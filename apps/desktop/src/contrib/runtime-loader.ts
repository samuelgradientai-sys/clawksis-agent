 ...
 *   (`@clawk/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default ClawksisPlugin -> register(ctx)
 *
 ...
 * and `<clawk home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { getStatus } from '@/clawk'
import { installPluginSdk, sdkImportMap } from '@/sdk/runtime'
import { notifyError } from '@/store/notifications'

import { createPluginContext, type ClawksisPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@clawk/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @clawk/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: ClawksisPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default ClawksisPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<clawk home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    const { clawk_home } = await getStatus()
    const { entries } = await desktop.readDir(`${clawk_home}/desktop-plugins`)
