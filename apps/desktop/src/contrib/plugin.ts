 ...
 * `ClawksisPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/clawk'
...
export type { PluginRestOptions } from '@/clawk'
 ...
 *  under `clawk.plugin.<id>.` — plugins can't read or clobber each other. */
...
export interface ClawksisPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `clawk.plugin.${pluginId}.${key}`
