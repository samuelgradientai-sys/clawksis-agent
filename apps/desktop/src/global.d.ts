...
declare global {
  interface Window {
    clawkDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ClawksisConnection>
      ...
      // Clawksis Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new CLAWK_HOME (reloads the window). Pass null to
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ClawksisGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ClawksisGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ClawksisGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ClawksisRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ClawksisReviewScope, baseRef?: null | string) => Promise<ClawksisReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ClawksisReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ClawksisReviewShipInfo>
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Clawksis Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Clawksis Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Clawksis Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the clawk gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Clawksis Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ClawksisGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ClawksisGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface ClawksisGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ClawksisRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ClawksisRepoStatus {
  branch: null | string
...
// turn began.
export type ClawksisReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ClawksisReviewFile {
  path: string
  added: number
  removed: number
...
export interface ClawksisReviewList {
  files: ClawksisReviewFile[]
...
export interface ClawksisReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ClawksisReviewShipInfo {
  ghReady: boolean
  pr: ClawksisReviewPr | null
}
