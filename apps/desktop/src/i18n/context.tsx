...
import { getClawksisConfigRecord, type ClawksisConfigRecord, saveClawksisConfig } from '@/clawk'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ClawksisConfigRecord>
  saveConfig: (config: ClawksisConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.clawkDesktop?.api) {
      return Promise.resolve({})
    }

    return getClawksisConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.clawkDesktop?.api) {
    ...
    return saveClawksisConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ClawksisConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ClawksisConfigRecord, locale: Locale): ClawksisConfigRecord {
