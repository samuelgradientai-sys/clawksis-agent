...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      startingClawksisDesktop: '正在启动 Clawksis Desktop…'
    },
    errors: {
  ...
  settings: {
    ...
    fieldDescriptions: {
      ...
      file_read_max_chars: 'Clawksis 单次文件读取可读取的最大字符数。',
      'approvals.mode': 'Clawksis 如何处理需要显式审批的命令。',
      ...
      'agent.max_turns': 'Clawksis 停止一次运行前工具调用轮次的上限。'
    },
    about: {
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server,暴露其 API,然后用服务器密码把 Clawksis 指向该 URL。',
      ...
      weixin: '登录微信公众平台,复制 AppID 和 Token,并把消息回调 URL 指向 Clawksis。',
      qqbot: '在 QQ 开放平台(q.qq.com)注册一个应用,复制 App ID 和 Client Secret。',
      api_server: '把 Clawksis 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥,然后把 Open WebUI / LobeChat 等指向 host:port。',
