...
const CLAWK_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ClawksisRefType = (typeof CLAWK_REF_TYPES)[number]
...
const ICON_PATHS: Record<ClawksisRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ClawksisRefType] ?? ICON_FALLBACK
...
const CLAWK_DIRECTIVE_RE = new RegExp(
...
export const clawkDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CLAWK_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ClawksisRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ClawksisRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Clawksis directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => clawkDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.clawkDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.clawkDesktop?.readFileDataUrl(id)
