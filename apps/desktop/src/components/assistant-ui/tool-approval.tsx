...
const ApprovalBar: FC<{ request: ApprovalRequest }> = ({ request }) => {
  ...
  // "Always allow" persists the pattern to ~/.clawksis/config.yaml permanently, so
  ...
  const respond = useCallback(
    async (choice: ApprovalChoice) => {
      ...
      if (!gateway) {
        notifyError(new Error('Clawksis gateway is not connected'), 'Could not send approval response')
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <code className="font-mono text-xs">~/.clawksis/config.yaml</code>). Clawksis won’t ask again for commands
