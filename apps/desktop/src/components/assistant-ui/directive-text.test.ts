...
import { formatRefValue, clawkDirectiveFormatter } from './directive-text'
...
describe('clawkDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = clawkDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = clawkDirectiveFormatter.parse('@file:src/main.tsx the entry point')
