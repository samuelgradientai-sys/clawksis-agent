...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Clawksis gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.clawkDesktop
  Object.defineProperty(window, 'clawkDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'clawkDesktop', { configurable: true, value: original })
}
