...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'clawk:saveImageFromUrl'")
...
export function ZoomableImage({ className, containerClassName, src, alt, slot, ...props }: ZoomableImageProps) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      if (window.clawkDesktop?.saveImageFromUrl) {

        const saved = await window.clawkDesktop.saveImageFromUrl(src)
      ...
      if (isMissingIpcHandler(error)) {
        ...
        try {
          ...
          notify({
            ...
            message: 'Restart Clawksis Desktop to use Save Image.'

          })

        } catch (fallbackError) {

          notifyError(fallbackError, 'Restart Clawksis Desktop to save images')
