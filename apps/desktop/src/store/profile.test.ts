...
const { getProfiles } = await import('@/clawk')
...
const remoteConn = (over: Partial<ClawksisConnection> = {}): ClawksisConnection =>
  ({ baseUrl: 'https://clawk-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ClawksisConnection

const localConn = (over: Partial<ClawksisConnection> = {}): ClawksisConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ClawksisConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ClawksisConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { clawkDesktop: { getConnection } })
