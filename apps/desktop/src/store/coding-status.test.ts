...
import type { ClawksisRepoStatus } from '@/global'

import { $repoStatus, $repoStatusLoading, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: ClawksisRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<ClawksisRepoStatus | null>) {
  ;(window as unknown as { clawkDesktop?: unknown }).clawkDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh', async () => {
    const resolvers: Array<(status: ClawksisRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
