...
//
// Scope is always "uncommitted" — Clawksis' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'clawk.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'clawk.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'clawk.desktop.reviewTreeMode'
const SELECTED_KEY = 'clawk.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ClawksisReviewFile[]>([])
...
export const $reviewShipInfo = atom<ClawksisReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['clawkDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ClawksisReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.clawkDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.clawkDesktop?.openExternal?.(url)
    }
