import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@clawk/shared'
...
const isOpen = (gateway: ClawksisGateway | null): boolean => gateway?.connectionState === 'open'
...
let primaryGateway: ClawksisGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ClawksisGateway | null, profile = 'default'): void {
...
export function activeGateway(): ClawksisGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ClawksisGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
