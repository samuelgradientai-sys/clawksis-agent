 ...
 * app's bounds and stays visible while Clawksis is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'clawk.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'clawk.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.clawkDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.clawkDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.clawkDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.clawkDesktop?.petOverlay?.close()
}
