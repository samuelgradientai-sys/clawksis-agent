...
import { getClawksisConfigRecord, saveClawksisConfig } from '@/clawk'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getClawksisConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveClawksisConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
