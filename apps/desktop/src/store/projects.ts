...
import type { ClawksisGitBaseBranch, ClawksisGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/clawk'
...
const PROJECT_SCOPE_KEY = 'clawk.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Clawksis gateway is not connected')
  }
...
// repo, returning where Clawksis should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ClawksisGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ClawksisGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.clawkDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.clawkDesktop?.writeClipboard?.(path)
  }
}
