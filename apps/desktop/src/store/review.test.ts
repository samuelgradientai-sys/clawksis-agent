...
import type { ClawksisReviewFile, ClawksisReviewShipInfo } from '@/global'
...
// reaches the gateway); everything else routes through window.clawkDesktop.git,
...
function file(path: string, over: Partial<ClawksisReviewFile> = {}): ClawksisReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as ClawksisReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.clawkDesktop. Any op not supplied defaults
...
function stubReview(over: ReviewStub = {}) {
  ...
  ;(window as unknown as { clawkDesktop?: unknown }).clawkDesktop = {
    git: { review },
    openExternal: vi.fn()
  }
...
afterEach(() => {
  delete (window as unknown as { clawkDesktop?: unknown }).clawkDesktop
})
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as ClawksisReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.clawkDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: ClawksisReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as ClawksisReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { clawkDesktop?: unknown }).clawkDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as ClawksisReviewShipInfo)
