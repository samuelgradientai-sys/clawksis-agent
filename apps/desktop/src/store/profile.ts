...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/clawk'
...
import type { ProfileInfo } from '@/types/clawk'
...
// /api/profiles/active `current`). "default" is the root ~/.clawksis. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'clawk.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'clawk.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.clawkDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new CLAWK_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.clawkDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.clawkDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'clawk.desktop.showAllProfiles'
...
// Switch to the default (root ~/.clawksis) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.clawkDesktop?.touchBackend?.(target).catch(() => undefined)
}
