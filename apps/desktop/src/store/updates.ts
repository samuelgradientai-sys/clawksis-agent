...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: 'Update Clawksis', onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
    message: 'Your Clawksis backend is older than this desktop build and may not work correctly. Update to align them.',
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    ...
    // The update check pulls the latest clawk_cli + bundled package metadata
...
function onFocus() {
  ...
  // updated Clawksis from another window/CLI between focuses, and About should
