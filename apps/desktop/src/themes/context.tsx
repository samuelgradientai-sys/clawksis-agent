...
const SKIN_KEY = 'clawk-desktop-theme-v2'
const MODE_KEY = 'clawk-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'clawk-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'clawk-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'clawk-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.clawkTheme = skinName
  root.dataset.clawkMode = rendered
  ...
  window.clawkDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('clawk-boot-background', c.background)
    window.localStorage.setItem('clawk-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.clawkThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.clawkDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Clawksis backend theme on connect. */
