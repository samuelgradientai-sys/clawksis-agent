...
import { getElevenLabsVoices, getClawksisConfigSchema, saveClawksisConfig } from '@/clawk'
import { useI18n } from '@/i18n'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, ClawksisConfigRecord } from '@/types/clawk'

import { setClawksisConfigCache, useClawksisConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: ClawksisConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ClawksisConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useClawksisConfigRecord()
  ...
  const {
    ...
    queryKey: ['clawk-config-schema'],
    queryFn: getClawksisConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: ClawksisConfigRecord) => {
