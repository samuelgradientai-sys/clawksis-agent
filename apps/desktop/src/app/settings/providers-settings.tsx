...
import { disconnectOAuthProvider, listOAuthProviders } from '@/clawk'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/clawk'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.clawkDesktop?.terminal)
...
//      in clawk_cli/provider_catalog.py) — the SAME provider identity
//      `clawk model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Clawksis can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Clawksis can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Clawksis never deletes creds another tool owns behind a silent API call.
