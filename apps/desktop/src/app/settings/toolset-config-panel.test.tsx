...
import type { ToolsetConfig } from '@/types/clawk'
...
const getClawksisConfigRecord = vi.fn()
const getClawksisConfigSchema = vi.fn()
const saveClawksisConfig = vi.fn()
...
vi.mock('@/clawk', () => ({
  ...
  getClawksisConfigRecord: () => getClawksisConfigRecord(),
  getClawksisConfigSchema: () => getClawksisConfigSchema(),
  saveClawksisConfig: (config: unknown) => saveClawksisConfig(config),
...
beforeEach(() => {
  ...
  getClawksisConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getClawksisConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveClawksisConfig.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveClawksisConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveClawksisConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
