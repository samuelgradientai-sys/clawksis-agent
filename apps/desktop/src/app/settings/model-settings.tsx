...
import { invalidateClawksisConfig, setClawksisConfigCache, useClawksisConfigRecord } from '../hooks/use-config-record'
...
// Clawksis' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Clawksis default (medium), shown as Medium.
...
// backend now surfaces the full `clawk model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in clawk_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useClawksisConfigRecord()
  const setConfig = setClawksisConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `clawk model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Clawksis runs the flow for you.`}
          </p>
        )}
