...
export function GatewaySettings() {
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.clawkDesktop
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.clawkDesktop.applyConnectionConfig(payload())
        : await window.clawkDesktop.saveConnectionConfig(payload())
      ...
      notify({
        ...
        message: apply ? 'Clawksis Desktop will reconnect using the saved settings.' : 'Saved for the next restart.'
      })
    } catch (err) {
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.clawkDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.clawkDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.clawkDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.clawkDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.clawkDesktop.getConnectionConfig(scope)
      setState(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.clawkDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
      ...
      const message = `Connected to ${result.baseUrl}${result.version ? ` · Clawksis ${result.version}` : ''}`
      setLastTest(message)
  ...
  if (!window.clawkDesktop?.getConnectionConfig) {
    return (
      <EmptyState
    ...
    <SettingsContent>
      <div className="mb-5">
        ...
        <p className="mt-2 max-w-2xl text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-(--ui-text-tertiary)">
          Clawksis Desktop starts its own local gateway by default. Use a remote gateway when you want this app to control
          an already-running Clawksis backend on another machine or behind a trusted proxy. Pick a profile below to give it
          its own remote host.
        </p>
      </div>
        ...
        <div className="mb-5 flex items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/10 px-3 py-2.5 text-[length:var(--conversation-caption-font-size)] text-destructive">
          ...
          <div>
            ...
            <div className="mt-1 leading-5">
              Unset <code>CLAWK_DESKTOP_REMOTE_URL</code> and <code>CLAWK_DESKTOP_REMOTE_TOKEN</code> to use the saved
              setting below.
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <ModeCard
          ...
          description="Start a private Clawksis backend on localhost. This is the default and works offline."
        ...
        <ModeCard
          ...
          description="Connect this desktop shell to a remote Clawksis backend. Hosted gateways use OAuth or a username and password; self-hosted ones may use a session token."
          disabled={state.envOverride}
          icon={Globe}
        ...
        <ListRow
          action={
            <Input
              ...
              placeholder="https://gateway.example.com/clawk"
              value={state.remoteUrl}
            />
          }
          description="Base URL for the remote dashboard backend. Path prefixes are supported, for example /clawk."
          title="Remote URL"
        />
      ...
      <div className="mt-6 grid gap-1">
        <ListRow
          action={
            <Button onClick={() => void window.clawkDesktop?.revealLogs()} size="sm" variant="textStrong">
