...
import { getClawksisConfigDefaults, getClawksisConfigRecord, saveClawksisConfig } from '@/clawk'
...
export function SettingsView({ gateway, onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getClawksisConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'clawk-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    if (!window.confirm('Reset all settings to Clawksis defaults?')) {
      return
    }

    try {
      await saveClawksisConfig(await getClawksisConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
