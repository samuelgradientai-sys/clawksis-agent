...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    window.clawkDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  // clawk:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
  ...
  useEffect(() => {
    const unsubscribe = window.clawkDesktop?.onClosePreviewRequested?.(() => void closeActiveTab())
