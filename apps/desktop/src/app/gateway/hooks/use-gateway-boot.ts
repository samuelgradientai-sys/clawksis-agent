import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@clawk/shared'
import { useEffect, useRef } from 'react'

import type { ClawksisConnection } from '@/global'
import { ClawksisGateway } from '@/clawk'
...
import type { RpcEvent } from '@/types/clawk'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.clawkDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ClawksisGateway | null) => void
  refreshClawksisConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ClawksisConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Clawksis...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Clawksis…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Clawksis gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshClawksisConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshClawksisConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startClawksis re-emits progress — ignore so the
    ...
    const gateway = new ClawksisGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshClawksisConfig(),
          ...
          // retry it), not brick boot behind the "Clawksis couldn't start"
