  ...
  useEffect(() => {
    ...
    const offExit = desktop.onBackendExit(() => {
      if ($desktopBoot.get().running || $desktopBoot.get().visible) {
        failDesktopBoot('Clawksis background process exited during startup.')
      }

      notify({
        kind: 'error',
        title: 'Backend stopped',
        message: 'Clawksis background process exited.',
        durationMs: 0
      })
    })
    ...
    async function boot() {
      try {
        ...
        setDesktopBootStep({
          ...
          message: 'Loading Clawksis settings',
          progress: 97
        })
        await callbacksRef.current.refreshClawksisConfig()
