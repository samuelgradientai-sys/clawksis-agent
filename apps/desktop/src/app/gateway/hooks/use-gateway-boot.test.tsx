...
// bug that drives the REAL useGatewayBoot hook + REAL ClawksisGateway through a
...
function Harness({ refreshSessions }: { refreshSessions?: () => Promise<void> } = {}) {
  useGatewayBoot({
    ...
    refreshClawksisConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { clawkDesktop?: unknown }).clawkDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { clawkDesktop?: unknown }).clawkDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForClawksis) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startClawksis()'s remote branch awaits waitForClawksis() for 45s before it
    ...
    ;(window as { clawkDesktop?: unknown }).clawkDesktop = desktop
    ...
    // After ~45s waitForClawksis gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Clawksis backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Clawksis couldn't start" overlay — the socket is open
