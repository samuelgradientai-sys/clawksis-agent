...
import { getProfiles, transcribeAudio } from '@/clawk'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.clawkDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `clawk gateway` process performs the actual transfer, so we
