...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshClawksisConfig — two REST calls (config + defaults) per event, per
    ...
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        if (apply) {
          ...
          // everything refreshClawksisConfig applies is either active-session
        ...
        const errorMessage = payload?.message || 'Clawksis reported an error'
