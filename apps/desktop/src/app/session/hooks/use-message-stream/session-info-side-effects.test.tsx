...
let refreshClawksisConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  handleEvent = null
  refreshClawksisConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshClawksisConfig).not.toHaveBeenCalled()

    await act(async () => {
      await vi.advanceTimersByTimeAsync(400)
    })

    expect(refreshClawksisConfig).toHaveBeenCalledTimes(1)
  })
