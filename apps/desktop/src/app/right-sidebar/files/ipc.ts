...
import type { ClawksisReadDirEntry, ClawksisReadDirResult } from '@/global'



export type ProjectTreeEntry = ClawksisReadDirEntry
...
async function gitRootFor(start: string) {
  ...
  if (!window.clawkDesktop?.gitRoot) {
  ...
  if (!cached) {
    ...
    cached = window.clawkDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  ...
  if (!window.clawkDesktop?.readDir || !window.clawkDesktop.readFileDataUrl) {
  ...
  try {
    ...
    const listing = await window.clawkDesktop.readDir(dir)
    ...
    const text = decodeDataUrl(await window.clawkDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: ClawksisReadDirEntry) {
...
async function filterIgnored(entries: ClawksisReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ClawksisReadDirResult> {

  if (!window.clawkDesktop) {
  ...
  const result = await window.clawkDesktop.readDir(dirPath)
