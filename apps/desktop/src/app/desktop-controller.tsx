...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../clawk'
...
import { useClawksisConfig } from './session/hooks/use-clawk-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.clawkDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.clawkDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.clawkDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.clawkDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // clawk:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.clawkDesktop?.onDeepLink?.(payload => {
    ...
    void window.clawkDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.clawkDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshClawksisConfig, sttEnabled, voiceMaxRecordingSeconds } = useClawksisConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshClawksisConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshClawksisConfig])
