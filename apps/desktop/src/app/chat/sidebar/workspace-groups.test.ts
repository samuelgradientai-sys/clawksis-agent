...
import type { ClawksisWorktreeInfo } from '@/global'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/clawksis-agent/apps/desktop'), makeSession('/a/clawksis-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/clawksis-agent/apps/desktop'), makeSession('/a/clawksis-agent-wt-rtl/apps/desktop')])
    ).toEqual(['clawksis-agent/apps/desktop', 'clawksis-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/clawksis-agent/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<ClawksisWorktreeInfo> & Pick<ClawksisWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): ClawksisWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/clawksis-agent'), makeSession('/www/clawksis-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('clawksis-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['clawksis-agent', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/clawksis-agent') {
        return info({ repoRoot: '/www/clawksis-agent', worktreeRoot: '/www/clawksis-agent', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/clawksis-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/clawksis-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['clawksis-agent', 'rtl'])
  })
