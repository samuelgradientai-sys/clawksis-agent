...
import type { ClawksisGitBranch } from '@/global'
...
const branchActionLabel = (branch: ClawksisGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<ClawksisGitBranch[]>([])
  ...
  const convert = async (branch: ClawksisGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
