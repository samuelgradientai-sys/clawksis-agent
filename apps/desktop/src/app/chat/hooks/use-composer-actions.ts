...
export const CLAWK_PATHS_MIME = 'application/x-clawk-paths'
 ...
 * triples. Internal Clawksis sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.clawkDesktop?.getPathForFile
  ...
  try {
    ...
    const internalRaw = transfer.getData(CLAWK_PATHS_MIME)
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  const pickContextPaths = useCallback(
    ...
    async (kind: 'file' | 'folder') => {
      ...
      const paths = await window.clawkDesktop?.selectPaths({
  ...
  const attachImagePath = useCallback(async (filePath: string) => {
    ...
    try {
      ...
      const previewUrl = await window.clawkDesktop?.readFileDataUrl(filePath)
  ...
  const attachImageBlob = useCallback(
    ...
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.clawkDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(async () => {
    ...
    try {
      ...
      const path = await window.clawkDesktop?.saveClipboardImage()
    ...
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          ...
          !knownPath && window.clawkDesktop?.getPathForFile ? window.clawkDesktop.getPathForFile(file) : ''
