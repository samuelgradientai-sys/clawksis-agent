    ...
    <PreviewEmptyState
        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.clawkDesktop?.openExternal(error.url)
            }}
          >
      ...
      secondaryAction={
        onRestartServer
          ? {
              disabled: restarting,
              label: restarting ? 'Clawksis is restarting...' : 'Ask Clawksis to restart the server',
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Clawksis to restart the server" looked like it did nothing —
    ...
    try {
      ...
      appendConsoleEntry({
        level: 1,
        message: `Clawksis is looking for a preview server to restart (${taskId})`
      })

      notify({
        kind: 'info',
        title: 'Restarting preview server',
        message: 'Clawksis is working in the background. Watch the preview console for progress.',
        durationMs: 4000
      })
    } catch (error) {
      appendConsoleEntry({
  ...
  useEffect(() => {
      ...
      message:
        previewServerRestart.status === 'running'
          ...
          : previewServerRestart.status === 'complete'
            ? `Clawksis finished restarting the preview server${
    ...
    if (previewServerRestart.status === 'complete') {
      ...
      notify({
        ...
        message: previewServerRestart.message?.slice(0, 200) || 'Clawksis could not restart the server.',
        durationMs: 6000
      })
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      failPreviewServerRestart(
        taskId,
        'Clawksis is still working, but no restart result has arrived yet. The server command may be running in the foreground.'
      )
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.clawkDesktop?.watchPreviewFile ||
      !window.clawkDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.clawkDesktop.onPreviewFileChanged(payload => {
    ...
    void window.clawkDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.clawkDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.clawkDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:clawk-preview')
