...
import { clawkDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, CLAWK_PATHS_MIME } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // cold start ("Starting Clawksis...") from a dropped connection we're trying to
  // restore (e.g. after the Mac slept) so the stuck state reads as recoverable.
  const placeholder = disabled
    ? gatewayState === 'closed' || gatewayState === 'error'
      ? 'Reconnecting to Clawksis…'
      : 'Starting Clawksis...'
    : restingPlaceholder
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = clawkDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, CLAWK_PATHS_MIME)) {
      return
    }
  ...
  const handleDrop = (event: ReactDragEvent<HTMLFormElement>) => {
    ...
    if (Array.from(event.dataTransfer.types || []).includes(CLAWK_PATHS_MIME)) {
