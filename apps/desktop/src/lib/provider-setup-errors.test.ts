...
describe('isProviderSetupErrorMessage', () => {
  ...
  it('matches generic missing-provider copy', () => {
    ...
    expect(isProviderSetupErrorMessage('No inference provider configured. Run `clawk model` to choose one.')).toBe(
    ...
    expect(isProviderSetupErrorMessage('No Clawksis provider is configured.')).toBe(true)

    expect(isProviderSetupErrorMessage('set an API key (OPENROUTER_API_KEY) in ~/.clawksis/.env')).toBe(true)
