...
/** Why a known Clawksis command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Clawksis profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Clawksis version', surface: exec() },
...
function isKnownClawksisSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Clawksis' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownClawksisSlashCommand(normalized)
}
