...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Clawksis doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in clawk_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `clawk model` uses — instead of
...
export interface ClawksisConfig {
  agent?: {
...
export type ClawksisConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `clawk model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors clawk_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Clawksis, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `clawk tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Clawksis); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/clawk/update/check` — the backend's own update state.
