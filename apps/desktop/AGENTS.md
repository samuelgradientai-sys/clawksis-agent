...
How to build Clawksis Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Clawksis surface can also
...
- A **runtime home change** (switching the underlying `CLAWK_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Clawksis — do not assume
