...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'clawk-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'clawk-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Clawksis Test'], { cwd: dir })
