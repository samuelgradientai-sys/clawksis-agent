/**
 * windows-clawk-path.ts
 *
 * Pure, dependency-injected pieces of Windows `clawk` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvClawksisCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `clawk` shim shadowed the real
 *      clawk.cmd/clawk.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\clawk.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvClawksisCommand() — unwrapWindowsVenvClawksisCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `clawk`) must
 * not shadow `clawk.cmd`/`clawk.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv clawk shim, bootstrap-complete
 * marker) — gating on just the clawk.exe console-script shim alone is the
...
export interface ResolveVenvClawksisCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportClawksisCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `clawk`/`clawk.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/clawk(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m clawk_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportClawksisCli(). A
 * venv whose update died mid-`pip install` still has python.exe + clawk.exe
 ...
 *
 * Returns null when `command` is not a venv clawk shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvClawksisCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvClawksisCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^clawk(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Clawksis at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Clawksis Python at ${python}`,
    command: python,
