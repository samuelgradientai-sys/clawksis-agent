...
async function resolveInstallScript({ installStamp, sourceRepoRoot, clawkHome, emit }) {
  ...
  const cached = cachedScriptPath(clawkHome, installStamp.commit)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, clawkHome } = {}) {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, clawkHome } = {}) {
...
function buildPosixPinArgs({ installStamp, activeRoot, clawkHome }) {
...
async function fetchManifest({ scriptPath, installerKind, emit, clawkHome, activeRoot, installStamp }) {

  const isPosix = installerKind === 'posix'

  const args = isPosix

    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, clawkHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, clawkHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ...
    ? [
        ...
        ...buildPosixPinArgs({ installStamp, activeRoot, clawkHome })
