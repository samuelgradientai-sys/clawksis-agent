 ...
 *     `X-Clawksis-Session-Token` header; WS uses `?token=`.
...
// clawk_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - clawk_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - clawk_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / clawk #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-clawk_session_at', '__Secure-clawk_session_at', 'clawk_session_at']
const RT_COOKIE_VARIANTS = ['__Host-clawk_session_rt', '__Secure-clawk_session_rt', 'clawk_session_rt']

// The Nous portal (NAS) does NOT use Clawksis gateway session cookies — it is a
...
// Clawksis Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://clawk.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a clawk session ACCESS-token cookie
 ...
 * the Clawksis gateway session cookies, so cloud sign-in / discovery liveness
