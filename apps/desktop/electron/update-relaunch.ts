 ...
 * Background
 * ----------
 * After `clawk update` + `clawk desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything CLAWK_DESKTOP_* is preserved
// plus CLAWK_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['CLAWK_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['CLAWK_DESKTOP_']
