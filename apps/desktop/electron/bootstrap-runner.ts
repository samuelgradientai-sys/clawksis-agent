 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Clawksis by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.clawk-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(clawkHome) {
  return path.join(clawkHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.clawksis/clawksis-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(clawkHome) {
  if (!clawkHome) {
    return null
  }

  const candidate = path.join(clawkHome, 'clawksis-agent', 'scripts', installScriptName())
...
function cachedScriptPath(clawkHome, commit) {
  return path.join(bootstrapCacheDir(clawkHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/samuelgradientai-sys/clawksis-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(clawkHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(clawkHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, clawkHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass CLAWK_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          CLAWK_HOME: clawkHome || process.env.CLAWK_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, clawkHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, clawkHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--clawk-home', clawkHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, clawkHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, clawkHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, clawkHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(clawkHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, clawkHome, emit })
