// Profile-delete routing logic for the `clawk:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_clawk_home() and
...
/**
 * Parse a `clawk:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `clawk:api` request away from the primary/window backend
 ...
 * ensure_clawk_home() recreates the directory the delete just removed.
 */
