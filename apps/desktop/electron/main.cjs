...
// scripts/install.ps1's $ClawksisHome and scripts/install.sh's $CLAWK_HOME.
//
// Defaults:
...
function resolveClawksisHome() {
  if (process.env.CLAWK_HOME) return path.resolve(process.env.CLAWK_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'clawk-home')
...
const CLAWK_HOME = resolveClawksisHome()
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `clawk update --branch ${branch}`
        }
      } catch {
    ...
    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Clawksis updater…', percent: 100 })
    ...
    const child = spawn(updater, updaterArgs, {
      ...
      env: {
        ...process.env,
        CLAWK_HOME,
        PATH: [path.join(CLAWK_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
function resolveClawksisCliBinary(updateRoot) {
  ...
  if (fileExists(venvClawksis)) return venvClawksis
...
async function applyUpdatesPosixInApp() {
  ...
  const extraPath = [path.join(CLAWK_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    CLAWK_HOME,
  ...
  // `clawk update` reaps stale `clawk dashboard` backends (a code update
  ...
  if (clawkProcess && Number.isInteger(clawkProcess.pid)) {
    env.CLAWK_DESKTOP_CHILD_PID = String(clawkProcess.pid)
  }
  ...
  const rebuilt = await runStreamedUpdate(clawk, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
...
function isBootstrapComplete() {
  ...
  return isClawksisSourceRoot(ACTIVE_CLAWK_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'clawk_cli.main', ...dashboardArgs],
    env: {
...
function resolveClawksisBackend(dashboardArgs) {
  ...
  if (overrideRoot && isClawksisSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Clawksis source at ${overrideRoot}`, dashboardArgs)
  ...
  if (!IS_PACKAGED && isClawksisSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Clawksis source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }
  ...
  if (process.env.CLAWK_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    const clawkOverride = process.env.CLAWK_DESKTOP_HERMES
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Clawksis install was cancelled.')
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Clawksis gateway',
...
async function freshGatewayWsUrl() {
  const connection = await startClawksis()
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig()) {
  ...
  return {
    ...
    envOverride: Boolean(process.env.CLAWK_DESKTOP_REMOTE_URL)
  }
}
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (config.mode === 'remote') {
    ...
    const remote = (await resolveRemoteBackend()) || (await startClawksis())
...
function resetClawksisConnection() {
...
async function startClawksis() {
  ...
  connectionPromise = (async () => {
    ...
    const backend = await ensureRuntime(resolveClawksisBackend(dashboardArgs))
    ...
    clawkProcess = spawn(backend.command, backend.args, {
      cwd: clawkCwd,
      env: {
        ...process.env,
        // Explicitly pin CLAWK_HOME for the child so Python's get_clawk_home()
        // resolves to the SAME location our resolveClawksisHome() picked. Without
...
ipcMain.handle('clawk:connection', async () => startClawksis())
ipcMain.handle('clawk:gateway:ws-url', async () => freshGatewayWsUrl())
...
ipcMain.handle('clawk:connection-config:get', async () => sanitizeDesktopConnectionConfig())
...
ipcMain.handle('clawk:connection-config:apply', async (_event, payload) => {
  ...
  // Capture the reference before resetClawksisConnection() nulls clawkProcess,
...
ipcMain.handle('clawk:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('clawk:setting:defaultProjectDir:get', async () => ({
  ...
  defaultLabel: path.join(app.getPath('home'), 'clawk-projects')
}))
...
ipcMain.handle('clawk:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('clawk:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('clawk:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Clawksis.')
  }
