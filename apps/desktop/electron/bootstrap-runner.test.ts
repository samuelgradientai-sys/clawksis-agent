...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'clawk-bootstrap-test-'))
}
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'clawksis-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'clawksis-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/clawksis-agent',
      clawkHome: '/tmp/clawk'
    }),
    ['--dir', '/tmp/clawksis-agent', '--clawk-home', '/tmp/clawk', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/clawksis-agent', '--clawk-home', '/tmp/clawk', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/clawk',
      clawkHome: '/tmp/home'
    }),
    ['--dir', '/tmp/clawk', '--clawk-home', '/tmp/home', '--branch', 'main']
  )
})
