// Unit tests for the pure Windows `clawk` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvClawksisCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `clawk` shim shadows
//      the real clawk.cmd/clawk.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the clawk.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvClawksisCommand() — must probe the venv python via
//      canImportClawksisCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvClawksisCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportClawksisCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    clawkHome: '/fake/clawk-home',
...
test('resolveVenvClawksisCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvClawksisCommand('/root/venv/Scripts/clawk.exe', [], deps), null)
})

test('resolveVenvClawksisCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvClawksisCommand('/root/venv/Scripts/clawk.cmd', [], deps), null)
})

test('resolveVenvClawksisCommand: returns null when the basename is not clawk/clawk.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvClawksisCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvClawksisCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvClawksisCommand('/root/venv/bin/clawk.exe', [], deps), null)
})

test('resolveVenvClawksisCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvClawksisCommand('/root/venv/Scripts/clawk.exe', [], deps), null)
})

test('resolveVenvClawksisCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportClawksisCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvClawksisCommand('/root/venv/Scripts/clawk.exe', ['serve'], deps)
...
test('resolveVenvClawksisCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvClawksisCommand('/root/venv/Scripts/clawk.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'clawk_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvClawksisCommand: is case-insensitive on clawk.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvClawksisCommand('/root/venv/Scripts/CLAWKSIS.EXE', [], deps))
  assert.ok(resolveVenvClawksisCommand('/root/venv/SCRIPTS/clawk.exe', [], deps))
})
