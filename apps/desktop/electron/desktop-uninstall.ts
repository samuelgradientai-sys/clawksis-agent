 ...
 * the three user-facing uninstall modes to the `clawk uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m clawk_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `clawk_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'clawk_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Clawksis.app/Contents/MacOS/Clawksis  → …/Clawksis.app
 *   Windows: …\Clawksis\Clawksis.exe                 → …\Clawksis  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Clawksis.app/Contents/MacOS/Clawksis → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Clawksis.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Clawksis.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Clawksis$/i.test(dir) || /[\\/]clawk-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/clawk
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import clawk_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, clawkHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import clawk_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Clawksis install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "CLAWK_HOME=${String(clawkHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
