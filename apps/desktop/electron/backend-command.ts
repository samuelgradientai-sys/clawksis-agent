// Backend subcommand routing for the desktop-managed Clawksis process.
//
// The desktop app launches its own headless backend via `clawk serve` — it
...
// app hasn't updated yet, or an older `clawk` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Clawksis profile to pin via `--profile`.
 */
 ...
 * `-m clawk_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `clawk_cli/subcommands/dashboard.py` source registers
