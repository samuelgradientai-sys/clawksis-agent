...
export function markerPath(clawkHome) {
  return path.join(clawkHome, '.clawk-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(clawkHome)
  let raw
 ...
 *
 * The Tauri-based clawk-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(clawkHome, pid, { now = Date.now } = {}) {
  const file = markerPath(clawkHome)
