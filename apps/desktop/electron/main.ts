...
import { buildDesktopBackendEnv, normalizeClawkHomeRoot } from './backend-env'
import { canImportClawksisCli, verifyClawksisCli } from './backend-probes'
...
import {
  ...
  resolveVenvClawksisCommand
} from './windows-clawk-path'
import {
  alreadyHasNoSandbox,
...
const USER_DATA_OVERRIDE = process.env.CLAWK_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CLAWK_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.CLAWK_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with CLAWK_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[clawk] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[clawk] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `clawk desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Clawksis sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[clawk] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[clawk] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[clawk] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[clawk] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[clawk] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('clawk:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[clawk] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[clawk] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[clawk] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[clawk] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CLAWK_HOME — the user-facing root for everything Clawksis-related. Mirrors
// scripts/install.ps1's $ClawkHome and scripts/install.sh's $CLAWK_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\clawk (matches install.ps1)
//   macOS / Linux: ~/.clawksis (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.clawksis directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\clawk yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CLAWK_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CLAWK_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.clawksis / %LOCALAPPDATA%\clawk.
function resolveClawkHome() {
  if (process.env.CLAWK_HOME) {
    return normalizeClawkHomeRoot(process.env.CLAWK_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'clawk-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a CLAWK_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\clawk and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('CLAWK_HOME')

    if (fromRegistry) {
      return normalizeClawkHomeRoot(fromRegistry)
    }
  }
  ...
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    ...
    // ~/.clawksis setup (no LOCALAPPDATA install yet) so users don't lose state.
...
const CLAWK_HOME = resolveClawkHome()

function clawkManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_clawk_node_dirs() in
  // clawk_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(CLAWK_HOME, 'node')
...
function pathWithClawksisManagedNode(...entries) {
  return [...clawkManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_CLAWK_ROOT — the canonical mutable Clawksis install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_CLAWK_ROOT = path.join(CLAWK_HOME, 'clawksis-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_CLAWK_ROOT, 'venv')
...
// boots, going straight to `resolveClawksisBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CLAWK_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CLAWK_ROOT, '.clawk-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Clawksis profile the desktop launches its
// local backend as. When set, startClawksis() passes `clawk --profile <name>
// dashboard …`, which deterministically pins CLAWK_HOME (see
// _apply_profile_override in clawk_cli/main.py) and bypasses the sticky
// ~/.clawksis/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors clawk_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// clawkDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CLAWK_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by clawk_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(CLAWK_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (clawk_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.CLAWK_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.CLAWK_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.CLAWK_DESKTOP_APP_NAME || 'Clawksis'
...
// 'clawk:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.clawk) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.clawk')
}

// Seed the native About panel with the live Clawksis version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `clawk update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveClawksisVersion(),
...
const MEDIA_PROTOCOL = 'clawk-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(clawk-media://…)`.
...
// startClawksis(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.CLAWK_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.CLAWK_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startClawksis() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning clawk serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Clawksis backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (clawkLog.length > 300) {
    clawkLog.splice(0, clawkLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-clawk-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('clawk:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the clawk:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('clawk:bootstrap:event', ev)
}
...
// The Tauri updater writes CLAWK_HOME/.clawk-update-in-progress for the whole
...
// (`force_kill_other_clawk`, taskkill /IM clawk.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(CLAWK_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Clawksis will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(CLAWK_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `clawk` — must not shadow `clawk.cmd`/`clawk.exe`.
...
function unwrapWindowsVenvClawksisCommand(command, backendArgs) {
  return resolveVenvClawksisCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
    canImportClawksisCli,
    getVenvPython,
    getVenvSitePackagesEntries,
    buildDesktopBackendEnv,
    clawkHome: CLAWK_HOME,
...
// spawns `clawk serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `clawk` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'clawk_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, CLAWK_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isClawksisSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'clawk_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.CLAWK_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Clawksis deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Clawksis' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\clawk\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clawk', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'clawk', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal CLAWK_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\clawk\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clawk', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'clawk', 'git', 'bin', 'git.exe'))
  }
...
function recentClawksisLog() {
  return clawkLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's clawk root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CLAWK_ROOT.
// CLAWK_DESKTOP_CLAWK_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CLAWK_DESKTOP_CLAWK_ROOT && path.resolve(process.env.CLAWK_DESKTOP_CLAWK_ROOT),
    !IS_PACKAGED && isClawksisSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isClawksisSourceRoot(ACTIVE_CLAWK_ROOT) ? ACTIVE_CLAWK_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CLAWK_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('clawk:updates:progress', merged)
  }
}
...
// CLAWK_HOME/clawk-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_clawk_home). That binary owns
// ALL repo mutation — running `clawk update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'clawk-setup.exe' : 'clawk-setup'
  const candidate = path.join(CLAWK_HOME, name)
...
// Path to the venv shim whose lock decides whether `clawk update` can write
// fresh entry points. On Windows this is the file the running backend
// `clawk.exe` holds open; on POSIX it's never mandatory-locked.
function venvClawksisShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'clawk.exe')
    : path.join(updateRoot, 'venv', 'bin', 'clawk')
}
...
// only signals the direct child, so on Windows a backend `clawk.exe` that
// spawned its own grandchildren (a `clawk` REPL, a pty terminal session, the
...
// `clawkProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `clawk.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a clawk
// REPL, a pty terminal, the gateway) keep `clawk.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const clawkProcess = backendConnectionState.getProcess()

  if (clawkProcess && Number.isInteger(clawkProcess.pid)) {
    pids.push(clawkProcess.pid)
  }
  ...
  if (clawkProcess && !clawkProcess.killed) {
    try {
      clawkProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvClawksisShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentClawksisProcess = backendConnectionState.getProcess()

    if (currentClawksisProcess && Number.isInteger(currentClawksisProcess.pid)) {
      stragglers.push(currentClawksisProcess.pid)
    }
...
// `clawk update`). Instead we spawn the staged Clawksis-Setup binary with
// --update and quit, so it can run `clawk update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri clawk-setup. Unlike Windows
      ...
      // whole update itself: `clawk update` (backend) + `clawk desktop
    ...
    if (!updater) {
      ...
      // `clawk desktop`, never the Tauri installer that self-copies
      // clawk-setup.exe into CLAWK_HOME). They DO have a working `clawk`
      ...
      // checkout they're on — bare `clawk update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `clawk update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `clawk update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, clawkRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Clawksis — this window will close and the updater will open. Don’t reopen Clawksis yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // clawk.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running clawk, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Clawksis install open ' +
        '(a second Clawksis window or a terminal running clawk?). Close it and retry.'
    ...
    // `clawk update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: CLAWK_HOME,
      env: {
        ...process.env,
        CLAWK_HOME,
        PATH: pathWithClawksisManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(CLAWK_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvClawksis = path.join(venvBin, IS_WINDOWS ? 'clawk.exe' : 'clawk')
  ...
  // not just the `clawk.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvClawksis) || fileExists(path.join(updateRoot, '.clawk-bootstrap-complete'))
...
// Resolve the clawk CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `clawk` on PATH.
function resolveClawksisCliBinary(updateRoot) {
  const venvClawksis = path.join(updateRoot, 'venv', 'bin', 'clawk')

  if (fileExists(venvClawksis)) {
    return venvClawksis
  }

  return findOnPath('clawk') || null
}
...
// macOS/Linux in-app update: backend (`clawk update`) + OS-aware GUI rebuild
// (`clawk desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const clawk = resolveClawksisCliBinary(updateRoot)

  if (!clawk) {
    emitUpdateProgress({ stage: 'manual', message: 'clawk update', percent: null })

    return { ok: true, manual: true, command: 'clawk update', clawkRoot: updateRoot }
  }

  // Put the Clawksis-managed Node and the venv on PATH so `clawk desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\clawk\node, not node\bin.
  // PYTHONUNBUFFERED: `clawk update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    CLAWK_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithClawksisManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `clawk update` reaps stale `clawk serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads CLAWK_DESKTOP_CHILD_PID and excludes
  ...
  if (clawkProcess && Number.isInteger(clawkProcess.pid)) {
    desktopChildPids.push(clawkProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.CLAWK_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Clawksis (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(clawk, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'clawk update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'clawk update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(clawk, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Clawksis to retry.',
  ...
  // Linux in-app update terminal state (#45205). `clawk desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Clawksis…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `clawk-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Clawksis to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Clawksis desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Clawksis to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Clawksis.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Clawksis.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Clawksis to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.clawk-update-new"; then
    rm -rf "$DST.clawk-update-old" 2>/dev/null || true
    mv "$DST" "$DST.clawk-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.clawk-update-new" "$DST"
    rm -rf "$DST.clawk-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Clawksis to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_CLAWK_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isClawksisSourceRoot(ACTIVE_CLAWK_ROOT) &&
    fileExists(venvPython) &&
    canImportClawksisCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_CLAWK_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `clawk
...
function resolveWebDist() {
  const override = process.env.CLAWK_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set CLAWK_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: clawk desktop --force-build`
  )
...
function resolveClawksisCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Clawksis.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveClawksisCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'clawk_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_CLAWK_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Clawksis at ${ACTIVE_CLAWK_ROOT}`,
    command,
    args: ['-m', 'clawk_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      clawkHome: CLAWK_HOME,
      pythonPathEntries: [ACTIVE_CLAWK_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_CLAWK_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveClawksisBackend(backendArgs) {
  // 1. Explicit override -- CLAWK_DESKTOP_CLAWK_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.CLAWK_DESKTOP_CLAWK_ROOT && path.resolve(process.env.CLAWK_DESKTOP_CLAWK_ROOT)

  if (overrideRoot && isClawksisSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Clawksis source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `clawk` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isClawksisSourceRoot.)
  if (!IS_PACKAGED && isClawksisSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Clawksis source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_CLAWK_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\clawk\clawksis-agent (Windows) or ~/.clawksis/clawksis-agent.
  ...
  //    to spawning clawk. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `clawk update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `clawk` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CLAWK_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CLAWK_DESKTOP_IGNORE_EXISTING !== '1') {
    let clawkCommand = null
    const clawkOverride = process.env.CLAWK_DESKTOP_CLAWKSIS

    if (clawkOverride) {
      const resolvedOverride = findOnPath(clawkOverride)

      if (resolvedOverride) {
        clawkCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(clawkOverride, { isWsl: IS_WSL })) {
        clawkCommand = clawkOverride
      } else {
        rememberLog(`Ignoring Windows Clawksis override under WSL: ${clawkOverride}`)
      }
    } else {
      clawkCommand = findOnPath('clawk')
    }

    if (clawkCommand) {
      if (looksLikeDesktopAppBinary(clawkCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Clawksis CLI: ${clawkCommand}`)
        clawkCommand = null
      }
    }

    if (clawkCommand) {
      const unwrapped = unwrapWindowsVenvClawksisCommand(clawkCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `clawk` shim
      ...
      const shellForProbe = isCommandScript(clawkCommand)

      if (verifyClawksisCli(clawkCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvClawksisCommand(clawkCommand, backendArgs) || {
            label: `existing Clawksis CLI at ${clawkCommand}`,
            command: clawkCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Clawksis CLI at ${clawkCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed clawk_cli module via system Python.
  ...
  if (python) {
    ...
    // having clawk_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\clawk\clawksis-agent\venv.
    if (canImportClawksisCli(python)) {
      return {
        kind: 'python',
        label: `installed clawk_cli module via ${python}`,
        command: python,
        args: ['-m', 'clawk_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: clawk_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveClawksisBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Clawksis not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveClawksisBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Clawksis install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Clawksis recovery was handed off to Clawksis Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      clawkHome: CLAWK_HOME,
      logRoot: path.join(CLAWK_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Clawksis install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Clawksis bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(CLAWK_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startClawksis() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // clawk:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveClawksisBackend(backend.args))
  }
  ...
  if (!isClawksisSourceRoot(ACTIVE_CLAWK_ROOT)) {
    throw new Error(
      `Clawksis install at ${ACTIVE_CLAWK_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Clawksis' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\clawk\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `clawk` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Clawksis on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Clawksis.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isClawksisSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Clawksis venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Clawksis at ${ACTIVE_CLAWK_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Clawksis runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----clawk-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Clawksis backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-Clawksis-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Clawksis backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Clawksis backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Clawksis-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('clawk:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveClawksisCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('clawk:preview-file-changed', payload)
}
...
async function waitForClawksis(baseUrl, token) {
  ...
  throw new Error(`Clawksis backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('clawk:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('clawk:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Clawksis...".
function sendPowerResume() {
  ...
  webContents.send('clawk:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('clawk:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('clawk:window-state-changed', state)
}
...
//
// Hosted Clawksis gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``clawk_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / clawk #37247).
//     Both are set as HttpOnly cookies (``clawk_session_at`` ~15 min,
//     ``clawk_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:clawk-remote-oauth'
...
// "not signed in", which on the initial boot path (startClawksis → the renderer's
// single-shot boot() with no retry) surfaced as the "Clawksis couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Clawksis
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Clawksis Cloud agent…' : 'Sign in to Clawksis gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Clawksis backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Clawksis Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:clawk-remote-oauth`:
...
// convention (clawk_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Clawksis surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.CLAWK_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Clawksis gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for clawk_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Clawksis Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Clawksis Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Clawksis Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Clawksis Cloud. Open Settings → Gateway, choose Clawksis Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Clawksis Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Clawksis Cloud session has expired. Sign in to Clawksis Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Clawksis Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root CLAWK_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.CLAWK_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.CLAWK_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  // The env override forces a plain remote connection. Otherwise reflect the
  // saved mode, preserving 'cloud' (a Clawksis Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted Clawksis Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by CLAWK_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Clawksis Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Clawksis Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (clawk #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Clawksis gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Clawksis gateway is selected, but no session token is saved. ' +
...
//   2. env override (CLAWK_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.CLAWK_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.CLAWK_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'CLAWK_DESKTOP_REMOTE_URL is set but CLAWK_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Clawksis backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.CLAWK_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startClawksis() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Clawksis
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startClawksis())
  ...
  // connect to Clawksis gateway". Mirror the renderer's connect here so the test
...
function resetClawksisConnection({ soft = false } = {}) {
  backendStartFailure = null
  const clawkProcess = backendConnectionState.invalidate()
  stopBackendChild(clawkProcess)
...
// startClawksis() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetClawksisConnection() invalidates it.
  const clawkProcess = backendConnectionState.getProcess()
  const dying = clawkProcess && !clawkProcess.killed ? clawkProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('clawk:connection:applied')
}
...
// profile to startClawksis() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startClawksis() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForClawksis(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: clawkLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited CLAWK_HOME env (see _apply_profile_override
  // step 3 in clawk_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveClawksisBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const clawkCwd = resolveClawksisCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Clawksis backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: clawkCwd,
        CLAWK_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        CLAWK_DESKTOP: '1',
        CLAWK_WEB_DIST: webDist,
        ...(readyFile ? { CLAWK_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Clawksis backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Clawksis backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Clawksis backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForClawksis(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Clawksis backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_clawk_home() recreates the deleted profile directory.
//
...
async function startClawksis() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startClawksis() call re-throws the same error
  ...
  const connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Clawksis backend', 8)
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Clawksis backend at ${remote.baseUrl}`, 24)
      await waitForClawksis(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Clawksis backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.clawksis/active_profile file) and
    // resolves CLAWK_HOME the same way `clawk -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Clawksis runtime', 28)
    const backend = await ensureRuntime(resolveClawksisBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const clawkCwd = resolveClawksisCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Clawksis backend via ${backend.label}`, 84)
    rememberLog(`Starting Clawksis backend via ${backend.label}`)

    const clawkProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: clawkCwd,
        env: {
          ...process.env,
          // Explicitly pin CLAWK_HOME for the child so Python's get_clawk_home()
          // resolves to the SAME location our resolveClawkHome() picked. Without
          // this pin, Python falls back to ~/.clawksis on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\clawk, which differs from C:\Users\<u>\.clawk.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets CLAWK_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, clawkProcess)

    if (!processOwner) {
      stopBackendChild(clawkProcess)
      throw new Error('Clawksis backend start was superseded by a newer connection attempt.')
    }

    clawkProcess.stdout.on('data', rememberLog)
    clawkProcess.stderr.on('data', rememberLog)
    ...
    clawkProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Clawksis backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Clawksis backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Clawksis backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Clawksis backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    clawkProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Clawksis backend exit (${signal || code})`)
      ...
      rememberLog(`Clawksis backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Clawksis backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Clawksis backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentClawksisLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Clawksis backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(clawkProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Clawksis backend to become ready', 90)
    await Promise.race([waitForClawksis(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => clawkProcess.exitCode === null && !clawkProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Clawksis backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Clawksis is
...
// pushes pet state over IPC (clawk:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via clawk:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Clawksis app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Clawksis icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see clawk:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Clawksis in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('clawk:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  startClawksis().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('clawk:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Clawksis…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('clawk:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetClawksisConnection only
    // clears the connection promise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Clawksis backend failed liveness probe; dropping stale connection.')
    resetClawksisConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('clawk:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('clawk:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('clawk:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('clawk:window:openNewSession', async () => {
...
ipcMain.handle('clawk:zoom:get', event => {
...
ipcMain.on('clawk:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('clawk:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('clawk:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('clawk:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('clawk:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('clawk:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('clawk:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('clawk:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('clawk:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('clawk:pet-overlay:control', payload)
})
ipcMain.handle('clawk:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startClawksis() call restarts the
...
ipcMain.handle('clawk:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startClawksis() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('clawk:bootstrap:cancel', async () => {
...
ipcMain.handle('clawk:boot-progress:get', async () => bootProgressState)
ipcMain.handle('clawk:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('clawk:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('clawk:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('clawk:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('clawk:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('clawk:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Clawksis Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('clawk:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('clawk:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('clawk:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on clawk:connection:applied.
...
ipcMain.handle('clawk:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('clawk:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CLAWK_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('clawk:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('clawk:requestMicrophoneAccess', async () => {
...
ipcMain.handle('clawk:api', async (_event, request) => {
  ...
  // backend calls ensure_clawk_home() which recreates the profile directory,
...
ipcMain.handle('clawk:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('clawk:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('clawk:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('clawk:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('clawk:readFileText', async (_event, filePath) => {
...
ipcMain.handle('clawk:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('clawk:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('clawk:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('clawk:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('clawk:saveClipboardImage', async () => {
...
ipcMain.handle('clawk:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('clawk:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('clawk:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('clawk:titlebar-theme', (_event, payload) => {
...
ipcMain.on('clawk:native-theme', (_event, mode) => {
...
ipcMain.on('clawk:translucency', (_event, payload) => {
...
ipcMain.handle('clawk:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('clawk:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveClawksisCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('clawk:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveClawksisCwd()
}))

ipcMain.handle('clawk:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('clawk:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('clawk:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('clawk:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('clawk:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('clawk:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: clawkLog.slice(-200) }))
...
function terminalShellCommand() {
  // CLAWK_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.CLAWK_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Clawksis' TUI into
  ...
  env.TERM_PROGRAM = 'Clawksis'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a clawk/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from CLAWK_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.CLAWK_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `clawk:terminal:${id}:${suffix}`
}
...
ipcMain.handle('clawk:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('clawk:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('clawk:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('clawk:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('clawk:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('clawk:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('clawk:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('clawk:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('clawk:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('clawk:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('clawk:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('clawk:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('clawk:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('clawk:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('clawk:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('clawk:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('clawk:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('clawk:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('clawk:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('clawk:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('clawk:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('clawk:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('clawk:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('clawk:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('clawk:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('clawk:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('clawk:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('clawk:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('clawk:terminal:write', (_event, id, data) => {
...
ipcMain.handle('clawk:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('clawk:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('clawk:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('clawk:updates:check', async () =>
...
ipcMain.handle('clawk:updates:apply', async (_event, payload) =>
...
ipcMain.handle('clawk:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('clawk:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Clawksis version (the one `release.py` bumps in
// clawk_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Clawksis version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveClawksisVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'clawk_cli', '__init__.py')
...
// Re-resolve the live Clawksis version and push it into the native About panel
// just before showing it, so an in-place `clawk update` is reflected without
...
ipcMain.handle('clawk:version', async () => ({
  appVersion: resolveClawksisVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  clawkRoot: resolveUpdateRoot()
}))
...
// the actual removal via `clawk uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (clawk_cli/uninstall.py + clawk_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_CLAWK_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isClawksisSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'clawk_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, CLAWK_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Clawksis agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import clawk_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_CLAWK_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `clawk-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `clawk-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('clawk:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('clawk:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('clawk:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('clawk:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// clawk:// deep links (e.g. clawk://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const CLAWK_PROTOCOL = 'clawk'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${CLAWK_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // clawk://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('clawk:deep-link', payload)
...
ipcMain.handle('clawk:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${CLAWK_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(CLAWK_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(CLAWK_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `clawk://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching clawk:// URL is in our own argv.
