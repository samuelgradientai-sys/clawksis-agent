...
// no longer bundles the Clawksis Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Clawksis.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Clawksis'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Clawksis.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'clawk'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// CLAWK_HOME and never touches this.
const DEFAULT_CLAWK_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'clawk')
  }
  return path.join(os.homedir(), '.clawk')
})()
const VENV_ROOT = path.join(DEFAULT_CLAWK_HOME, 'clawksis-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'clawk-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.CLAWK_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Clawksis-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Clawksis-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Clawksis-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Clawksis-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CLAWK_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CLAWK_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const clawkHome = path.join(sandbox, 'clawk-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(clawkHome, { recursive: true })
  ...
  env.CLAWK_DESKTOP_CWD = cwd
  env.CLAWK_DESKTOP_IGNORE_EXISTING = '1'
  env.CLAWK_DESKTOP_TEST_MODE = 'fresh-install'
  env.CLAWK_DESKTOP_USER_DATA_DIR = userDataDir
  env.CLAWK_HOME = clawkHome
  delete env.CLAWK_DESKTOP_HERMES
  delete env.CLAWK_DESKTOP_CLAWK_ROOT
  ...
  console.log(`  CLAWK_HOME: ${clawkHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(clawkHome, 'clawksis-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Clawksis Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of clawk_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'clawksis-agent', 'clawk_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Clawksis
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CLAWK_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CLAWK_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
