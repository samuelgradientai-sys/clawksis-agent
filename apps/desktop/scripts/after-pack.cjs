 ...
 *
 * Stamps the Clawksis icon + identity onto the packed Windows Clawksis.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `clawk desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Clawksis')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Clawksis'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Clawksis.exe keeps the stock Electron icon`)
  }
}
