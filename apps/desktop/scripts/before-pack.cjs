 ...
 * product name (`Clawksis`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Clawksis'
 *
 * This is a hard failure with no obvious cause for the user — `clawk desktop`
 ...
 * (the mac-unpacked Clawksis.app bundle) and Windows (win-unpacked), so we
