//! Clawksis Setup — Tauri entrypoint.
...
/// desktop app handing off via `Clawksis-Setup.exe --update`).
...
pub fn run() {
    ...
    // Tracing → bootstrap-installer.log under CLAWK_HOME/logs/ so install
    ...
    // Clawksis is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Clawksis installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Clawksis is already installed should NOT show the installer or

            // rebuild — it should just open the app, so the /Applications

            // "Clawksis" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Clawksis" shortcuts that install.ps1 creates, and a

            // reliable detached relaunch there needs the DETACHED_PROCESS +

            // startup-grace handling used by launch_clawk_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                ...
                let install_root = paths::clawk_home().join("clawksis-agent");

                if bootstrap::clawk_is_installed(&install_root) {
                    ...
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        ...
                        Ok(()) => {
                            ...
                            // before we exit (mirrors launch_clawk_desktop).

                            std::thread::sleep(std::time::Duration::from_millis(200));

                            tracing::info!(

                                "clawk already installed — relaunched desktop; exiting installer"
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            bootstrap::launch_clawk_desktop,

            // Diagnostics

            paths::get_log_path,

            paths::get_clawk_home,
        ...
        .expect("error while running Clawksis Setup");
