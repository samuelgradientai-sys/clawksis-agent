...
//!   1. Dev shortcut: a sibling repo checkout via $CLAWK_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Clawksis-Setup.exe is meant
...
/// `pin` is the commit-or-branch from either Clawksis-Setup's build-time
...
pub async fn resolve(
    ...
    if let Ok(repo_root) = std::env::var("CLAWK_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    ...
    let response = reqwest::Client::new()
        ...
        .header("User-Agent", "clawk-setup/0.0.1")
