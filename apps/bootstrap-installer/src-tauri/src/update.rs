...
//! Driven when the installer is launched as `Clawksis-Setup.exe --update` (see

//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:

//!

//!   1. wait for the old Clawksis desktop process to fully exit (so the venv

//!      shim is free; otherwise `clawk update` aborts with exit code 2),

//!   2. run `clawk update --yes --gateway` (Python/repo update; this does NOT

//!      rebuild apps/desktop by design — see cmd_update in clawk_cli/main.py),

//!   3. run `clawk desktop --build-only` (the rebuild step update skips),
...
//! Cross-platform note: `clawk update` already handles macOS/Linux (git/pip).

//! The only OS-specific bits here are the venv shim path (resolve_clawk) and
...
/// `clawk update` exit code meaning "another clawk process is holding the

/// venv shim open / dirty precondition" — see _cmd_update_impl in

/// clawk_cli/main.py (sys.exit(2)). We surface a targeted message for this.
...
/// before giving up and letting `clawk update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
...

    {
        ...
        let mut stages = vec![
            ...
            stage_info("update", "Updating Clawksis"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let clawk_home = crate::paths::clawk_home();

    let install_root = clawk_home.join("clawksis-agent");
    ...
    let clawk = resolve_clawk(&install_root).ok_or_else(|| {

        let msg = format!(

            "Could not find the clawk CLI under {}. Is Clawksis installed? \
    ...
    // async on Windows. If it still holds the venv shim, `clawk update`
    ...
    // ---- stage 1: clawk update -----------------------------------------

    // Pass --branch so `clawk update` targets the branch this installer was
    ...
    // without --branch, `clawk update` switches the checkout to `main` (a
    ...
    // --force skips `clawk update`'s Windows running-exe guard (which would
    ...
    // time `clawk update` runs there is no legitimate clawk.exe to protect,

    // and the guard would only produce a false "Clawksis is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `clawk update` lazily imports
    ...
    // an updater-path contract across that boundary (e.g. upstream #39780's

    // `_UvResult`, whose `__iter__` injected a bool into the argv and crashed

    // Windows `list2cmdline` with `TypeError: sequence item 1: expected str

    // instance, bool found`, fixed in #39820) therefore kills the FIRST update

    // on the parked population — even though the fix is already on disk by then.

    // A second `clawk update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Clawksis" state
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            ...
            let msg = "Clawksis is still running. Close all Clawksis windows and try \
        ...
        other => {
            ...
            let msg = format!(
                ...
                "clawk update failed (exit {:?}). See {} for details.",

                other,

                crate::paths::clawk_home()
    ...
    // ---- stage 2: clawk desktop --build-only ----------------------------

    // `clawk update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        ...
        let msg = format!(
            ...
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             ...
             applied but the app could not be rebuilt; run `clawk desktop` \
    ...
    if let Some(target_app) = launch_target {
        ...
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            ...
            emit_log(
                ...
                &format!("[update] could not auto-launch desktop: {err}. Launch Clawksis manually."),
        ...
        crate::bootstrap::launch_clawk_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    ...
    let shim = venv_clawk(install_root);

    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;



    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Clawksis to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Last resort: a backend clawk.exe (or a grandchild it spawned)
            ...
            // every clawk.exe except ourselves, then give the OS a beat to
            ...
            emit_log(
                ...
                "[update] Clawksis still holding the venv shim; force-killing stragglers…",

            );

            force_kill_other_clawk();
...
/// Force-kill any `clawk.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `clawk.exe` image tree via
...
/// which completes BEFORE we spawn `venv\Scripts\clawk.exe update`. At this

/// point no update-driven clawk.exe exists yet, so the only clawk.exe images
...
/// Spawn `clawk <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv clawk shim under an install root, regardless of existence.

fn venv_clawk(install_root: &Path) -> PathBuf {

    if cfg!(target_os = "windows") {

        install_root.join("venv").join("Scripts").join("clawk.exe")

    } else {

        install_root.join("venv").join("bin").join("clawk")
...
/// Resolve the clawk CLI to drive. Prefer the venv shim in the install we

/// just updated; fall back to `clawk` on PATH.

fn resolve_clawk(install_root: &Path) -> Option<PathBuf> {
    ...
    let exe = if cfg!(target_os = "windows") { "clawk.exe" } else { "clawk" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        ...
        "CLAWK_HOME".to_string(),

        clawk_home.as_os_str().to_os_string(),

    )];

    if let Some(path) = path_with_prepended_entries(&[

        clawk_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_clawk_desktop_app(install_root).ok_or_else(|| {

        anyhow!(

            "desktop rebuild succeeded but no Clawksis.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.clawk-update-new", target_app.display()));

    let old = PathBuf::from(format!("{}.clawk-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        ...
        .current_dir(crate::paths::clawk_home())
...
mod tests {
    ...
    fn venv_clawk_is_under_install_root() {

        let root = Path::new("/x/clawksis-agent");

        let shim = venv_clawk(root);
    ...
    fn parses_only_app_targets() {
        ...
        assert_eq!(
            ...
            target_app_from_args(["--update", "--target-app", "/Applications/Clawksis.app"]),

            Some(PathBuf::from("/Applications/Clawksis.app"))
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Clawksis.app");

        let tmp = base.join("Clawksis.app.clawk-update-new");

        let old = base.join("Clawksis.app.clawk-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let tmp = base.join("Clawksis.app.clawk-update-new"); // intentionally absent
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let tmp = base.join("Clawksis.app.clawk-update-new"); // absent
