...
export const $clawkHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.clawksis/logs/bootstrap-installer.log')
    $clawkHome.set('~/.clawksis')
  ...
  try {
    const [logPath, clawkHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_clawk_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $clawkHome.set(clawkHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Clawksis" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Clawksis-Setup.exe --update);
...
export async function launchClawksisDesktop(): Promise<void> {
  ...
  await invoke('launch_clawk_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Clawksis repository' },
