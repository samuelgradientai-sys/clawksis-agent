# Clawksis Has Had "Routines" Since March
...
Clawksis equivalent — works today:
```bash
...
Clawksis equivalent — works today:
```bash
clawk webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Clawksis implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Clawksis |
|---|---|---|
...
## Things Clawksis Does That Routines Can't
...
Clawksis has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Clawksis is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
clawk gateway setup    # enable webhooks
clawk webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [clawksis-agent.nousresearch.com/docs/guides/automation-templates](https://clawksis-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [clawksis-agent.nousresearch.com](https://clawksis-agent.nousresearch.com)

GitHub: [github.com/samuelgradientai-sys/clawksis-agent](https://github.com/samuelgradientai-sys/clawksis-agent)

---

*Clawksis is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
