...
``clawk_cli/main.py``) so the LSP module ships self-contained.
...
def register_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Wire the ``clawk lsp`` subcommand tree into the main argparse."""
...
def run_lsp_command(args: argparse.Namespace) -> int:
    """Top-level dispatcher for ``clawk lsp <subcommand>``."""
