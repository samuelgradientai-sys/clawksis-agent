...
#
# When a user invokes a /skill (or /bundle), the turn is expanded into a
# model-facing message that embeds the full skill body plus scaffolding. That
# expanded text flows into the agent loop — and into memory providers via
# MemoryManager. Providers that store/embed the raw user turn (mem0, hindsight,
# supermemory, …) would otherwise capture the entire skill body instead of what
# the user actually asked. ``extract_user_instruction_from_skill_message``
# recovers just the user's instruction so memory stays clean.
#
# These markers MUST stay byte-identical to the builders that produce them:
# ``_build_skill_message`` (below, single skill) and
# ``build_bundle_invocation_message`` (agent/skill_bundles.py, bundles).
# ---------------------------------------------------------------------------
...
def extract_user_instruction_from_skill_message(content: Any) -> Optional[str]:
    ...
    Returns:
        ...
        - ``None`` when the content is skill scaffolding with no user
          instruction (a bare ``/skill`` invocation). Callers feeding memory
          providers should skip the turn in that case — there is no user
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``CLAWK_PLATFORM`` env var and

    ``CLAWK_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("CLAWK_PLATFORM") or get_session_env(
            "CLAWK_SESSION_PLATFORM"
        )

    except Exception:
        resolved_platform = os.getenv("CLAWK_PLATFORM")
...
def _load_skill_payload(
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.clawksis/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.clawk.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_clawk_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.clawksis/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.clawksis/skills/`` and any ``skills.external_dirs`` so the
