...
A pre-run snapshot of ``~/.clawksis/skills/`` (excluding ``.curator_backups/``

itself) is taken before any mutating curator pass. Snapshots are tar.gz

files under ``~/.clawksis/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.clawksis/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    ...
    return get_clawk_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.clawksis/cron/jobs.json``)."""

    return get_clawk_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    ...
    if not skills.exists():
        logger.debug("No ~/.clawksis/skills/ directory — nothing to back up")
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.clawksis/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `clawk curator rollback --list` to see available snapshots)",
            None,
        )
