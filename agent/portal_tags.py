...
Every Clawksis request that hits the Nous Portal — main agent loop, auxiliary

client (compression / titles / vision / web_extract / session_search / etc.),

and any future code path — must carry the same product-attribution tags so

Nous can attribute usage to Clawksis and bucket it by client release.
...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        ...
        "product=clawksis-agent",

        "client=clawk-client-v<__version__>",

    ]



The version is sourced live from ``clawk_cli.__version__`` so it auto-aligns
...
``clawk_cli.__version__`` is the canonical source of truth.
...
def _clawk_version() -> str:
    """Return the current Clawksis release version, e.g. ``"0.13.0"``.



    Falls back to ``"unknown"`` if ``clawk_cli`` cannot be imported (should
...
def clawk_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.



    Format: ``client=clawk-client-v<MAJOR>.<MINOR>.<PATCH>``.

    """

    return f"client=clawk-client-v{_clawk_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    return ["product=clawksis-agent", clawk_client_tag()]
