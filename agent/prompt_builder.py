...
_CLAWK_MD_NAMES = (".clawk.md", "CLAWKSIS.md")


def _find_clawk_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.clawk.md`` or ``CLAWKSIS.md``.
    ...
    # could pick up a .clawk.md planted in /tmp, /home, etc.
...
CLAWK_AGENT_HELP_GUIDANCE = (
    ...
    "information. Load the `clawksis-agent` skill with skill_view(name='clawksis-agent') "
...
# Fork-only: Scrapling / anti-bot escalation ladder for web fetching.
# Imported by agent/system_prompt.py — do NOT drop on upstream syncs.
SCRAPING_GUIDANCE = (
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.clawksis/kanban.db`. Your task id is in "
    "`$CLAWK_KANBAN_TASK`; your workspace is `$CLAWK_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $CLAWK_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $CLAWK_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${CLAWK_KANBAN_BRANCH:-wt/$CLAWK_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$CLAWK_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`clawk profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `clawk kanban <verb>` for board operations. Use "
...
#
# The clawksis-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to clawksis-agent's Python
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "\n"
    "<tool_use_discretion>\n"
    "- Use tools only when they materially improve correctness, verification, file access, execution, browsing, automation, or safety.\n"
    "- For simple conversational replies, greetings, definitions, short examples, short creative outputs, small lists, or plain-language explanations, answer directly without tools.\n"
    "- Do not use `execute_code` for trivial examples, tiny arithmetic, short random/example lists, greetings, definitions, or simple explanatory answers.\n"
    "- If the user explicitly asks you to run code, test code, inspect files, verify something, calculate precisely, or produce reproducible data, then use the appropriate tool.\n"
    "</tool_use_discretion>\n"
    "\n"
    "<mandatory_tool_use>\n"
    "NEVER answer these from memory or mental computation — ALWAYS use a tool:\n"
    "- Non-trivial arithmetic, math, calculations, data processing, simulations, or results that must be exact/reproducible → use terminal or execute_code\n"
    "- Trivial arithmetic, simple estimates, examples, or short random/example lists may be answered directly when exact reproducibility is not required\n"
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `clawk computer-use doctor` and share the output. That command "
...
PLATFORM_HINTS = {
    ...
    "desktop": (
        "You are chatting inside the Clawksis desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Clawksis WebUI, a browser-based chat interface. "
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Clawksis itself is running. The host OS, home, and cwd "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Clawksis "
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Clawksis itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `clawksis-agent` skill "
            "first. It has the actual commands (e.g. `clawk config set …`, `clawk tools`, "
            "`clawk setup`) so you don't have to guess or invent workarounds.\n"
...
def _load_clawk_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.clawk.md / CLAWKSIS.md — walk to git root."""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .clawk.md / CLAWKSIS.md  (walk to git root)
    ...
    # Never let a FALLBACK-picked directory inside the Clawksis install/source
    ...
    # explicitly configured cwd is honored verbatim — the Clawksis tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Clawksis in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Clawksis install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_clawk_md(cwd_path, context_length)
