...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``CLAWK_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `clawk update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Clawksis owns auxiliary retry + provider/model fallback policy (the
    ...
    # Clawksis sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Clawksis control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Clawksis triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/samuelgradientai-sys/clawksis-agent",
    "X-Title": "Clawksis",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``CLAWK_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``CLAWK_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("CLAWK_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("CLAWK_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_AUTH_JSON_PATH = get_clawk_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Clawksis)",
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Clawksis plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``clawk_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.clawksis/auth.json for an active Nous provider.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``clawk auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``clawk_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Clawksis auth store.
    ...
    try:
        from clawk_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from clawk_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `clawk setup` or `clawk model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from clawk_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _get_provider_chain() -> List[tuple]:
    ...
    NOTE: ``nous`` is deliberately NOT in this chain either — Clawksis is
    BYOK and does not probe Nous Portal automatically.
    """
    return [
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second clawk
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``clawk config aux reset``)."""
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Clawksis' built-in discovery
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `clawk model` but the
    #    old OPENAI_BASE_URL lingers in ~/.clawksis/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: clawk model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.clawksis/.env",
    ...
    if main_provider == "moa":
        try:
            from clawk_cli.config import load_config
            from clawk_cli.moa_config import resolve_moa_preset
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from clawk_cli.runtime_provider import _get_named_custom_provider
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Clawksis owns
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: clawk model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: clawk model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: clawk doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("clawk_cli.auth not available for provider %s", provider)
        return None, None
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`clawk_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    # the normalized Clawksis reasoning config through a private kwarg so the
...
def call_llm(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Clawksis reasoning config for direct model calls
