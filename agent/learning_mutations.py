...
- **skills** → the skill name (e.g. ``"debugging-clawk-desktop"``)
...
shared by the CLI (``clawk journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``clawk curator restore``); deleting a memory rewrites its
...
def _memories_dir() -> Path:
    ...
    return get_clawk_home() / "memories"
