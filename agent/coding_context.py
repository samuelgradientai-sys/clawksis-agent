"""Coding-context awareness — base Clawksis, every interactive surface.

When the user runs Clawksis inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Clawksis shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("clawk.coding_context")
...
# in _CLAWK_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``CLAWK_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Clawksis should operate in its coding posture right now."""
