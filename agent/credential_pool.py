...
# un-throttled log fires on *every* selection. On Windows several Clawksis
...
# readiness handshake ("Timed out connecting to Clawksis backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``clawk_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``clawk model`` / ``clawk auth`` which
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``clawk`` invocation that defaults to the active provider
        (e.g. setup wizard, ``clawk auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # clawk_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Clawksis
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``CLAWK_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "CLAWK_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "CLAWK_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("clawk_pkce"),
                )
                updated = replace(
                    entry,
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Clawksis process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `clawk model` / `clawk auth` after a 429/401,
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``clawk auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `clawk auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `clawk auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Clawksis PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `clawk setup` ("Claude
        ...
        # Claude Code CLI, clawk_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.clawksis/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `clawk setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `clawk auth add nous --label <name>`).  Fall back to the
        ...
        # Clawksis auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.clawksis/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `clawk auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `clawk auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Clawksis auth store.  Without this gate the removal is instantly
        ...
        # Clawksis owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``clawk model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``clawk auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from clawk_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.clawksis/.env over os.environ — the user's config file is the
    # authoritative source for Clawksis credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_clawk_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `clawk auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.clawksis/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.clawksis/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `clawk auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Clawksis
