"""Anthropic Messages API adapter for Clawksis.

Translates between Clawksis's internal OpenAI-style message format and
...
# Clawksis effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See clawksis-agent#13848, #17057.
    """
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Clawksis notices an expired token, Claude Code may have already rotated it.
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``clawk models``) never mutates ``~/.clawksis/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Clawksis)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.clawksis/auth.json)
    ...
    # 1. Clawksis-managed OAuth/setup token env var
    ...
    # 4. Clawksis credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Clawksis configs.
...
# ── Clawksis-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.clawksis/.anthropic_oauth.json (our own file).
...
def run_clawk_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Clawksis-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Clawksis with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from clawk_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_clawk_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Clawksis-managed OAuth credentials from ~/.clawksis/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Clawksis OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See clawksis-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See clawksis-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See clawksis-agent#13848 (Kimi) and
    clawksis-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Clawksis", "Claude Code")
                text = text.replace("Clawksis agent", "Claude Code")
                text = text.replace("clawksis-agent", "claude-code")
        ...
        #      a) bare Clawksis-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Clawksis surfaces in its CLI. We
