...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Clawksis with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.clawksis/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Clawksis
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``clawk tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``clawk setup`` for
