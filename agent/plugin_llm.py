...
Plugins built on Clawksis often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Clawksis
...
:class:`~clawk_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Clawksis' main config
...
handles every provider, fallback chain, and per-task override Clawksis
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`clawk_cli.plugins.PluginContext`
