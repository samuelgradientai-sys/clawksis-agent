...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    blocked_exact.add(clawk_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(clawk_home / rel for rel in _SENSITIVE_CLAWK_DIRS)
    ...
    # message text into reference expansion, so `@file:~/.clawksis/auth.json` from a
