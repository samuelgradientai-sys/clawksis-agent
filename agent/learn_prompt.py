...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the Clawksis
     skill-authoring standards (description <=60 chars, the modern section
     order, Clawksis-tool framing, no invented commands).
...
# (HARDLINE)" and the clawksis-agent-dev new-skill salvage reference. Embedded in
# the prompt so the agent authors skills the way a maintainer would by hand.
_AUTHORING_STANDARDS = """\
Follow the Clawksis skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Clawksis`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Clawksis.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.clawk.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Clawksis tools.
...
Clawksis-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Clawksis tools by name in backticks: `terminal`, `read_file`,
