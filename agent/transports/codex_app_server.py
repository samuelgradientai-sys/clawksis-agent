...
projection into Clawksis' display, approval bridging, transcript projection into
...
"codex_app_server"`. Clawksis' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Clawksis secret — gateway
        ...
        spawn_env = clawk_subprocess_env(inherit_credentials=True)
        ...
        if spawn_env.get("CLAWK_KANBAN_TASK"):
            kanban_db = spawn_env.get("CLAWK_KANBAN_DB")
            ...
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "CLAWK_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("CLAWK_HOME", os.path.expanduser("~/.clawksis")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "clawk",
        client_title: str = "Clawksis",
