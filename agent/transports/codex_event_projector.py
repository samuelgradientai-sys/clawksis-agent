"""Projects codex app-server events into Clawksis' messages list.



The translator that lets Clawksis' memory/skill review keep working under the
...
Clawksis' message-alternation invariants (system → user → assistant → user/tool

→ assistant → ...). Multiple Codex tool calls within one Codex turn produce

multiple consecutive (assistant, tool) pairs, which is the same shape Clawksis
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Clawksis' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Clawksis stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Clawksis already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Clawksis' messages store text only.
