...
Owns one Codex thread per Clawksis session. Drives `turn/start`, consumes
...
# Clawksis' tools.terminal.security_mode → Codex's permissions profile id.

# Defaults if config is missing → workspace-write (matches Codex's own default).

_CLAWK_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Clawksis/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Clawksis session, lifetime owned by AIAgent.
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="clawk",
            client_title="Clawksis",
            client_version=_get_clawk_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Clawksis' text path is the common case).
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Clawksis' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own clawk-tools callback we

            # auto-accept — the user already approved Clawksis' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Clawksis approval choices onto codex's CommandExecutionApprovalDecision

    / FileChangeApprovalDecision wire values.



    Clawksis returns 'once', 'session', 'always', or 'deny'.
...
def _get_clawk_version() -> str:
    """Best-effort Clawksis version string for codex's userAgent line."""
