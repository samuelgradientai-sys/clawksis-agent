...
# Provider ID mapping: Clawksis ↔ models.dev

# ---------------------------------------------------------------------------


# Clawksis provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Clawksis (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_clawk_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``clawk config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Clawksis provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Clawksis traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Clawksis provider ID (e.g. "kilocode") or a models.dev

    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.

    """

    # Resolve Clawksis ID → models.dev ID
...
def get_model_info(provider_id: str, model_id: str) -> Optional[ModelInfo]:
    ...
    Accepts Clawksis or models.dev provider ID.  Tries exact match then
