...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Clawksis accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    Clawksis' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, Clawksis should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Clawksis state.

    The app-server owns the compacted thread context, so Clawksis should not
...
# ---------------------------------------------------------------------------
# Codex app-server → Clawksis UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal Clawksis tool loop. Without this bridge gateway
...
# Codex item types that map to a Clawksis tool_call in the projector (and
...
# clawk-tools-mcp-server subprocess that has no access to the parent
...
# the display event for those calls; we strip the mcp.clawk-tools.*
# namespacing and emit the bare tool name (web_search, browser_navigate,
# vision_analyze, ...) since the user thinks of these as Clawksis tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "clawk-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Clawksis tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (Clawksis' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into Clawksis' gateway UI callbacks.
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Clawksis'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Clawksis' standard prompt flow if a
        ...
        # explicitly opted out of Clawksis approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / CLAWK_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing Clawksis UI. Defaults (manual/smart/unset) preserve the
        ...
        # item/agentMessage/delta, ...) into Clawksis' gateway UI callbacks
