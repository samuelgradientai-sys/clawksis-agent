...
``auth.json``.  They intentionally have no dependency on ``clawk_cli.auth`` so
...
# Sources Clawksis owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Clawksis-owned OAuth/device-code state)
