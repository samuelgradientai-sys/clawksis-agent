...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Clawksis also honours CLAWK_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("CLAWK_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_clawk_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Clawksis.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Clawksis from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Clawksis' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Clawksis minimum.
...
def fetch_endpoint_model_metadata(
    ...
    for candidate in candidates:
        ...
        try:
            response = requests.get(
                url, headers=headers, timeout=(5, 10), verify=_resolve_requests_verify()
            )
            response.raise_for_status()
            payload = response.json()
            cache: Dict[str, Dict[str, Any]] = {}
...
def _get_context_cache_path() -> Path:
    ...
    return get_clawk_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Clawksis grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from clawk_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Clawksis sends to providers:
