...
Storage
-------
Bundles live in ``~/.clawksis/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CLAWK_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``clawk bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CLAWK_HOME.

    Honors ``CLAWK_BUNDLES_DIR`` for tests; falls back to
    ``<CLAWK_HOME>/skill-bundles``.
    """
    override = os.environ.get("CLAWK_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_clawk_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `clawk bundles` CLI subcommand.
# ---------------------------------------------------------------------------
