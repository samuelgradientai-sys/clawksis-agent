...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Clawksis starts and finds ``~/.openclaw/``.



    Points users at ``clawk claw migrate`` (non-destructive port of config,

    memory, and skills) first. ``clawk claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Clawksis, run "
        "`clawk claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `clawk claw cleanup` (renames it to ~/.openclaw.pre-migration — "
