...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `clawk backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - clawk_home (str): The active CLAWK_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.clawksis``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "clawk").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'clawk memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], clawk_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'clawk memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``clawk_home`` is the active CLAWK_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE CLAWK_HOME.

        ``clawk backup`` only walks CLAWK_HOME, so any provider state kept
        ...
        of the archive, and ``clawk import`` restores them to their original
