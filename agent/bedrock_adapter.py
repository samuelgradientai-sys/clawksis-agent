"""AWS Bedrock Converse API adapter for Clawksis.
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Clawksis with Bedrock support: pip install -e '.[bedrock]'"
        )
    # converse() / converse_stream() were added in boto3 1.34.59.
    # When Clawksis is installed editable into system Python, the system boto3
...
def convert_messages_to_converse(
    ...
    if converse_msgs and converse_msgs[-1]["role"] != "user":
        converse_msgs.append({
            "role": "user",
            "content": [{"text": _EMPTY_TEXT_PLACEHOLDER}],
        })
