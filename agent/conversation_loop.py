...
def run_conversation(
    ...
    while (
        ...
        if _runtime_context_error:
            ...
            agent._emit_status(
                "❌ Ollama runtime context is too small for Clawksis tool use"
            )
        ...
        while retry_count < max_retries:
            try:
                ...
                if (
                    ...
                    print(
                        f"{agent.log_prefix}     • Re-authenticate: clawk auth add nous"
                    )
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(
                            f"{agent.log_prefix}   Run `clawk doctor` for credential-chain diagnostics, or"
                        )
                    ...
                    print(
                        f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Clawksis-managed OAuth/setup tokens"
                    )
                    ...
                    print(
                        f'{agent.log_prefix}     • Legacy cleanup: clawk config set ANTHROPIC_TOKEN ""'
                    )

                    print(
                        f'{agent.log_prefix}     • Clear stale keys: clawk config set ANTHROPIC_API_KEY ""'
                    )
                    ...
                    if (
                        ...
                        if (
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(
                                    f"{agent.log_prefix}      2. Then run `clawk auth` to re-authenticate.",
                                    force=True,
                                )
                                ...
                                agent._vprint(
                                    f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `clawk model`.",
                                    force=True,
                                )
                                ...
                                agent._vprint(
                                    f"{agent.log_prefix}      1. Re-authenticate: clawk portal",
                                    force=True,
                                )
                            ...
                            agent._vprint(
                                f"{agent.log_prefix}      • Is the key valid? Run: clawk setup",
                                force=True,
                            )
