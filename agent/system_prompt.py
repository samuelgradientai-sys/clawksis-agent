...
upstream prefix cache warm.  See ``clawksis-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``CLAWK_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("CLAWK_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(
    ...
    AIAgent.  Clawksis never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CLAWK_HOME persona while keeping
    ...
    # Pointer to the clawksis-agent skill + docs for user questions about Clawksis itself.
    stable_parts.append(CLAWK_AGENT_HELP_GUIDANCE)
    ...
    # Coding posture (base Clawksis, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Clawksis profile the agent is running
    # under so it doesn't conflate ~/.clawksis/skills/ (default profile) with
    # ~/.clawksis/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        ...
        stable_parts.append(
            f"Active Clawksis profile: {active_profile}. This session reads "
            f"and writes {get_clawk_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_clawk_home()}/skills/, {get_clawk_home()}/plugins/, "
            f"{get_clawk_home()}/cron/, {get_clawk_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Clawksis). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Clawksis never rebuilds or reinjects parts of it
