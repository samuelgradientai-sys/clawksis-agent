...
Clawksis pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.clawksis/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<clawk_home>/bin/bws`` on
  first use.  Clawksis pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.clawksis/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``clawk`` invocations don't
  hammer the API.
* Failures NEVER block Clawksis startup.  Missing binary, no network,
...
# In-process cache so repeated load_clawk_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `clawk chat -q ...`
...
# <clawk_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.clawksis/.env (which
...
def _resolve_home(home_path: Optional[Path] = None) -> Optional[Path]:
    """Resolve the Clawksis home for cache paths (profile-aware).

    Clawksis-specific: the shared ``DiskCache`` falls back to a raw
    ``$CLAWK_HOME`` / ``~/.clawk`` read, which ignores the active profile.
    Resolving through :func:`clawk_constants.get_clawk_home` first keeps the
    bws cache inside the same home every other Clawksis surface uses.
    """
    if home_path is None:
        from clawk_constants import get_clawk_home

        home_path = get_clawk_home()
    return home_path


def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under clawk_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to the profile-aware Clawksis home when None.
    """
    return _DISK_CACHE.path(_resolve_home(home_path))
...
def _clawk_bin_dir() -> Path:
    """Where Clawksis stores its managed binaries.  Profile-aware."""
    from clawk_constants import get_clawk_home

    return get_clawk_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<clawk_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _clawk_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``clawk secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _clawk_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="clawk-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "clawksis-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<clawk_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
    ...
    if use_cache:
        ...
        disk_cached = _DISK_CACHE.read(
            cache_key, cache_ttl_seconds, _resolve_home(home_path)
        )
...
# ---------------------------------------------------------------------------
# Public entry point — called from clawk_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_clawk_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `clawk secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `clawk secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `clawk secrets bitwarden setup` to install."
        )
        return result
...
def _reset_cache_for_tests(home_path: Optional[Path] = None) -> None:
    ...
    """
    _CACHE.clear()
    _DISK_CACHE.clear(_resolve_home(home_path))
