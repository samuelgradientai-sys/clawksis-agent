...
cache problem must never block Clawksis startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Clawksis home used for cache paths.

    ``home_path`` is whatever ``load_clawk_dotenv()`` already resolved;
    falling back to ``$CLAWK_HOME`` / ``~/.clawksis`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<clawk_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if CLAWK_HOME is ever made traversable.
            try:
