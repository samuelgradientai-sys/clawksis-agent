"""Unified removal contract for every credential source Clawksis reads from.



Clawksis seeds its credential pool from many places:



    env:<VAR>     — os.environ / ~/.clawksis/.env

    claude_code   — ~/.claude/.credentials.json

    clawk_pkce   — ~/.clawksis/.anthropic_oauth.json
    ...
    manual        — user ran `clawk auth add`
...
unify here is **removal**:
    ...
    ``clawk auth remove <provider> <N>`` must make the pool entry stay gone.
...
qwen-cli, nous device_code (partial), clawk_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      ...
      1. Var lives only in ~/.clawksis/.env  → clear it
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.clawksis/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Clawksis.",
            f"  The pool entry is now suppressed — Clawksis will ignore "
            f"{env_var} until you run `clawk auth add {provider}`.",
        ])
...
def _remove_clawk_pkce(provider: str, removed) -> RemovalResult:
    """~/.clawksis/.anthropic_oauth.json is ours — delete it outright."""

    from clawk_constants import get_clawk_home

    result = RemovalResult()

    oauth_file = get_clawk_home() / ".anthropic_oauth.json"

    if oauth_file.exists():
        try:
            oauth_file.unlink()

            result.cleaned.append("Cleared Clawksis Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `clawk auth add nous` (or any other path that writes providers.nous)

    from re-seeding before the user has decided to.  Suppression forces

    them to go through `clawk auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``clawk auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``clawk auth add
    ...
    result.hints.append(
        "Run `clawk model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Clawksis auth store is not enough — _seed_from_singletons() would
    ...
    via ``clawk auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `clawk auth add openai-codex` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Clawksis stops picking the token up.
