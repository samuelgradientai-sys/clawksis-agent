"""Upload a Clawksis session transcript to Hugging Face as an agent trace.

Clawksis stores sessions in its own SQLite store (``clawk_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``clawk trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Clawksis' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "clawk-traces"
_CLAWK_VERSION = "clawksis-agent"
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Clawksis' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Clawksis conversation messages as Claude Code JSONL text.
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/clawk-traces`` dataset. Returns a
