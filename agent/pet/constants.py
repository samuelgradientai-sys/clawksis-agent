...
row taxonomy is inferred from the atlas shape so Clawksis can render both legacy
...
# rows (1536x1872), while older Clawksis/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Clawksis' activity state names. They are not always identical to the
...
# Legacy Clawksis/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Clawksis activity names -> accepted row-name aliases in descending
