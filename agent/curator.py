...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `clawk update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``clawk curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``clawk update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``clawk curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `clawk curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Clawksis' background skill CURATOR. This is an "
    ...
    "into ~/.clawksis/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: clawk-config-*, clawksis-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.clawksis/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.clawksis/logs/curator/``)
    ...
    authored skill data in ``~/.clawksis/skills/``.

    ``ensure_clawk_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_clawk_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every clawk install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        keep an umbrella stable: clawk curator pin document-tools
    ...
    lines.append("full report: clawk curator status")
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.clawksis/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.clawksis/skills/.archive/` for "
            "safety and can be restored via `clawk curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.clawksis/skills/.archive/`. "
            "Restore any via `clawk curator restore <name>`._\n"
        )
        SHOW = 50
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``clawk curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `clawk curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `clawk update` only if "
        ...
        # recorded in state so `clawk curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
    ...
    # When consolidation is off there is no LLM work to do — ``_llm_pass``
    # only records the prune-only run and writes the report. Run it inline
    # instead of spawning a background thread (no aux model is contacted, so
    # a thread buys nothing and makes the "no LLM was used" contract
    # unobservable to callers/tests).
    ...
    if not consolidate:
        # Explicit signal for callers/UI: the deterministic prune ran, the
        # LLM umbrella-consolidation pass did not.
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``clawk model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `clawk model` → auxiliary
