...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: clawk config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$CLAWK_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_clawk_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$CLAWK_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .clawk.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / CLAWK_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.clawksis/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from clawk_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.clawksis/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from clawk_logging import setup_logging, setup_verbose_logging
    setup_logging(clawk_home=_ra()._clawk_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that clawk_logging.setup_logging() does not
        ...
        # handler level inside clawk_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from clawk_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `clawk model` to "
                        "select a provider, or run `clawk setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff CLAWK_KANBAN_TASK is set).
    ...
    # Session logs go into ~/.clawksis/sessions/ alongside gateway sessions
    clawk_home = get_clawk_home()
    agent.logs_dir = clawk_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.clawksis/sessions/session_{sid}.json)
    ...
    try:
        from clawk_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from clawk_cli.config import load_config as _load_agent_config
    ...
    # management API (the historical Clawksis behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "clawk"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Clawksis' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "clawk", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, clawk, off.",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from clawk_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from clawk_cli.plugins import get_plugin_context_engine
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from clawk_cli.model_switch import _check_clawk_model_warning

            _clawk_warn = _check_clawk_model_warning(agent.model or "")
            if _clawk_warn:
                _user_msg = (
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_clawk_warn)
        except Exception:
            pass
