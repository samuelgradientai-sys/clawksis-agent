...
/* ------------------------------------------------------------------ */
/* Clawksis — Nous DS with the LENS_0 (Clawksis teal) lens applied   */
/* statically. Mirrors nousnet-web/(clawksis-agent)/layout.tsx so the    */
/* canonical Clawksis palette is the default — teal canvas + cream      */
/* accent — without relying on leva/gsap at runtime.                  */
/* ------------------------------------------------------------------ */

:root {
  /* LENS_0 — from design-language/src/ui/components/overlays/index.tsx.
     These are the defaults for the `default` (Clawksis Teal) dashboard theme;
  ...
  /* Data-series accents — consumed by Analytics + Models pages for the
     ...
     legend swatches). Defaults are tuned for the Clawksis-teal LENS_0
...
/* Nousnet's clawksis-agent layout bumps `small` and `code` to readable
