/**
 * Clawksis Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__CLAWK_PLUGIN_SDK__`` and
 * ``window.__CLAWK_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__CLAWK_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``ClawksisPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@clawk/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__CLAWK_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__CLAWK_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__CLAWK_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ClawksisPluginSDK {
...
declare global {
  interface Window {
    __CLAWK_PLUGIN_SDK__?: ClawksisPluginSDK;
    __CLAWK_PLUGINS__?: PluginRegistry;
  }
}
