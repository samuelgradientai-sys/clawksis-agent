...
Clawksis CLI - Interactive Terminal Interface



A beautiful command-line interface for the Clawksis, inspired by Claude Code.
...
Usage:
    ...
    python cli.py --skills clawksis-agent-dev,github-auth
...
os.environ["CLAWK_QUIET"] = "1"  # Our own modules
...
try:
    ...
    from clawk_cli.pt_input_extras import (
...
from clawk_cli.banner import _format_context_length, format_banner_version_label
...
from clawk_cli.browser_connect import (
...
load_clawk_dotenv(clawk_home=_clawk_home, project_env=_project_env)
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    env_path = os.getenv("CLAWK_PREFILL_MESSAGES_FILE", "").strip()
...
def load_cli_config() -> Dict[str, Any]:
    ...
    1. ~/.clawksis/config.yaml (user config - preferred)
    ...
    If CLAWK_IGNORE_USER_CONFIG=1 is set (via ``clawk chat --ignore-user-config``),

    the user config at ``~/.clawksis/config.yaml`` is skipped entirely and only the
    ...
    # Check user config first ({CLAWK_HOME}/config.yaml)

    user_config_path = _clawk_home / 'config.yaml'
    ...
    ignore_user_config = os.environ.get("CLAWK_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Clawksis, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",

                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",

                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",

                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Clawksis - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            ...
            # Recap tuning for /resume — see clawk_cli/config.py DEFAULT_CONFIG.
        ...
        try:
            ...
            if "model" in file_config:
                ...
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and ClawksisCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (clawk_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && clawk` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _CLAWK_GATEWAY marker)

    # where it was already set correctly by gateway/run.py's config bridge.

    _is_gateway = os.environ.get("_CLAWK_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            ...
            os.environ["CLAWK_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.clawksis/logs/.
...
try:
    ...
    from clawk_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``clawk --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    ...
    from clawk_cli.mcp_startup import wait_for_mcp_discovery
...
from clawk_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if os.environ.get("CLAWK_DEFER_AGENT_STARTUP") != "1":

        return

    _deferred_agent_startup_done = True

    _accept_hooks = os.environ.get("CLAWK_ACCEPT_HOOKS", "").lower() in {
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run clawk -w")
    ...
    wt_name = f"clawk-{short_id}"

    branch_name = f"clawk/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`clawk_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from clawk_constants import get_clawk_home as _get_clawk_home

        _clawk_home_maint = _get_clawk_home()
        ...
        try:
            ...
            if not session_db.get_meta("ghost_session_prune_v1"):
                ...
                pruned = session_db.prune_empty_ghost_sessions(
                    ...
                    sessions_dir=_clawk_home_maint / "sessions"
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`clawk_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``clawk/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        ...
        if not entry.is_dir() or not entry.name.startswith("clawk-"):
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    """Delete local ``clawk/clawk-*`` and ``pr-*`` branches with no worktree.



    These are auto-generated by ``clawk -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("clawk/clawk-") or b.startswith("pr-"))
...
#   1. CLAWK_LIGHT / CLAWK_TUI_LIGHT env (true/false) — explicit override

#   2. CLAWK_TUI_THEME=light|dark — explicit theme

#   3. CLAWK_TUI_BACKGROUND=#RRGGBB — explicit bg hint

#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light

#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly

#   6. Default: assume dark (matches the legacy Clawksis assumption)
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("CLAWK_LIGHT", "CLAWK_TUI_LIGHT"):
        ...
        theme = (os.environ.get("CLAWK_TUI_THEME") or "").strip().lower()
        ...
        bg_hint = os.environ.get("CLAWK_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        ...
        from clawk_cli.skin_engine import SkinConfig  # type: ignore[import]

    except Exception:

        return

    if getattr(SkinConfig, "_clawk_light_mode_hook_installed", False):
    ...
    SkinConfig._clawk_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Clawksis output it's common to display cron schedules verbatim.
...
from clawk_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_clawk_bp_timeout_patched`` sentinel on the module.
    ...
    try:
        ...
        if getattr(_vt100_mod, "_clawk_bp_timeout_patched", False):
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            ...
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._clawk_bp_start = None
                    ...
                    bp_start = getattr(self_parser, "_clawk_bp_start", None)

                    now = time.monotonic()

                    if bp_start is None:

                        self_parser._clawk_bp_start = now
        ...
        _vt100_mod._clawk_bp_timeout_patched = True
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Clawksis") if _skin else "Clawksis"
    ...
    if os.environ.get("CLAWK_FAST_STARTUP_BANNER") == "1":

        from clawk_cli import __release_date__ as _release_date

        from clawk_cli import __version__ as _version



        version_line = f"Clawksis v{_version} ({_release_date})"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        ...
        from clawk_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.clawksis/config.yaml (user config - preferred, used if it exists)
    ...
    try:
        ...
        # Ensure parent directory exists (for ~/.clawksis/config.yaml on first use)
...
class ClawksisCLI:
    ...
    def __init__(
        ...
        self.requested_provider = (
            ...
            or os.getenv("CLAWK_INFERENCE_PROVIDER")
        ...
        elif os.getenv("CLAWK_MAX_ITERATIONS"):

            try:

                self.max_turns = int(os.getenv("CLAWK_MAX_ITERATIONS", ""))
        ...
        # by `clawk chat --ignore-rules` in clawk_cli/main.py. When true we

        # pass skip_context_files=True and skip_memory=True to AIAgent so

        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.

        self.ignore_rules = ignore_rules or os.environ.get("CLAWK_IGNORE_RULES") == "1"
        ...
        self.system_prompt = (
            ...
            os.getenv("CLAWK_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Clawksis processes for this CLAWK_HOME.
        ...
        self._history_file = _clawk_home / ".clawk_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        try:
            ...
            from clawk_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            ...
            from clawk_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            ...
            try:
                ...
                from clawk_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            ...
            try:
                ...
                from clawk_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            fill = w - 2 - ClawksisCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        # `clawk chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `clawk auth add openai-codex`

        # without `clawk model`), fall back to the provider's first catalog
            ...
            if not session_meta:
                ...
                if _quiet_mode:
                    ...
                    print(
                        ...
                        "Use a session ID from a previous CLI run (clawk sessions list).",
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``clawk doctor``.

        Banner-cache rate-limits this to once per 24h per advisory; full

        remediation lives behind ``clawk doctor`` so the banner stays
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                ...
                f"[dim]   Clawksis needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
        ...
        # Warn if the configured model is a Nous Clawksis LLM (not agentic)

        from clawk_cli.model_switch import is_nous_clawk_non_agentic
        ...
        if is_nous_clawk_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "[bold yellow]⚠  Nous Research Clawksis 3 & 4 models are NOT agentic and are not "

                "designed for use with Clawksis.[/]"
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            ...
            if role == "user":
                ...
                lines.append("  ◆ Clawksis: ", style=f"bold {_assistant_label_c}")
                ...
                lines.append("  ◆ Clawksis: ", style=f"dim bold {_assistant_label_c}")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.clawksis/images/ and appends the path to

        ``_attached_images``.  Returns True if an image was attached.

        """

        from clawk_cli.clipboard import save_clipboard_image



        img_dir = get_clawk_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: clawk --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        """Handle /snapshot — lightweight state snapshots for Clawksis config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            print(f"  State snapshots ({display_clawk_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from clawk_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run clawk chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'clawk setup' to configure[/]")
    ...
    def show_help(self):
        ...
        from clawk_cli.commands import COMMANDS_BY_CATEGORY



        try:

            from clawk_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Clawksis!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in clawk_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from clawk_cli.main import _relative_time
            ...
            if self._session_db:
                ...
                try:
                    ...
                    self._session_db.create_session(
                        ...
                        source=os.environ.get("CLAWK_SESSION_SOURCE", "cli"),
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        _cprint("  Timed out waiting for the gateway. Is `clawk gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            _cprint("  Tip:   Use /history or `clawk sessions list` to find sessions.")
        ...
        if target.isdigit():
            ...
            from clawk_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `clawk sessions list` to see available sessions.")
    ...
    def save_conversation(self):
        ...
        """Save the current conversation to a JSON snapshot under ~/.clawksis/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``clawk --resume <id>``
        ...
        saved_dir = get_clawk_home() / "sessions" / "saved"
        ...
        path = saved_dir / f"clawk_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                ...
                print(f"       Resume the live session with: clawk --resume {self.session_id}")
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        from clawk_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `clawk model` and gateway pickers).
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Clawksis default (chat_completions)
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to clawk_cli.curator so the CLI and the `clawk curator`
        ...
        try:
            ...
            from clawk_cli.curator import cli_main
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        """Handle /skills slash command — delegates to clawk_cli.skills_hub."""

        from clawk_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print(f"  Configuration file: {display_clawk_home()}/config.yaml")
            ...
            print(f"    2. Or configure settings in {display_clawk_home()}/config.yaml")
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in clawk_cli/commands.py instead of touching every dispatch site.

        from clawk_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from clawk_cli.config import reload_env
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_clawk_home()}/plugins/ to get started.")
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``clawk bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                ...
                f"  {_DIM}Create one with: clawk bundles create "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Clawksis keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            ...
            from clawk_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print(f"  Custom skins: drop a YAML file in {display_clawk_home()}/skins/\n")
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        from clawk_cli.colors import Colors as _Colors
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``CLAWK_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
    ...
    def _toggle_yolo(self):
        ...
        ``CLAWK_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        """Handle /busy — control what Enter does while Clawksis is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            ...
            if arg == "queue":
                ...
                behavior = "Enter will queue follow-up input while Clawksis is busy."
                ...
                behavior = "Enter will interrupt the current run while Clawksis is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            ...
            from clawk_cli.models import _is_anthropic_fast_model
    ...
    def _handle_update_command(self) -> bool:
        ...
        """Handle /update — update Clawksis to the latest version.



        In the classic CLI this exits the session and relaunches as

        ``clawk update`` so the user sees update output directly and gets
        ...
        if is_managed():
            ...
            print(f"  ✗ {format_managed_message('update Clawksis')}")
        ...
        choices = [
            ...
            ("once", "Update Now", "exit the current session and update Clawksis"),
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from clawk_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        ...
        """Reload skills: rescan ~/.clawksis/skills/ and queue a note for the
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "clawk_voice"), exist_ok=True)
                    ...
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                ...
                                _dbg = _clawk_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard CLAWK_HOME, so no -p needed.
            ...
            print(f"  clawk --resume {self.session_id}{profile_flag}")

            if session_title:

                print(f"  clawk -c \"{session_title}\"{profile_flag}")
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        try:
            ...
            from clawk_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            ...
            from clawk_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Clawksis! Type your message or /help for commands.")

            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")

        except Exception:

            _welcome_text = "Welcome to Clawksis! Type your message or /help for commands."
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                ...
                self._console_print(
                    ...
                    f"(CLAWK_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Clawksis migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    ...
                    from clawk_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in clawk_cli.pt_input_extras
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Clawksis binding. See issue #22379.
        ...
        def handle_ctrl_z(event):
            ...
            agent_name = get_active_skin().get_branding("agent_name", "Clawksis")
        ...
        # Dynamic prompt: shows Clawksis symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Clawksis needs your input", preview_lines)
            ...
            lines.append(('class:clarify-title', 'Clawksis needs your input'))

            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Clawksis needs your input") - 3)) + '╮\n'))
        ...
        def _get_model_picker_display():
            ...
            scroll_offset, visible = ClawksisCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_clawk_osd_patched", False):
                ...
                _pt_renderer._clawk_osd_patched = True
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``CLAWK_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                ...
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        ...
                        _grace = float(os.getenv("CLAWK_SIGTERM_GRACE", "1.5"))
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the clawk process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: clawk setup"
...
def _run_kanban_goal_loop_q(cli: "ClawksisCLI", first_response: str) -> None:
    ...
    only when ``CLAWK_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("CLAWK_KANBAN_TASK") or "").strip()
    ...
    from clawk_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    Clawksis CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Clawksis Gateway (messaging platforms)...")
    ...
    # Default to clawk-cli toolset which includes cronjob management tools
    ...
    # mode registers its own inside ClawksisCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  CLAWK_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("CLAWK_KANBAN_TASK"):
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``clawk chat -q "work kanban task <id>"``;
        ...
        _kanban_task_id = os.environ.get("CLAWK_KANBAN_TASK", "").strip()
