...
for-secrets-only rule), provisioned by NAS at deploy time (Phase 3):
    ...
    CLAWK_DASHBOARD_DRAIN_SECRET   # the per-agent shared secret (>=43 url-safe-b64 chars)
...
def register(ctx) -> None:
    ...
    No-op (records a skip reason) when ``CLAWK_DASHBOARD_DRAIN_SECRET`` is
    ...
    secret = os.environ.get("CLAWK_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        LAST_SKIP_REASON = (
    ...
    if reason is not None:
        LAST_SKIP_REASON = (
            f"CLAWK_DASHBOARD_DRAIN_SECRET rejected — {reason}. "
    ...
    try:
        from clawk_cli.dashboard_auth.token_auth import register_token_route
