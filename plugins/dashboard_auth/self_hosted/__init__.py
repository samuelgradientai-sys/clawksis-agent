...
A standards-compliant OpenID Connect Relying Party for the ``clawk dashboard``
...
:class:`~clawk_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``clawk_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~clawk_cli.dashboard_auth.Session`.
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.clawksis/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    CLAWK_DASHBOARD_OIDC_ISSUER
    CLAWK_DASHBOARD_OIDC_CLIENT_ID
    CLAWK_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    CLAWK_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Clawksis does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``CLAWK_DASHBOARD_OIDC_*`` env vars or the
    ...
    scopes = (
        _resolve_setting("CLAWK_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.clawksis/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "CLAWK_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(CLAWK_DASHBOARD_OIDC_ISSUER + CLAWK_DASHBOARD_OIDC_CLIENT_ID) "
