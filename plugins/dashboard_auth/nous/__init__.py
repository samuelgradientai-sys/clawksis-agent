...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      CLAWK_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``

      CLAWK_DASHBOARD_PORTAL_URL       — defaults to
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``clawk-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``clawk_session_pkce``
...
Skip reasons:
  ...
  ("Set CLAWK_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via CLAWK_DASHBOARD_PORTAL_URL for
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"clawk_session_pkce\"]``
        ...
        cookie_payload = {
            "clawk_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between CLAWK_DASHBOARD_PORTAL_URL / CLAWK_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      ...
      1. ``CLAWK_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    env = os.environ.get("CLAWK_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
...
def _resolve_portal_url() -> str:
    ...
    Order:
      ...
      1. ``CLAWK_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
    ...
    env = os.environ.get("CLAWK_DASHBOARD_PORTAL_URL", "").strip()
...
def register(ctx) -> None:
    ...
    configured (either via ``CLAWK_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set CLAWK_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``clawk_cli.web_server.should_require_auth`` +
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"CLAWK_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
