/**
 * Clawksis Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.clawksis/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__CLAWK_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__CLAWK_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "clawk-kanban-dot-triage",
    todo: "clawk-kanban-dot-todo",
    ready: "clawk-kanban-dot-ready",
    running: "clawk-kanban-dot-running",
    blocked: "clawk-kanban-dot-blocked",
    done: "clawk-kanban-dot-done",
    archived: "clawk-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-clawk-task";
  ...
  const DOCS_URL = "https://clawksis-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://clawksis-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "clawk.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="clawk-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "clawk-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "clawk-kanban-md",
  ...
  // standard `drop` event and our `clawk-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("clawk-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("clawk-kanban-column--drop");
          if (target) target.classList.add("clawk-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("clawk-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("clawk-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("clawk-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__CLAWK_SESSION_TOKEN__ directly (the old
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "clawk-kanban-attention",
        "clawk-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "clawk-kanban-attention-bar" },
        h("span", { className: "clawk-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "clawk-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "clawk-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "clawk-kanban-attention-row",
                  "clawk-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "clawk-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "clawk-kanban-attention-row-id" }, task.id),
                h("span", { className: "clawk-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "clawk-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "clawk-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "clawk-kanban-diag-action-btn",
      action.suggested ? "clawk-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " clawk-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".clawk-kanban-drawer-comment-row input, .clawk-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "clawk-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("clawk-kanban-diag", sevClass) },
      h("div", { className: "clawk-kanban-diag-header" },
        h("span", { className: "clawk-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "clawk-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "clawk-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "clawk-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "clawk-kanban-diag-data-row" },
                  h("span", { className: "clawk-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "clawk-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "clawk-kanban-diag-data-row" },
                h("span", { className: "clawk-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "clawk-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "clawk-kanban-diag-reassign-row" },
            h("span", { className: "clawk-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "clawk-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "clawk-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "clawk-kanban-diag-msg",
              msg.ok ? "clawk-kanban-diag-msg--ok" : "clawk-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "clawk-kanban-section" },
      h("div", { className: "clawk-kanban-section-head-row" },
        h("span", { className: "clawk-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "clawk-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "clawk-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "clawk-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "clawk-kanban-docs-link",
      title: "Open Clawksis Kanban docs in a new tab",
      "aria-label": "Clawksis Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `clawk kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "clawk-kanban-msg-ok" : "clawk-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "clawk-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "clawk-kanban-boardswitcher" },
      h("div", { className: "clawk-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "clawk-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "clawk-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "clawk-kanban-dialog-title" },
        ...
        h("div", { className: "clawk-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "clawk-kanban-bulk" },
      h("span", { className: "clawk-kanban-bulk-count" },
      ...
      h("div", { className: "clawk-kanban-bulk-priority",
      ...
      h("div", { className: "clawk-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Clawksis profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "clawk-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("clawk-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("clawk-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "clawk-kanban-trash",
        dragOver ? "clawk-kanban-trash--drop" : "",
        props.draggingTaskId ? "clawk-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "clawk-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "clawk-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".clawk-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "clawk-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("clawk-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("clawk-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "clawk-kanban-column",
        dragOver ? "clawk-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "clawk-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "clawk-kanban-col-check",
        ...
        h("span", { className: cn("clawk-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "clawk-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "clawk-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "clawk-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "clawk-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "clawk-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "clawk-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "clawk-kanban-lane" },
                  h("div", { className: "clawk-kanban-lane-head" },
                    h("span", { className: "clawk-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "clawk-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "clawk-kanban-card--stale-red";
    if (age >= tier.amber) return "clawk-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".clawk-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "clawk-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "clawk-kanban-card",
        props.selected ? "clawk-kanban-card--selected" : "",
        props.failed ? "clawk-kanban-card--failed" : "",
        props.draggingSource ? "clawk-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "clawk-kanban-card-content" },
          h("div", { className: "clawk-kanban-card-row" },
            h("label", {
              className: "clawk-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "clawk-kanban-card-check",
            ...
            h("span", { className: "clawk-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or clawk kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "clawk-kanban-warning-badge",
                    "clawk-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "clawk-kanban-needs-assignee",
          ...
          h("div", { className: "clawk-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "clawk-kanban-card-row clawk-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "clawk-kanban-assignee",
                            title: `Assigned to Clawksis profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "clawk-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "clawk-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "clawk-kanban-ago",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "clawk-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "clawk-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "clawk-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "clawk-kanban-drawer-close",
        ...
        data ? h("div", { className: "clawk-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "clawk-kanban-section" },
      h("div", { className: "clawk-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "clawk-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "clawk-kanban-drawer-body" },
      h("div", { className: "clawk-kanban-drawer-title" },
        h("span", { className: cn("clawk-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "clawk-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "clawk-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "clawk-kanban-section" },
        h("div", { className: "clawk-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "clawk-kanban-section" },
        h("div", { className: "clawk-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "clawk-kanban-comment" },
                h("div", { className: "clawk-kanban-comment-head" },
                  h("span", { className: "clawk-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "clawk-kanban-comment-ago" },
      ...
      h("div", { className: "clawk-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "clawk-kanban-event",
              isDiag ? "clawk-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "clawk-kanban-event-header" },
                  h("span", { className: "clawk-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "clawk-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "clawk-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "clawk-kanban-event-header-plain" },
                  h("span", { className: "clawk-kanban-event-kind" }, e.kind),
                  h("span", { className: "clawk-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "clawk-kanban-event-phantom-row" },
                  h("span", { className: "clawk-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "clawk-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "clawk-kanban-event-payload" },
    ...
    return h("div", { className: "clawk-kanban-section" },
      h("div", { className: "clawk-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "clawk-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `clawk-kanban-run--${r.outcome || r.status || "ended"}`
          : "clawk-kanban-run--active";
        return h("div", { key: r.id, className: cn("clawk-kanban-run", outcomeClass) },
          h("div", { className: "clawk-kanban-run-head" },
            h("span", { className: "clawk-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "clawk-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "clawk-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "clawk-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "clawk-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "clawk-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "clawk-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "clawk-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "clawk-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "clawk-kanban-pre clawk-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "clawk-kanban-meta-row" },
      h("span", { className: "clawk-kanban-meta-label" }, props.label),
      h("span", { className: "clawk-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "clawk-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clawk-kanban-meta-row" },
        h("span", { className: "clawk-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "clawk-kanban-meta-value clawk-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clawk-kanban-meta-row" },
        h("span", { className: "clawk-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "clawk-kanban-meta-value clawk-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "clawk-kanban-section" },
      h("div", { className: "clawk-kanban-section-head-row" },
        h("span", { className: "clawk-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "clawk-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "clawk-kanban-section" },
      h("div", { className: "clawk-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "clawk-kanban-deps-row" },
        h("span", { className: "clawk-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "clawk-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "clawk-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "clawk-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "clawk-kanban-dep-chip-x",
      ...
      h("div", { className: "clawk-kanban-deps-row" },
        h("span", { className: "clawk-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "clawk-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "clawk-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "clawk-kanban-section" },
      ...
      h("div", { className: "clawk-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "clawk-kanban-home-sub clawk-kanban-home-sub--on"
              : "clawk-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__CLAWK_PLUGINS__ && typeof window.__CLAWK_PLUGINS__.register === "function") {
    window.__CLAWK_PLUGINS__.register("kanban", KanbanPage);
  }
})();
