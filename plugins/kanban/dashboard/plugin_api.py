...
``clawk_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__CLAWK_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``clawk_cli/web_server.py``.



This means ``clawk dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``clawk_cli.web_server._ws_auth_ok``).
    ...
    try:
        from clawk_cli import web_server as _ws
...
def _compute_task_diagnostics(
    ...
    Uses ``clawk_cli.kanban_diagnostics`` — see that module for the
...
def get_board(
    ...
    through to the active board (``CLAWK_KANBAN_BOARD`` env → on-disk
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``clawk kanban
...
# spawn failures, stuck-blocked). See clawk_cli.kanban_diagnostics for
...
def list_diagnostics(
    ...
    AND for ``clawk kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``clawk kanban reclaim <task_id> --reason ...``.
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``clawk kanban specify <task_id>``.
    ...
    # CLAWK_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from clawk_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``clawk kanban reassign <task_id> <profile> [--reclaim]``.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.clawksis/profiles/*`` on disk and every
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``clawk profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``clawk kanban decompose <task_id>``.
    ...
    # CLAWK_KANBAN_BOARD env var would let concurrent requests for
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.clawksis/config.yaml.
