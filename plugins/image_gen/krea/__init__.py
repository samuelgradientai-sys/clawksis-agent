...
try:  # Shared helper; ships with the upstream ``image_gen_provider`` module.
    from agent.image_gen_provider import normalize_reference_images
except ImportError:  # pragma: no cover - older provider module without it

    def normalize_reference_images(value: Any) -> Optional[List[str]]:
        """Coerce a reference-image argument into a clean list of strings."""
        if value is None:
            return None
...
# Clawksis uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # BYOK: Krea is always reached directly with the user's own
        # ``KREA_API_KEY``.
        base_url = BASE_URL
        ...
        if not auth_token:
            return error_response(
                error=(
                    "KREA_API_KEY not set. Run `clawk tools` → Image "
        ...
        headers = {
            ...
            "User-Agent": "Clawksis/1.0 (krea-image-gen)",
        }
