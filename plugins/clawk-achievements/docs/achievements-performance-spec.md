# Clawksis Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: clawk-achievements plugin
...
### `GET /api/plugins/clawk-achievements/achievements`
...
### `POST /api/plugins/clawk-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/clawk-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/clawk-achievements/overview`
...
- Persist plugin data under:
  - `~/.clawksis/plugins/clawk-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.clawksis/plugins/clawk-achievements/scan_checkpoint.json`
...
- `~/.clawksis/plugins/clawk-achievements/`
