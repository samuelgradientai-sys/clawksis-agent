...
clawk plugins enable security-guidance

# or edit ~/.clawksis/config.yaml manually:
...
* **No LLM diff review.** Anthropic's layer 2 spawns an auxiliary LLM call
  ...
  on every agent turn that touched files. On clawk that would route
...
* **No project-local rules file.** Anthropic's `.claude/claude-security-guidance.md`
  ...
  add an analogous `.clawksis/security-guidance.md` once layer 2 lands.
...
* `__init__.py`, `plugin.yaml`, `README.md`, and tests are original work by
  ...
  NousResearch, MIT-licensed alongside the rest of clawksis-agent.
