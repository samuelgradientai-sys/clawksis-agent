...
def _xai_user_agent() -> str:
    try:
        ...
        return clawk_xai_user_agent()
    except Exception:
        return "clawksis-agent/video_gen"
...
def _raise_if_blocked_local_input(ref: str) -> None:
    """Refuse to read a local media path that Clawksis' read deny-list blocks.
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``clawk_cli/tools_config.py``) so the picker doesn't blindly
        ...
        return {
            ...
            "tag": tag,
            "desc": (
                "Grok's video models from xAI; uses your Grok account or an "
                "xAI API key."
            ),
            "env_vars": [],
            "post_setup": "xai_grok",
...
def _auth_required_response(prompt: str) -> Dict[str, Any]:
    return error_response(
        error=(
            "No xAI credentials found. Sign in via `clawk auth add xai-oauth` "
