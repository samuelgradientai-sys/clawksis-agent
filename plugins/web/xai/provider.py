...
``{title, url, description, position}`` rows every other Clawksis web
...
prefers Clawksis-managed xAI Grok OAuth (via ``clawk auth``) and falls back

to ``XAI_API_KEY`` (resolved through ``~/.clawksis/.env``, then
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``clawk tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `clawk auth` to sign in with "
