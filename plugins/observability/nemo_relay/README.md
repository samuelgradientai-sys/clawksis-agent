...
Optional Clawksis observability plugin that maps Clawksis observer hooks to
...
not replace Clawksis's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Clawksis emit NeMo Relay lifecycle events for
the work Clawksis already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Clawksis can:

- Preserve Clawksis execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
clawk plugins enable observability/nemo_relay
```

The `CLAWK_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `CLAWK_HOME` that the
agent run will use:

```bash
env CLAWK_HOME=/tmp/clawk-nemo-relay-test \
  clawk plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`CLAWK_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`CLAWK_HOME` is the Clawksis profile/config home used by both
`clawk plugins enable ...` and the later `clawk chat ...` run. If unset,
Clawksis uses the user's default home, usually `~/.clawksis`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export CLAWK_HOME=/tmp/clawk-nemo-relay-test
clawk plugins enable observability/nemo_relay
clawk chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `clawk` command you run is built from the
...
uv run clawk plugins enable observability/nemo_relay
uv run clawk chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/clawk_agent-*.whl
...
The plugin can configure exporters directly from `CLAWK_NEMO_RELAY_*`
...
```bash
export CLAWK_NEMO_RELAY_ATOF_ENABLED=1
export CLAWK_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export CLAWK_NEMO_RELAY_ATIF_ENABLED=1
export CLAWK_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `CLAWK_NEMO_RELAY_ATOF_FILENAME`
- `CLAWK_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `CLAWK_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `CLAWK_NEMO_RELAY_ATIF_AGENT_NAME`
- `CLAWK_NEMO_RELAY_ATIF_AGENT_VERSION`
- `CLAWK_NEMO_RELAY_ATIF_MODEL_NAME`
- `CLAWK_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Clawksis at it:

```bash
export CLAWK_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `CLAWK_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`CLAWK_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`CLAWK_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Clawksis does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Clawksis keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Clawksis routes LLM and tool
...
Clawksis feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with Clawksis-owned
...
with this Clawksis-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Clawksis rejects
...
Clawksis activates these plugins before registering its managed LLM and tool
...
activation API, Clawksis logs an actionable warning and continues with the
...
For the full generic Clawksis middleware contract, see
...
export CLAWK_HOME=/tmp/clawk-nemo-relay-docs/clawk-home
mkdir -p "$CLAWK_HOME"

cat > "$CLAWK_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Clawksis session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export CLAWK_NEMO_RELAY_ATOF_ENABLED=1
export CLAWK_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/clawk-nemo-relay-docs/subagent/atof
export CLAWK_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export CLAWK_NEMO_RELAY_ATOF_MODE=overwrite
export CLAWK_NEMO_RELAY_ATIF_ENABLED=1
export CLAWK_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/clawk-nemo-relay-docs/subagent/atif
export CLAWK_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export CLAWK_NEMO_RELAY_ATIF_AGENT_NAME='Clawksis E2E'
export CLAWK_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export CLAWK_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"clawk.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Clawksis E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Clawksis dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/clawk-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/clawk-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/clawk-nemo-relay-docs/workdir/beta.txt
cd /tmp/clawk-nemo-relay-docs/workdir

export CLAWK_NEMO_RELAY_ATOF_ENABLED=1
export CLAWK_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/clawk-nemo-relay-docs/parallel/atof
export CLAWK_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export CLAWK_NEMO_RELAY_ATOF_MODE=overwrite
export CLAWK_NEMO_RELAY_ATIF_ENABLED=1
export CLAWK_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/clawk-nemo-relay-docs/parallel/atif
export CLAWK_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export CLAWK_NEMO_RELAY_ATIF_AGENT_NAME='Clawksis E2E'
export CLAWK_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Clawksis sessions map to `agent` scopes.
- Clawksis API request hooks map to `llm` scope start/end events.
- Clawksis tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Clawksis execution middleware to hand
...
Enable it for Clawksis:

```bash
export CLAWK_NEMO_RELAY_PLUGINS_TOML=/tmp/clawk-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Clawksis routes execution
through these boundaries:

```text
Clawksis provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Clawksis provider adapter next_call(...)

Clawksis tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Clawksis tool dispatcher next_call(...)
```
...
middleware for a local Clawksis run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by the supported
0.x release range beginning with 0.5.

```bash
export CLAWK_HOME=/tmp/clawk-middleware-test/clawk-home
mkdir -p "$CLAWK_HOME" /tmp/clawk-middleware-test/nemo-relay
...
cat > /tmp/clawk-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/clawk-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/clawk-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Clawksis Middleware E2E"
agent_version = "local"
...
export CLAWK_NEMO_RELAY_PLUGINS_TOML=/tmp/clawk-middleware-test/nemo-relay/plugins.toml
