"""nemo_relay — optional Clawksis plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atof_filename: str = "clawk-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "clawk-atif-{session_id}.json"
    atif_subagent_export_mode: str = "embedded"
    atif_agent_name: str = "Clawksis"
...
class _Runtime:
    def __init__(self, nemo_relay: Any, settings: _Settings) -> None:
        ...
        self._atof_subscriber_name = "clawk.nemo_relay.atof"
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("clawk.subagent.stop", kwargs)
    ...
    def _run_managed_with_downstream_preservation(
        ...
        *,
        ...
        # execution so Clawksis retry classification still sees it. The LLM and tool
...
def register(ctx) -> None:
    # Activate dynamic plugins before Clawksis installs the managed execution
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("clawk.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("clawk.turn.end", kwargs))
...
def on_post_api_request(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("clawk.api.response.unmatched", kwargs)
            return
...
def on_api_request_error(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        span = state.llm_spans.pop(_api_key(kwargs), None)
        if span is None:
            runtime.mark("clawk.api.error", kwargs)
            return
        runtime.nemo_relay.llm.call_end(
            span,
...
def on_post_tool_call(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("clawk.tool.response.unmatched", kwargs)
            return
...
def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("clawk.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("clawk.approval.response", kwargs))
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("CLAWK_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("CLAWK_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("CLAWK_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("CLAWK_NEMO_RELAY_ATOF_FILENAME") or "clawk-atof.jsonl",
        ...
        atif_model_name=_env("CLAWK_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Clawksis cannot activate Relay gateway [[plugins.dynamic]] records because "
                "the Python binding does not expose the CLI lifecycle resolver for "
                "enablement, trust policy, and worker environments. Use Clawksis-owned "
...
def _atif_subagent_export_mode() -> str:
    mode = _env("CLAWK_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
...
def _is_relay_wrapped_callback_error(exc: Exception, callback_error: Exception | None) -> bool:
    ...
    # leading ``internal error:`` shape entirely, this returns False and Clawksis
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="clawk-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()
