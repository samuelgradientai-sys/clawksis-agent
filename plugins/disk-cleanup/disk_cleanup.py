"""disk_cleanup — ephemeral file cleanup for Clawksis.
...
Rules:
  ...
  - empty dirs    → always delete (under CLAWK_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CLAWK_HOME and /tmp/clawk-*
Never touches: ~/.clawksis/logs/ or any system directory.
"""
...
try:
    ...
    def get_clawk_home() -> Path:  # type: ignore[no-redef]
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CLAWK_HOME/logs/``."""
    return get_clawk_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CLAWK_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CLAWK_HOME or ``/tmp/clawk-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    clawk_home = get_clawk_home()
    try:
        path.resolve().relative_to(clawk_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/clawk-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("clawk-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "clawksis-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $CLAWK_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so CLAWK_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        clawk_home = get_clawk_home()
        for parent in ("cron", "cronjobs"):
            base = clawk_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CLAWK_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CLAWK_HOME, but never recurse into known
    # durable state trees.  Some installs place the Clawksis checkout, venv,
    # and desktop build under CLAWK_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    clawk_home = get_clawk_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in clawk_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    clawk_home = get_clawk_home()
    try:
        rel = path.resolve().relative_to(clawk_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "clawksis-agent",
        }:
            return None
        ...
        # Path isn't under CLAWK_HOME (e.g. /tmp/clawk-*) — fall through.
        pass
