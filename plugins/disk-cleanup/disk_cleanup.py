...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CLAWK_HOME (but leave CLAWK_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(clawk_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == clawk_home:
                continue
            try:
                rel_parts = dirpath.relative_to(clawk_home).parts
            except ValueError:
                continue
