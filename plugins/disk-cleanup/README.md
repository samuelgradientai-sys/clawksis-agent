...
Auto-tracks and cleans up ephemeral files created during Clawksis
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CLAWK_HOME` and `/tmp/clawk-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CLAWK_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CLAWK_HOME` or `/tmp/clawk-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CLAWK_HOME/disk-cleanup/` is itself excluded
- `$CLAWK_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
