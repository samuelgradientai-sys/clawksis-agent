"""`clawk meet node ...` subcommand tree.

Wired into the existing ``clawk meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``clawk meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="clawk-meet-node")
