...
Let the clawk agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where clawk runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `clawk meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `clawk meet` CLI |
...
| `cli.py` | `clawk meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CLAWK_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `clawk meet node {run,list,approve,remove,status,ping}` |
...
clawk meet install                                      # pip + Chromium
clawk meet setup                                        # preflight
clawk meet auth                                         # optional
clawk meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
clawk meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.clawksis/.env
clawk meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
clawk meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
clawk meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.clawksis/.env
clawk meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, clawk will **not** switch your system audio input automatically — the
...
clawk plugins enable google_meet
clawk meet node run --display-name my-mac --host 0.0.0.0 --port 18789
