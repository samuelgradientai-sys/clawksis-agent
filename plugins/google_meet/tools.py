...
def _resolve_node_client(node: Optional[str]):
    ...
    if entry is None:
        raise RuntimeError(
            ...
            "run `clawk meet node approve <name> <url> <token>` first"
        )
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "guest_name": {
                "type": "string",
                "description": (
                    "Display name to use when joining as guest. Defaults to 'Clawksis'."
                ),
            },
            "duration": {
                "type": "string",
            ...
            "node": {
                ...
                "description": (
                    ...
                    "approved via `clawk meet node approve`."
                ),
            },
        },
        "required": ["url"],
    ...
    if client is not None:
        ...
        try:
            res = client.start_bot(
                url=url,
                guest_name=str(args.get("guest_name") or "Clawksis"),
