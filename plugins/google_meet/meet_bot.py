...
``$CLAWK_HOME/workspace/meetings/<meeting-id>/``. The main clawk process
...
Run standalone for debugging::
    ...
    CLAWK_MEET_URL=https://meet.google.com/abc-defg-hij \\
    CLAWK_MEET_OUT_DIR=/tmp/meet-debug \\
    CLAWK_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``CLAWK_MEET_OUT_DIR``.
...
# and exposes ``window.__clawkMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__clawkMeetInstalled) return;
  window.__clawkMeetInstalled = true;
  window.__clawkMeetQueue = [];
  ...
  window.__clawkMeetDrain = () => {
    const out = window.__clawkMeetQueue.slice();
    window.__clawkMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "clawk_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("CLAWK_MEET_URL", "").strip()
    out_dir_env = os.environ.get("CLAWK_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("CLAWK_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("CLAWK_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("CLAWK_MEET_GUEST_NAME", "Clawksis")
    duration_s = _parse_duration(os.environ.get("CLAWK_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when CLAWK_MEET_MODE=realtime.
    mode = os.environ.get("CLAWK_MEET_MODE", "transcribe").strip().lower()
    ...
    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — CLAWK_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: CLAWK_MEET_OUT_DIR is required\n")
        return 2
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("CLAWK_MEET_LOBBY_TIMEOUT", "300")
            )
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__clawkMeetInstalled) {
