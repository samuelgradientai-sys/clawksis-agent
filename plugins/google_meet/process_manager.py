...
session-scoped state file under ``$CLAWK_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CLAWK_HOME):
...
def _root() -> Path:
    ...
    return Path(get_clawk_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this clawk install, leave it first —
    ...
    env["CLAWK_MEET_URL"] = url

    env["CLAWK_MEET_OUT_DIR"] = str(out)

    env["CLAWK_MEET_GUEST_NAME"] = guest_name

    if headed:
        env["CLAWK_MEET_HEADED"] = "1"

    if auth_state:
        env["CLAWK_MEET_AUTH_STATE"] = auth_state

    if duration:
        env["CLAWK_MEET_DURATION"] = duration

    # v2: realtime mode + passthroughs. The bot defaults to transcribe

    # mode if CLAWK_MEET_MODE isn't set, matching v1 behavior.

    if mode:
        env["CLAWK_MEET_MODE"] = mode

    if realtime_model:
        env["CLAWK_MEET_REALTIME_MODEL"] = realtime_model

    if realtime_voice:
        env["CLAWK_MEET_REALTIME_VOICE"] = realtime_voice

    if realtime_instructions:
        env["CLAWK_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions

    if realtime_api_key:
        env["CLAWK_MEET_REALTIME_KEY"] = realtime_api_key
