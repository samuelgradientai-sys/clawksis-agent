"""
LINE Messaging API platform adapter for Clawksis.
...
to Clawksis. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CLAWK_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            clawk_home = Path.home().joinpath(".clawk").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``clawk status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``clawk status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clawk setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.clawksis/.env`` via ``clawk_cli.config``.
    """
    print()
    ...
    try:
        from clawk_cli.config import get_env_var, set_env_var
    except ImportError:
        print("clawk_cli.config not available; set LINE_* vars manually in ~/.clawksis/.env")
        return
