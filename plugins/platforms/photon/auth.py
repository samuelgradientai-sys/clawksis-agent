...
Credential storage mirrors every other Clawksis channel:

    * runtime SDK creds  -> ``~/.clawksis/.env``  (``PHOTON_PROJECT_ID`` =
      spectrumProjectId, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.clawksis/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a clawk dependency
...
# registers Clawksis as its own client_id.
...
# Default name of the project Clawksis provisions for the operator.
DEFAULT_PROJECT_NAME = "Clawksis"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of clawksis-agent.


def _auth_json_path() -> Path:
    """Resolve ``~/.clawksis/auth.json`` honouring the active Clawksis profile."""
    try:
        from clawk_constants import get_clawk_home

        return Path(get_clawk_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.clawksis")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.clawksis/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.clawksis/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.clawksis/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: clawk_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels: Dict[str, str] = {}
    labels["device_token"] = (
        "✓ stored" if load_photon_token() else "✗ missing (run `clawk photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `clawk photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `clawk photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_token() -> str:
        return (
            ...
            else "✗ missing (run `clawk photon setup`)"
        )
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `clawk photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `clawk photon setup`)"
