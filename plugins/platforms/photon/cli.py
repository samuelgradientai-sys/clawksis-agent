"""
``clawk photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Clawksis
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Clawksis" project.
    ...
    print("  Start the gateway:  clawk gateway start")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.clawksis/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.clawksis/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  clawk gateway restart")
    return 0
...
#
# `clawk gateway setup` discovers platforms via the registry and calls each
