...
This plugin connects Clawksis to iMessage (and other Spectrum
...
TypeScript-only, Clawksis runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Clawksis-Sidecar-Token`.
...
There is no separate `login` command; like every other Clawksis channel,
...
Run `clawk photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.clawksis/.env` (the same place every other
...
Management metadata lives in `~/.clawksis/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `clawk photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `clawk photon status`, a DM and a group roundtrip,
