"""
Photon Spectrum (iMessage) platform adapter for Clawksis.
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `clawk photon setup` will
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Clawksis defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Clawksis-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Clawksis checkout's sidecar entry point.
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Clawksis-Sidecar-Token": token}
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `clawk photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: clawk photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `clawk gateway setup` alongside every other
    ...
    # Register CLI subcommands — `clawk photon ...`
