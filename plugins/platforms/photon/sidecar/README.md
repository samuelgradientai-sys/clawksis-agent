...
Small Node helper that bridges Clawksis to Photon's Spectrum SDK
(`spectrum-ts`).  Clawksis is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Clawksis-Sidecar-Token`)
...
The Clawksis plugin's `clawk photon setup` command runs `npm install`
here automatically.
