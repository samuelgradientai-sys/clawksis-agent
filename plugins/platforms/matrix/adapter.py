...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Clawksis command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Clawksis accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Clawksis ``/command`` text."""
...
# Uses get_clawk_home() so each profile gets its own Matrix store.
from clawk_constants import get_clawk_dir as _get_clawk_dir

_STORE_DIR = _get_clawk_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("CLAWK_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("CLAWK_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "clawk"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Clawksis" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in clawk_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces clawk_cli/setup.py::_setup_matrix
...
def _is_connected(config) -> bool:
    ...
    configured. Read via clawk_cli.gateway.get_env_value so setup-status
