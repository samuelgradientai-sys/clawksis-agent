"""
Email platform adapter for the Clawksis gateway.

Allows users to interact with Clawksis by sending emails.
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `clawk config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `clawk gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Clawksis")
        ...
        msg_id = f"<clawk-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# clawk_cli/gateway.py, and the _send_email dispatch in
