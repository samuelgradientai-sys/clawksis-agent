...
required — uses aiohttp which is already a Clawksis dependency.
...
def validate_mattermost_config(config: PlatformConfig) -> bool:
    ...
    token = (
        getattr(config, "token", None) or os.getenv("MATTERMOST_TOKEN", "")
    ).strip()
...
def interactive_setup() -> None:
    ...
    ``clawk_cli/setup.py::_setup_mattermost`` function this migration
    removes.
    """
    ...
    print()
    print_info(
        "📬 Home Channel: where Clawksis delivers cron job results and notifications."
    )
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # clawk_cli/setup.py::_setup_mattermost function.
