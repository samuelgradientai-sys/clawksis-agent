...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known Clawksis media cache directory.
    ...
    ``<kind>_cache`` layout that ``get_clawk_dir`` may return.
    """
    try:
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not creds_path.exists():
            ...
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — pair from the dashboard or run `clawk whatsapp`.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `clawk update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".clawk-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # Clawksis-managed portable Node before falling back to PATH.
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `clawk update`, so without this check it
            ...
            # with_clawk_node_path() copies os.environ when called with no arg.
            bridge_env = with_clawk_node_path()
            if self._reply_prefix is not None:
                bridge_env["WHATSAPP_REPLY_PREFIX"] = self._reply_prefix
            # Pass the profile-aware cache directories so the bridge writes
            # media where the Python side reads it.  Without these the bridge
            # hardcodes ~/.clawksis/{image,audio,document}_cache, which diverges
            # under CLAWK_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
...
# dict in clawk_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in clawk_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``clawk setup`` even
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via clawk_cli.gateway.get_env_value (not os.getenv) so setup-status
