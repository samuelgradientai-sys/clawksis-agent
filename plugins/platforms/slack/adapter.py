...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `clawk slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_clawk_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Clawksis does not currently need to route them into the agent loop.
            ...
            # /model work directly without /clawk prefix). A single regex
            ...
            # manifest (see `clawk slack manifest`). In Socket Mode, Slack
            ...
            from clawk_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/clawk$")

            @self._app.command(_slash_pattern)
            async def handle_clawk_command(ack, command):
            ...
            for _action_id in (
                "clawk_approve_once",
                "clawk_approve_session",
                "clawk_approve_always",
                "clawk_deny",
            ):
            ...
            for _action_id in (
                "clawk_confirm_once",
                "clawk_confirm_always",
                "clawk_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("clawk_feedback")(self._handle_feedback_action)
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Clawksis handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Clawksis session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Clawksis owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@clawk`` still resolve.
        ...
        try:
            ...
            if not smart_denied:
                actions.append({
                    ...
                    "action_id": "clawk_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "clawk_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "clawk_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "clawk_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "clawk_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "clawk_confirm_once": "once",
            "clawk_confirm_always": "always",
            "clawk_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "clawk_approve_once": "once",
            "clawk_approve_session": "session",
            "clawk_approve_always": "always",
            "clawk_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/clawk <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/clawk
        ...
        if slash_name in {"clawk", ""}:
            # Legacy /clawk <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from clawk_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/clawk   ``).
        ...
        # "Running /cmd…" ack shown by handle_clawk_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/clawk <question>" must produce public replies so
...
# ``clawk_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``clawk_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from clawk_cli.config import get_env_value, save_env_value
    from clawk_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Clawksis",
                bot_description="Your Clawksis agent on Slack",
            )
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``clawk_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'clawksis-agent[slack]'",
        # Interactive setup wizard — replaces clawk_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in clawk_cli/gateway.py.
