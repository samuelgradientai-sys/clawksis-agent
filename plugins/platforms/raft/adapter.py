...
and injects content-free wake hints into Clawksis' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Clawksis turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    """Interactive ``clawk gateway setup`` flow for the Raft platform.

    Lazy-imports CLI helpers so the plugin stays importable in gateway runtime
    and test contexts. The flow persists ``RAFT_PROFILE`` to the Clawksis env
