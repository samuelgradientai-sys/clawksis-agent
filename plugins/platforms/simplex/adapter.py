...
Optional environment variables:
    ...
    CLAWK_SIMPLEX_TEXT_BATCH_DELAY
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("CLAWK_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    ...
    auto-accept / home channel. Writes to ``~/.clawksis/.env`` via
    ``clawk_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "clawk_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.clawksis/.env"
        )
        return
