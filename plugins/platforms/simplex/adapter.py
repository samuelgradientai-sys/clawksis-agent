"""SimpleX Chat platform adapter (Clawksis plugin).
...
This adapter ships as a Clawksis platform plugin under

``plugins/platforms/simplex/``. The Clawksis plugin loader scans the
...
discoverable and `clawk setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Clawksis — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    async def _handle_new_chat_item(self, wrapper: dict) -> None:
        ...
        if is_group:
            ...
            contact_id = str(
                contact_info.get("contactId") or contact_info.get("id") or ""
            )
        ...
        if is_group and member:
            ...
            sender_name = (
                member.get("displayName") or member.get("localDisplayName") or sender_id
            )
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"clawk-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clawk setup gateway`` → SimpleX.



    Prompts for the WebSocket URL and the optional allowlist / home channel.

    Writes to ``~/.clawksis/.env`` via ``clawk_cli.config``.
    ...
    try:
        ...
        print(
            "clawk_cli.config not available; set SIMPLEX_* vars manually in ~/.clawksis/.env"
        )
    ...
    _prompt(
        "SIMPLEX_ALLOWED_USERS", "Allowed contact IDs (comma-separated; blank=skip)"
    )

    _prompt("SIMPLEX_HOME_CHANNEL", "Home channel contact/group ID (or empty)")

    print(
        "Done. Make sure the simplex-chat daemon is running before starting the gateway."
    )
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `clawk gateway status` without
