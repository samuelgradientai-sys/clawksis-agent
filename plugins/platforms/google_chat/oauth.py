...
- Per-user tokens (keyed by sender email):
    ...
    ``${CLAWK_HOME}/google_chat_user_tokens/<sanitized_email>.json``

- Legacy single-user token (fallback, untouched for backward compat):

    ``${CLAWK_HOME}/google_chat_user_token.json``
...
- Shared OAuth client (one per host, anchored at the default Clawksis root so

  every profile sees it; a profile-local copy under ``${CLAWK_HOME}`` wins

  when present):

    ``<default-root>/google_chat_user_client_secret.json`` (default ``~/.clawksis``)
...
try:
    ...
    def get_default_clawk_root() -> Path:
        ...
        # Mirror clawk_constants.get_default_clawk_root(): resolve the
...
def _clawk_home() -> Path:
    """Resolve CLAWK_HOME at call time (NOT module import).



    Tests and ``CLAWK_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.clawksis/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    ...
    return _clawk_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:

    return _clawk_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:

    return _clawk_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:

    return _clawk_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    ...
    so it is anchored at the default Clawksis root (``~/.clawksis`` — or the

    Docker root) rather than the active profile's ``CLAWK_HOME``. That way
    ...
    A profile-local secret (``$CLAWK_HOME/google_chat_user_client_secret.json``)
    ...
    profile_local = _clawk_home() / "google_chat_user_client_secret.json"

    if profile_local.exists():
        return profile_local

    return get_default_clawk_root() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install clawksis-agent[google_chat]."
        )
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'clawksis-agent[google_chat]'")
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into CLAWK_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_clawk_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_clawk_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Clawksis (native attachment delivery)"
    )
