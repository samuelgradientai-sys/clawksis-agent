...
# ``model_tools`` import time, so ``clawk status``, ``clawk chat``, etc.
...
# ``clawk_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Clawksis is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a clawk MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "clawk-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CLAWK_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Clawksis is thinking…".
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CLAWK_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a clawk MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Clawksis is thinking…' marker message.
        ...
        "Clawksis is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Clawksis is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Clawksis is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Clawksis is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``clawk setup``.

    The setup wizard at ``clawk_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.clawksis/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. clawk-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.clawksis/.env")
    print_info("Restart the gateway: clawk gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``clawk cron`` running as a
    separate process from ``clawk gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Clawksis plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'clawksis-agent[google_chat]'",
        ...
        # the "Clawksis is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Clawksis is thinking…' marker message "
