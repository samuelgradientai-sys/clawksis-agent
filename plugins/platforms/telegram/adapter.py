...
# trigger its own recovery path. Refs: samuelgradientai-sys/clawksis-agent#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: samuelgradientai-sys/clawksis-agent#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float(
            "CLAWK_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8
        )
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "CLAWK_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "CLAWK_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Clawksis-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on Clawksis' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Clawksis must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _start_polling_resilient(
        self, *, drop_pending_updates: bool, error_callback
    ) -> bool:
        ...
        try:
            if app and app.updater and app.updater.running:
                try:
                    ...
                    # Refs: samuelgradientai-sys/clawksis-agent#58270
                    await asyncio.wait_for(
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `clawk update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Clawksis or OpenClaw instance is running "
            "with this token, then restart the gateway with 'clawk gateway restart'."
            % (
                MAX_CONFLICT_RETRIES,
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # payload size limit (~4KB total).  Clawksis defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the Clawksis process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("CLAWK_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("CLAWK_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float(
                    "CLAWK_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0
                ),
                "read_timeout": _env_float("CLAWK_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("CLAWK_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }
            ...
            disable_fallback = os.getenv(
                "CLAWK_TELEGRAM_DISABLE_FALLBACK_IPS", ""
            ...
            _max_connect = 8
            _init_timeout = _env_float("CLAWK_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set CLAWK_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``clawk update --gateway``
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``clawk model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from clawk_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from clawk_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.clawksis/scripts/gmail-triage/. `arg` from the callback
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@clawk_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Clawksis bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the Clawksis inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(
        ...
        # unread in the Clawksis inbox (#52363).
        if self._is_own_message(message):
            return False
    ...
    def _enqueue_text_event(self, event: MessageEvent) -> None:
        ...
        if self._should_drop_delayed_delivery():
            logger.debug(
                "[Telegram] Dropping text batch enqueue after disconnect started"
            )
            return
    ...
    async def _flush_text_batch(self, key: str) -> None:
        ...
        try:
            ...
            if self._should_drop_delayed_delivery():
                logger.debug(
                    "[Telegram] Dropping text batch flush after disconnect started"
                )
                return
            logger.info(
    ...
    async def _flush_photo_batch(self, batch_key: str) -> None:
        ...
        try:
            ...
            if self._should_drop_delayed_delivery():
                logger.debug(
                    "[Telegram] Dropping photo batch flush after disconnect started"
                )
                return
            logger.info(
    ...
    def _enqueue_photo_event(self, batch_key: str, event: MessageEvent) -> None:
        ...
        if self._should_drop_delayed_delivery():
            logger.debug(
                "[Telegram] Dropping photo batch enqueue after disconnect started"
            )
            return
    ...
    async def _queue_media_group_event(
        ...
        if self._should_drop_delayed_delivery():
            logger.debug(
                "[Telegram] Dropping media group enqueue after disconnect started"
            )
            return
    ...
    async def _flush_media_group_event(self, media_group_id: str) -> None:
        ...
        try:
            ...
            if event is not None:
                if self._should_drop_delayed_delivery():
                    logger.debug(
                        "[Telegram] Dropping media group flush after disconnect started"
                    )
                    return
                await self.handle_message(event)
        except asyncio.CancelledError:
...
# clawk_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("CLAWK_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in clawk_cli/gateway.py.
    """
    from clawk_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'clawksis-agent[telegram]'",
