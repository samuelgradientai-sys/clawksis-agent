...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("CLAWK_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )

        self._text_batch_split_delay_seconds = float(
            os.getenv("CLAWK_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    async def _install_voice_mixer(self, guild_id: int, vc) -> None:
        ...
        def _after(error):
            ...
            if error:
                logger.error(
                    "Voice mixer stream error (guild=%d): %s", guild_id, error
                )
