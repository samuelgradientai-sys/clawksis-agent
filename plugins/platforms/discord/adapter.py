...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.clawksis/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="CLAWK_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="CLAWK_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Clawksis gateway
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``CLAWK_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_clawk_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Clawksis response. Emoji-only acks are
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Clawksis/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:clawk|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Clawksis pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="status", description="Show Clawksis session status")
        ...
        @tree.command(name="stop", description="Stop the running Clawksis agent")
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Clawksis skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from clawk_cli.commands import discord_skill_commands_by_category
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Clawksis has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Clawksis"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Clawksis: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "Clawksis session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Clawksis handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Clawksis to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Clawksis needs your input",
                description=body,
                color=discord.Color.orange(),
            )
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Clawksis could not create a Discord thread for "
...
def _component_check_auth(
    ...
    the pairing store (``clawk pairing approve``).
    ...
    # so users approved via ``clawk pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``clawk update`` prompts.
...
# is not in this process (e.g. ``clawk cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "CLAWK_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "CLAWK_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``clawk_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'clawksis-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # clawk_cli/setup.py::_setup_discord function.  Same shape as Teams.
