...
``_check_spotify_available()`` — when the user has not run ``clawk auth
spotify``, the tools remain registered (so they appear in ``clawk
...
The Spotify auth flow (``clawk auth spotify``), CLI plumbing, and docs
