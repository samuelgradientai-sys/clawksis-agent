...
- Hosted: API key from [app.supermemory.ai/integrations?connect=clawk](http://app.supermemory.ai/integrations?connect=clawk)
...
Before running `clawk memory setup`, add the local endpoint to
...
Then run `clawk memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$CLAWK_HOME/supermemory.json`
...
| `container_tag` | `clawk` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `clawk-{identity}` → `clawk-coder`). |
...
`https://api.supermemory.ai`. Clawksis resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: clawk`, and document writes stamp
`metadata.sm_source: clawk`. This is a **functional routing key, not telemetry**:
it groups Clawksis-written memories into a dedicated "Clawksis" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Clawksis profile:

```json
{
  "container_tag": "clawk-{identity}"
}
```

For a profile named `coder`, this resolves to `clawk-coder`. The default profile resolves to `clawk-default`. Without `{identity}`, all profiles share the same container.
