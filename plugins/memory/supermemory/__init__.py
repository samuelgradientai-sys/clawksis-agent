...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=clawk"
...
def _load_supermemory_config(clawk_home: str) -> dict:
    config = _default_config()
    config_path = Path(clawk_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, clawk_home: str) -> None:
    config_path = Path(clawk_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "clawk"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "clawk", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `clawk memory setup`.
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, clawk_home)
    ...
    def post_setup(self, clawk_home: str, config: dict) -> None:
        ...
        from clawk_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(clawk_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._config = _load_supermemory_config(self._clawk_home)
