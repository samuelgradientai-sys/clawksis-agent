...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Clawksis peer ID in OpenViking (default: clawk)
...
_DEFAULT_AGENT = "clawk"
_AGENT_PROMPT_LABEL = "Clawksis peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "CLAWK_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Clawksis slash-skill scaffolding before sending content to OpenViking.
...
def _load_clawk_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("CLAWK_HOME", "")).expanduser() if os.environ.get("CLAWK_HOME") else Path.home() / ".clawk"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Clawksis session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Clawksis only", "write values only to Clawksis .env"),
    ...
    _save_clawk_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Clawksis .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See clawksis-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_clawk_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Clawksis peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, clawk_home: str, config: dict) -> None:
        ...
        from clawk_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        clawk_home_path = Path(clawk_home)
        env_path = clawk_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_clawk_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Clawksis run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Clawksis' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Clawksis canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See clawksis-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/clawksis/.overview.md).
