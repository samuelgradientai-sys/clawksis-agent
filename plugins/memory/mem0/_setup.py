...
def _save_mem0_json(clawk_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(clawk_home) / "mem0.json"
    existing = {}
...
def _setup_platform(clawk_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.clawksis/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.clawksis/.env (or unset it) or Clawksis will keep "
...
def _setup_oss(clawk_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(clawk_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "clawk-user")
...
def _prompt_api_key(label: str, env_var: str, clawk_home: str) -> str:
...
_PGVECTOR_CONTAINER = "clawk-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "clawk"
...
def _setup_oss_interactive(clawk_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], clawk_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'clawk-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "clawk-user")

    agent_id = input("  Agent ID [clawk]: ").strip()
    agent_id = agent_id or "clawk"
...
def post_setup(clawk_home: str, config: dict) -> None:
    """Entry point called by clawk memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(clawk_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(clawk_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(clawk_home, config, flags)
        return
