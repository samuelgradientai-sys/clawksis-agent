...
Behavioral settings live in `$CLAWK_HOME/mem0.json` (set them via `clawk memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.clawksis/.env`.
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Clawksis session and call `mem0_search` — it connects to your server.
...
```bash
clawk memory setup mem0 --mode oss \
...
```bash
clawk memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$CLAWK_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.clawksis/mem0_qdrant"}}
  }
}
```
...
```bash
clawk memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$CLAWK_HOME/mem0.json`).
