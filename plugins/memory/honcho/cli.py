...
Handles: clawk honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "clawk" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Clawksis profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'clawk honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `clawk update` -- no output, no exceptions.
    """
    try:
        from clawk_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Clawksis profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CLAWK_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_clawk_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        clawk_host.get("pinUserPeer"),
        clawk_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in clawk_host:
        aliases_src = clawk_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in clawk_host:
        prefix_src = clawk_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(clawk_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom clawk_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    clawk_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        clawk_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(clawk_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = clawk_host.get("userPeerAliases")
    prefix = clawk_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(clawk_host.get('pinUserPeer'))}")
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        clawk_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        clawk_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Clawksis persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = clawk_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            clawk_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(clawk_host)
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'clawk honcho setup' to retry, or choose an API key instead.\n")
                return
            clawk_host["apiKey"] = cred.access_token
            clawk_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                clawk_host["peerName"] = cred.consent_peer_name
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'clawk honcho setup' again once you have a key.\n")
                return
    ...
    current_ai = clawk_host.get("aiPeer") or cfg.get("aiPeer", "clawk")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        clawk_host["aiPeer"] = new_ai

    current_workspace = clawk_host.get("workspace") or cfg.get("workspace", "clawk")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        clawk_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Clawksis GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, clawk_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Clawksis gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(clawk_host)
            clawk_host["pinUserPeer"] = False
            if prior_aliases:
                clawk_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                clawk_host, current_prefix, prefix_from_root,
            ...
            if merged:
                clawk_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                clawk_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                clawk_host, current_pin, current_aliases, current_prefix,
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        clawk_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = clawk_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        clawk_host.pop("contextTokens", None)
    ...
    try:
        ...
        if val >= 1:
            clawk_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        clawk_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        clawk_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        clawk_host["dialecticReasoningLevel"] = new_reasoning
    else:
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        clawk_host["sessionStrategy"] = new_strat

    clawk_host["enabled"] = True
    clawk_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        print("  Run: clawk config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    clawk honcho status     -- show full config")
    print("    clawk honcho mode       -- change recall/observation mode")
    print("    clawk honcho tokens     -- tune context and dialectic budgets")
    print("    clawk honcho peer       -- update peer names")
    print("    clawk honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Clawksis profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: clawk honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'clawk honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'clawk honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: clawk honcho map <session-name>")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: clawk honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'clawk honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'clawk honcho identity <file>' to seed one.")
        print()
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Clawksis + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Clawksis\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  2. Run:  clawk honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'clawk honcho setup' now?", default="y")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  clawk honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'clawk'")
        print("  with Honcho configured and no prior session history.")
        print("  (Clawksis calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    clawk honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'clawk honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Clawksis, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'clawk honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    clawk honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  clawk honcho identity <path/to/SOUL.md>")
    ...
    print("    Clawksis:   Honcho observes every message and updates representations")
    ...
    print("    Clawksis:   per-session by default — each run gets its own session")
    print("              Map a custom name:  clawk honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. clawk honcho setup              — configure API key (required)")
        print("  2. clawk honcho migrate            — re-run this walkthrough")
    else:
        print("  1. clawk honcho status             — verify Honcho connection")
...
def register_cli(subparser) -> None:
    """Build the ``clawk honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``clawk honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to clawk memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Clawksis Honcho",
    )
