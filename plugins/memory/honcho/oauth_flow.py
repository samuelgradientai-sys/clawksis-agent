...
``clawk://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Clawksis OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "clawksis-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``clawk-desktop`` / ``clawk-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Clawksis.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local CLAWK_HOME override can't reach.
