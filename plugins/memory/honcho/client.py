...
Resolution order for config file:
  ...
  1. $CLAWK_HOME/honcho.json  (instance-local, enables isolated Clawksis instances)
...
from clawk_cli.profiles import _get_default_clawk_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Clawksis profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Clawksis profile.



    Resolution order:

      1. CLAWK_HONCHO_HOST env var (explicit override)

      2. Active profile name via profiles system -> ``clawk.<profile>``

      3. Fallback: ``"clawk"`` (default profile)

    """

    explicit = os.environ.get("CLAWK_HONCHO_HOST", "").strip()
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      ...
      1. $CLAWK_HOME/honcho.json      (profile-local, if it exists)

      2. ~/.clawksis/honcho.json          (default profile — shared host blocks live here)
    ...
    local_path = get_clawk_home() / "honcho.json"
    ...
    default_path = _get_default_clawk_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Clawksis system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.clawk
    ...
    def from_global_config(
        ...
        Resolution: $CLAWK_HOME/honcho.json -> ~/.honcho/config.json -> env vars.

        When host is None, derives it from the active Clawksis profile.
        ...
        # A hosts.clawk block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Clawksis session title (from /title command)

          3. Gateway session key (stable per-chat identifier from gateway platforms)

          4. per-session strategy — Clawksis session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Clawksis session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'clawk honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run clawk honcho setup" hint they used to.
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `clawk honcho setup` to configure)."
        )
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            clawk_cfg = load_config()

            honcho_cfg = clawk_cfg.get("honcho", {})
