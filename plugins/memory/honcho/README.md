...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/clawk>
...
```bash
clawk memory setup honcho   # configure Honcho directly (works on a fresh install)
clawk memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
clawk config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.clawksis/.env
```

> `clawk honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `clawk memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `clawk model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CLAWK_HOME/honcho.json` | Profile-local (isolated Clawksis instances) |
| 2 | `~/.clawksis/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Clawksis profile: `clawk` (default) or `clawk_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `clawk honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Clawksis session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`clawksis-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`clawk`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-clawksis-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening clawk in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Clawksis session ID (timestamp + hex). Every `clawk` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Clawksis profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`clawk`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Clawksis profile: `clawk` (default) or `clawk_<profile>` (e.g. `clawk -p coder` -> host key `clawk_coder`). Older `clawk.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `CLAWK_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `clawksis-agent`) |
...
|---------|-------------|
| `clawk memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `clawk honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `clawk memory setup`) |
| `clawk honcho status` | Show resolved config for active profile |
| `clawk honcho enable` / `disable` | Toggle Honcho for active profile |
| `clawk honcho mode <mode>` | Change recall or observation mode |
| `clawk honcho peer --user <name>` | Update user peer name |
| `clawk honcho peer --ai <name>` | Update AI peer name |
| `clawk honcho tokens --context <N>` | Set context token budget |
| `clawk honcho tokens --dialectic <N>` | Set dialectic max chars |
| `clawk honcho map <name>` | Map current directory to a session name |
| `clawk honcho sync` | Create host blocks for all Clawksis profiles |
