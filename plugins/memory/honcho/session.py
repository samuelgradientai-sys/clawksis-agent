...
class HonchoSessionManager:
    ...
    Runs alongside clawk' existing SQLite state and file-based memory,
    ...
    def get_or_create(self, key: str) -> HonchoSession:
        ...
        assistant_peer_id = self._sanitize_id(
            self._config.ai_peer if self._config else "clawk-assistant"
        )
    ...
    def dialectic_query(
        ...
        try:
            ...
            # Only automatic injection uses the Clawksis-side character cap.
            if (
                apply_injection_cap
                and result
    ...
    def get_prefetch_context(
        ...
        # Also fetch AI peer's own representation so Clawksis knows itself.
        try:
    ...
    def migrate_memory_files(self, session_key: str, memory_dir: str) -> bool:
        ...
        Args:
            ...
            memory_dir: Path to the memories directory (~/.clawksis/memories/).
