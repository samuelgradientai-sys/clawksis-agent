"""Windows UTF-8 bootstrap for Clawksis entry points.
...
should be imported at the very top of every Clawksis entry point
(``clawk``, ``clawksis-agent``, ``clawk-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Clawksis modules.

    Clawksis ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Clawksis source root
...
def activate_durable_lazy_target() -> None:
    ...
    (``CLAWK_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("CLAWK_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import clawk_bootstrap``
# (or ``from clawk_bootstrap import apply_windows_utf8_bootstrap``) at
