...
from clawk_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    clawk_logging._logging_initialized = False
    ...
    clawk_logging._reset_queued_handlers()
    ...
    clawk_logging._install_session_record_factory()
    yield
    ...
    clawk_logging._logging_initialized = False
    clawk_logging.clear_session_context()


@pytest.fixture
def clawk_home(tmp_path, monkeypatch):
    """Provide an isolated CLAWK_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_clawk_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["CLAWK_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, clawk_home):
        log_dir = clawk_logging.setup_logging(clawk_home=clawk_home)
        assert log_dir == clawk_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, clawk_home):
    ...
    def test_creates_errors_log_handler(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)
        root = logging.getLogger()
    ...
    def test_force_reinitializes(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        clawk_logging.setup_logging(clawk_home=clawk_home, force=True)
    ...
    def test_custom_log_level(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, clawk_home):
        clawk_logging.setup_logging(
            clawk_home=clawk_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)
    ...
    def test_writes_to_agent_log(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)

        test_logger = logging.getLogger("test_clawk_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        clawk_logging.flush_log_queue()

        agent_log = clawk_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)

        test_logger = logging.getLogger("test_clawk_logging.warning_test")
        test_logger.warning("this is a warning")

        clawk_logging.flush_log_queue()

        agent_log = clawk_home / "logs" / "agent.log"
        errors_log = clawk_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)

        test_logger = logging.getLogger("test_clawk_logging.info_test")
    ...
    def test_explicit_params_override_config(self, clawk_home):
        ...
        clawk_logging.setup_logging(clawk_home=clawk_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, clawk_home):
        """The custom record factory injects session_tag on all records."""
        clawk_logging.setup_logging(clawk_home=clawk_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_clawk_session_injector", False), (
            "Record factory should have _clawk_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, clawk_home):
    ...
    def test_gateway_log_not_created_in_cli_mode(self, clawk_home):
    ...
    def test_gateway_log_created_after_cli_init(self, clawk_home):
        ...
        gw_log = clawk_home / "logs" / "gateway.log"
    ...
    def test_gateway_log_receives_gateway_records(self, clawk_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, clawk_home):
    ...
    def test_agent_log_still_receives_all(self, clawk_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home, mode="cli")
        clawk_logging.setup_logging(clawk_home=clawk_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home, mode="gui")

        logging.getLogger("clawk_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        clawk_logging.flush_log_queue()

        gui_log = clawk_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, clawk_home):
        """When session context is set, log lines include [session_id]."""
        clawk_logging.setup_logging(clawk_home=clawk_home)
        clawk_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, clawk_home):
    ...
    def test_clear_session_context(self, clawk_home):
        ...
        clawk_logging.setup_logging(clawk_home=clawk_home)
        clawk_logging.set_session_context("xyz789")
        clawk_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, clawk_home):
        ...
        def thread_a():
            clawk_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            clawk_logging.flush_log_queue()

        def thread_b():
            clawk_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        clawk_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        clawk_logging.set_session_context("any_handler_test")
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in clawk_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the clawk_plugins
        ...
        # break this test.
        gateway_prefixes = clawk_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "clawk_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = clawk_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == clawk_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = clawk_logging.COMPONENT_PREFIXES["cli"]
        assert "clawk_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == clawk_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = clawk_logging.COMPONENT_PREFIXES["gui"]
        assert "clawk_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)
        clawk_logging.setup_verbose_logging()
    ...
    def test_idempotent(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)
        clawk_logging.setup_verbose_logging()
        clawk_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        clawk_logging.set_session_context("factory_test")
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(clawk_logging.sys, "platform", "win32"):
            assert clawk_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
            assert not clawk_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("some other logging failure")
            )

        with patch.object(clawk_logging.sys, "platform", "linux"):
            assert not clawk_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, clawk_home):
        level, max_size, backup = clawk_logging._read_logging_config()
...
class TestExternalRotationRecovery:
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        clawk_logging.setup_logging(clawk_home=clawk_home, mode="gateway")
        gw_path = clawk_home / "logs" / "gateway.log"
        rotated = clawk_home / "logs" / "gateway.log.1"
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)
        root = logging.getLogger()
    ...
    def test_records_reach_file_through_queue(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)
    ...
    def test_queue_preserves_per_handler_levels(self, clawk_home):
        clawk_logging.setup_logging(clawk_home=clawk_home)
