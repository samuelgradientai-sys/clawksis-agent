...
_CLAWK_BEHAVIORAL_VARS = frozenset({
    ...
    "CLAWK_SESSION_CHAT_NAME",
    "CLAWK_SESSION_THREAD_ID",
    "CLAWK_SESSION_SOURCE",
    "CLAWK_SESSION_KEY",
    ...
    "CLAWK_BACKGROUND_NOTIFICATIONS",
    "CLAWK_EXEC_ASK",
    "CLAWK_HOME_MODE",
    "CLAWK_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.clawksis/kanban.db instead of the per-test CLAWK_HOME.
    "CLAWK_KANBAN_DB",
    "CLAWK_KANBAN_BOARD",
    "CLAWK_KANBAN_HOME",
    "CLAWK_KANBAN_WORKSPACES_ROOT",
    "CLAWK_KANBAN_LOGS_ROOT",
    "CLAWK_KANBAN_TASK",
    "CLAWK_KANBAN_WORKSPACE",
    "CLAWK_KANBAN_RUN_ID",
    ...
    "CLAWK_DASHBOARD_OAUTH_CLIENT_ID",
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live clawk-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting clawk/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `clawk update` (or the
        # equivalent `python -m clawk_cli.main update`).  These commands
        ...
        if "update" in low and (
            # clawk update / clawk update --gateway / setsid bash -c ... clawk update
            ("clawk" in low and "update" in low.split())
            or
            # python -m clawk_cli.main update --gateway
            ("clawk_cli" in low and "update" in low.split())
            or
            # venv/bin/clawk update  (absolute path variant used in tests)
            (".venv/bin/clawk" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`clawk update` against the real checkout, fetching "
