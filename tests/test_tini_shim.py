...
These run without Docker: the shim's CLAWK_TINI_SHIM_TARGET /
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["CLAWK_TINI_SHIM_TARGET"] = str(init)
    env["CLAWK_TINI_SHIM_WRAPPER"] = str(wrapper)
...
def test_shim_script_is_executable_bit_friendly() -> None:
    ...
    assert "CLAWK_TINI_SHIM_TARGET" in text
