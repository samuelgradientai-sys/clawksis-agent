...
which is already bounded by CLAWK_CRON_TIMEOUT — the worker thread submitted
...
timeout (CLAWK_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
def _hanging_session_db(never_set: threading.Event):
    """Stand-in for clawk_state.SessionDB() that blocks until released —
...
class TestSessionDbInitTimeout:
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CLAWK_CRON_SESSION_DB_TIMEOUT", raising=False)
