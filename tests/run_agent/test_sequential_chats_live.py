...
AlexKucera reported on Discord (2026-04-16) that after ``clawk update`` pulled
...
Opt-in — not part of default CI:
    ...
    CLAWK_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v



Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.clawksis/.env).
...
# Load ~/.clawksis/.env so live runs pick up OPENROUTER_API_KEY without
...
def _load_user_env() -> None:
    ...
    env_file = Path.home() / ".clawksis" / ".env"
