...
def test_build_api_kwargs_codex(monkeypatch):
    ...
    assert kwargs["instructions"] == "You are Clawksis."
...
def test_run_codex_stream_ignores_completed_response_with_null_output(monkeypatch):
    ...
    'NoneType' object is not iterable``. The Clawksis runtime consumes raw
...
def test_copilot_final_preflight_sanitizes_both_middleware_layers(monkeypatch):
    ...
    monkeypatch.setattr(
        "clawk_cli.middleware.apply_llm_request_middleware",
        _request_middleware,
    )
    monkeypatch.setattr(
        "clawk_cli.middleware.run_llm_execution_middleware",
...
def test_mid_turn_compaction_does_not_double_persist_in_place_rows(monkeypatch, tmp_path):
    ...
    def _fake_compress_context(messages, system_message, *, approx_tokens=None, task_id="default", focus_topic=None):
        ...
        return compacted, "You are Clawksis."
...
def test_dump_api_request_debug_redacts_request_and_error_secrets(monkeypatch, tmp_path, capsys):
    ...
    _patch_agent_bootstrap(monkeypatch)
    monkeypatch.setenv("CLAWK_DUMP_REQUEST_STDOUT", "1")
    agent = run_agent.AIAgent(
        model="gpt-4o",
        base_url="http://127.0.0.1:9208/v1",
...
def test_normalize_codex_response_reasoning_only_completed_is_stop_for_other_backends(monkeypatch):
    ...
    See https://github.com/samuelgradientai-sys/clawksis-agent/issues/64434
    """
    agent = _build_agent(monkeypatch)
    from agent.codex_responses_adapter import _normalize_codex_response
...
def test_duplicate_detection_distinguishes_different_codex_message_items(monkeypatch):
    """Fork-only: consecutive incomplete turns carrying *different* visible
    commentary must each survive as their own interim, with their own message
    id/status — the visible-content dedup (#52711) must not collapse them.
    """
    agent = _build_agent(monkeypatch)
    responses = [
        SimpleNamespace(
        ...
        SimpleNamespace(
            output=[
                SimpleNamespace(
                    ...
                    phase="commentary",
                    status="in_progress",
                    content=[SimpleNamespace(type="output_text", text="Still working, almost there...")],
                )
            ],
            usage=SimpleNamespace(input_tokens=50, output_tokens=10, total_tokens=60),
            status="in_progress",
