...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    clawk_state = ModuleType("clawk_state")
    clawk_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "clawk_state", clawk_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Clawksis"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Clawksis"
