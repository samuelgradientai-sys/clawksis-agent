...
async def test_first_run_slack_home_channel_onboarding_uses_parent_command(monkeypatch):
    ...
    assert "/clawk sethome" in onboarding
...
async def test_status_command_bypasses_active_session_guard():
    ...
    async def fake_handler(event):
        ...
        return "📊 **Clawksis Gateway Status**\n**Agent Running:** Yes ⚡"
...
async def test_profile_command_reports_custom_root_profile(monkeypatch, tmp_path):
    """Gateway /profile detects custom-root profiles (not under ~/.clawksis)."""
...
async def test_profile_command_reports_source_stamped_profile(monkeypatch, tmp_path):
    ...
    profile_home = clawk_home / "profiles" / "milo"
...
@pytest.mark.asyncio
async def test_profile_command_ignores_stamp_when_multiplexing_off(
    ...
    assert f"**Home:** `{clawk_home}`" in result
...
async def test_post_delivery_callback_generation_snapshot_happens_after_bind():
    ...
    _clawk_run_generation is set on the interrupt event by
    ...
    async def fake_handler(event):
        ...
        setattr(interrupt_event, "_clawk_run_generation", 1)
