...
def _make_runner_with_adapter(session_id: str = None):
    ...
    # module-import time, defeating per-test CLAWK_HOME monkeypatches).
...
async def test_goal_verdict_done_sent_via_adapter_send(clawk_home):
    ...
    with patch("clawk_cli.goals.judge_goal", return_value=("done", "the feature shipped", False)):
...
async def test_goal_verdict_continue_enqueues_continuation(clawk_home):
    ...
    with patch("clawk_cli.goals.judge_goal", return_value=("continue", "still needs work", False)):
...
async def test_goal_verdict_budget_exhausted_sends_pause(clawk_home):
    ...
    from clawk_cli.goals import GoalManager, save_goal
    ...
    with patch("clawk_cli.goals.judge_goal", return_value=("continue", "keep going", False)):
...
async def test_goal_verdict_skipped_when_no_active_goal(clawk_home):
...
async def test_goal_verdict_survives_adapter_without_send(clawk_home):
    ...
    with patch("clawk_cli.goals.judge_goal", return_value=("done", "ok", False)):
