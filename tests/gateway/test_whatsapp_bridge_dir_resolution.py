...
(/opt/clawksis/scripts/whatsapp-bridge) is read-only, so `npm install` fails
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    # Nothing mirrored into CLAWK_HOME.
    assert not (clawk_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_clawk_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable CLAWK_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = clawk_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the CLAWK_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    clawk_home = tmp_path / "clawk_home"
    mirror = clawk_home / "scripts" / "whatsapp-bridge"
