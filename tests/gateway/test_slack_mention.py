...
def test_mpim_unmentioned_strict_mention_ignored():
    ...
    adapter = _make_adapter(
        require_mention=True, strict_mention=True, free_response_channels=[]
    )
...
def test_mpim_mentioned_processed():
    ...
    assert (
        _would_process(adapter, channel_type="mpim", mentioned=True, text="hello")
        is True
    )
...
def test_mpim_in_free_response_processed_without_mention():
    ...
    adapter = _make_adapter(require_mention=True, free_response_channels=["G_MPIM"])
    assert (
        _would_process(adapter, channel_type="mpim", channel_id="G_MPIM", text="hello")
        is True
    )
...
def test_mention_patterns_list_matches():
    adapter = _make_adapter(mention_patterns=["hey clawk", "clawk,"])
    assert (
        adapter._slack_message_matches_mention_patterns("hey clawk, you there?") is True
    )
    assert adapter._slack_message_matches_mention_patterns("just chatting") is False


def test_mention_patterns_case_insensitive():
    adapter = _make_adapter(mention_patterns=["hey clawk"])
    assert adapter._slack_message_matches_mention_patterns("HEY CLAWKSIS!") is True


def test_mention_patterns_single_string():
    adapter = _make_adapter(mention_patterns="^clawk")
    assert adapter._slack_message_matches_mention_patterns("clawk do this") is True
    assert adapter._slack_message_matches_mention_patterns("ok clawk") is False


def test_mention_patterns_invalid_regex_skipped_without_crash():
    # An invalid pattern is dropped; valid siblings still work.
    adapter = _make_adapter(mention_patterns=["(unclosed", "hey clawk"])
    assert adapter._slack_message_matches_mention_patterns("hey clawk") is True


def test_mention_patterns_env_var_fallback(monkeypatch):
    monkeypatch.setenv("SLACK_MENTION_PATTERNS", '["hey clawk", "clawk,"]')
    adapter = _make_adapter()  # no config value -> falls back to env
    assert adapter._slack_message_matches_mention_patterns("hey clawk") is True
...
def test_mention_patterns_env_var_csv_fallback_splits_patterns(monkeypatch):
    ...
    assert [pattern.pattern for pattern in patterns] == ["hey clawk", "clawk"]
    assert adapter._slack_message_matches_mention_patterns("hey clawk") is True
...
def test_mention_patterns_trigger_in_channel_without_literal_mention():
    ...
    adapter = _make_adapter(require_mention=True, mention_patterns=["hey clawk"])
    assert _would_process(adapter, text="hey clawk what's the status") is True
