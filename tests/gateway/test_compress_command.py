...
async def test_compress_command_preserves_platform_and_gateway_session_key():
    ...
    assert kwargs.get("gateway_session_key") == runner._session_key_for_source(
        _make_source()
    )
...
async def test_compress_command_overrides_stale_resolver_identity():
    ...
    assert kwargs["gateway_session_key"] == runner._session_key_for_source(
        _make_source()
    )
