...
def adapter(tmp_path):
    ...
    ~/.clawksis/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.clawksis/...)
...
class TestHttpEventIngress:
    ...
    def test_verify_google_id_token_uses_cached_request_and_configured_audience(
        self, monkeypatch
    ):
        request = object()
        captured = {}
        ...
        monkeypatch.setattr(
            sys.modules["google.oauth2"], "id_token", FakeIdToken, raising=False
        )

        assert _gc_mod._verify_google_id_token(
            "signed-token", "https://callback.example/events"
    ...
    def test_verify_http_event_request_accepts_expected_google_identity(
        self, monkeypatch
    ):
        cfg = PlatformConfig(enabled=True)
        cfg.extra.update({
...
class TestConnectModes:
    @pytest.mark.asyncio
    async def test_connect_http_mode_skips_pubsub_subscriber(
        ...
        monkeypatch.setattr(
            a, "_load_sa_credentials", MagicMock(return_value=MagicMock())
        )
        monkeypatch.setattr(
            _gc_mod, "build_service", MagicMock(return_value=MagicMock())
        )
        subscriber_client = MagicMock()
        monkeypatch.setattr(
            _gc_mod, "pubsub_v1", MagicMock(SubscriberClient=subscriber_client)
        )
...
class TestOnPubsubMessage:
    ...
    def test_relay_flat_bot_sender_is_filtered_end_to_end(self, adapter):
        ...
        envelope = {
            ...
            "sender_display_name": "ClawksisBot",
            "sender_type": "BOT",
            "text": "reply from bot",
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the Clawksis host. The relay flattens Chat
...
class TestSend:
    ...
    def test_card_spec_to_cards_v2_builds_button_card(self):
        ...
        assert {"key": "choice", "value": "yes"} in button["onClick"]["action"][
            "parameters"
        ]
    ...
    async def test_send_clarify_posts_choice_card(self, adapter):
        ...
        assert buttons[0]["onClick"]["action"]["function"] == "clawk_clarify"
        assert {"key": "choice", "value": "Simple"} in buttons[0]["onClick"]["action"][
            "parameters"
        ]
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_default_status_text(self, adapter):
        ...
        await adapter.send_typing("spaces/S")
        body = adapter._create_message.await_args.args[1]
        assert body["text"] == "Clawksis is thinking…"
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'Clawksis is thinking…' stuck in chat. Race fix via
...
class TestUserOAuthHelper:
    ...
    def test_profile_local_client_secret_takes_precedence(self, tmp_path, monkeypatch):
        """Fork guard: the OAuth client secret is resolved per CLAWK_HOME, so a
        gateway running under a named profile reads its own profile-local
        secret (separate OAuth app per bot) and never the default root's."""
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/clawk-chat"
            ),
        ...
        monkeypatch.setattr("clawk_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("clawk_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "clawk_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr("clawk_cli.cli_output.print_info", lambda *_a, **_kw: None)
        monkeypatch.setattr(
            "clawk_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "clawk_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
