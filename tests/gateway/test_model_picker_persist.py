...
closure the PR changed, against a real temp ``CLAWK_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "clawk_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
async def test_multiplex_picker_keeps_profile_adapter_and_callback_scope(
    ...
    (named_home / ".env").write_text(
        "PROFILE_MODEL_KEY=named-secret\n", encoding="utf-8"
    )
    ...
    monkeypatch.setattr("clawk_cli.model_switch.switch_model", _profile_switch)
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    named_adapter = _FakePickerAdapter()
    runner = _make_named_runner(monkeypatch, default_adapter, named_adapter, named_home)
    monkeypatch.setattr(gateway_run, "_clawk_home", default_home)
    _stub_picker_dependencies(monkeypatch)
