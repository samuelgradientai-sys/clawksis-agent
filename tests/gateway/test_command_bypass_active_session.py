...
class TestBypassWithBotnameSuffix:
    ...
    async def test_stop_with_botname(self):
        """/stop@MyClawksisBot must bypass the guard."""
        ...
        await adapter.handle_message(_make_event("/stop@MyClawksisBot"))

        assert sk not in adapter._pending_messages, (
            "/stop@MyClawksisBot was queued instead of bypassing"
        )
    ...
    async def test_new_with_botname(self):
        """/new@MyClawksisBot must bypass the guard."""
        ...
        await adapter.handle_message(_make_event("/new@MyClawksisBot"))
