...
class TestMattermostWebSocketParsing:
    def setup_method(self):
        ...
        self.adapter._bot_username = "clawk-bot"
...
class TestMattermostMentionBehavior:
    ...
    async def test_mention_stripped_from_text(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MATTERMOST_REQUIRE_MENTION", None)
            await self.adapter._handle_ws_event(
                self._make_event("@clawk-bot what is 2+2")
            )
            assert self.adapter.handle_message.called
            msg = self.adapter.handle_message.call_args[0][0]
            assert "@clawk-bot" not in msg.text
...
async def test_mattermost_top_level_channel_post_is_thread_root():
    ...
    adapter._bot_username = "clawk-bot"
    adapter.handle_message = AsyncMock()
    post_data = {
        "id": "top_post_123",
        "user_id": "user_123",
        "channel_id": "chan_456",
        "message": "@clawk-bot start work",
        "root_id": "",
    }
    event = {
        "event": "posted",
