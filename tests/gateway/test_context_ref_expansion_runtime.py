...
``ClawksisCLI``, which does carry ``self.model``/``self.base_url`` — see
...
async def test_at_reference_resolves_model_via_session_runtime(monkeypatch):
    ...
    async def _passthrough_preprocess(
        message, *, cwd, context_length, url_fetcher=None, allowed_root=None
    ):
...
@pytest.mark.asyncio
async def test_at_reference_uses_routed_profile_scope_when_multiplexed(
...
async def test_at_reference_passes_compatible_custom_provider_context(monkeypatch):
    ...
    monkeypatch.setattr(
        config_mod, "get_compatible_custom_providers", lambda _cfg: custom_providers
    )
...
async def test_at_reference_applies_custom_runtime_budget_to_preprocessor(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_load_gateway_config",
        lambda: {
            "model": {"default": "global/model", "context_length": 128000},
            "custom_providers": custom_providers,
        },
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        config_mod, "get_custom_provider_context_length", lambda **_kwargs: 32768
    )
    ...
    async def _preprocess(message, *, context_length, **_kwargs):
        ...
        return ContextReferenceResult(
            message="expanded", original_message=message, expanded=True
        )
...
@pytest.mark.asyncio
async def test_at_reference_ignores_global_context_for_session_model_override(
    monkeypatch,
):
