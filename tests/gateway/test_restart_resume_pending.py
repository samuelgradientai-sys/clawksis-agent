...
class TestResumePendingSystemNote:
    ...
    def test_new_message_guidance_identical_regardless_of_interactivity(self):
        ...
        a = build_resume_recovery_note(
            "restart_timeout", "do the thing", interactive=True
        )
        b = build_resume_recovery_note(
            "restart_timeout", "do the thing", interactive=False
        )
        assert a == b
...
class TestFreshnessHelpers:
    ...
    def test_auto_continue_freshness_window_malformed_falls_back(self, monkeypatch):
        monkeypatch.setenv("CLAWK_AUTO_CONTINUE_FRESHNESS", "not-a-number")
        assert _auto_continue_freshness_window() == 3600.0

    def test_auto_continue_freshness_window_empty_falls_back(self, monkeypatch):
        monkeypatch.setenv("CLAWK_AUTO_CONTINUE_FRESHNESS", "")
        assert _auto_continue_freshness_window() == 3600.0
