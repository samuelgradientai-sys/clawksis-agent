...
async def test_expect_edits_metadata_keeps_preview_on_legacy_path():
    ...
    # rich messages until Clawksis wires rich_message edits directly.
    bot = adapter._bot
    assert bot is not None
    bot.do_api_request.assert_not_called()
    bot.send_message.assert_awaited()
...
@pytest.mark.asyncio
async def test_rich_transport_error_redacts_bot_token_even_when_redaction_disabled(
    monkeypatch,
):
...
@pytest.mark.asyncio
async def test_legacy_send_error_redacts_bot_token_without_traceback(
    monkeypatch, caplog
):
    import agent.redact as redact
...
@pytest.mark.asyncio
async def test_legacy_edit_error_logs_redacted_bot_token_without_traceback(
    monkeypatch, caplog
):
    import agent.redact as redact
