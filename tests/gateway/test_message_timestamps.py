...
def test_render_user_content_adds_single_context_timestamp():
    ...
    assert rendered == (
        "[Tue 2026-04-28 13:40:53 CEST] [Example User] Timestamp should be in context"
    )
...
def test_render_user_content_deduplicates_existing_timestamp_and_preserves_embedded_time():
    ...
    stored_content = (
        "[Mon 2026-04-27 15:54:44 CEST] [Example User] This should go on our todo list"
    )
...
def test_message_timestamps_enabled_defaults_off():
    ...
    assert _message_timestamps_enabled({"gateway": {"message_timestamps": {}}}) is False
...
def test_message_timestamps_enabled_when_opted_in():
    ...
    assert (
        _message_timestamps_enabled({"gateway": {"message_timestamps": True}}) is True
    )
