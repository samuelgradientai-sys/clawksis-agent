...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="ClawksisBot"),
    )
...
async def test_auto_registers_plugin_commands_for_discord(adapter):
    ...
    with patch(
        "clawk_cli.plugins.get_plugin_commands",
        return_value={
            "metricas": {
...
async def test_slash_command_registration_stays_under_discord_limit(adapter):
    ...
    with patch("clawk_cli.plugins.get_plugin_commands", return_value=many_plugins):
        adapter._register_slash_commands()
...
async def test_auto_create_thread_uses_message_content_as_name(adapter):
    ...
    assert thread._clawk_auto_thread_initial_name == "Hello world, how are you?"
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_clawk_when_only_mentions(adapter):
    """If a message contains only mention syntax, the stripped content is
    empty — fall back to the 'Clawksis' default rather than ''."""
    thread = SimpleNamespace(id=999, name="Clawksis")
    message = SimpleNamespace(
...
async def test_rename_thread_edits_only_when_current_name_matches(adapter):
    ...
    thread.edit.assert_awaited_once_with(
        ...
        reason="Clawksis semantic session title",
    )
...
async def test_auto_thread_source_carries_initial_name_for_semantic_rename(adapter, monkeypatch):
    ...
    thread = SimpleNamespace(
        id=999,
        name="raw user prompt",
        _clawk_auto_thread_initial_name="raw user prompt",
    )
    adapter._auto_create_thread = AsyncMock(return_value=thread)
