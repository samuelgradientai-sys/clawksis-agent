...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "clawk-gateway"
    ...
    def test_get_running_pid_accepts_no_supervisor_restart_runtime(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            status,
            "_read_process_cmdline",
            lambda pid: "python -m clawk_cli.main gateway restart",
        )
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level CLAWK_HOME.
        ...
        the process-level CLAWK_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("CLAWK_HOME", str(process_home))

        # Simulate a profile context override being active during write.
        token = set_clawk_home_override(str(profile_home))
        try:
            status.write_pid_file()
        finally:
...
class TestGatewayRuntimeStatus:
    ...
    def test_write_runtime_status_overwrites_stale_argv_on_restart(self, tmp_path, monkeypatch):
        ...
        payload = status.read_runtime_status()
        assert payload["argv"] == ["/new/path/clawk", "gateway", "run"]
        assert payload["pid"] == os.getpid()
    ...
    def test_runtime_status_running_pid_rejects_stale_record_for_supervisor_pid(self, monkeypatch):
        ...
        payload = {
            ...
            "gateway_state": "running",
            "kind": "clawk-gateway",
            "argv": ["/opt/clawksis/.venv/bin/clawk", "gateway", "run", "--replace"],
        }
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``clawk gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "clawk gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "clawk -p coder gateway run --replace",
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_clawk_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's CLAWK_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("CLAWK_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_clawk_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        CLAWK_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.clawksis flap
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``clawk gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00clawk_cli/main.py\x00gateway\x00"
    ...
    def test_windows_skips_ps_fallback_and_uses_psutil(self, monkeypatch):
        ...
        class _Proc:
            ...
            def cmdline(self):
                return ["pythonw.exe", "-m", "clawk_cli.main", "gateway", "run"]
        ...
        assert result == "pythonw.exe -m clawk_cli.main gateway run"
        assert ps_calls == []
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CLAWK_HOME", str(tmp_path))
        from clawk_cli.gateway import generate_launchd_plist
