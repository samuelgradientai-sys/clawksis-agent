...
def _isolate_env(tmp_path, monkeypatch):
    ...
    The global hermetic fixture already redirects CLAWK_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    CLAWK_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "clawk_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["clawk_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "clawksis-agent" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "clawksis-agent":
                raise AssertionError("quick() descended into protected clawksis-agent/")
