...
def test_setup_linux_loads_null_sink_and_virtual_source():
    ...
    assert any(a.startswith("sink_name=clawk_meet_sink") for a in calls[0])

    assert calls[1][0] == "pactl" and calls[1][1] == "load-module"

    assert "module-virtual-source" in calls[1]

    assert any(a.startswith("source_name=clawk_meet_src") for a in calls[1])
    ...
    assert info["device_name"] == "clawk_meet_src"

    assert info["write_target"] == "clawk_meet_sink"
