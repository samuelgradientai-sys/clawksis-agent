...
def test_nemo_relay_plugin_emits_llm_tool_and_exports_atif(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CLAWK_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "atof"))
    monkeypatch.setenv("CLAWK_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("CLAWK_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
    ...
    assert (tmp_path / "atif" / "clawk-atif-s1.json").exists()
...
def test_nemo_relay_plugin_emits_approval_marks(monkeypatch):
    ...
    assert "clawk.approval.response" in mark_names
...
def test_nemo_relay_plugin_emits_unmatched_fallback_marks(monkeypatch):
    ...
    assert "clawk.api.error" in mark_names
    assert "clawk.tool.response.unmatched" in mark_names
...
def test_nemo_relay_plugin_reparents_child_session_scope_for_embedded_atif(monkeypatch):
    ...
    child_push = next(
        event
        for event in fake.events
        if event[0] == "scope.push" and event[1] == "clawk-session-child-session"
    )
    child_kwargs = child_push[3]
    assert child_kwargs["handle"] == ("scope", "clawk-session-parent-session")
...
def test_nemo_relay_plugin_skips_embedded_child_atif_file_by_default(
    ...
    assert (tmp_path / "atif" / "clawk-atif-parent-session.json").exists()
...
def test_nemo_relay_plugin_can_write_embedded_child_atif_file_in_all_mode(
    ...
    monkeypatch.setenv("CLAWK_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("CLAWK_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
    monkeypatch.setenv("CLAWK_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE", "all")
    ...
    assert (tmp_path / "atif" / "clawk-atif-parent-session.json").exists()
    assert (tmp_path / "atif" / "clawk-atif-child-session.json").exists()
...
""",
    ...
    assert "plugin.clear" not in event_names
    assert "atif.register" in event_names
    assert (tmp_path / "direct-atif" / "clawk-atif-s1.json").exists()
...
def test_nemo_relay_downstream_unwrap_matches_real_middleware_wrapper_shape(
    ...
    # ``clawk_cli.middleware._run_execution_chain`` hands to a middleware
    ...
    from clawk_cli import middleware
