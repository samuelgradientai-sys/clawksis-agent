...
Run from the PR worktree:
    ...
    cd ~/.clawksis/clawksis-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.clawksis/clawksis-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.clawksis/clawksis-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a clawksis-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a clawksis-agent checkout"
)
...
# Isolated CLAWK_HOME for the config write.
