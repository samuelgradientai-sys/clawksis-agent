...
@pytest.fixture
def tmp_clawk_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    home = tmp_path / "clawk"
    home.mkdir()
...
def test_store_and_load_photon_token(tmp_clawk_home: Path) -> None:
    ...
    auth_json = json.loads((tmp_clawk_home / "auth.json").read_text())
...
def test_store_project_credentials_writes_env(tmp_clawk_home: Path) -> None:
    photon_auth.store_project_credentials(
        spectrum_project_id="sp-789",
        project_secret="sek-ret",
        dashboard_project_id="dash-1",
    )
    env_text = (tmp_clawk_home / ".env").read_text()
    assert "PHOTON_PROJECT_ID=sp-789" in env_text
    assert "PHOTON_PROJECT_SECRET=sek-ret" in env_text


def test_store_user_numbers_round_trip(tmp_clawk_home: Path) -> None:
...
def test_create_project_sends_spectrum_true(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(photon_auth.httpx, "post", fake_post)
    data = photon_auth.create_project("tok", name="Clawksis")
    assert data["id"] == "new-proj"
    assert captured["body"]["spectrum"] is True
    assert captured["body"]["name"] == "Clawksis"
