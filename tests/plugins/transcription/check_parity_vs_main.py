...
the legacy ``CLAWK_LOCAL_STT_COMMAND`` escape hatch exists) or this PR's
...
def _resolve_main_dir() -> Path:
    ...
    sibling = REPO_ROOT.parent / "clawksis-agent-main"
...
# Isolated CLAWK_HOME so the config write is hermetic.
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'clawksis-agent-main' checkout pinned to "
