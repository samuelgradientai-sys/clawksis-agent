...
def test_post_setup_create_remote_user_profile_can_mirror_to_openviking_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            ...
            "Clawksis peer ID in OpenViking": "clawk",
...
def test_post_setup_create_remote_user_can_keep_clawk_only(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    clawk_home = tmp_path / "clawk"
    clawk_home.mkdir()
    monkeypatch.setenv("CLAWK_HOME", str(clawk_home))
...
def test_post_setup_user_key_path_can_route_detected_root_key_to_root_setup(tmp_path, monkeypatch):
    ...
    assert prompt_events.count("Clawksis peer ID in OpenViking") == 1
    env_text = (clawk_home / ".env").read_text(encoding="utf-8")
...
def test_https_local_endpoint_is_not_runtime_autostart_eligible(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory disabled for this Clawksis run. "
...
def test_runtime_openviking_waiter_warns_when_background_start_times_out(monkeypatch):
    ...
    assert warnings == [
        ...
        "within 60 seconds. OpenViking memory disabled for this Clawksis run."
    ]
...
def test_tool_add_resource_rejects_clawk_credential_file_upload(tmp_path, monkeypatch):
...
def test_tool_add_resource_directory_zip_skips_clawk_credential_files(tmp_path, monkeypatch):
    ...
    clawk_home = tmp_path / "clawk_home"
    clawk_home.mkdir()
    (clawk_home / "guide.md").write_text("# Guide\n", encoding="utf-8")
    ...
    provider._client.post.return_value = {
        "status": "ok",
        "result": {"root_uri": "viking://resources/clawk_home"},
    }

    result = json.loads(provider._tool_add_resource({"url": str(clawk_home)}))
...
def test_handle_tool_call_forget_deletes_exact_memory_file_uri():
    uri = "viking://user/peers/clawk/memories/preferences/mem_abc123.md"
    provider = OpenVikingMemoryProvider()
    provider._client = MagicMock()
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "clawk"
...
def test_viking_client_headers_include_bearer_when_api_key_set():
    ...
    assert headers["X-OpenViking-Actor-Peer"] == "clawk"
    assert "X-OpenViking-Agent" not in headers
    assert "X-OpenViking-Account" not in headers
    assert "X-OpenViking-User" not in headers
...
def test_viking_client_health_sends_auth_headers(monkeypatch):
    ...
    assert captured["headers"]["X-OpenViking-Actor-Peer"] == "clawk"
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for clawksis-agent#28296 review (M3): once a session is marked
...
def test_on_session_switch_does_not_block_caller_on_slow_drain():
    """Regression for clawksis-agent#28296 review (H1): on_session_switch must
...
def test_on_session_switch_defers_old_commit_to_finalizer_thread():
    ...
    -_DEFERRED_COMMIT_TIMEOUT drain or the commit POST. See clawksis-agent#28296
...
def test_on_memory_write_uses_content_write_independent_of_session_rotation():
    ...
    assert captured_payloads[0]["uri"].startswith(
        "viking://user/peers/clawk/memories/preferences/mem_"
    )
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/clawk/memories/events/mem_1.md",
                            "score": 0.9,
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/clawk/memories/events/mem_2.md",
                            "score": 0.8,
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/clawk/memories/events/mem_3.md",
                            "score": 0.9,
                            "level": 2,
                            "category": "events",
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/clawk/memories/one.md",
                            "score": 0.9,
                            "abstract": "",
                        },
                    ],
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": f"viking://user/peers/clawk/memories/events/mem_{idx}.md",
                            "score": 0.9 - (idx * 0.01),
                            "level": 2,
                            "category": "events",
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/clawk/memories/events/mem_timeout.md",
                            "score": 0.9,
                            "level": 2,
                            "category": "events",
                            "abstract": "short abstract",
