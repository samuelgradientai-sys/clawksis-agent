...
class TestMem0Prefetch:
    ...
    def test_prefetch_returns_memories_on_first_call(self):
        ...
        backend = FakeBackend(
            search_results=[{"id": "m1", "memory": "lives in Berlin"}]
        )
        provider = self._make_provider(backend)
