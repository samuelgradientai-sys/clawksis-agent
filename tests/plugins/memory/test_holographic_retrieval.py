...
def test_prefetch_recovers_prose_query(retriever_with_facts):
    ...
    """
    results = retriever_with_facts.search("what happened with the deployment rollback")
...
def test_prefetch_single_keyword_still_works(retriever_with_facts):
    ...
    assert len(results) >= 1
    assert (
        "Compaction" in results[0]["content"]
        or "compaction" in results[0]["content"].lower()
    )
