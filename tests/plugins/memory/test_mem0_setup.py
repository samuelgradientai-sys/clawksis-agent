...
class TestBuildOSSConfig:
    ...
    def test_openai_custom_urls_use_mem0_provider_specific_keys(self):
        ...
        assert (
            oss["embedder"]["config"]["openai_base_url"] == "https://embed.example/v1"
        )
...
class TestPostSetup:
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "sys.argv", ["clawk", "--mode", "platform", "--api-key", "sk-test"]
        )
        monkeypatch.setattr(
            "plugins.memory.mem0._setup.get_clawk_home", lambda: tmp_path
        )
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        _inject_fake_clawk_cli(monkeypatch)
        monkeypatch.setattr(
            "plugins.memory.mem0._setup._check_selfhosted_server", lambda h: None
        )
        config = {"memory": {}}
        post_setup(str(tmp_path), config)
        assert config["memory"]["provider"] == "mem0"
...
class TestDryRun:
    ...
    def test_dry_run_platform_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "sys.argv",
            ["clawk", "--mode", "platform", "--api-key", "sk-test", "--dry-run"],
        )
        monkeypatch.setattr(
            "plugins.memory.mem0._setup.get_clawk_home", lambda: tmp_path
        )
