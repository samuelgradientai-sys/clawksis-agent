...
from clawk_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    provider.initialize(session_id="test-session", clawk_home=str(tmp_path), platform="cli")
    return provider
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Clawksis and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        assert cfg["banks"]["clawk"]["bankId"] == "env-bank"
        assert cfg["banks"]["clawk"]["budget"] == "high"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        run = MagicMock()
        monkeypatch.setattr("clawk_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        provider = HindsightMemoryProvider()
        provider.post_setup(str(clawk_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (clawk_home / ".env").exists()
        assert not (clawk_home / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "clawk.env").exists()
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "clawk.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(clawk_home))
        provider.post_setup(str(clawk_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        clawk_env = user_home / ".hindsight" / "profiles" / "clawk.env"
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(clawk_home))
        ...
        saved = json.loads((clawk_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Clawksis and the User"
        ...
        assert item["metadata"]["source"] == "clawk"
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("clawk") == "clawk"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "clawk-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="clawk",
        )
        assert p._bank_id == "clawk-coder"
        assert p._bank_id_template == "clawk-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "clawk-" which collapses to "clawk"
        p.initialize(session_id="s1", clawk_home=str(tmp_path), platform="cli")
        assert p._bank_id == "clawk"
