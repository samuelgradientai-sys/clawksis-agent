...
Covers four shapes from Phase 4 of ``.clawksis/plans/2026-05-21-dashboard-oauth-auth.md``:
    ...
    def test_rejects_malformed_client_id(self):
        ...
        with pytest.raises(ValueError, match="agent:"):
            nous_plugin.NousDashboardAuthProvider(
                client_id="clawksis-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    ...
    def test_registers_with_default_portal_url_when_only_client_id_set(
        ...
        """Phase 7 follow-up: CLAWK_DASHBOARD_PORTAL_URL is optional —

        defaults to the production Nous Portal. The user shouldn't have

        to set it for the common production deployment path."""

        monkeypatch.setenv("CLAWK_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
    ...
    def test_skips_when_client_id_malformed(self, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_DASHBOARD_OAUTH_CLIENT_ID", "clawksis-dashboard")

        monkeypatch.setenv("CLAWK_DASHBOARD_PORTAL_URL", "https://p.example")
        ...
        assert "clawksis-dashboard" in nous_plugin.LAST_SKIP_REASON
    ...
    def test_strips_whitespace_from_env_vars(self, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_DASHBOARD_OAUTH_CLIENT_ID", "  agent:x  ")
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_DASHBOARD_PORTAL_URL", "")
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``CLAWK_DASHBOARD_OAUTH_CLIENT_ID``

    and ``CLAWK_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``clawk_cli.config.load_config``
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        CLAWK_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST

        honour it even if a stale config.yaml ships in the image."""

        monkeypatch.setenv("CLAWK_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
    ...
    def test_empty_env_string_does_not_shadow_config(self, patch_config, monkeypatch):
        """``CLAWK_DASHBOARD_OAUTH_CLIENT_ID=`` (set but empty) is
        ...
        monkeypatch.setenv("CLAWK_DASHBOARD_OAUTH_CLIENT_ID", "")
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(self, monkeypatch):
        ...
        monkeypatch.setattr("clawk_cli.config.load_config", _broken_load)
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        ...
        result = provider.start_login(
            redirect_uri="https://clawk.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://clawk.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "clawk_session_pkce" in result.cookie_payload

        pkce = result.cookie_payload["clawk_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        a = provider.start_login(redirect_uri="https://clawk.fly.dev/auth/callback")

        b = provider.start_login(redirect_uri="https://clawk.fly.dev/auth/callback")

        assert (
            a.cookie_payload["clawk_session_pkce"]
            != b.cookie_payload["clawk_session_pkce"]
        )
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://clawk.fly.dev/auth/callback")
...
class TestVerifySession:
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between CLAWK_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
