...
class TestConfigYamlSource:
    ...
    def test_env_overrides_config_portal_url(self, patch_config, monkeypatch):
        monkeypatch.setenv("CLAWK_DASHBOARD_OAUTH_CLIENT_ID", "agent:x")
        monkeypatch.setenv(
            "CLAWK_DASHBOARD_PORTAL_URL", "https://env.portal.example",
        )
        patch_config({
            "client_id": "agent:x",
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "clawk_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        nous_plugin.register(ctx)
...
class TestStartLogin:
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["clawk_session_pkce"] != b.cookie_payload[
            "clawk_session_pkce"
        ]
