"""Tests for the bundled clawk-achievements dashboard plugin.
...
``plugins/clawk-achievements/tests/`` covering the achievement engine

internals (tier math, secret-state handling, catalog invariants). These

tests live at the clawksis-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.clawksis so state/snapshot files don't collide.
...
class _FakeSessionDB:
    """Stand-in for clawk_state.SessionDB that records scan calls."""
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['clawk_state']`` swap is auto-restored at test teardown

    and cannot leak into unrelated tests in the same xdist worker.

    """

    fake_module = type(sys)("clawk_state")

    fake_module.SessionDB = lambda: fake_db

    plugin_api._test_monkeypatch.setitem(sys.modules, "clawk_state", fake_module)
