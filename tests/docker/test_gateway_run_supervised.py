...
Before the s6 migration, ``docker run nousresearch/clawksis-agent gateway
...
crash and a supervised dashboard alongside (when ``CLAWK_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``CLAWK_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``CLAWK_S6_SUPERVISED_CHILD`` sentinel breaks the loop.
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'clawk.*gateway'`` (the gateway is running) or
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → clawk gateway run) can take
    ...
    # (main-wrapper.sh → python → clawk gateway run) can take well over
    ...
    # top), NOT the static main-clawk service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-clawk
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``clawk gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "CLAWK_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    gateway slot would never actually run a python ``clawk gateway
    ...
    # (main-wrapper.sh → python → clawk gateway run) can take well
    ...
    # started either — so we'd see the CMD's `clawk gateway run`
    ...
    # `clawk gateway run`. If the recursion guard fails, s6 would
    ...
    assert n <= 1, (
        f"expected at most one supervised python `clawk gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-clawk`
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${CLAWK_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Clawksis Gateway Starting…"
    ...
    # Poll docker logs for the banner glyph (⚕) or "Clawksis Gateway
    ...
    assert "⚕" in combined or "Clawksis Gateway Starting" in combined, (
