"""Regression test: ``clawk dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``clawk dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$CLAWK_GIT_SHA`` build-arg to ``/opt/clawksis/.clawk_build_sha`` and
``clawk_cli/build_info.py`` reads it as a fallback.
...
is absent and ``clawk dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/clawksis/.clawk_build_sha`` exists in the image, assert that
  ``clawk dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's clawk user (UID 10000) can bootstrap
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``CLAWK_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``clawk dump``
