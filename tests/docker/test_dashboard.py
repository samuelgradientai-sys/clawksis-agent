"""Harness: dashboard opt-in via CLAWK_DASHBOARD.



Today (tini): dashboard starts once when CLAWK_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``clawk`` user
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    false-positive `clawk doctor` and any other health check).
    ...
    CLAWK_DASHBOARD is unset, so the slot reflects reality.
    ...
    assert "down" in r.stdout, (
        ...
        f"Dashboard slot should be 'down' without CLAWK_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    subprocess.run(
        ...
        ["docker", "run", "-d", "--name", container_name,
         ...
         # CLAWK_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
...
def test_dashboard_port_override(
    ...
    subprocess.run(
        ...
        ["docker", "run", "-d", "--name", container_name,
         ...
         "-e", "CLAWK_DASHBOARD=1", "-e", "CLAWK_DASHBOARD_PORT=9120",
...
def test_dashboard_restarts_after_crash(
    ...
    # Kill the dashboard. The dashboard process runs as clawk, so the

    # clawk user can kill it (same UID).
...
# whenever `CLAWK_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``CLAWK_DASHBOARD_OAUTH_CLIENT_ID`` is
    ...
    subprocess.run(
        ...
        ["docker", "run", "-d", "--name", container_name,
         ...
         "-e", "CLAWK_DASHBOARD_HOST=0.0.0.0",

         "-e", "CLAWK_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
    ...
    assert "nous" in provider_names, (
        ...
        f"CLAWK_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and CLAWK_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_opts_out_of_gate(
    ...
    """``CLAWK_DASHBOARD_INSECURE=1`` re-enables the legacy no-gate mode
    ...
    assert status.get("auth_required") is False, (
        ...
        "CLAWK_DASHBOARD_INSECURE=1 must disable the auth gate (explicit "
