...
Override the image with ``CLAWK_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CLAWK_TEST_IMAGE", "clawksis-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CLAWK_TEST_IMAGE`` env var to point at a pre-built

    image (faster local iteration).

    """

    if os.environ.get("CLAWK_TEST_IMAGE"):
        return IMAGE_TAG
...
# docker_exec — default to the unprivileged clawk user

# ---------------------------------------------------------------------------

#

# Background: every Clawksis runtime path inside the container drops to UID

# 10000 (the ``clawk`` user) via ``s6-setuidgid clawk``. ``docker exec``
...
# fails (PermissionError swallowed) for clawk, so a test that ran as root
...
# helpers below run every probe under ``-u clawk`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: clawk).
