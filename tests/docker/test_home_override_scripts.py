...
def test_dashboard_service_resets_home(
    ...
    container with CLAWK_DASHBOARD=1 and verify the dashboard process
    ...
    r = docker_exec_sh(
        container_name,
        # Find the dashboard process (clawk dashboard) and read its HOME
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to clawk:clawk on every boot.
    ...
    # Verify files are now owned by clawk.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "clawk" in r.stdout, (
        f"expected clawk-owned files after restart, got: {r.stdout!r} — "
