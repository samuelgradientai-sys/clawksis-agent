...
The shim (docker/clawk-exec-shim.sh, installed at /opt/clawksis/bin/clawk)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> clawk login` would write /opt/data/auth.json as
...
failed: Clawksis is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> clawk …`` (defaulting to root) gets dropped to the
   clawk user before the real binary runs.
2. ``docker exec --user clawk <c> clawk …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $CLAWK_HOME from a ``docker exec`` session land
   as clawk:clawk — the actual user-visible invariant.
4. The CLAWK_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-clawk-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown clawk:clawk`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to clawk:clawk,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$CLAWK_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (clawk_cli.container_boot), which s6 runs
    *strictly after* ``01-clawk-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_clawk_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `clawk` with a Python-style `-c` shim equivalent — there's no
    pure-clawk "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `clawk` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `clawk`
    do anything that writes to disk, then check the file's owner.

    Use `clawk config set` which writes config.yaml under CLAWK_HOME.
    ...
    assert r.stdout.strip() == "clawk:clawk", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected clawk:clawk. "
        "The shim did not drop privileges before invoking clawk."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user clawk already runs as 10000; shim should be a no-op.
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """CLAWK_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``CLAWK_DOCKER_EXEC_AS_ROOT=0``) doesn't
    ...
    assert r.stdout.strip() == "clawk:clawk", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected clawk:clawk"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/clawksis/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid clawk clawk <args>` which resolves `clawk`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> clawk login`` (root) wrote
    ...
    "Clawksis is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `clawk config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is clawk-readable.

    Specifically: pretend the operator ran `clawk login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the clawk UID. We use `clawk auth list` since that touches the
    ...
    # `clawk auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `clawk config set` first to
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the clawk user "
