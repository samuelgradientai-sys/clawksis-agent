...
from clawk_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(clawk_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <clawk_home>/plugins/<name> and opt it in."""
    plugin_dir = clawk_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CLAWK_HOME", str(tmp_path / "clawk_empty"))
