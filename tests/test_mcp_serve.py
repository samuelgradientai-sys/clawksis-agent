"""
Tests for mcp_serve — Clawksis MCP server.
...
def _isolate_clawk_home(tmp_path, monkeypatch):
    """Redirect CLAWK_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking clawk_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(self, tmp_path, monkeypatch):
        ...
        # _poll_once reads <CLAWK_HOME>/state.db for its mtime gate; the autouse
        # fixture points CLAWK_HOME at tmp_path.
        db_path = tmp_path / "state.db"
