"""Tests for clawk_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Clawksis entry point
(clawk, clawksis-agent, clawk-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Clawksis entry point imports clawk_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported clawk_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("clawk_bootstrap", None)
    import clawk_bootstrap  # noqa: WPS433
    return clawk_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Clawksis CLI
...
class TestEntryPointsImportBootstrap:
    """Every Clawksis entry point must import clawk_bootstrap as its
    ...
    # Entry points that invoke Clawksis as a process.  Each one must
    # import clawk_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "clawk_cli/main.py",   # clawk CLI (console_script)
        "run_agent.py",          # clawksis-agent (console_script)
        "acp_adapter/entry.py",  # clawk-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import clawk_bootstrap' and that
        ...
        half-finished ``clawk update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``clawk_bootstrap`` as a top-level module) leaves clawk
        ...
        """
        # Resolve relative to the clawksis-agent repo root.  Tests live
        # at tests/test_clawk_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # clawk start even when clawk_bootstrap hasn't been
        ...
        assert first_import_name == "clawk_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'clawk_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import clawk_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Clawksis's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("CLAWK_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["CLAWK_PYTHON_SRC_ROOT"] = env
            elif "CLAWK_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["CLAWK_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/clawksis")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CLAWK_PYTHON_SRC_ROOT", None)
            else:
                os.environ["CLAWK_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/clawksis", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Clawksis root.  The guard must relocate Clawksis to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/clawksis"])
        assert result[0] == "/opt/clawksis"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Clawksis root.
        assert result.index("/opt/clawksis") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/clawksis", "/opt/clawksis", ""])
        assert result.count("/opt/clawksis") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["CLAWK_PYTHON_SRC_ROOT"] = "/env/clawk"
            hb.harden_import_path()
            assert sys.path[0] == "/env/clawk"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CLAWK_PYTHON_SRC_ROOT", None)
