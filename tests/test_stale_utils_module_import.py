...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.clawksis/clawksis-agent/utils.py)
...
2. ``clawk update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``clawk_cli/main.py`` (the ``clawk update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``clawk_constants`` after the
