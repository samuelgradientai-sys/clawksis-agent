"""Regression tests for the Clawksis-managed Node's npm global prefix.

When the installer falls back to a bundled Node under ``$CLAWK_HOME/node``,
npm's default global prefix is that Node dir, so ``npm install -g <pkg>``
drops the package binary in ``$CLAWK_HOME/node/bin`` — which is NOT on PATH
...
def test_install_sh_repairs_existing_managed_node_on_rerun() -> None:
    ...
    assert '[ -x "$CLAWK_HOME/node/bin/npm" ] || return 0' in text
