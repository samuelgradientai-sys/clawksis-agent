...
- 96vc: ACP didn't set CLAWK_EXEC_ASK, so `check_all_command_guards`
...
1. Setting CLAWK_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds CLAWK_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set CLAWK_INTERACTIVE so
    ...
    (CLAWK_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When CLAWK_INTERACTIVE is set and an approval callback is
        ...
        assert called_with == [], (
            "without CLAWK_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With CLAWK_INTERACTIVE: callback IS called, approval flows through it
    ...
    def test_interactive_context_var_routes_to_callback_without_env(
        ...
        tok = set_clawk_interactive_context(True)
        try:
            result = check_all_command_guards(
            ...
            reset_clawk_interactive_context(tok)

        assert called_with, (
            "set_clawk_interactive_context(True) should route dangerous "
            "commands through the callback without CLAWK_INTERACTIVE in env"
        )
        assert result["approved"] is True
