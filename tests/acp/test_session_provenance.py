...
def test_root_session_no_compression(db):
    ...
    assert prov["currentClawksisSessionId"] == "root1"
    assert prov["rootClawksisSessionId"] == "root1"
    assert prov["parentClawksisSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    assert prov["previousClawksisSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootClawksisSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootClawksisSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"clawk"}
    assert "sessionProvenance" in meta["clawk"]
    assert meta["clawk"]["sessionProvenance"]["currentClawksisSessionId"] == "root1"
