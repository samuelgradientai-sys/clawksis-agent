...
class TestToolKindMap:
    def test_all_clawk_tools_have_kind(self):
        """Every common clawk tool should appear in TOOL_KIND_MAP."""
        for tool in COMMON_CLAWK_TOOLS:
...
class TestBuildToolComplete:
    ...
    def test_build_tool_complete_for_skill_manage_summarizes_without_raw_json(self):
        result = build_tool_complete(
            "tc-skill-manage",
            "skill_manage",
            '{"success":true,"message":"Patched references/clawk-acp-zed-rendering.md in skill \'clawksis-agent-operations\' (1 replacement)."}',
            function_args={
                "action": "patch",
                "name": "clawksis-agent-operations",
                "file_path": "references/clawk-acp-zed-rendering.md",
            },
        )
        text = result.content[0].content.text
    ...
    def test_build_tool_complete_for_search_files_files_only_formats_file_list(self):
        result = build_tool_complete(
            "tc-search-files",
            "search_files",
            '{"total_count":36,"files":["/home/nour/.clawksis/config.yaml","/home/nour/.clawksis/profiles/recall-test/config.yaml"],"truncated":true}',
        )
        text = result.content[0].content.text
        assert "File search results" in text
        assert "Found 36 files; showing 2." in text
        assert "/home/nour/.clawksis/config.yaml" in text
    ...
    def test_build_tool_complete_for_write_file_summarizes_without_repeating_diff(self, tmp_path):
        ...
        result = build_tool_complete(
            ...
            function_args={"path": str(target), "content": "hello from clawk\n"},
            snapshot=snapshot,
        )
        assert isinstance(result, ToolCallProgress)
        assert len(result.content) == 1
