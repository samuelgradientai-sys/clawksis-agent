...
from clawk_cli.active_sessions import active_session_registry_snapshot
from clawk_cli.browser_connect import ChromeDebugLaunch
...
def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "Clawksis is at the active session limit (1/1). "
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["CLAWK_VOICE"]``
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "clawk_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("CLAWK_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "clawk_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    # _get_platform_tools (a non-configurable platform toolset in clawk-cli's
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "clawk_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("CLAWK_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "clawk_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown CLAWK_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("clawk_state.SessionDB", lambda db_path=None: profile_db)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("CLAWK_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("CLAWK_MODEL", raising=False)
    monkeypatch.delenv("CLAWK_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/clawk-test "}}
    )

    assert server._resolve_model() == "nous/clawk-test"
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set CLAWK_INFERENCE_MODEL as a provision-time seed;
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `clawk --tui -m <model>` sets CLAWK_MODEL/CLAWK_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("CLAWK_MODEL", "one-shot/model")
    monkeypatch.setenv("CLAWK_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("CLAWK_MODEL", "seed/model")
    monkeypatch.setenv("CLAWK_INFERENCE_MODEL", "seed/model")
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("CLAWK_MODEL", "nous/clawk-test")
    monkeypatch.setenv("CLAWK_TUI_PROVIDER", "nous")
    monkeypatch.delenv("CLAWK_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/clawk-test", "nous")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setenv("CLAWK_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "clawk_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/clawk-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("CLAWK_MODEL", "sonnet")
    monkeypatch.delenv("CLAWK_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("CLAWK_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "clawk_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with ClawksisCLI / gateway: fallback_providers stays first and keeps
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "clawk_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_get_approval_mode_uses_manual_default_when_key_is_missing(
    ...
    """Fork divergence: upstream's missing-key default is ``smart``; Clawksis
    ships ``manual`` (BYOK — a fresh install has no key to spend on the
    auxiliary smart-approval call, so it must ask the owner)."""
    import yaml
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("CLAWK_EXEC_ASK", raising=False)
    ...
    assert server.os.environ["CLAWK_GATEWAY_SESSION"] == "1"
    assert server.os.environ["CLAWK_EXEC_ASK"] == "1"
    assert server.os.environ["CLAWK_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("clawk_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("clawk_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "clawk_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing CLAWK_INFERENCE_PROVIDER on a switch leaked the new
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "CLAWK_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write CLAWK_MODEL / CLAWK_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("CLAWK_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("CLAWK_MODEL", raising=False)
    monkeypatch.delenv("CLAWK_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["CLAWK_TUI_PROVIDER"] == "openai-codex"
        assert "CLAWK_MODEL" not in os.environ
        assert "CLAWK_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("CLAWK_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("CLAWK_MODEL", "old/model")
    monkeypatch.setattr(
    ...
    try:
        ...
        assert os.environ["CLAWK_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["CLAWK_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors ClawksisCLI._manual_compress's
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".clawk" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Clawksis TUI Status" in out
...
def test_session_info_includes_install_warning_for_pip(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.config.detect_install_method", lambda: "pip")
    ...
    # Fork divergence: the warning links the rebranded Clawksis install docs
    # instead of upstream's #platform-support anchor.
    assert "#installation" in info["install_warning"]


def test_session_info_omits_install_warning_for_git(monkeypatch):
    monkeypatch.setattr("clawk_cli.config.detect_install_method", lambda: "git")
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("clawk_state")
    ...
    monkeypatch.setitem(sys.modules, "clawk_state", fake_mod)
...
# Fork divergence: upstream refuses to delete ANY session bound to a live TUI
# session (test_session_delete_refuses_active_session). Clawksis only keeps the
...
def test_session_delete_finalizes_idle_live_session(monkeypatch):
    ...
    and proceeds with the DB delete."""
    called: list[str] = []
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_clawk_home
    # fixture pins CLAWK_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `clawk model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `clawk model` and the classic CLI /model picker.
    ...
    with patch(
        ...
        # guarding against.
        with patch("clawk_cli.models.provider_model_ids") as live_fetch:
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from clawk_cli.inventory import ConfigContext
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with (
            patch("clawk_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
...
def test_reload_env_rpc_calls_clawk_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.clawksis/.env
    ...
    with patch.dict(sys.modules, {"clawk_cli.config": fake}):
...
def test_make_agent_waits_for_shared_mcp_discovery(monkeypatch):
    ...
    from clawk_cli import mcp_startup
...
def test_session_save_writes_under_clawk_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Clawksis profile
    ...
    Regression: the gateway handler wrote ``clawk_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Clawksis.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("clawk_conversation_*.json"))
    ...
    assert payload["model"] == "clawk-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Clawksis."
...
# NOTE(sync v2026.7.20): upstream's Nous-Portal billing/subscription RPC tests
# (billing.state serialization, billing error codes, subscription.preview/
# change/resume/upgrade) are NOT adopted. Clawksis purged Nous Portal — there is
# no clawk_cli.nous_billing module and tui_gateway.server has no _usage_payload/
# _serialize_billing_state. Re-add only if managed billing ever returns.
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._clawk_home, which
    # is captured at import time — patch it directly (set_clawk_home_override
