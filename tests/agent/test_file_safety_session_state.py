...
manipulation. Clawksis keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point CLAWK_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_clawk_home /
    get_default_clawk_root) instead of monkeypatching private helpers —
    ...
    CLAWK_HOME=<root>/profiles/<name> makes get_default_clawk_root()
