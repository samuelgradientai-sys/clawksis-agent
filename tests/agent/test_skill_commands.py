...
class TestScanSkillCommands:
    ...
    def test_get_skill_commands_rescans_when_platform_scope_changes(self, tmp_path):
        ...
        def _disabled_skills():
            platform = os.getenv("CLAWK_PLATFORM")
        ...
        with (
            ...
            with patch.dict(os.environ, {"CLAWK_PLATFORM": "telegram"}):
            ...
            with patch.dict(os.environ, {"CLAWK_PLATFORM": "discord"}):
    ...
    def test_get_skill_commands_rescans_when_session_platform_changes(self, tmp_path):
        """``CLAWK_SESSION_PLATFORM`` from the gateway session context must
        also trigger a rescan, not just ``CLAWK_PLATFORM`` (#14536).

        Exercises the real ContextVar path: the gateway sets the active
        adapter via ``set_session_vars(platform=...)`` and the resolver
        reads it via ``get_session_env``. Setting ``CLAWK_SESSION_PLATFORM``
    ...
    def test_get_skill_commands_rescans_when_leaving_platform_scope(
        ...
        def _disabled_skills():
            if os.getenv("CLAWK_PLATFORM") == "telegram":
                return {"telegram-only"}
            return set()
        ...
        with (
            ...
            monkeypatch.setenv("CLAWK_PLATFORM", "telegram")
            telegram_commands = dict(get_skill_commands())
            assert "/telegram-only" not in telegram_commands

            # Drop back to no platform scope — bare CLI / cron / RL rollouts.
            monkeypatch.delenv("CLAWK_PLATFORM", raising=False)
    ...
    def test_get_skill_commands_does_not_rescan_when_platform_unchanged(self, tmp_path):
        ...
        with (
            ...
            patch.object(sc_mod, "_skill_commands", {}),
            patch.object(sc_mod, "_skill_commands_platform", None),
            patch.dict(os.environ, {"CLAWK_PLATFORM": "telegram"}),
        ):
            _make_skill(tmp_path, "shared")
            # Prime the cache.
            get_skill_commands()
    ...
    def test_skill_collides_with_core_command_is_skipped(self, tmp_path):
        """A skill whose auto-generated /command collides with a core Clawksis
...
class TestBuildPreloadedSkillsPrompt:
    ...
    def test_skips_disabled_skill(self, tmp_path, monkeypatch):
        ...
        CLAWK_TUI_SKILLS preloading (mirrors the bundle gate, #59156)."""
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
    ...
    def test_loads_normally_when_nothing_disabled(self, tmp_path, monkeypatch):
        ...
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            ...
            monkeypatch.setattr(
                su_module, "get_disabled_skill_names", lambda platform=None: set()
            )
...
class TestTemplateVarSubstitution:
    """``${CLAWK_SKILL_DIR}`` and ``${CLAWK_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${CLAWK_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
        ...
        assert "${CLAWK_SKILL_DIR}" not in msg.split("[Skill directory:")[0]
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert "Session: ${CLAWK_SESSION_ID}" in msg
    ...
    def test_disable_template_vars_via_config(self, tmp_path):
        ...
        assert "${CLAWK_SKILL_DIR}/scripts/foo.js" in msg
...
class TestStackedSkillCommands:
    ...
    def test_split_dedupes_repeated_skill(self, tmp_path):
        ...
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            keys, instruction = split_stacked_skill_commands("/skill-b /skill-b go")
    ...
    def test_stacked_message_skips_missing_skills(self, tmp_path):
        ...
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            self._setup_three_skills(tmp_path)
            scan_skill_commands()
            result = build_stacked_skill_invocation_message(["/skill-a", "/gone"], "go")
        assert result is not None
        msg, loaded, missing = result
...
class TestExtractUserInstructionFromSkillMessage:
    ...
    Fork-specific coverage (clawksis): agent/memory_manager.py and the
    openviking memory plugin call this helper on every turn, so the extraction
    contract is load-bearing outside the stacked-skill path exercised above.
    """
