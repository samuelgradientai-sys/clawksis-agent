...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_clawk_env(self):
        ...
        # Note: clawk internal .env is in ~/.clawksis/.env which is NOT a project-local

        # path, but the basename check applies to ANY .env. This is intentional —

        # even ~/.clawksis/.env should not be readable via read_file.

        error = get_read_block_error(os.path.expanduser("~/.clawksis/.env"))
...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = clawk_home / "skills" / ".hub" / "index-cache" / "data.json"
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        hub = clawk_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        cache = clawk_home / "skills" / ".hub" / "index-cache" / "x"
