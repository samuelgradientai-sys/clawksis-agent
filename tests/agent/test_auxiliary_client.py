...
class TestResolveTaskProviderModel:
    ...
    @pytest.mark.parametrize(
        "provider", ["", "auto", "custom", "custom:local", "unknown-provider"]
    )
    def test_explicit_base_url_without_first_class_provider_routes_as_custom(
        self, provider
    ):
        resolved_provider, model, base_url, api_key, api_mode = (
            _resolve_task_provider_model(
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(
        ...
        with (
            ...
            patch(
                "clawk_cli.auth._read_codex_tokens",
                return_value={
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(
        ...
        ``clawk auth status`` already reports these as logged in; compression
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(
        ...
        monkeypatch.setenv("CLAWK_HOME", str(clawk_home))
        monkeypatch.setenv("CLAWK_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``clawk model``, no per-task override is
...
class TestExpiredCodexFallback:
    ...
    def test_clawk_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with (
            patch(
                "agent.anthropic_adapter.resolve_anthropic_token",
                return_value="sk-ant-oat-clawk-token",
            ),
            patch("agent.anthropic_adapter.build_anthropic_client") as mock_build,
            patch(
                "agent.auxiliary_client._select_pool_entry", return_value=(False, None)
...
class TestStaleFallbackCandidateSkip:
    ...
    def test_stale_anthropic_fallback_refreshes_and_retries(self, monkeypatch):
        ...
        stale_fb.chat.completions.create.side_effect = _AuxAuth401(
    ...
    def test_unrefreshable_stale_candidate_is_skipped_to_next(self, monkeypatch):
        ...
        healthy_fb.chat.completions.create.return_value = _DummyResponse(
            "openrouter-serves"
        )
        ...
        with (
            ...
            patch(
                "agent.auxiliary_client._try_payment_fallback", side_effect=fb_walks
            ) as mock_fb,
            patch(
                "agent.auxiliary_client._refresh_provider_credentials",
                return_value=False,
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Clawksis auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Clawksis owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with (
            patch("clawk_cli.config.load_config", return_value=config),
            patch(
                "agent.auxiliary_client._get_cached_client",
    ...
    def test_reasoning_effort_shorthand_folds_into_extra_body(self):
        ...
        config = {"auxiliary": {"session_search": {"reasoning_effort": "low"}}}
    ...
    def test_empty_reasoning_effort_is_noop(self):
        ...
        with patch("clawk_cli.config.load_config", return_value=config):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Clawksis reasoning.

    MoA slot reasoning is carried through call_llm as a Clawksis
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/samuelgradientai-sys/clawksis-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/samuelgradientai-sys/clawksis-agent/issues/18478
    """
...
class TestCompressionFallbackContextFilter:
    ...
    def test_main_chain_skips_too_small_candidate_for_compression(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "clawk_cli.fallback_config.get_fallback_chain",
            lambda cfg: chain,
        )
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with (
            patch("clawk_cli.config.load_config", return_value=fake_config),
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with (
            patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"),
            patch.object(ac, "_RUNTIME_MAIN_BASE_URL", "https://gw.example.com/v1"),
            patch("clawk_cli.config.load_config", return_value={"model": {}}),
            patch.object(ac, "_create_openai_client", side_effect=_capture_create),
        ):
            client, model = resolve_provider_client(
                "custom",
