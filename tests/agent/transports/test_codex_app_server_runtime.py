...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("CLAWK_HOME", "/users/alice/.clawksis/profiles/backend-worker")
        monkeypatch.setenv("CLAWK_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "CLAWK_KANBAN_DB",
            "/users/alice/.clawksis/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.clawksis/kanban/boards/smoke"]'
            in cmd
        )
