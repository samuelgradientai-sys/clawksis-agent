...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Clawksis passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("CLAWK_HOME", "/users/alice/.clawk/profiles/backend-worker")
        monkeypatch.setenv("CLAWK_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "CLAWK_KANBAN_DB",
            "/users/alice/.clawk/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.clawk/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through clawk_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Clawksis secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "CLAWK_DASHBOARD_SESSION_TOKEN": "dash-secret",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through clawk_subprocess_env must not
