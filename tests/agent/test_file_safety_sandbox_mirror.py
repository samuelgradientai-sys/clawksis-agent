...
config.yaml, memories/*.md) because the host Clawksis process never reads
...
class TestClassifySandboxMirrorTarget:
    def test_docker_mirror_soul_md_classified(self, tmp_path):
        ...
        target = (
            tmp_path
            / "profiles" / "group1"
            / "sandboxes" / "docker" / "default" / "home" / ".clawk"
    ...
    def test_other_backends_and_inner_files_match(self, tmp_path, backend, inner):
        ...
        target = (
            tmp_path
            / "sandboxes" / backend / "task-42" / "home" / ".clawk"
            / Path(inner)
        )
    ...
    def test_path_outside_sandbox_returns_none(self, tmp_path):
        ...
        target = tmp_path / ".clawk" / "profiles" / "group1" / "SOUL.md"
        target.parent.mkdir(parents=True)
    ...
    def test_sandboxes_segment_with_home_but_no_clawk_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-clawk`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
