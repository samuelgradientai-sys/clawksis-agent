...
class TestQueryLocalContextLengthOllama:
    ...
    def test_ollama_num_ctx_wins_over_model_info(self):
        ...
        training max — using it would let Clawksis grow conversations past
        the runtime limit and Ollama would silently truncate.

        Concrete example: clawk-brain:qwen3-14b-ctx32k is a Modelfile
        derived from qwen3:14b with `num_ctx 32768`, but the underlying
        GGUF reports `qwen3.context_length: 40960` (training max). If
        Clawksis used 40960 it would let the conversation grow past 32768
        ...
        with (
            ...
            result = _query_local_context_length(
                "clawk-brain:qwen3-14b-ctx32k", "http://100.77.243.5:11434/v1"
            )

        assert result == 32768, (
            f"Expected num_ctx (32768) to win over model_info (40960), got {result}. "
            "If Clawksis uses the GGUF training max, conversations will silently truncate."
        )
...
class TestGetModelContextLengthLocalFallback:
    ...
    def test_local_endpoint_stale_cache_reconciled_from_live_probe(self):
        ...
        with (
            patch(
                "agent.model_metadata.get_cached_context_length", return_value=131072
            ),
            patch(
                "agent.model_metadata.fetch_endpoint_model_metadata", return_value={}
            ),
            ...
            patch("agent.model_metadata.is_local_endpoint", return_value=True),
            patch(
                "agent.model_metadata._query_local_context_length", return_value=32768
            ),
            patch(
    ...
    def test_local_endpoint_stale_cache_reconciled_to_valid_live_probe(self):
        ...
        with (
            ...
            patch("agent.model_metadata.is_local_endpoint", return_value=True),
            patch(
                "agent.model_metadata._query_local_context_length", return_value=65536
            ),
            patch(
                "agent.model_metadata._invalidate_cached_context_length"
            ) as mock_invalidate,
...
class TestLocalContextProbeTTLCache:
    ...
    def test_none_result_not_cached(self):
        ...
        assert detect.call_count == 2, (
            "None result was wrongly cached; retry did not re-probe"
        )
