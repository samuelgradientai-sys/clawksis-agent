...
def test_add_entry_normalizes_before_persisting(tmp_path, monkeypatch):
    ...
    persisted = json.loads((clawk_home / "auth.json").read_text())
    assert entry.auth_type == AUTH_TYPE_OAUTH
...
def test_profile_global_fallback_normalizes_in_memory_without_writing(
    ...
    assert entry.auth_type == AUTH_TYPE_OAUTH
    assert (
        persisted["credential_pool"]["anthropic"][0]["auth_type"] == AUTH_TYPE_API_KEY
    )
