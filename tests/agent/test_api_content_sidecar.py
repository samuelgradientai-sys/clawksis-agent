...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("clawk_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated CLAWK_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="clawk_api_content_")
    os.makedirs(os.path.join(test_home, ".clawk"))
    prev_home = os.environ.get("CLAWK_HOME")
    os.environ["CLAWK_HOME"] = os.path.join(test_home, ".clawk")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["CLAWK_HOME"] = prev_home
