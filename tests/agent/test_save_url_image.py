...
def http_server(tmp_path, monkeypatch):
    ...
    """Spin up a localhost HTTP server and isolate CLAWK_HOME under tmp_path."""
    ...
    # Force the constants/image cache helpers to re-read CLAWK_HOME.

    import sys

    for mod in list(sys.modules):

        if mod.startswith("clawk_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    ...
    def test_writes_real_bytes_to_clawk_home_cache(self, http_server):
        ...
        # The cache directory must be under CLAWK_HOME — gateway cleanup
