...
class TestToOpenaiBaseUrl:
    def test_minimax_global_anthropic_suffix_replaced(self):
        assert (
            _to_openai_base_url("https://api.minimax.io/anthropic")
            == "https://api.minimax.io/v1"
        )

    def test_minimax_cn_anthropic_suffix_replaced(self):
        assert (
            _to_openai_base_url("https://api.minimaxi.com/anthropic")
    ...
    def test_v1_url_unchanged(self):
        assert (
            _to_openai_base_url("https://api.openai.com/v1")
            == "https://api.openai.com/v1"
        )

    def test_openrouter_url_unchanged(self):
        assert (
            _to_openai_base_url("https://openrouter.ai/api/v1")
    ...
    def test_anthropic_domain_unchanged(self):
        ...
        assert (
            _to_openai_base_url("https://api.anthropic.com")
            == "https://api.anthropic.com"
        )

    def test_anthropic_in_subpath_unchanged(self):
        assert (
            _to_openai_base_url("https://example.com/anthropic/extra")
            == "https://example.com/anthropic/extra"
        )
