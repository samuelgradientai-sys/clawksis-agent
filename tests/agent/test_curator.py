...
def curator_env(tmp_path, monkeypatch):
    ...
    # save_state() resolves the state path from CLAWK_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.clawksis once monkeypatch
    ...
    # thread here, while CLAWK_HOME is still pinned to this test's tmp home
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `clawk curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_run_review_consolidate_override_runs_llm(curator_env, monkeypatch):
    ...
    the LLM consolidation pass — mirrors `clawk curator run --consolidate`."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
# `clawk model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_review_runtime_carries_auxiliary_extra_body(curator_env):
    ...
    assert binding.request_overrides == {"extra_body": {"slot_flag": "slot-value"}}
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from clawk_cli.main import _AUX_TASKS
