...
def test_translate_native_response_surfaces_reasoning_and_tool_calls():
        ...
        "candidates": [
            {
                "content": {
                    "parts": [
                        ...
                        {"functionCall": {"name": "search", "args": {"q": "clawk"}}},
                    ]
                },
                "finishReason": "STOP",
            }
    ...
    assert json.loads(choice.message.tool_calls[0].function.arguments) == {"q": "clawk"}
...
def test_max_tokens_none_defaults_to_gemini_output_ceiling():
    ...
    maxOutputTokens is absent, truncating tool calls mid-stream. Clawksis passes
...
def test_x_goog_api_client_header_is_set():
    ...
    assert "clawksis-agent/" in headers["X-Goog-Api-Client"], (
        "clawksis-agent not found in X-Goog-Api-Client header"
    )


def test_x_goog_api_client_header_format():
    """Header value should be 'clawksis-agent/<version>' matching the package version."""
    from agent.gemini_native_adapter import GeminiNativeClient, _CLAWK_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    expected = f"clawksis-agent/{_CLAWK_VERSION}"
    assert headers["X-Goog-Api-Client"] == expected


def test_user_agent_contains_version():
    """User-Agent should include the clawksis-agent version."""
    from agent.gemini_native_adapter import GeminiNativeClient, _CLAWK_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    assert f"clawksis-agent/{_CLAWK_VERSION}" in headers["User-Agent"]


def test_clawk_version_is_valid():
    """_CLAWK_VERSION should be a non-empty string."""
    from agent.gemini_native_adapter import _CLAWK_VERSION

    assert isinstance(_CLAWK_VERSION, str)
    assert len(_CLAWK_VERSION) > 0
    assert _CLAWK_VERSION != "0.0.0", (
        "Version should resolve from clawk_cli.__version__, not the fallback"
    )
