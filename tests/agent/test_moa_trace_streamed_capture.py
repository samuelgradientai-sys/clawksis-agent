...
path via ``clawk chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp CLAWK_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point CLAWK_HOME at a temp dir and turn moa.save_traces on."""
    clawk_home = tmp_path / ".clawk"
    clawk_home.mkdir()
    monkeypatch.setenv("CLAWK_HOME", str(clawk_home))

    # save_moa_turn reads config via clawk_cli.config.load_config; stub it to
