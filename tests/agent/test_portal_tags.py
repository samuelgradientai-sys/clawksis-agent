...
def test_clawk_client_tag_includes_current_version():
    """The client tag must reflect clawk_cli.__version__ verbatim."""
    from clawk_cli import __version__
    from agent.portal_tags import clawk_client_tag

    assert clawk_client_tag() == f"client=clawk-client-v{__version__}"


def test_clawk_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import clawk_client_tag

    tag = clawk_client_tag()
    assert tag.startswith("client=clawk-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import clawk_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=clawksis-agent" in tags
    assert clawk_client_tag() in tags
    assert len(tags) == 2
