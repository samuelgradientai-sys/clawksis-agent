...
Damien's incident (Discord, 2026-05-28): a long Clawksis session in a Discord
...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``clawk_state`` module, whose ``SessionDB``
...
_NominalSessionDBImpostor.__module__ = "clawk_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``clawk_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = clawk_state.SessionDB
    monkeypatch.setattr(clawk_state, "SessionDB", legacy_type)
    try:
