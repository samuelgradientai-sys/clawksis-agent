...
class TestParseResponse:
    ...
    def test_pre_verify_without_message_is_noop(self):
        ...
        assert (
            shell_hooks._parse_response("pre_verify", '{"action": "continue"}') is None
        )
        assert (
            shell_hooks._parse_response("pre_verify", '{"decision": "allow"}') is None
        )
...
class TestCallbackSubprocess:
    ...
    def test_block_translation_end_to_end(self, tmp_path):
        ...
        must translate it to the canonical Clawksis block shape so that
    ...
    def test_block_aggregation_through_plugin_manager(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_ACCEPT_HOOKS", "1")
...
class TestAllowlistConcurrency:
    ...
    def test_save_allowlist_failure_logs_actionable_warning(
        ...
        re-prompt consequence so "clawk keeps asking" is debuggable."""
        import logging
    ...
    def test_command_script_path_resolution(self):
        ...
        ``clawk hooks doctor``'s executability check and silently
