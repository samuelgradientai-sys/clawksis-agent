...
class TestRegistry:
    ...
    def test_alias_lookup(self):
        ...
        # Fork note: the ``nous``/``nous-portal`` profile is not bundled in
        # Clawksis (provider removed from the picker), so no alias assertion.
...
class TestOpenRouterProfile:
    ...
    def test_mandatory_anthropic_effort_routes_to_verbosity(self):
        ...
        ``clawk_constants.parse_reasoning_effort`` —
