...
Verifies that:
 ...
 2. User plugins at $CLAWK_HOME/plugins/model-providers/<name>/ override bundled
...
def test_bundled_plugins_discovered():
    ...
    assert len(child_dirs) >= 27, (
        f"Expected at least 27 provider plugins, found {len(child_dirs)}"
    )
...
def test_user_plugin_overrides_bundled(tmp_path, monkeypatch):
    ...
    # Point CLAWK_HOME at a fresh temp dir
    ...
    # get_clawk_home() may be module-cached depending on codebase; ensure the

    # env var is the source of truth. Most code paths re-read it each call.

    # Drop a user plugin that replaces 'gmi'

    user_gmi = clawk_home / "plugins" / "model-providers" / "gmi"
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from clawk_cli import plugins as plugin_mod
