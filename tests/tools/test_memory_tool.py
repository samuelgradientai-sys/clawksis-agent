...
class TestScanMemoryContent:
    ...
    def test_clawk_config_mod_blocked(self):
        result = _scan_memory_content("edit .clawk/config.yaml to change settings")
        assert "Blocked" in result
        assert "clawk_config_mod" in result
        result = _scan_memory_content("update .clawk/SOUL.md with new personality")
        assert "Blocked" in result
        assert "clawk_config_mod" in result
    ...
    def test_clawk_config_mod_no_false_positives(self):
        """Merely mentioning clawk config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .clawk/config.yaml for settings") is None
        assert _scan_memory_content("Read .clawk/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .clawk/config.yaml file contains runtime options") is None
...
# =========================================================================
# Batch: store-level API (fork addition, kept through the upstream sync)
#
# Upstream's TestMemoryBatch drives batches through ``memory_tool(operations=...)``.
# These cover the two surfaces upstream does not: ``MemoryStore.apply_batch()``
# called directly, and the explicit ``action="batch"`` dispatch branch.
# =========================================================================
