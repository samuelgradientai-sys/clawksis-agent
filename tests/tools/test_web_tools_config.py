...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_clawk_home_override(self, tmp_path):
        """Auth lookup should read from CLAWK_HOME/auth.json, not ~/.clawksis/auth.json."""
...
class TestBackendSelection:
    ...
    ``clawk tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from clawk_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.clawksis/.env were invisible to the provider."""

    def test_direct_config_reads_via_get_env_value(
    ...
    def test_direct_config_reads_url_via_get_env_value(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.clawksis/.env), not bare os.getenv. Parametrized over the four
