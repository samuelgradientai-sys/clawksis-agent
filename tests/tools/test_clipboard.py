...
class TestMacosPngpaste:
    ...
    def test_timeout_returns_false(self, tmp_path):
        ...
        with patch(
            "clawk_cli.clipboard.subprocess.run",
...
class TestWaylandSave:
    ...
    def test_jpeg_extraction_converts_to_real_png(self, tmp_path):
        ...
        with patch("clawk_cli.clipboard.subprocess.run", side_effect=fake_run):
            with patch(
                "clawk_cli.clipboard._convert_to_png", side_effect=fake_convert
            ) as mock_convert:
                assert _wayland_save(dest) is True
        ...
        with patch("clawk_cli.clipboard._is_wsl", return_value=True):
            with patch("clawk_cli.clipboard._wsl_save", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch(
                        "clawk_cli.clipboard._xclip_save", return_value=True
                    ) as m:
                        assert _linux_save(dest) is True
...
class TestHasClipboardImageWin32:
    ...
    def test_dispatches_on_win32(self):
        ...
        with patch("clawk_cli.clipboard.sys") as mock_sys:
            ...
            with patch(
                "clawk_cli.clipboard._windows_has_image", return_value=True
            ) as m:
...
class TestConvertToPng:
    ...
    def test_file_still_usable_when_no_converter(self, tmp_path):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch(
                "clawk_cli.clipboard.subprocess.run", side_effect=FileNotFoundError
            ):
...
class TestHasClipboardImage:
    ...
    def test_linux_wsl_dispatch(self):
        ...
        with patch("clawk_cli.clipboard.sys") as mock_sys:
            ...
            with patch("clawk_cli.clipboard._is_wsl", return_value=True):
                with patch(
                    "clawk_cli.clipboard._wsl_has_image", return_value=True
                ) as m:
                    assert has_clipboard_image() is True
    ...
    def test_wsl_falls_through_to_wayland_when_windows_path_empty(self):
        ...
        with patch("clawk_cli.clipboard.sys") as mock_sys:
            ...
            with patch("clawk_cli.clipboard._is_wsl", return_value=True):
                with patch(
                    "clawk_cli.clipboard._wsl_has_image", return_value=False
                ) as wsl:
                    with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                        with patch(
                            "clawk_cli.clipboard._wayland_has_image", return_value=True
                        ) as wl:
                            assert has_clipboard_image() is True
    ...
    def test_linux_x11_dispatch(self):
        ...
        with patch("clawk_cli.clipboard.sys") as mock_sys:
            ...
            with patch("clawk_cli.clipboard._is_wsl", return_value=False):
                with patch.dict(os.environ, {}, clear=True):
                    with patch(
                        "clawk_cli.clipboard._xclip_has_image", return_value=True
                    ) as m:
                        assert has_clipboard_image() is True
