...
def test_resolve_stdio_command_falls_back_to_clawk_node_bin(tmp_path):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$CLAWK_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the Clawksis Docker image
    ...
    # the other candidates ($CLAWK_HOME/node/bin/npx and ~/.local/bin/npx)
