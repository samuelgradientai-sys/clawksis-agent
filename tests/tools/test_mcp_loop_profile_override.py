"""Regression tests for CLAWK_HOME override propagation onto the MCP loop.
...
get_clawk_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: ClawksisTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(ClawksisTokenStorage("probe-srv")._tokens_path())
...
def test_concurrent_scopes_do_not_interfere(tmp_path, monkeypatch, mcp_loop):
    ...
    def scoped_call(key, home):
        token = set_clawk_home_override(str(home))
        try:
