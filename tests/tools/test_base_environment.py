...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_static_part_is_quoted_bashpid_outside(self):
        ...
        env = _TestableEnv()
        env._snapshot_ready = True
        env._snapshot_path = "/tmp/has space/clawk-snap-x.sh"
        wrapped = env._wrap_command("echo hi", "/tmp")
