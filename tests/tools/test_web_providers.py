...
class TestDispatchersTriggerPluginDiscovery:
    ...
    def test_web_extract_tool_runs_discovery_before_registry_lookup(self, monkeypatch):
        ...
        try:
            ...
            # underlying ``clawk_cli.plugins._ensure_plugins_discovered`` so
    ...
    def test_extract_tool_reports_disabled_plugin(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(
                ...
                "_read_config_key",
                lambda *path: (
                    "firecrawl" if path == ("web", "extract_backend") else None
                ),
            )
            self._patch_manager(
                monkeypatch,
            ...
            assert "clawk plugins enable" in err
    ...
    def test_search_tool_reports_disabled_plugin(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(
                ...
                "_read_config_key",
                lambda *path: (
                    "firecrawl" if path == ("web", "search_backend") else None
                ),
            )
            self._patch_manager(
                monkeypatch,
