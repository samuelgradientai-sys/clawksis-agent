...
Clawksis' existing ``cache_image_from_bytes`` so a ``MEDIA:<path>`` tag
...
def _png_bytes():
    ...
    Clawksis' ``cache_image_from_bytes`` has a format-sniff guard that rejects
...
class TestCacheMcpImageBlock:
    def test_returns_media_tag_for_valid_image_block(self, tmp_path, monkeypatch):
        ...
        # The cached file should be in Clawksis' image cache dir
        ...
        assert tag.startswith(f"MEDIA:{cache_dir}"), (
            f"cached file not under CLAWK_HOME image cache dir. "
