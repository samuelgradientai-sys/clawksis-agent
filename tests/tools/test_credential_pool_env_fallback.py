...
- _seed_from_env reads API keys from ~/.clawksis/.env when not in os.environ
- _resolve_api_key_provider_secret falls back to credential_pool when env vars are empty
- ~/.clawksis/.env takes priority over os.environ for Clawksis-managed credentials
...
class TestCredentialPoolSeedsFromDotEnv:
    ...
    load_clawk_dotenv, the credential pool must still discover it.
    """
    ...
    def test_dotenv_wins_over_stale_os_environ(self, isolated_clawk_home, monkeypatch):
        """Regression for #20591: a fresh key rotated into ~/.clawksis/.env must
        ...
        """
        _write_env_file(isolated_clawk_home, DEEPSEEK_API_KEY="sk-dotenv-fresh")
...
class TestAuthResolvesFromDotEnv:
    ...
    def test_dotenv_wins_over_stale_os_environ_on_resolve(
        self, isolated_clawk_home, monkeypatch
    ):
        """Regression for #20591: when both ~/.clawksis/.env and os.environ define
        ...
        """
        _write_env_file(isolated_clawk_home, DEEPSEEK_API_KEY="dotenv-fresh-deepseek")
    ...
    def test_get_anthropic_key_prefers_dotenv_over_stale_os_environ(
        ...
        prefer ~/.clawksis/.env over a stale shell export. This path resolves
        ...
        """
        _write_env_file(isolated_clawk_home, ANTHROPIC_API_KEY="dotenv-fresh-anthropic")
...
class TestAnthropicEnvAuthTypeClassification:
    ...
    def test_admin_key_classified_as_api_key(self, isolated_clawk_home):
