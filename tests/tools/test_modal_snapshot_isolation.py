...
def _restore_tool_modules():
    original_clawk_home = os.environ.get("CLAWK_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_clawk_home is None:
            os.environ.pop("CLAWK_HOME", None)
        else:
            os.environ["CLAWK_HOME"] = original_clawk_home
        _reset_modules(("tools", "clawk_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    clawk_cli = types.ModuleType("clawk_cli")
    clawk_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["clawk_cli"] = clawk_cli
    clawk_home = tmp_path / "clawk-home"
    os.environ["CLAWK_HOME"] = str(clawk_home)
    sys.modules["clawk_cli.config"] = types.SimpleNamespace(
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="clawksis-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": clawk_home / "modal_snapshots.json",
