...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "clawk_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "clawk_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    sys.modules["clawk_cli.config"] = types.SimpleNamespace(
        get_clawk_home=lambda: Path(tempfile.gettempdir()) / "clawk-home",
    )
        ...
        credential_mounts=[
            {
                ...
                "container_path": "/root/.clawksis/token.json",
            }
        ]
    )
