...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.clawksis/.env must not leak."""
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _CLAWK_FORCE_ prefix."""
...
class TestClawksisBinDirOnPath:
    ...
    def test_resolves_via_which(self, monkeypatch):
        ...
        monkeypatch.setattr(
            local_mod.os.path, "isdir", lambda p: p == "/opt/clawksis/bin"
        )
