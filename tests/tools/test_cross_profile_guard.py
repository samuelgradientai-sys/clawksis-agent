...
def fake_clawk(tmp_path, monkeypatch):
    """Build a two-profile Clawksis layout and point CLAWK_HOME at

    the clawk-security profile (matching the original-incident shape).
    ...
    monkeypatch.setenv("CLAWK_HOME", str(sec_home))
    ...
    monkeypatch.setattr(fs, "_clawk_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_clawk):

        from tools.file_tools import write_file_tool

        target = fake_clawk["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_clawk):
        ...
        target = fake_clawk["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "clawk-security" in result["error"]

        # File untouched.

        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_clawk):
    ...
    def test_non_clawk_path_unaffected(self, fake_clawk, tmp_path):
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_clawk):
    ...
    def test_cross_profile_patch_bypass(self, fake_clawk):
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_clawk):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        self._make_skill_in_profile(fake_clawk["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up CLAWK_HOME (set in
        ...
        assert "not found in active profile 'clawk-security'" in err

        assert "default" in err

        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_clawk, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""

        self._make_skill_in_profile(fake_clawk["root"], "everywhere-skill")

        self._make_skill_in_profile(fake_clawk["coder_home"], "everywhere-skill")
        ...
        assert "clawk -p" in err

    def test_genuinely_missing_skill_keeps_helpful_hint(self, fake_clawk, monkeypatch):
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.clawksis/profiles/<name>/."""

        # Don't set CLAWK_HOME — falls back to default.

        import agent.file_safety as fs

        monkeypatch.setattr(fs, "_clawk_home_path", lambda: tmp_path / "fake")

        monkeypatch.setattr(fs, "_clawk_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_clawk):
        """When active profile is 'clawk-security', the prompt warns
        ...
        assert "Active Clawksis profile" in src

        assert "cross_profile=True" in src

        assert "~/.clawksis/profiles/" in src

        # Both branches present (default and named profile).

        assert "Active Clawksis profile: default" in src

        assert "Active Clawksis profile: {active_profile}" in src
