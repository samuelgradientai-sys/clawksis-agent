...
dicts) keeps the test independent of the user's ~/.clawksis/config.yaml and
mirrors the pattern used in tests/clawk_cli/test_config_drift.py.
"""
...
def test_save_config_set_supports_critical_bridged_keys():
    """``clawk config set terminal.X true`` must propagate to .env for
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on Clawksis exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether Clawksis sweeps stale
...
def test_docker_volumes_is_bridged_everywhere():
    ...
    ``clawk config set terminal.docker_volumes '["/host:/workspace"]'`` wrote
...
def test_docker_forward_env_is_bridged_everywhere():
    ...
    ``clawk config set terminal.docker_forward_env '["GITHUB_TOKEN"]'`` had no
