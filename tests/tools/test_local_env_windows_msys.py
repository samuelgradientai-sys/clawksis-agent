...
class TestBashSafePath:
    ...
    def test_forward_slash_native_path_becomes_msys(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
        assert (
            _bash_safe_path("C:/Users/Alexander/.clawk/cache/terminal/clawk-snap-x.sh")
            == "/c/Users/Alexander/.clawk/cache/terminal/clawk-snap-x.sh"
        )
    ...
    def test_mixed_msys_path_normalizes_backslashes(self, monkeypatch):
        ...
        assert (
            _bash_safe_path(mixed) == "/c/Users/Alexander/Documents/NewTEST/readme.txt"
        )
    ...
    def test_quote_bash_path_quotes_mixed_windows_path(self, monkeypatch):
        ...
        quoted = _quote_bash_path(
            r"C:\Users\Alexander\AppData\Local\Temp\clawk-snap-abc.sh"
        )
        assert "/c/Users/Alexander/AppData/Local/Temp/clawk-snap-abc.sh" in quoted
...
class TestWindowsMsysPathconvDefaults:
    ...
    def test_sanitize_subprocess_env_sets_msys_no_pathconv_on_windows(
        self, monkeypatch
    ):
        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
    ...
    def test_clawk_subprocess_env_sets_msys_no_pathconv_on_windows(self, monkeypatch):
        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
    ...
    def test_msys2_arg_conv_excl_set_on_windows(self, monkeypatch):
        ...
        assert clawk_subprocess_env().get("MSYS2_ARG_CONV_EXCL") == "*"
...
class TestGitBashCoreutilsOnPath:
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
        monkeypatch.setattr(
            local_mod, "_git_bash_bin_dirs_cache", ["/pg/usr/bin", "/pg/bin"]
        )
    ...
    def test_make_run_env_prepends_coreutils_on_windows(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
        monkeypatch.setattr(
            local_mod, "_git_bash_bin_dirs_cache", ["/pg/mingw64/bin", "/pg/usr/bin"]
        )
...
class TestWrapCommandWindowsNativeCwd:
    ...
    def test_init_session_bootstrap_converts_native_cwd_for_cd(self, monkeypatch):
        ...
        def fake_run_bash(
            ...
            captured.setdefault(
                "script", cmd_string
            )  # bootstrap only; ignore the failure-path probe
    ...
    def test_init_session_bootstrap_quotes_snapshot_paths_in_msys_form(
        self, monkeypatch
    ):
        ...
        snap = "C:/Users/Alexander/.clawk/cache/terminal/clawk-snap-deadbeef.sh"
        ...
        assert (
            "/c/Users/Alexander/.clawk/cache/terminal/clawk-snap-deadbeef.sh" in script
        )
        assert "C:/Users/Alexander" not in script
        assert r"C:\Users\Alexander" not in script

    def test_init_session_bootstrap_rewrites_backslash_snapshot_paths(
        self, monkeypatch
    ):
        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
        ...
        snap = r"C:\Users\Alexander\AppData\Local\Temp\clawk-snap-deadbeef.sh"
        with patch.object(LocalEnvironment, "__init__", lambda self, **kw: None):
            env = LocalEnvironment.__new__(LocalEnvironment)
            BaseEnvironment.__init__(
                env,
        ...
        script = captured["script"]
        assert "/c/Users/Alexander/AppData/Local/Temp/clawk-snap-deadbeef.sh" in script
