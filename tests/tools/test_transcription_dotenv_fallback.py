...
``~/.clawksis/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.clawksis/.env`` would be told
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        """Importing STT while clawk_cli.config.get_env_value is patched must
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``clawk_cli.config.load_env`` to simulate
    ``~/.clawksis/.env`` carrying the key while ``os.environ`` does not.
