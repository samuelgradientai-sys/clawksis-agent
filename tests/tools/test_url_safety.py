...
class TestNormalizeUrlForRequest:
    ...
    def test_does_not_collapse_embedded_scheme_separator_in_query(self):
        assert (
            normalize_url_for_request(
                "https://example.com/r?next=https:// evil.example"
            )
...
class TestGlobalAllowPrivateUrls:
    ...
    def test_default_is_false(self, monkeypatch):
        ...
        with patch(
            "clawk_cli.config.read_raw_config", side_effect=Exception("no config")
        ):
            assert _global_allow_private_urls() is False

    def test_env_var_true(self, monkeypatch):
        """CLAWK_ALLOW_PRIVATE_URLS=true enables the toggle."""
    ...
    def test_env_var_1(self, monkeypatch):
        """CLAWK_ALLOW_PRIVATE_URLS=1 enables the toggle."""
        monkeypatch.setenv("CLAWK_ALLOW_PRIVATE_URLS", "1")
        assert _global_allow_private_urls() is True

    def test_env_var_yes(self, monkeypatch):
        """CLAWK_ALLOW_PRIVATE_URLS=yes enables the toggle."""
        monkeypatch.setenv("CLAWK_ALLOW_PRIVATE_URLS", "yes")
        assert _global_allow_private_urls() is True

    def test_env_var_false(self, monkeypatch):
        """CLAWK_ALLOW_PRIVATE_URLS=false keeps it disabled."""
        monkeypatch.setenv("CLAWK_ALLOW_PRIVATE_URLS", "false")
        assert _global_allow_private_urls() is False
