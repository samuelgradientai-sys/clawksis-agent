...
Verifies:
  - Tools are gated on CLAWK_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    ...
    schema = registry.get_definitions(set(resolve_toolset("clawk-cli")), quiet=True)
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the CLAWK_KANBAN_TASK env var means it's a focused
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("CLAWK_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated CLAWK_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to CLAWK_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    under an authoritative-looking name like ``clawk-system`` and
    ...
    try:
        ...
        # caller-supplied "clawk-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_stamps_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("CLAWK_SESSION_ID", "acp-sess-abc")
    from tools import kanban_tools as kt
    from clawk_cli import kanban_db as kb
    out = kt._handle_create({
        "title": "from chat",
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("CLAWK_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from clawk_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("CLAWK_HOME", str(home))
    monkeypatch.setenv("CLAWK_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
#
# A worker process has CLAWK_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no CLAWK_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # CLAWK_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("CLAWK_KANBAN_RUN_ID", str(run1.id))
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via CLAWK_KANBAN_BOARD env var,
...
# restarting Clawksis. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    ...
    CLAWK_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".clawk"
    home.mkdir()
    monkeypatch.setenv("CLAWK_HOME", str(home))
    # Make sure neither CLAWK_KANBAN_DB nor CLAWK_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("CLAWK_KANBAN_DB", raising=False)
    monkeypatch.delenv("CLAWK_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("CLAWK_KANBAN_TASK", raising=False)
    monkeypatch.setenv("CLAWK_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("CLAWK_HOME", str(home))
    monkeypatch.setenv("CLAWK_PROFILE", "alt-worker")
    monkeypatch.delenv("CLAWK_KANBAN_DB", raising=False)
    monkeypatch.delenv("CLAWK_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: CLAWK_SESSION_PLATFORM + CLAWK_SESSION_CHAT_ID set.
# - TUI sessions: CLAWK_SESSION_KEY (or CLAWK_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("CLAWK_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("CLAWK_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("CLAWK_SESSION_USER_ID", "user-9")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports CLAWK_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("CLAWK_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("CLAWK_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("CLAWK_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("CLAWK_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("CLAWK_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("CLAWK_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("CLAWK_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("CLAWK_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("CLAWK_SESSION_KEY", raising=False)
    monkeypatch.delenv("CLAWK_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.clawk; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".clawk"
    ...
    monkeypatch.setenv("CLAWK_SESSION_CHAT_ID", "channel-1")
...
def test_create_partial_session_context_no_subscribe(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("CLAWK_SESSION_PLATFORM", "slack")
    monkeypatch.delenv("CLAWK_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("CLAWK_SESSION_KEY", raising=False)
    monkeypatch.delenv("CLAWK_SESSION_ID", raising=False)
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting CLAWK_ALLOW_PRIVATE_URLS on a private network.
    Resets the url_safety process-lifetime cache on both sides so the
    override neither leaks in nor out of the test.
    """
    from tools import url_safety

    monkeypatch.setenv("CLAWK_ALLOW_PRIVATE_URLS", "true")
...
def default_url_guard(monkeypatch):
    ...
    Clears CLAWK_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
    cache on both sides so a prior test's opt-in can't leak in.
    """
    from tools import url_safety

    monkeypatch.delenv("CLAWK_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
