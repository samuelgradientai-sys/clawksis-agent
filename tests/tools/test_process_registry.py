...
from tools.environments.local import _CLAWK_PROVIDER_ENV_FORCE_PREFIX
...
def test_close_terminal_tool_gated_on_desktop(monkeypatch):
    """Hidden unless CLAWK_DESKTOP is set (mirrors read_terminal gating)."""
...
class TestOrphanedPipeReconciliation:
    ...
    `clawk update` in Feishu spawned a background subprocess that restarted
    ...
    def test_wait_wakes_when_session_moves_to_finished(self, registry):
        ...
        assert (
            elapsed < 0.9
        )  # must stay under the old 1s poll tick being regression-tested, f"wait() should wake on completion; took {elapsed:.3f}s"
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with (
            ...
            registry.spawn_local(
                ...
                env_vars={
                    ...
                    f"{_CLAWK_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN": "forced-bot-token",
                },
            )
        ...
        assert f"{_CLAWK_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN" not in env
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/clawk_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/clawk_bg_" not in bg_command
        assert "cat /tmp/clawk_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with (
            ...
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/clawk_bg.log",
                "/path with spaces/clawk_bg.pid",
                "/path with spaces/clawk_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/clawk_bg.log' 2>/dev/null"
        assert (
            env.commands[1][0]
            == "kill -0 \"$(cat '/path with spaces/clawk_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        )
        assert env.commands[2][0] == "cat '/path with spaces/clawk_bg.exit' 2>/dev/null"
...
@pytest.mark.parametrize("skip_state", ["_poll_observed", "_completion_consumed"])
def test_drain_notifications_routes_foreign_before_local_skip(registry, skip_state):
    event = {
        "type": "completion",
...
def test_drain_notifications_filters_addressed_completion_by_owns_event(
    ...
    assert [event["session_id"] for event, _ in results] == [f"proc_owned_{exit_code}"]
...
def test_drain_notifications_owns_event_callback_fails_closed():
    ...
    try:
        ...
        assert results == []
        assert (
            process_registry.completion_queue.get_nowait()["delegation_id"] == "deleg_x"
        )
    finally:
        while not process_registry.completion_queue.empty():
            process_registry.completion_queue.get_nowait()
