...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.clawksis/skills/."""
    with (
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("clawk_cli.config.load_config", return_value={"skills": {}}):
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.clawksis/skills/ as a
    workaround.
    """
            ...
            try:
                ...
                with (
                    patch(
                        ...
                        return_value={
                            "manual-skill": {"created_by": None, "use_count": 50}
                        },
                    ),
                    patch(
                        "tools.skill_usage.get_record",
                        side_effect=lambda n: (
                            {"created_by": None, "use_count": 50}
                            if n == "manual-skill"
                            else {}
                        ),
                    ),
            ...
            try:
                ...
                with (
                    patch(
                        "tools.skill_usage.load_usage",
                        return_value={
                            "agent-skill": {"created_by": "agent", "use_count": 5}
                        },
                    ),
                    patch(
                        "tools.skill_usage.get_record",
                        side_effect=lambda n: (
                            {"created_by": "agent", "use_count": 5}
                            if n == "agent-skill"
                            else {}
                        ),
                    ),
                    patch(
                        "tools.skill_usage.is_curation_eligible",
                        return_value=True,
                    ),
                    patch(
...
# come up. The user unpins via `clawk curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "clawk curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    Points CLAWK_HOME at ``tmp_path/.clawk`` so skill_usage's archive path
    (``get_clawk_home()``) resolves into the same tree the skill manager
    searches, and flips ``is_background_review()`` → True so the consolidation
    guard fires.
    """
    clawk_home = tmp_path / ".clawk"
    skills_root = clawk_home / "skills"
...
class TestCuratorConsolidationDeleteGuard:
    ...
    def test_dispatcher_preserves_usage_record_on_curator_archive(
        ...
        # `clawk curator restore` can bring it back.
    ...
    def test_background_review_patch_requires_skill_view_first(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_background_review_support_file_overwrite_requires_that_file_read(
        ...
        with _curator_pass(tmp_path, monkeypatch=monkeypatch):
            _create_skill("reviewed", _skill_content("reviewed"))
            ref = tmp_path / ".clawk" / "skills" / "reviewed" / "references"
            ref.mkdir()
