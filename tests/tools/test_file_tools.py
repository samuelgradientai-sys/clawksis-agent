...
class TestWindowsMsysPathResolution:
    ...
    def test_absolute_msys_path_normalized_before_windows_resolve(self, monkeypatch):
        ...
        monkeypatch.setattr(
            file_tools, "_uses_container_paths", lambda task_id="default": False
        )
    ...
    def test_container_paths_skip_msys_translation(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod, "_IS_WINDOWS", True)
        monkeypatch.setattr(
            file_tools, "_uses_container_paths", lambda task_id="default": True
        )
        monkeypatch.setattr(
            file_tools,
            "_authoritative_workspace_root",
...
class TestSensitivePathCheck:
    ...
    def test_clawk_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._clawk_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._clawk_config_resolved_loaded", True)
        ...
        assert "error" in result
        assert "Clawksis config" in result["error"]

    def test_clawk_config_blocked_via_tilde_path(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._clawk_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._clawk_config_resolved_loaded", True)
    ...
    def test_clawk_config_blocked_for_patch(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr(
            "tools.file_tools._clawk_config_resolved", "/some/other/path"
        )
        monkeypatch.setattr("tools.file_tools._clawk_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr(
            "tools.file_tools._clawk_config_resolved", "/home/user/.clawk/config.yaml"
        )
        monkeypatch.setattr("tools.file_tools._clawk_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
