...
Older images made /opt/clawksis/ui-tui and /opt/clawksis/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV CLAWK_TUI_DIR=/opt/clawksis/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R clawk:clawk /opt/clawksis/ui-tui" not in text
    assert "chown -R clawk:clawk /opt/clawksis/node_modules" not in text
