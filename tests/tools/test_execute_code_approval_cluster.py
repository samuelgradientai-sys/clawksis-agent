...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad CLAWK_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: CLAWK_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_logs_dropped_clawk_vars(caplog):
    ...
    leave users guessing why a sandbox script sees an unset CLAWK_* var."""
    import logging
    ...
    assert "CLAWK_HOME" in out and "PATH" in out
    assert "CLAWK_BASE_URL" not in out and "CLAWK_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "CLAWK_BASE_URL" in msgs and "CLAWK_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "CLAWK_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped CLAWK_* vars."""
    import logging
