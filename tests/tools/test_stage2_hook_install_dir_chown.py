...
When CLAWK_UID is remapped at container boot, ``usermod -u`` only rewrites

files under the clawk user's home directory ($CLAWK_HOME == /opt/data).

Runtime-writable trees under ``/opt/clawksis`` must be explicitly chowned to the

new UID before services drop privileges. ``/opt/clawksis/gateway`` is one such
