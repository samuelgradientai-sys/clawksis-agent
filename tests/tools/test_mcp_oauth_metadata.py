...
- :class:`ClawksisTokenStorage` ``.meta.json`` roundtrip (save / load / remove)

- The production manager provider

  (:class:`tools.mcp_oauth_manager.ClawksisMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _CLAWK_PROVIDER_CLS
...
# ClawksisTokenStorage metadata roundtrip
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = ClawksisTokenStorage("example-server")
    ...
    def test_load_missing_returns_none(self, tmp_path, monkeypatch):
        ...
        storage = ClawksisTokenStorage("nonexistent")
    ...
    def test_load_corrupt_returns_none(self, tmp_path, monkeypatch):
        ...
        storage = ClawksisTokenStorage("corrupt-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        ...
        storage = ClawksisTokenStorage("cleanup-server")
...
# Manager-path provider (ClawksisMCPOAuthProvider) — production code path

# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: ClawksisTokenStorage, **context_attrs):
    ...
    provider = _CLAWK_PROVIDER_CLS.__new__(_CLAWK_PROVIDER_CLS)

    provider._clawk_server_name = context_attrs.get("server_name", "srv")
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        storage = ClawksisTokenStorage("mgr-srv")
        ...
        with patch.object(
            _CLAWK_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(
        ...
        storage = ClawksisTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(
        ...
        storage = ClawksisTokenStorage("persist-srv")
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        storage = ClawksisTokenStorage("noop-srv")
        ...
        with patch.object(ClawksisTokenStorage, "save_oauth_metadata") as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        storage = ClawksisTokenStorage("flow-srv")
        ...
        with (
            patch.object(
                _CLAWK_PROVIDER_CLS.__bases__[0],
                "async_auth_flow",
