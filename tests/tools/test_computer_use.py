...
class TestCuaDriverSessionReconnect:
    ...
    def test_is_transient_daemon_error_matches_eagain(self):
        ...
        assert (
            _CuaDriverSession._is_transient_daemon_error(
                RuntimeError("daemon proxy to /tmp/sock not ready")
            )
            is True
        )
    ...
    def test_call_tool_falls_back_to_cli_on_transient_error(self):
        ...
        def fake_cli(name, args, timeout):
            ...
            return {
                "data": "42 elements\ntree",
                "images": ["B64PNG"],
                "structuredContent": {"element_count": 42},
                "isError": False,
            }
    ...
    def test_cli_fallback_reads_screenshot_from_file(self, tmp_path):
        ...
        class FakeProc:
            ...
            stdout = (
                '{"element_count": 7, "tree_markdown": "- [0] AXButton",'
    ...
    def test_capture_refetches_via_cli_on_empty_gws(self):
        ...
        def cli_call(name, args, timeout):
            ...
            return {
                "data": "5 elements\n- [0] AXButton 'OK'",
                "images": [png_b64],
                "structuredContent": {"element_count": 5},
                "isError": False,
            }
...
class TestCaptureAppFilterNoMatch:
    ...
    def test_capture_logical_driver_error_disarms_target_and_tokens(self, tool_name):
        ...
        logical_error = {
            "data": f"{tool_name} rejected target",
            "images": [],
            "structuredContent": None,
            "isError": True,
        }
...
class TestCaptureAfterExactTarget:
    def test_followup_capture_reuses_exact_window_identity(self):
        ...
        _maybe_follow_capture(
            cast(Any, backend), ActionResult(ok=True, action="click"), True
        )
    ...
    def test_dispatch_capture_after_reuses_metadata_resolved_identity(
        self, monkeypatch
    ):
        ...
        assert all(
            call["pid"] == 7675 and call["window_id"] == 42 for call in gws_calls
        )
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Clawksis-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (samuelgradientai-sys/clawksis-agent#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (samuelgradientai-sys/clawksis-agent#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (samuelgradientai-sys/clawksis-agent#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Clawksis patch.
    ...
    def test_future_renamed_subcommand_is_honored(self):
        ...
        instead of `mcp` keeps working without a Clawksis patch."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so CLAWK_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (samuelgradientai-sys/clawksis-agent#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (samuelgradientai-sys/clawksis-agent#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (samuelgradientai-sys/clawksis-agent#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Clawksis never declared a cua-driver
    ...
    def test_session_id_unique_per_backend(self):
        ...
        assert a != b, "each Clawksis run should mint its own session id"
