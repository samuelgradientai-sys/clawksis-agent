...
class TestIsWriteDenied:
    ...
    def test_clawk_control_files_oauth_and_mcp_tokens_denied(self, path):
        """Clawksis control files, PKCE creds, mcp-tokens, and pairing entries must be write-denied."""

        from clawk_constants import get_clawk_home

        clawk_home = get_clawk_home()

        full_path = str(clawk_home / path)
    ...
    def test_clawk_control_files_and_oauth_traversal_denied(self, path):
        """Path traversal attempts to protected Clawksis files must be blocked."""
    ...
    def test_control_files_and_oauth_protected_in_profile_mode(
        ...
        global ~/.clawksis/{auth.json,config.yaml,webhook_subscriptions.json,
        ...
        # Simulate a profile-mode CLAWK_HOME layout:
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.clawksis/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        ...
        leaked = (
            "'\x07__CLAWK_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__CLAWK_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "CLAWK_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        ...
        leaked = (
            "__CLAWK_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__CLAWK_FENCE_a9f7b3__\n"
        )
