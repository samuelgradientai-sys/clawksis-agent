...
``clawk_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``CLAWK_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # CLAWK_* handling (#27303): only the operational allowlist passes;
        # every other CLAWK_* is dropped (the broad prefix was removed).
        "CLAWK_HOME": "/home/alice/.clawksis",        # allowlisted → kept
        "CLAWK_PROFILE": "default",                 # allowlisted → kept
        "CLAWK_INTERACTIVE": "1",                   # not allowlisted → dropped
        "CLAWK_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "CLAWK_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``clawk_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``clawk_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        from tools.code_execution_tool import generate_clawk_tools_module
        stub = generate_clawk_tools_module(["terminal"], transport="file")
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        from tools.code_execution_tool import generate_clawk_tools_module
        import tempfile, ast
        stub = generate_clawk_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_clawk_tools_module(["terminal"], transport="uds")
...
# let the child import clawk_tools, a user script that printed non-ASCII
