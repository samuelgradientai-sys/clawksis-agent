...
TTS provider tools must resolve API keys from ``~/.clawksis/.env`` (via
``clawk_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``clawk_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.clawksis/.env`` carries the
    ...
    ``clawk_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``clawk_cli.config.load_env`` to simulate ``~/.clawksis/.env`` carrying the
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(
        ...
        # Simulate ~/.clawksis/.env carrying the key (load_env returns the dict
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.clawksis/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
