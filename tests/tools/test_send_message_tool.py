...
class TestSendToPlatformChunking:
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        with patch("tools.send_message_tool._send_slack", send):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    SimpleNamespace(enabled=True, token="***", extra={}),
                    "C123",
                    "**hello** from [Clawksis](<https://example.com>)",
                )
            )
        ...
        send.assert_awaited_once_with(
            "***",
            "C123",
            "*hello* from <https://example.com|Clawksis>",
        )
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert result["success"] is True
        async_mock.assert_awaited_once_with({"bridge_port": 3000}, chat_id, "hello from clawk")
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``clawk cron``
    runs separately from ``clawk gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``CLAWK_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``CLAWK_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``CLAWK_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    3. ``is_gateway_running()`` returns True (CLI / orchestrator profile with
       a live gateway colocated under the same ``CLAWK_HOME``).
    4. None of the above → False, tool is hidden.
    """

    def test_kanban_task_env_grants_access(self, monkeypatch):
        """Workers spawned by the dispatcher (CLAWK_KANBAN_TASK set) must be
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        """Honoring CLAWK_KANBAN_TASK must not depend on importing or calling
        gateway.status — the worker may run with a CLAWK_HOME that has no
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        """``CLAWK_SESSION_PLATFORM=local`` means CLI-style — must defer to
