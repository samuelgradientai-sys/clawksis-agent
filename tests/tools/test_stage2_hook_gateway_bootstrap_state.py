...
CLAWK_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
CLAWK_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as

/etc/cont-init.d/01-clawk-setup, which runs lexicographically BEFORE
...
This mirrors the existing CLAWK_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def _seed_block(text: str) -> str:
    """Extract the ``if [ ! -f "$CLAWK_HOME/gateway_state.json" ] && … fi``

    block that seeds the gateway state file from the bootstrap env var."""

    m = re.search(
        r'(if \[ ! -f "\$CLAWK_HOME/gateway_state\.json" \] && \\\n'
        r"(?:.*\n)*?fi)",
        text,
    )
    ...
    assert m, (
        ...
        "guarded on CLAWK_GATEWAY_BOOTSTRAP_STATE"
    )
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$CLAWK_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )

    assert "CLAWK_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $CLAWK_HOME.



    ``env_value`` is the CLAWK_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export CLAWK_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset CLAWK_GATEWAY_BOOTSTRAP_STATE\n"
        )
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when CLAWK_GATEWAY_BOOTSTRAP_STATE is unset"
