...
code paths Clawksis takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``clawk_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect CLAWK_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("clawk_cli.stdio", None)
        # Fresh import now; tests import from clawk_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import clawk_cli.stdio as _s
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("CLAWK_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("CLAWK_DISABLE_WINDOWS_UTF8", optout)
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("clawk_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "clawk_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, clawk_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "clawk_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call clawk_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """clawk_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from clawk_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from clawk_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``clawk update``
        (which runs under clawk-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# clawk_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "clawk_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the CLAWK_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / clawk_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "clawk_cli/tools_config.py",
            "clawk_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_clawk_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(
            encoding="utf-8"
        )
        assert "if _IS_WINDOWS:" in source
        assert "get_clawk_home" in source
        assert 'cache_dir = get_clawk_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_clawk_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "clawk_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "clawk_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(
        ...
        clawk_cli._subprocess_compat; the watcher must call that helper from
    ...
    def test_watcher_rewrites_console_python_to_windowless(self):
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "CLAWK_HOME) so the windowless base pythonw resolves clawk_cli."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import clawk_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "clawk_cli.main", "gateway", "run"]
    ...
    def test_windows_rewrites_to_pythonw_and_preserves_tail(self):
        ...
        # ``from clawk_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import clawk_cli.config  # noqa: F401
        import clawk_cli.gateway  # noqa: F401
        ...
        # Mock get_clawk_home too: the real one calls Path.resolve(), which
        ...
        with (
            ...
            mock.patch.object(
                gw, "_stable_gateway_working_dir", return_value="C:/clawk"
            ),
            mock.patch("clawk_cli.config.get_clawk_home", return_value="C:/clawk"),
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
