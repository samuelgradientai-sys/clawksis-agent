...
from clawk_constants import (
    ...
    find_clawk_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_clawk_root,
    get_clawk_dir,
    get_clawk_home,
    get_process_clawk_home,
    heal_clawk_managed_node,
    clawk_managed_node_tree_present,
    iter_clawk_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultClawksisRoot:
    ...
    def test_no_clawk_home_returns_native(self, tmp_path, monkeypatch):
        ...
        assert get_default_clawk_root() == tmp_path / ".clawk"
    ...
    def test_clawk_home_is_native(self, tmp_path, monkeypatch):
        ...
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWK_HOME", str(native))
    ...
    def test_clawk_home_is_docker(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWK_HOME", str(docker_home))
    ...
    def test_clawk_home_is_custom_path(self, tmp_path, monkeypatch):
        ...
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWK_HOME", str(custom))
...
class TestGetProcessClawkHome:
    """Tests for get_process_clawk_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_clawk_home() == home

    def test_env_unset_returns_platform_default(self, tmp_path, monkeypatch):
        monkeypatch.delenv("CLAWK_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        assert get_process_clawk_home() == tmp_path / ".clawk"
    ...
    def test_ignores_context_local_override(self, tmp_path, monkeypatch):
        ...
        token = set_clawk_home_override(profile_home)
        try:
            # get_clawk_home() follows the override; the process-scoped
            # variant must not.
            assert get_clawk_home() == profile_home
            assert get_process_clawk_home() == launch_home
        finally:
            reset_clawk_home_override(token)


class TestClawksisManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_clawk_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(clawk_constants.sys, "platform", "win32")
        monkeypatch.setenv("CLAWK_HOME", str(home))
        monkeypatch.setattr(clawk_constants, "node_tool_runnable", lambda path: True)

        assert find_clawk_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(clawk_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(clawk_constants, "heal_clawk_managed_node", lambda: False)
        monkeypatch.setattr(
            clawk_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert clawk_managed_node_tree_present() is True
    ...
    def test_with_clawk_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        ...
        env = with_clawk_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(clawk_constants, "heal_clawk_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_clawk_node_executable("npm") == str(healed_npm)
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".clawk"
...
class TestGetClawksisDir:
    """Tests for ``get_clawk_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_clawk_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_clawk_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_clawk_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_clawk_dir("cache/audio", "audio_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_ignores_non_drive_paths(self):
        assert clawk_constants.windows_path_to_wsl("/home/alex") is None
    ...
    def test_wsl_unc_ignores_non_unc_paths(self):
        ...
        assert clawk_constants.wsl_unc_path_to_posix("/home/alex") is None
