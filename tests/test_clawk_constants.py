...
class TestGetDefaultClawksisRoot:
    ...
    def test_no_clawk_home_returns_native(self, tmp_path, monkeypatch):
        ...
        POSIX-only expectation — native Windows falls back to
        %LOCALAPPDATA%\\clawk (covered by the two win32 tests below), so pin
        the platform instead of failing when the suite runs on Windows.
        """

        monkeypatch.delenv("CLAWK_HOME", raising=False)

        monkeypatch.setattr(clawk_constants.sys, "platform", "linux")
...
class TestParseReasoningEffort:
    ...
    @pytest.mark.parametrize(
        # NOTE: "max" and "ultra" are *valid* efforts now
        # (VALID_REASONING_EFFORTS), so they no longer belong here.
        "value",
