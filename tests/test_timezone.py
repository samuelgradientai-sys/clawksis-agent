...
Tests for timezone support (clawk_time module + integration points).
...
def _reset_clawk_time_cache():
    """Reset the clawk_time module cache (replacement for removed reset_cache)."""

    clawk_time._cached_tz = None

    clawk_time._cached_tz_name = None

    clawk_time._cache_resolved = False


# =========================================================================

# clawk_time.now() — core helper

# =========================================================================


class TestClawksisTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):

        _reset_clawk_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["CLAWK_TIMEZONE"] = "Asia/Kolkata"

        result = clawk_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["CLAWK_TIMEZONE"] = "Mars/Olympus_Mons"

        with caplog.at_level(logging.WARNING, logger="clawk_time"):
            result = clawk_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        ...
        os.environ["CLAWK_TIMEZONE"] = "Europe/London"

        tz = clawk_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        ...
        os.environ["CLAWK_TIMEZONE"] = "Not/A/Timezone"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When CLAWK_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) CLAWK_TIMEZONE itself does NOT leak into the child env

        probe = (
            "import os; "
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("CLAWK_TIMEZONE=" + os.environ.get("CLAWK_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "CLAWK_TIMEZONE=NOT_SET" in result["output"], (
            "CLAWK_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When CLAWK_TIMEZONE is not set, child process has no TZ."""
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=clawk_tz) which shifted

        absolute time when system-local tz != Clawksis tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_clawk_tz(self):
        """Already-aware datetimes should be normalized to Clawksis tz."""
        ...
        # Must be in Clawksis tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(
        ...
        """Reproduce the actual bug: system tz ahead of Clawksis tz caused

        overdue jobs to appear as not-yet-due.



        Scenario: system is Asia/Kolkata (UTC+5:30), Clawksis is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Clawksis tz
        ...
        # Use a Clawksis timezone far behind UTC so that the numeric wall time

        # of the naive timestamp exceeds _clawk_now's wall time — this would

        # have caused a false "not due" with the old replace(tzinfo=...) approach.

        os.environ["CLAWK_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        ...
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Clawksis timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["CLAWK_TIMEZONE"] = "US/Eastern"
