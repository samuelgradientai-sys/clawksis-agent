...
Under a gateway (Telegram, Discord, Slack, ...) Clawksis passes the
...
class TestPeerResolutionOrder:
    ...
    def test_pin_does_not_affect_assistant_peer(self):
        ...
        session = mgr.get_or_create("telegram:86701400")
        assert session.user_peer_id == "Igor"
        assert session.assistant_peer_id == "clawk-assistant"


class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Clawksis via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Clawksis profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `clawk -p partner` pinned to a different person's peer).
