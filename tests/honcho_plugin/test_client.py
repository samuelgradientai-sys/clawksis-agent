...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "clawk"
        assert config.workspace_id == "clawk"
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="clawksis-agent"
        ):
            result = config.resolve_session_name("/home/user/clawksis-agent/subdir")
        assert result == "clawksis-agent"
...
class TestResolveConfigPath:
    def test_prefers_clawk_home_when_exists(self, tmp_path):
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: CLAWK_HOME points at ~/.clawksis/profiles/<name>, so
        # _get_default_clawk_home() must resolve back to ~/.clawksis — that's
        ...
        assert _get_default_clawk_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_clawk_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "clawk_coder"
        assert profile_host_key("default") == "clawk"

    def test_default_returns_clawk(self):
        with patch.dict(os.environ, {}, clear=True):
    ...
    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"CLAWK_HONCHO_HOST": "clawk.coder"}):
            assert resolve_active_host() == "clawk.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with (
            ...
            saved = sys.modules.get("clawk_cli.profiles")
            sys.modules["clawk_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "clawk"
            finally:
                if saved is not None:
                    sys.modules["clawk_cli.profiles"] = saved
                else:
                    sys.modules.pop("clawk_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="clawk_coder")
        assert config.host == "clawk_coder"
        assert config.workspace_id == "clawk"  # shared workspace
        assert config.ai_peer == "clawk_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="clawk")
        assert config.host == "clawk"
        assert config.workspace_id == "clawk"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        assert config.host == "clawk_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "clawk_dreamer"
...
class TestGetHonchoClient:
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_clawk_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(
        ...
        monkeypatch.setenv("CLAWK_MANAGED_DIR", str(managed_dir))
