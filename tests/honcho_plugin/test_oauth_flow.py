    ...
    def do_POST(self):  # noqa: N802
        ...
        if grant_type == "authorization_code":
            body["config"] = {
                ...
                "hosts": {"clawk": {"saveMessages": True, "recallMode": "hybrid"}},
            }
...
def fake_as(monkeypatch):
    ...
    monkeypatch.setenv("HONCHO_OAUTH_CLIENT_ID", "clawk-desktop")
    try:
        yield base
    finally:
        server.shutdown()
...
def test_full_loopback_flow_then_refresh(tmp_path, fake_as):
    ...
    assert saved["hosts"]["clawk"]["apiKey"] == "hch-at-1"
    assert saved["hosts"]["clawk"]["oauth"]["refreshToken"] == "hch-rt-1"
    assert saved["hosts"]["clawk"]["recallMode"] == "hybrid"
    ...
    token, refreshed = oauth.ensure_fresh_token(
        config_path, "clawk", now=saved["hosts"]["clawk"]["oauth"]["expiresAt"] + 10
    )
    assert refreshed is True
    assert token == "hch-at-2"
    rotated = json.loads(config_path.read_text())["hosts"]["clawk"]["oauth"]
...
def test_source_tags_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, source="clawk-cli")
    assert "source=clawk-cli" in url
    untagged, _ = oauth_flow.begin_authorization(endpoints)
    assert "source=" not in untagged


def test_client_id_defaults_to_clawk_agent(monkeypatch):
    ...
    assert oauth_flow.resolve_endpoints(**common).client_id == "clawksis-agent"
...
def test_grant_persists_default_client_id(tmp_path, fake_as, monkeypatch):
    ...
    # store client_id=clawksis-agent so refresh reuses the right client.
    monkeypatch.delenv("HONCHO_OAUTH_CLIENT_ID", raising=False)
    config_path = tmp_path / "honcho.json"
    ...
    saved = json.loads(config_path.read_text())
    assert saved["hosts"]["clawk"]["oauth"]["clientId"] == "clawksis-agent"
...
def test_cli_flow_stores_tokens_without_applying_config(tmp_path, fake_as):
    ...
    config_path = tmp_path / "honcho.json"
    config_path.write_text(json.dumps({"hosts": {"clawk": {"saveMessages": False}}}))
...
def test_get_flow_status_reports_stored_connection(tmp_path, monkeypatch, reset_flow):
    ...
    monkeypatch.setattr(honcho_client, "resolve_active_host", lambda: "clawk")
    monkeypatch.delenv("HONCHO_API_KEY", raising=False)

    cfgfile.write_text(json.dumps({"hosts": {"clawk": {}}}))
    assert oauth_flow.get_flow_status()["connected"] is False

    cfgfile.write_text(json.dumps({"hosts": {"clawk": {"apiKey": "hch-v3-static"}}}))
