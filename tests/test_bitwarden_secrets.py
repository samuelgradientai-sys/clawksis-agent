...
binary download are exercised manually by `clawk secrets bitwarden
...
def clawk_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_clawk_home; clear if needed.

    import clawk_constants

    if hasattr(clawk_constants, "_CLAWK_HOME_CACHE"):
        clawk_constants._CLAWK_HOME_CACHE = None  # type: ignore[attr-defined]
...
def test_install_bws_rejects_malicious_member(clawk_home, monkeypatch):
...
def test_install_bws_happy_path(clawk_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(clawk_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(clawk_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from clawk_cli.env_loader import _apply_external_secret_sources
