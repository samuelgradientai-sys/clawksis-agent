...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and clawksis-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),  # clawksis-agent (tests/skills/)
]
...
def test_browser_clears_captcha_logic():
    ...
    assert (
        config.browser_clears_captcha(
            {"browser_backend": "auto"}, env={"BROWSERBASE_API_KEY": "x"}
        )
        is True
    )
...
def test_encryption_age_round_trip():
    ...
    with temp_env():
        ...
        assert (
            dossier.load(d["subject_id"])["identity"]["full_name"] == "Jane Q. Public"
        )
...
def test_seed_broker_db_loads_and_is_well_formed():
    ...
    for b in everyone:
        assert (
            b.get("id")
            and b.get("name")
            and b.get("priority") in {"crucial", "high", "standard", "long_tail"}
        )
...
def test_hard_human_requirements_force_t3():
    ...
    assert (
        tiers.select_tier(brokers.get("thatsthem"), browser_clears_captcha=True) == "T1"
    )
...
def test_disclosure_maps_street_when_broker_requires_it():
    ...
    out = dossier.select_disclosure(
        d, ["full_name", "street", "city", "state", "postal"]
    )
...
def _mini_broker(bid, owns=None, requires=None, notes="", quirks=None):
    return {
        ...
        "optout": {
            "method": "web_form",
            "url": f"https://{bid}.example/optout",
...
def test_batch_plan_orders_found_parents_first():
    ...
    ledger = {
        "standalone": {"state": "found"},
        "smallparent": {"state": "found"},
    ...
    assert (
        phase2
        and "PARENTS FIRST" in phase2[0]
        and "bigparent -> smallparent" in phase2[0]
    )
...
def test_parent_playbook_has_bespoke_and_synthesised_steps():
    ...
    bespoke["optout"]["playbook"] = [
        "Step one from the record",
        "SUPPRESSION != DELETION warning",
    ]
    bl = [
        bespoke,
        _mini_broker(
...
def test_new_can_record_scan_outcome_directly():
    with temp_env():
        assert (
            ledger.transition("sub_test01", "thatsthem", "found", found=True)["state"]
            == "found"
        )
        assert (
            ledger.transition("sub_test01", "radaris", "not_found")["state"]
            == "not_found"
        )
        ...
        assert (
            ledger.transition("sub_test01", "spokeo", "blocked")["state"] == "blocked"
        )
        assert (
            ledger.transition("sub_test01", "radaris", "blocked")["state"] == "blocked"
        )
        ...
        assert (
            ledger.transition("sub_test01", "spokeo", "not_found")["state"]
            == "not_found"
        )
...
def test_indirect_exposure_state_and_transitions():
    with temp_env():
        ...
        assert (
            ledger.transition(sid, "radaris", "indirect_exposure")["state"]
            == "indirect_exposure"
        )
...
def test_ledger_illegal_transition_raises():
    with temp_env():
        try:
            ledger.transition(
                "sub_test01", "spokeo", "confirmed_removed"
            )  # new -> confirmed_removed
        except ValueError:
            pass
        else:
...
def test_ledger_disclosure_log():
    with temp_env():
        ledger.log_disclosure(
            "sub_test01", "spokeo", ["full_name", "contact_email"], "web_form"
        )
...
def test_consent_gate():
    ...
    try:
        ...
        raise AssertionError(
            "require_authorized should raise for non-consenting subject"
        )


def test_least_disclosure_selection():
    d = _consenting()
    got = dossier.select_disclosure(
        d, ["full_name", "contact_email", "profile_url", "ssn", "date_of_birth"]
    )
...
def test_all_names_and_locations_dedupe():
    d = _consenting()
    d["identity"]["also_known_as"] = [
        "Jane Public",
        "Jane Q. Public",
    ]  # 2nd dups primary
    d["identity"]["prior_addresses"] = [
        {"city": "Berkeley", "state": "CA"},
        {"city": "Oakland", "state": "CA"},
    ]
    assert dossier.all_names(d) == ["Jane Q. Public", "Jane Public"]
    assert [loc["city"] for loc in dossier.all_locations(d)] == [
        "Oakland",
        "Berkeley",
...
def test_search_vectors_respect_broker_capabilities():
    ...
    assert v and all(
        x["by"] == "name" for x in v
    )  # broker can't search email -> no email vectors


def test_search_vectors_address_needs_line1():
    d = _consenting()
    d["identity"]["current_address"] = {
        "line1": "123 Main St",
        "city": "Oakland",
        "state": "CA",
        "postal": "94601",
    ...
    assert (
        len(v) == 1
        and v[0]["by"] == "address"
        and v[0]["query"]["line1"] == "123 Main St"
    )
...
def test_subject_id_is_opaque_no_name_leak():
    ...
    assert (
        dossier.new_subject_id("Maiden Married Person") != sid
...
def test_cdp_default_profile_uses_clawk_home():
    prev = os.environ.get("CLAWK_HOME")
    with tempfile.TemporaryDirectory() as d:
        os.environ["CLAWK_HOME"] = d
        try:
            ...
            if prev is None:
                ...
                os.environ["CLAWK_HOME"] = prev


def test_cdp_endpoint_status_parses_live_and_handles_down():
    orig = cdp._http_get
    cdp._http_get = lambda url, timeout: (
        b'{"Browser":"Chrome/1.2","webSocketDebuggerUrl":"ws://x"}'
    )
    try:
        st = cdp.endpoint_status(port=9222)
        assert (
            st
            and st["Browser"] == "Chrome/1.2"
            and st["webSocketDebuggerUrl"] == "ws://x"
        )
    finally:
        cdp._http_get = orig
    ...
    try:
        assert (
            cdp.endpoint_status(port=9222) is None
        )  # nothing listening -> None, never raises
    finally:
        cdp._http_get = orig
...
def test_cdp_find_browser_override():
    ...
    assert (
        cdp.find_browser("definitely-not-a-real-browser-xyz") is None
    )  # bogus -> None (no crash)
...
def test_plan_surfaces_antibot():
    ...
    broker = {
        "id": "tps",
        "optout": {"requires": {}},
        "search": {"antibot": "datadome", "by": ["name"]},
    }
...
def test_plan_prewarns_when_dob_required_but_missing():
    ...
    broker = {
        ...
        "search": {"by": ["name"]},
        "optout": {
            "requires": {"dob": True, "email_verification": True},
...
def test_plan_surfaces_optout_quirks_and_email():
    ...
    broker = {
        ...
        "optout": {
            "requires": {},
            "email": "x@broker.test",
            "quirks": ["no profile URL -> email fallback"],
        },
    }
...
def test_render_ccpa_indirect_request_names_only_own_identifiers():
    ...
    out = legal.render_request(
        ...
        b,
        {
            ...
            "my_identifiers": [
                "jane@example.com",
                'the name "Jane Q. Public" where it appears as a relative',
            ],
...
def test_extract_verification_link_ignores_unrelated_only():
    assert (
        email_modes.extract_verification_link("see https://example.com/news today")
        is None
    )
...
def test_badbool_merge_keeps_curated_and_adds_new():
    with temp_env():
        badbool.refresh(
            __import__("paths").brokers_cache_path(), markdown=BADBOOL_FIXTURE
        )
...
def test_auto_configure_picks_most_autonomous():
    with temp_env():
        ...
        assert (
            config.auto_configure(env={"AGENTMAIL_API_KEY": "am"})["email_mode"]
            == "alias"
        )
        # encryption auto-on exactly when age is installed (free privacy, zero human cost)
        assert config.auto_configure(env={})["encryption"] == (
            "age" if _AGE else "none"
        )
...
def test_emailer_settings_inference_and_floor():
    ...
    s = emailer.smtp_settings({
        **corp,
        "EMAIL_SMTP_HOST": "mail.corp.example",
        "EMAIL_SMTP_PORT": "465",
    })
...
def test_emailer_send_requires_config_and_broker_address():
    ...
    try:
        ...
        raise AssertionError(
            "unconfigured SMTP must raise (draft fallback, not a crash)"
        )
    try:
        emailer.send(
            {"id": "y", "optout": {}},
...
def test_browser_email_mode_is_autonomous_without_smtp_or_imap():
    with temp_env():
        ...
        q = autopilot.next_actions(
            d, [mailer, verifier], _auto_cfg(email_mode="browser"), led, env={}
        )
        sends = [a for a in q["actions"] if a["type"] == "optout_email_send"]
        assert (
            sends
            and sends[0]["send_via"] == "browser"
            and sends[0]["to"] == "privacy@mailer.example"
        )
...
def test_verification_link_from_messages_is_domain_scoped():
    ...
    real = {
        "from": "no-reply@spokeo.com",
        "subject": "Confirm your opt out",
...
def test_verification_pending_to_awaiting_processing_is_legal():
    with temp_env():
        ...
        assert (
            ledger.transition(sid, "intelius", "awaiting_processing")["state"]
            == "awaiting_processing"
        )
...
def test_followup_stamps_and_due_queue():
    ...
    assert (
        f_done["next_recheck_at"] > f_sub["next_recheck_at"]
    )  # 30d rescan > 10d processing
    assert (
        ledger.followup_fields("found", broker, d) == {}
    )  # scan verdicts get no stamp
    led = {
        "a": {
            "broker_id": "a",
        ...
        "b": {
            ...
            "next_recheck_at": "2999-01-01T00:00:00Z",
        },
    }
...
def test_badbool_auto_records_have_processing_estimate():
    ...
    assert (
        recs[0]["optout"]["est_processing_days"] == 14
    )  # drives next_recheck_at for live records
...
def test_next_actions_scan_first_then_optouts_parents_first():
    with temp_env():
        ...
        assert not any(
            t.startswith("optout") for t in types
        )  # never act before the crawl
        ...
        assert [a["broker_id"] for a in opt] == [
            "parent",
            "solo",
        ]  # kid covered by parent
...
def test_next_actions_email_send_vs_draft_digest():
    with temp_env():
        ...
        env = {"EMAIL_ADDRESS": "agent@gmail.com", "EMAIL_PASSWORD": "p"}
        q = autopilot.next_actions(
            d, [b], _auto_cfg(email_mode="programmatic"), led, env=env
        )
        ...
        assert any(
            "render-email" in " ".join(t["agent_prep"]) for t in q2["human_digest"]
        )
...
def test_next_actions_poll_verification_and_due_rechecks():
    with temp_env():
        ...
        q = autopilot.next_actions(
            d,
            [b, _mini_broker("done1")],
            _auto_cfg(email_mode="programmatic"),
            led,
            env=env,
        )
        types = [a["type"] for a in q["actions"]]
...
def test_next_actions_blocked_stealth_or_operator_browser():
    with temp_env():
        ...
        q = autopilot.next_actions(
            d, [b], _auto_cfg(), led, env={"BROWSERBASE_API_KEY": "bb"}
        )
...
def test_parked_and_reappeared_states_group_correctly():
    ...
    with temp_env():
        ...
        led = {
            "parked": {"state": "human_task_queued"},
        ...
        q = autopilot.next_actions(d, bl, _auto_cfg(), led, env={})
        assert not any(
            a["type"] in ("scan_inline", "fanout_scan") for a in q["actions"]
        )
        assert {
            a["broker_id"] for a in q["actions"] if a["type"] == "optout_web_form"
        } == {"chosen", "back"}
...
def test_cluster_parents_have_playbook_and_deletion_lane():
    ...
    for b in brokers._load_curated():
        ...
        assert d.get("email") or d.get("via"), (
            f"{bid}: cluster parent missing deletion lane"
        )
        # every declared email must be a legal send-email recipient
        for addr in [opt.get("email"), d.get("email")]:
            if addr:
                assert addr in emailer.broker_addresses(b), (
                    f"{bid}: {addr} not sendable"
                )
...
def test_curated_intelius_suppress_first_not_delete():
    ...
    assert (
        d["email"] == "privacy@peopleconnect.us"
    )  # rights-request address for the data-purge path
...
def test_deletion_prefer_flag_controls_autopilot_note():
    with temp_env():
        ...
        pc["optout"]["deletion"] = {
            "via": "in_flow",
            "prefer": False,
            "email": "privacy@pc.example",
            "notes": "delete undoes suppression",
        }
        q = autopilot.next_actions(
            d,
            [pc, _mini_broker("kid")],
...
def test_curated_whitepages_email_lane_is_autonomous():
    ...
    assert (
        opt["requires"]["phone_callback"] is False
    )  # the callback is only the ALT tool
...
def test_request_kind_is_residency_honest():
    ...
    assert (
        autopilot.request_kind(tx) == "generic"
    )  # never claim CCPA for a non-CA resident
...
def test_email_lane_routing_and_rescue():
    with temp_env():
        ...
        hard["optout"]["deletion"] = {
            "email": "privacy@hardsite.example",
            "kinds": ["ccpa", "generic"],
        }
        ...
        q = autopilot.next_actions(
            d, [mailer, hard, cb], _auto_cfg(email_mode="programmatic"), led, env=env
        )
        sends = {
            a["broker_id"]: a for a in q["actions"] if a["type"] == "optout_email_send"
        }
...
def test_human_tasks_digest_markdown():
    with temp_env():
        ...
        ledger.transition(
            sid, "mylife", "human_task_queued", human_task_reason="gov ID demanded"
        )
...
def test_registry_refresh_all_ingests_csv_and_lists_portals():
    with temp_env():
        res = registry.refresh_all(
            paths.registry_cache_path(), fetched={"ca": _registry_csv()}
        )
        assert res["total"] == 2
        assert (
            res["sources"]["ca"]["parsed"] == 2
            and res["sources"]["ca"]["added_after_dedupe"] == 2
        )
        assert (
            res["sources"]["vt"]["format"] == "portal"
        )  # no bulk export, surfaced as portal
...
def test_email_rate_limit_paces_sends():
    with temp_env() as data:
        ...
        slept, now = [], [1000.0]
        emailer._respect_rate_limit(
            20, lambda s: slept.append(s), lambda: now[0], state
        )
        ...
        assert (
            slept and abs(slept[0] - 15) < 0.01
        )  # waited the remaining 15s of the 20s window
...
def test_show_reads_back_case_state_and_evidence():
    with temp_env():
        ...
        _run([
            ...
            "true",
            "--evidence",
            '{"listing_urls": ["https://radaris.com/p/x"]}',
        ])
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    prev_home = os.environ.get("CLAWK_HOME")
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            assert (
                config.dotenv_env()["BROWSERBASE_API_KEY"] == "from_shell"
            )  # shell wins
        finally:
            for k, v in (("CLAWK_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:
...
def test_cdp_cli_detects_already_running_and_does_not_launch():
    ...
    cdp.endpoint_status = lambda *a, **k: {
        "Browser": "Chrome/9",
    ...
    def _no_launch(*a, **k):
        raise AssertionError(
            "launch() must not be called when a browser is already live"
        )
...
def test_registry_candidate_urls_newest_first_with_floor():
    ...
    assert urls[0].endswith("registry2027.csv") and urls[-1].endswith(
        "registry2025.csv"
    )
    assert registry.ca_candidate_urls(__import__("datetime").date(2024, 1, 1))[
        0
    ].endswith("registry2025.csv")
