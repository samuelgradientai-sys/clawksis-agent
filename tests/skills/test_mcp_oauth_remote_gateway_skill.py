...
SESSION_REVOKED / REFRESH_DEAD), the CLAWK_HOME resolution fallback, the
...
def test_clawk_home_env_fallback(tmp_path, monkeypatch):
    mod = load_module()
    monkeypatch.setenv("CLAWK_HOME", str(tmp_path / "custom-home"))
    # Block the clawk_constants import so the env fallback is exercised
    with patch.dict(sys.modules, {"clawk_constants": None}):
        home = mod._clawk_home()
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "Clawksis"  # human credited first
