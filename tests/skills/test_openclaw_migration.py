...
"""
    ...
    assert "Tyler Williams > Active Projects: Clawksis" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `clawk cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # clawk_config_mod  — print statements in the post-migration summary that

    #                      tell the user to *review* ~/.clawksis/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "clawk_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    # Mixed-case / capitalized matches → capital-H ``Clawksis``.

    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Clawksis prefers Python 3.11"

    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Clawksis to use dark mode"

    assert mod.rebrand_text("Open-Claw config is great") == "Clawksis config is great"

    assert mod.rebrand_text("OPENCLAW uses tools well") == "Clawksis uses tools well"

    # All-lowercase matches → lowercase ``clawk``; this preserves the

    # real filesystem path ``~/.clawksis`` (Clawksis home) when rebranding

    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.

    assert mod.rebrand_text("openclaw should always respond concisely") == "clawk should always respond concisely"
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Clawksis remembers my timezone"

    assert mod.rebrand_text("clawdbot prefers tabs") == "clawk prefers tabs"

    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Clawksis was configured for Spanish"

    assert mod.rebrand_text("moltbot uses Python") == "clawk uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Clawksis said to ask Clawksis about Clawksis settings"
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewrite to lowercase ``.clawk`` (the real Clawksis home), not the broken
    ...
    rewritten to ``~/.Clawksis/config.yaml`` — a path that doesn't exist —
    ...
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.clawksis/config.yaml"

    assert mod.rebrand_text("use .openclaw directory") == "use .clawk directory"

    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.clawk'"

    # Sentence with both lowercase path and capitalized prose.

    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "clawk config path: ~/.clawksis/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "You are Clawksis" in result
