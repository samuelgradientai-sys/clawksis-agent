...
# ---------------------------------------------------------------------------
# Guards de sync (v2026.7.20)
# ---------------------------------------------------------------------------
# ``clawk_cli/main.py`` es uno de los divergentes que sigue en merge manual
# (tools/upstream/pending/TO_AGENT.txt): conserva el flujo propio de
# ``clawk update`` (salida limpia + _clean_managed_worktree) y todavia NO trajo
# tres piezas de upstream. Los tests de upstream que las cubren quedan aca, con
# skip condicional, para que se activen solos en cuanto main.py las adopte.
def _has_source(fn, needle: str) -> bool:
    import inspect
...
_HAS_DISCARD_SETTING = hasattr(clawk_main, "_discard_stashed_changes")
_HAS_LOCKFILE_PAIR_GUARD = _has_source(
    clawk_main._discard_lockfile_churn, "dirty_package_dirs"
)
_HAS_SCOPED_FETCH = _has_source(clawk_main._cmd_update_impl, '"fetch", "origin", branch')
_GITIGNORES_BOOTSTRAP_MARKER = ".clawk-bootstrap-complete" in (
    Path(clawk_main.__file__).resolve().parents[1] / ".gitignore"
...
def _fast_update(monkeypatch):
    """Fork-only: keep cmd_update fast for the CI per-file cap: skip the long
    (~3s) post-restart sleep (preserving short install-simulation sleeps) and
    make the gateway-restart process sweep (psutil scan + drain wait + SIGKILL,
    slow on Linux CI) a no-op. These tests cover update logic, not the restart."""
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    Leaving conflict markers in source files makes clawk unrunnable (SyntaxError).
    ...
    assert "clawk_cli/main.py" in out
...
@pytest.mark.skipif(
    not _HAS_LOCKFILE_PAIR_GUARD,
    reason="fork main.py: _discard_lockfile_churn aun no ignora lockfiles cuyo package.json tambien esta sucio (merge manual pendiente)",
)
def test_discard_lockfile_churn_skips_lock_when_package_json_dirty(tmp_path):
    ...
    clawk_main._discard_lockfile_churn(["git"], tmp_path)
...
def test_cmd_update_restores_stash_and_branch_when_already_up_to_date(monkeypatch, tmp_path, capsys):
    ...
    # Fork: force the stash path (not the managed-clone clean path) so this test
    # exercises stash restore. A real fork, or a clone where the managed clean
    # fails, falls through to stash.
    monkeypatch.setattr(clawk_main, "_clean_managed_worktree", lambda *a, **kw: False)
    restore_calls = []
    monkeypatch.setattr(
...
@pytest.mark.skipif(
    not _HAS_SCOPED_FETCH,
    reason="fork main.py: cmd_update todavia hace `git fetch origin` sin scope de branch",
)
...
# ---------------------------------------------------------------------------
# Fork-only: managed clone cleans, fork stashes
# ---------------------------------------------------------------------------
# NOTE (sync v2026.7.20): el test propio
# ``test_cmd_update_aborts_when_fresh_managed_uv_rebuild_fails`` NO se
# reintroduce: upstream saco la llamada a ``managed_uv.rebuild_venv`` de
# ``cmd_update`` (solo queda mencionada en un docstring), asi que el guard que
# ese test cubria ya no existe.
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    # Fork: force the stash path so this test exercises the reset-failure
    # handling of the stash branch (not the managed-clone clean path).
    monkeypatch.setattr(clawk_main, "_clean_managed_worktree", lambda *a, **kw: False)
    restore_calls = []
    monkeypatch.setattr(
        clawk_main, "_restore_stashed_changes",
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        clawk_main, "_discard_stashed_changes",
...
@pytest.mark.skipif(
    not _HAS_DISCARD_SETTING,
    reason="fork main.py: falta updates.non_interactive_local_changes / _discard_stashed_changes",
)
def test_non_interactive_discard_throws_changes_away(monkeypatch, tmp_path):
    ...
    clawk_main.cmd_update(SimpleNamespace(gateway=True))
...
def test_interactive_update_ignores_discard_setting(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(clawk_main.sys.stdin, "isatty", lambda: True)
    monkeypatch.setattr(clawk_main.sys.stdout, "isatty", lambda: True)

    clawk_main.cmd_update(SimpleNamespace())  # no gateway, no --yes
...
def test_non_interactive_defaults_to_stash_when_setting_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(clawk_config, "load_config", lambda *a, **kw: {"model": {}})
...
@pytest.mark.skipif(
    not _GITIGNORES_BOOTSTRAP_MARKER,
    reason="fork .gitignore: falta .clawk-bootstrap-complete (viene con el merge de main.py)",
)
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``clawk update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(clawk_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".clawk-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by clawk update (clawk_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "clawk-update-autostash")

    assert marker.exists(), (
        ".clawk-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".clawk-bootstrap-complete" not in status
