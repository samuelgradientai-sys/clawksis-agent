...
Covers:
 ...
 - Normal path (tokens already in Clawksis auth store)
...
Note: auto-import from ~/.codex/auth.json was removed in #12360 — Clawksis

now owns its own openai-codex auth state, and users explicitly adopt

existing Codex CLI tokens via `clawk auth openai-codex`. The old
...
@pytest.fixture()
def clawk_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Clawksis auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(clawk_auth_only_env):
    """openai-codex appears when tokens are already in Clawksis auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Clawksis
