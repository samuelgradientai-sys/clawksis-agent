...
clawk_cli.config._load_config_impl, so the managed-scope merge has to be
...
def _load_cli_config(home):
    ...
    cli.py binds ``_clawk_home = get_clawk_home()`` at import time (module
    singleton), so monkeypatching CLAWK_HOME after import doesn't move it.
    ...
    this only matters for tests that swap CLAWK_HOME.)
    """
    import cli

    cli._clawk_home = home
