"""Tests for `_can_open_graphical_browser()` in clawk_cli.auth.

Guards the fix for the May 2026 report where `clawk auth add xai-oauth`
...
from clawk_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("clawk_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.auth.sys.platform", "darwin")
