"""Tests for issue #26670 — concurrent clawk.exe detection and improved

quarantine retry / reboot-deferred fallback during `clawk update` on Windows.
...
from clawk_cli import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_clawk_instances

# helper (and need the autouse stub in tests/clawk_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "clawk.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "clawk.exe").write_bytes(b"")

    (scripts_dir / "clawk-gateway.exe").write_bytes(b"")
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "clawk.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_clawk_process(_winp, tmp_path):
    ...
    assert result == [(other_pid, "clawk.exe")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_clawk_instances(tmp_path) == []
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: clawk.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "clawk.exe"),
    ]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_still_finds_unrelated_other_clawk(_winp, tmp_path):
    """A sibling clawk.exe outside our ancestor chain must still be reported."""
    ...
    sibling_pid = me + 200  # an UNRELATED clawk.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "clawk.exe"),
        _make_proc(sibling_pid, str(shim), "clawk.exe"),
    ]
    ...
    assert result == [(sibling_pid, "clawk.exe")]
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "clawk.exe"),
        _make_proc(grandparent_pid, str(shim), "clawk.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "clawk.exe"),
    ]
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    assert result == [(launcher_pid, "clawk.exe")]
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "clawk.exe"),
        _make_proc(other_pid, str(shim), "clawk.exe"),
    ]
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    ...
    matches = [(1234, "clawk.exe"), (5678, "clawk.exe")]
    ...
    assert "clawk.exe" in msg

    assert "Clawksis Desktop" in msg

    assert "--force" in msg

    # Mentions the file that would have been overwritten

    assert str(tmp_path / "clawk.exe") in msg
...
# _quarantine_running_clawk_exe — retry + reboot-deferred fallback
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    assert quarantine.name.startswith("clawk.exe.old.")
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Clawksis Desktop" in captured or "gateway" in captured.lower()
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another clawk.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "clawk.exe")])
