...
def test_full_login_round_trip_unlocks_gated_api(gated_app):
    ...
    assert any("clawk_session_at" in c for c in set_cookies)
    assert any("clawk_session_rt" in c for c in set_cookies)
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``clawk_session_at`` / ``clawk_session_rt``
...
def test_gated_zero_providers_login_page_renders_help_text(monkeypatch):
    # Fork-specific: with zero providers the fork's /login renders the
    # first-run SETUP form (see the test below), so the "sign-in unavailable"
    # help page only shows up once first-run setup is unavailable.
...
def test_gated_zero_providers_first_run_renders_setup_form(monkeypatch):
    # Fork-specific: cero providers y NADA configurado -> formulario de
    # primera vez (crear usuario + contraseña), no la página
    # "sign-in unavailable".
    from clawk_cli.dashboard_auth import first_run
...
class _UnreachableProvider(StubAuthProvider):
    ...
    def verify_session(self, *, access_token: str):
        from clawk_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["clawk_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
