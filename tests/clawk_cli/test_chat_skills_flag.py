...
def test_top_level_skills_flag_defaults_to_chat(monkeypatch):
    ...
    assert captured == {
        "skills": ["clawksis-agent-dev,github-auth"],
        "command": None,
    }
...
def test_chat_subcommand_accepts_skills_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["clawk", "chat", "-s", "github-auth", "-q", "hello"],
    )
...
def test_chat_subcommand_accepts_image_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["clawk", "chat", "-q", "hello", "--image", "~/storage/shared/Pictures/cat.png"],
    )
...
def test_continue_worktree_and_skills_flags_work_together(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["clawk", "-c", "-w", "-s", "clawksis-agent-dev"],
    )
