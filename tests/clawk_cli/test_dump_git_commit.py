"""Tests for clawk_cli.dump._get_git_commit — git SHA resolution for ``clawk dump``.



``clawk dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``CLAWK_GIT_SHA`` build-arg.
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``clawk dump`` doesn't have to special-case
    ...
    with patch("clawk_cli.dump.subprocess.run", return_value=git_result):
