...
# NOTA (sync v2026.7.20): el test propio ``test_worktree_workspace_returns_intended_path``
# NO se reintroduce: upstream reemplazo el contrato (``resolve_workspace`` ahora
# materializa un worktree enlazado de verdad en vez de solo devolver la ruta
# pretendida) y ese test ya fallaba contra el codigo de produccion mergeado.
# Los tests de abajo cubren el contrato nuevo.
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import clawk_cli.profiles as profiles
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CLAWK_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    ...
    ``shutil.rmtree`` Clawksis' DB/metadata/logs on task completion.
    """
...
# spawned with `clawk -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's CLAWK_HOME.
# ---------------------------------------------------------------------------
...
class TestSharedBoardPaths:
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # ~/.clawksis/profiles/<profile>/kanban.db. After the fix both
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # CLAWK_HOME and confirm equality. This is the property the
    ...
    def test_docker_custom_clawk_home_uses_env_path_directly(
        ...
        # `get_default_clawk_root()` returns env_home directly when it
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # `get_default_clawk_root()` walks up to /opt/clawksis because
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # CLAWK_HOME perspectives via a real SQLite file. Without the
    ...
    def test_clawk_kanban_db_pin_beats_kanban_home(
        ...
        # CLAWK_KANBAN_HOME and the `get_default_clawk_root()` path.
        ...
        # workspaces_root still follows CLAWK_KANBAN_HOME -- the pins
        # are independent.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWK_HOME", str(default_home))
        monkeypatch.setenv("CLAWK_KANBAN_DB", "   ")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # and CLAWK_KANBAN_WORKSPACES_ROOT into the worker env so the
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    (see clawksis-agent issue #22032).
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'clawk kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_clawk_argv()
#
# Workers spawned by the dispatcher must use a `clawk` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["clawk", ...]`
...
def test_resolve_clawk_argv_prefers_path_shim(monkeypatch):
    ...
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/clawk")
...
def test_resolve_clawk_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CLAWK_BIN", ".\\clawk.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)
...
def test_resolve_clawk_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "clawk.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("CLAWK_BIN", raising=False)
...
def test_resolve_clawk_argv_honors_clawk_bin_path_override(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CLAWK_BIN", str(shim))
...
def test_resolve_clawk_argv_clawk_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    cwd_clawk.write_text("wrong\n", encoding="utf-8")
    cwd_clawk.chmod(cwd_clawk.stat().st_mode | stat.S_IXUSR)
    path_clawk = tmp_path / "bin" / "clawk"
    path_clawk.parent.mkdir()
    path_clawk.write_text("right\n", encoding="utf-8")
    path_clawk.chmod(path_clawk.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_clawk.parent))
...
def test_resolve_clawk_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    Pins the correct module name (NOT `clawk` — there is no top-level
    `clawk` package). Regression for #23198: the original PR shipped
    `python -m clawk` which fails with `No module named clawk` on every
    invocation.
    """
    import shutil
    import sys
...
def test_resolve_clawk_argv_module_actually_runs():
    ...
    sufficient — if `clawk_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m clawk_cli.main --version`
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from clawk_cli.kanban_db import connect, write_txn
    import clawk_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from clawk_cli.kanban_db import connect, _check_file_length_invariant
