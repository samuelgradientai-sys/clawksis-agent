"""Tests for clawk_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
class TestFallbackChain:
    ...
    PRIMARY = "https://clawksis-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/samuelgradientai-sys/clawksis-agent"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import clawk_cli.model_catalog as model_catalog
        from clawk_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in clawk_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from clawk_cli import model_catalog
        from clawk_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``clawk model`` CLI — not the live
        ...
        # ``CLAWK_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` CLAWK_HOME directly instead.
        import importlib
        from clawk_cli import model_catalog
        from clawk_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
