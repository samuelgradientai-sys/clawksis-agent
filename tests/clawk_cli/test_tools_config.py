...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="clawk_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "clawk" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["clawk-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["clawk-cli", "bogus"]}}
...
# NOTA (sync v2026.7.20): los tests propios ``test_configurable_toolsets_include_messaging``
# y ``test_get_platform_tools_default_telegram_includes_messaging`` NO se reintroducen:
# upstream elimino el toolset ``messaging`` y ya fallaban contra el codigo mergeado.
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `clawk tools` → X (Twitter) Search to flip the
...
# via the platform's native composite. The composite ``clawk-discord``
...
def test_discord_composite_only_enables_discord_toolsets():
    """Layer 1: ``platform_toolsets.discord: [clawk-discord]`` is an explicit
    opt-in to the full Discord bundle (which includes the ``discord`` and
    ``discord_admin`` tools). They must not be silently stripped."""
    config = {"platform_toolsets": {"discord": ["clawk-discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled, "discord toolset missing from clawk-discord composite"
...
def test_discord_composite_plus_configurable_enables_discord_toolsets():
    ...
    config = {"platform_toolsets": {"discord": ["clawk-discord", "spotify"]}}
    enabled = _get_platform_tools(config, "discord")
...
def test_discord_composite_plus_partial_explicit_enables_sibling():
    """Layer 3: ``[clawk-discord, discord]`` lists discord explicitly but
    discord_admin arrives only via the composite. Both must survive."""
    config = {"platform_toolsets": {"discord": ["clawk-discord", "discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["clawk-telegram", "discord"]}}
...
def test_discord_explicit_workaround_still_works():
    ...
    config = {
        "platform_toolsets": {"discord": ["clawk-discord", "discord", "discord_admin"]}
    }
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    ...
    has_explicit_config branch used to drop ``clawk-cli`` on the floor,
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _CLAWK_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `clawk tools`
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/samuelgradientai-sys/clawksis-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    ...
    ``clawk tools``, but clawk-cli stays in the config and re-enables
...
def test_save_platform_tools_does_not_preserve_clawk_telegram():
    ...
    assert "clawk-telegram" not in saved
    assert "web" in saved
...
# NOTA (sync v2026.7.20): upstream trae aca 4 tests
# ``test_visible_providers_{include,show,force_fresh_shows}_nous_subscription*``
# que asumen que la fila "Nous Subscription" es visible. El fork es BYOK y
# ``_visible_providers`` filtra toda fila ``managed_nous_feature`` /
# ``requires_nous_auth``, asi que se conservan las versiones propias invertidas.
...
def test_visible_providers_force_fresh_still_hides_nous_subscription(monkeypatch):
    calls = []
...
def test_configure_all_platforms_configures_selected_tool_missing_provider(monkeypatch):
    """Regression: `clawk tools` → Configure all platforms → Web Search
    ...
    monkeypatch.setattr("clawk_cli.tools_config._prompt_choice", choose_by_label)
    monkeypatch.setattr(
        "clawk_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web"},
    )
    monkeypatch.setattr(
        "clawk_cli.tools_config._toolset_needs_configuration_prompt",
...
# NOTA (sync v2026.7.20): ``test_first_install_nous_auto_configures_managed_defaults``
# y ``test_first_install_nous_auto_configures_video_gen`` NO se adoptan: el fork es
# BYOK y ``tools_command`` no llama ``apply_nous_managed_defaults`` (esa funcion solo
# sobrevive en clawk_cli/web_server.py), asi que no hay auto-config gestionada.
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/samuelgradientai-sys/clawksis-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `clawk tools` must reach the
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    # Fork: se arma el composite con los toolsets web + terminal *resueltos en
    # vivo* para que el chequeo de recuperacion por subconjunto siga siendo
    # correcto a medida que evolucionan — p. ej. el toolset "web" del fork sumo
    # ``scrape`` (Scrapling), asi que una lista fija como
    # ["web_search", "web_extract"] ya no recupera "web".
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `clawk tools` → "reconfigure existing" shows
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    as ``clawk model`` and writes the selection to the auxiliary config keys
    the resolver reads.
    """
    monkeypatch.setenv("CLAWK_HOME", str(tmp_path))
...
def test_provider_readiness_env_vars_gate_keys(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.tools_config.get_env_value", lambda key: None)
    assert provider_readiness_status(provider, {}) == "needs_keys"

    monkeypatch.setattr("clawk_cli.tools_config.get_env_value", lambda key: "sk-x")
...
# `clawk tools post-setup <key>` child. On Windows each console child (npm,
...
def test_post_setup_no_window_flags_zero_on_posix(monkeypatch):
    from clawk_cli import _subprocess_compat
    from clawk_cli.tools_config import _post_setup_no_window_flags
...
def test_provider_readiness_agent_browser_tracks_local_install(monkeypatch):
    ...
    monkeypatch.setattr(
        "clawk_cli.nous_subscription._local_browser_runnable", lambda: False
    )
    assert provider_readiness_status(provider, {}) == "needs_setup"

    monkeypatch.setattr(
        "clawk_cli.nous_subscription._local_browser_runnable", lambda: True
    )
    assert provider_readiness_status(provider, {}) == "ready"
