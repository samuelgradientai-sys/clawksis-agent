...
def test_gateway_install_in_container_with_operational_systemd_uses_systemd(
    ...
    monkeypatch.setattr(
        gateway,
        "systemd_install",
        lambda force=False, system=False, run_as_user=None, enable_on_startup=True: (
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(
    ...
    assert "sudo clawk gateway install --system --run-as-user alice" in out

    assert "sudo clawk gateway start --system" in out
