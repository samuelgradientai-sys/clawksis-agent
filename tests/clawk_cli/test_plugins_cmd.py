"""Tests for clawk_cli.plugins_cmd — the ``clawk plugins`` CLI subcommand."""
...
class TestResolveGitExecutable:
    ...
    def test_prefers_shutil_which(self):
        import clawk_cli.plugins_cmd as pc
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="clawk_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from clawk_cli.plugins_cmd import cmd_install

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("clawk_cli.plugins_cmd._resolve_git_url")
    ...
    @patch("clawk_cli.plugins_cmd._display_after_install")
    @patch("clawk_cli.plugins_cmd.shutil.move")
    @patch("clawk_cli.plugins_cmd.shutil.rmtree")
    @patch("clawk_cli.plugins_cmd._plugins_dir")
    @patch("clawk_cli.plugins_cmd._read_manifest")
    @patch("clawk_cli.plugins_cmd.subprocess.run")
...
class TestCmdUpdate:
    ...
    @patch("clawk_cli.plugins_cmd._sanitize_plugin_name")
    @patch("clawk_cli.plugins_cmd._plugins_dir")
    @patch("clawk_cli.plugins_cmd.subprocess.run")
    def test_update_git_pull_success(self, mock_run, mock_plugins_dir, mock_sanitize):
        from clawk_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    def test_remove_deletes_plugin(self, mock_rmtree, mock_plugins_dir, mock_sanitize):
        from clawk_cli.plugins_cmd import cmd_remove
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir(self, mock_plugins_dir):
        from clawk_cli.plugins_cmd import cmd_list
...
class TestCopyExampleFiles:
    ...
    def test_handles_copy_error_gracefully(self, tmp_path):
        ...
        with patch(
            "clawk_cli.plugins_cmd.shutil.copy2",
...
class TestPromptPluginEnvVars:
    ...
    def test_skips_when_no_requires_env(self):
        from clawk_cli.plugins_cmd import _prompt_plugin_env_vars
        from unittest.mock import MagicMock
    ...
    def test_skips_already_set_vars(self, monkeypatch):
        ...
        console = MagicMock()
        with patch("clawk_cli.config.get_env_value", return_value="already-set"):
...
class TestSubdirInstallE2E:
    ...
    def test_installs_only_the_subdir_plugin(self, tmp_path, monkeypatch):
        ...
        from clawk_cli import plugins_cmd as pc
