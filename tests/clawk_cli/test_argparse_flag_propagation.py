...
subparser, placing the flag BEFORE the subcommand (e.g. 'clawk --yolo chat')
...
def _build_parser():
    """Build the clawk argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("clawk_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "clawk_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets CLAWK_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `clawk gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: clawk_cli.main is a very heavy
    # import (previously 11 separate `python -m clawk_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `clawk <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `clawk -t web chat` silently dropped the
    ...
    Uses the real `clawk_cli._parser.build_top_level_parser()` rather than
    ...
    def test_flag_before_chat_is_preserved(self, real_parser, flag, attr, value):
        ...
        assert getattr(args, attr, None) == value, (
            f"`clawk {flag} {value} chat` lost the flag — got "
    ...
    def test_store_true_flag_before_chat_is_preserved(
        ...
        would clobber the parent's `True` when used as `clawk --tui chat`."""
        args, _ = real_parser.parse_known_args([flag, "chat"])
        assert getattr(args, attr, None) is True, (
            f"`clawk {flag} chat` lost the flag — got "
