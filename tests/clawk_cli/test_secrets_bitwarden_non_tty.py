"""Regression tests for clawk secrets bitwarden setup non-TTY guard.
...
class TestCmdSetupNonTtyGuard:
    ...
    def test_missing_server_url_with_env_var_passes(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "clawk_cli.secrets_cli.bw.fetch_bitwarden_secrets",
            lambda **kw: ({"KEY": "val"}, []),
        )
    ...
    def test_tty_does_not_trigger_guard(self, monkeypatch):
        ...
        monkeypatch.setattr("clawk_cli.secrets_cli.get_env_path", lambda: "/tmp/.env")
        monkeypatch.setattr(
            "clawk_cli.secrets_cli._resolve_server_url",
