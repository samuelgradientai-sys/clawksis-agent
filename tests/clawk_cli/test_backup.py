...
def _make_clawk_tree(root: Path) -> None:
    ...
    for db_name in ("memory_store.db", "clawk_state.db"):
    ...
    (root / "clawksis-agent").mkdir(exist_ok=True)
    (root / "clawksis-agent" / "run_agent.py").write_text("# big file\n")
    (root / "clawksis-agent" / ".git").mkdir()
...
class TestShouldExclude:
    def test_excludes_clawk_agent(self):
        ...
        assert _should_exclude(Path("clawksis-agent/run_agent.py"))
    ...
    def test_includes_nested_clawk_agent_in_skills(self):
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under CLAWK_HOME must be skipped —
...
class TestBackup:
    ...
    def test_failed_sqlite_backup_never_raw_copies_live_wal_db(
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(
        ...
        out_zip = clawk_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, clawk_home)
    ...
    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under CLAWK_HOME must be
        ...
        venv_pkg = (
            clawk_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        )
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = clawk_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_clawk_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../clawksis-agent/ but NOT root clawksis-agent/."""
        clawk_home = tmp_path / ".clawksis"
        clawk_home.mkdir()
        _make_clawk_tree(clawk_home)

        # Add a nested clawksis-agent directory inside skills (like the real layout)
        nested = clawk_home / "skills" / "autonomous-ai-agents" / "clawksis-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Clawksis Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root clawksis-agent must be excluded
            root_agent = [n for n in names if n.startswith("clawksis-agent/")]
            assert root_agent == [], f"root clawksis-agent leaked: {root_agent}"
            # Nested skill clawksis-agent must be included
            assert "skills/autonomous-ai-agents/clawksis-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/clawksis-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        ...
        zips = list(tmp_path.glob("clawk-backup-*.zip"))
        assert len(zips) == 1
...
class TestImport:
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside clawk home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (clawk_home / "gateway_state.json").write_text(live_state)
        ...
        assert (clawk_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (clawk_home / "config.yaml").exists()
        assert not (clawk_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        clawk_home = tmp_path / ".clawksis"
        (clawk_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("CLAWK_HOME", str(clawk_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (clawk_home / "profiles" / "coder" / "gateway_state.json").write_text(
            live_state
        )
        ...
        assert (
            clawk_home / "profiles" / "coder" / "config.yaml"
        ).read_text() == "model: anthropic\n"
        assert (
            clawk_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (clawk_home / "gateway.pid").write_text("4242")
        (clawk_home / "processes.json").write_text('{"live": true}')
        ...
        assert (clawk_home / "gateway.pid").read_text() == "4242"
        assert (clawk_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (clawk_home / "cron.pid").exists()
        assert not (clawk_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (clawk_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("CLAWK_HOME", str(clawk_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_HOME", str(dst_home))
        ...
        assert not (dst_home / "clawksis-agent").exists()
...
class TestBackupEdgeCases:
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = clawk_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        clawk_home = tmp_path / ".clawksis"
        clawk_home.mkdir()
        (clawk_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("CLAWK_HOME", str(clawk_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        clawk_home = tmp_path / ".clawksis"
        clawk_home.mkdir()
        (clawk_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("CLAWK_HOME", str(clawk_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
class TestQuickSnapshot:
    ...
    def test_state_db_safely_copied(self, clawk_home):
        ...
        snap_id = create_quick_snapshot(clawk_home=clawk_home)
        db_copy = clawk_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_discord_recovery_ledger(self, clawk_home):
        from clawk_cli.backup import create_quick_snapshot

        gateway_dir = clawk_home / "gateway"
        gateway_dir.mkdir()
        ...
        copied = clawk_home / "state-snapshots" / snap_id / "gateway" / ledger.name
        assert copied.exists()
        conn = sqlite3.connect(copied)
        assert conn.execute("SELECT message_id FROM handled").fetchall() == [("123",)]
        conn.close()

    def test_copies_channel_aliases(self, clawk_home):
        from clawk_cli.backup import create_quick_snapshot

        snap_id = create_quick_snapshot(clawk_home=clawk_home)
        copied = clawk_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
    ...
    def test_max_file_size_skips_oversized_file(self, clawk_home, capsys):
        ...
        cap = 1024
        snap_id = create_quick_snapshot(clawk_home=clawk_home, max_file_size=cap)
        assert snap_id is not None
        snap_dir = clawk_home / "state-snapshots" / snap_id
    ...
    def test_max_file_size_none_copies_everything(self, clawk_home):
        """Default (no cap) preserves manual /snapshot behavior."""
        from clawk_cli.backup import create_quick_snapshot

        snap_id = create_quick_snapshot(clawk_home=clawk_home, max_file_size=None)
        assert (clawk_home / "state-snapshots" / snap_id / "state.db").exists()

    def test_max_file_size_under_cap_copies(self, clawk_home):
        from clawk_cli.backup import create_quick_snapshot

        snap_id = create_quick_snapshot(clawk_home=clawk_home, max_file_size=1 << 30)
        assert (clawk_home / "state-snapshots" / snap_id / "state.db").exists()
    ...
    def test_restore_recovers_pairing_data(self, clawk_home):
        ...
        feishu = clawk_home / "feishu_comment_pairing.json"
    ...
    def test_restore_rejects_snapshot_id_traversal(self, clawk_home):
        ...
        for hostile in hostile_ids:
            assert restore_quick_snapshot(hostile, clawk_home=clawk_home) is False, (
                f"hostile snapshot_id was not rejected: {hostile!r}"
            )

    def test_restore_rejects_manifest_rel_traversal(self, clawk_home):
        """A snapshot whose manifest.json contains a rel path that escapes
        the snapshot directory (e.g. ``../../outside.txt``) must skip that
        entry rather than restoring outside CLAWK_HOME."""
        from clawk_cli.backup import create_quick_snapshot, restore_quick_snapshot
        ...
        escape_dst = clawk_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped CLAWK_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from clawk_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, clawk_home):
        from clawk_cli.backup import create_quick_snapshot

        snap_id = create_quick_snapshot(clawk_home=clawk_home)
        copy = clawk_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, clawk_home):
        from clawk_cli.backup import create_quick_snapshot

        snap_id = create_quick_snapshot(clawk_home=clawk_home)
        copy = clawk_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, clawk_home):
        from clawk_cli.backup import create_quick_snapshot, restore_quick_snapshot

        snap_id = create_quick_snapshot(clawk_home=clawk_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(clawk_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, clawk_home):
        ...
        board_dir = clawk_home / "kanban" / "boards" / "work"
    ...
    def test_additional_per_profile_dbs_round_trip(self, clawk_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(clawk_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, clawk_home):
        ...
        board = clawk_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(clawk_home=clawk_home)
        snap = clawk_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, clawk_home, monkeypatch):
...
class TestPreUpdateBackup:
    ...
    def test_failed_sqlite_snapshot_removes_incomplete_archive(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_creates_backup_under_backups_dir(self, clawk_home):
        ...
        assert out.exists()
        assert out.parent == clawk_home / "backups"
    ...
    def test_backup_contents_match_full_backup(self, clawk_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # clawksis-agent repo excluded
        assert not any(n.startswith("clawksis-agent/") for n in names)
    ...
    def test_does_not_recurse_into_prior_backups(self, clawk_home):
        ...
        # First backup
        out1 = create_pre_update_backup(clawk_home=clawk_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(clawk_home=clawk_home)
    ...
    def test_keep_zero_does_not_delete_freshly_created_backup(self, clawk_home):
        ...
        out = create_pre_update_backup(clawk_home=clawk_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_zero_still_prunes_older_backups(self, clawk_home):
        ...
        _t.sleep(1.05)
        second = create_pre_update_backup(clawk_home=clawk_home, keep=5)
        _t.sleep(1.05)
...
class TestRunPreUpdateBackup:
    ...
    def clawk_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_clawk_tree(root)
        # Point CLAWK_HOME at the temp dir so config + backup paths resolve here
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, clawk_home):
        ...
        out = create_pre_migration_backup(clawk_home=clawk_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `clawk
    ...
    def test_restorable_with_clawk_import(self, clawk_home, tmp_path):
        ...
        from clawk_cli.backup import create_pre_migration_backup, _validate_backup_zip
    ...
    def test_does_not_recurse_into_prior_backups(self, clawk_home):
        ...
        out1 = create_pre_migration_backup(clawk_home=clawk_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(clawk_home=clawk_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_missing_clawk_home_returns_none(self, tmp_path):
        ...
        out = create_pre_migration_backup(clawk_home=missing)
        assert out is None
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_bom_live_file_still_counted(self, tmp_path):
        ...
        from clawk_cli.backup import _count_cron_jobs, restore_cron_jobs_if_emptied
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = clawk_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, clawk_home=clawk_home) is None
...
# location, NOT under CLAWK_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------


class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, clawk_home: Path) -> None:
        clawk_home.mkdir(parents=True, exist_ok=True)
        (clawk_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (clawk_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (clawk_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(
        ...
        clawk_home = tmp_path / ".clawksis"
        self._make_min_tree(clawk_home)
        # External provider state living OUTSIDE CLAWK_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(
        ...
        """_external/ members restore to ~/<relpath>, not under CLAWK_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        clawk_home = dst_home / ".clawksis"
        clawk_home.mkdir()
        ...
        # External state did NOT leak into CLAWK_HOME.
        assert not (clawk_home / "_external").exists()
