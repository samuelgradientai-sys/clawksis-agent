...
Covers:
- Defense 1: gateway stop/restart refuse when _CLAWK_GATEWAY=1
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_clawk_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade clawk then run clawk gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.clawk.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_clawk_gateway_once.sh). The script must live under
        # CLAWK_HOME/scripts so the scheduler — and the guard — resolve it.
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    Issue #37453: systemctl --user restart clawk-gateway runs as a child of the
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-clawk systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _CLAWK_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart clawk-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.clawk.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    clawk_cli.cron.cron_create. Enforcing at create_job covers both."""
