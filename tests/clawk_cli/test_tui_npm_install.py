...
@pytest.fixture
def main_mod():
    import clawk_cli.main as m

    return m


def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@clawk" / "ink" / "package.json"
...
def test_make_tui_argv_decodes_dev_prebuild_with_utf8_replace(
    ...
    ink_dir = tmp_path / "packages" / "clawk-ink"
    ink_dir.mkdir(parents=True)
...
def test_make_tui_argv_uses_bundled_tui_when_workspace_missing(
    ...
    clawk_cli/tui_dist/entry.js but never ships ui-tui/ (that directory only
    ...
    def which(name: str) -> str | None:
        ...
        raise AssertionError(
            f"unexpected shutil.which({name!r}) call — bundled path must not need npm/git"
        )
    ...
    def fail_run(*_args, **_kwargs):
        raise AssertionError(
            "bundled TUI path must not spawn any subprocess (no npm install/build, no git restore)"
        )
...
def test_make_tui_argv_exits_with_recovery_hint_when_workspace_unrecoverable(
    ...
    assert "clawk update --force" in err
...
def test_make_tui_argv_restores_missing_workspace_from_git(
    ...
    monkeypatch.delenv("CLAWK_TUI_DIR", raising=False)
    monkeypatch.delenv("CLAWK_QUIET", raising=False)
    monkeypatch.setattr(main_mod, "_ensure_tui_node", lambda: None)
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (clawk-ink etc.)
