"""Tests for clawk_cli.mcp_catalog and clawk_cli.mcp_picker.
...
def _default_mock_probe(monkeypatch):
    ...
    ``clawk_cli.mcp_catalog._probe_tools`` themselves.
    """
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("CLAWK_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_clawk_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp CLAWK_HOME."""
...
class TestInstall:
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from clawk_cli.config import load_config
        from clawk_cli.mcp_catalog import CatalogError, install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from clawk_cli.mcp_catalog import install_entry
        from clawk_cli.config import get_env_value, load_config
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from clawk_cli import mcp_catalog
        from clawk_cli.mcp_catalog import install_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from clawk_cli.mcp_catalog import install_entry, uninstall_entry
        from clawk_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from clawk_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from clawk_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_picker_surfaces_future_manifest_warning(self, catalog_dir, capsys, monkeypatch):
        ...
        assert "requires a newer Clawksis" in out
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from clawk_cli import mcp_catalog
        from clawk_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from clawk_cli import mcp_catalog
        from clawk_cli.mcp_catalog import _do_git_install, get_entry
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "clawk_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no CLAWK_OPTIONAL_MCPS
