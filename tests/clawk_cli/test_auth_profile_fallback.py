...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_clawk_home_override(profile_b)
        try:
...
#
# Fork-specific: the per-provider state/pool fallbacks let a profile *read* a
# provider that was only authenticated at the global root, but
# ``resolve_provider()`` picks the ``auto`` provider from ``active_provider``
# — which only ever read the profile store. A named profile running
# ``model.provider: auto`` could see the global login
# (``get_provider_auth_state(...)`` succeeds) yet still fail to select it.
# These pin the active_provider shadowing so the selection mirrors the
# state/pool fallbacks: profile wins when present, fall back to global when
# the profile never chose its own provider.
# ---------------------------------------------------------------------------
...
def test_resolve_provider_uses_global_active_provider(profile_env, monkeypatch):
    ...
    login check stays offline (no token refresh / network).
    """
    import clawk_cli.auth as auth
