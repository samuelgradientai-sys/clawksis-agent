...
Execution via the ``CLAWK_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``clawk_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.clawksis/plugins/`` source via
   ``os.environ.get("CLAWK_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``clawk_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
...
class TestEndToEndPocBlocked:
    ...
    ``CLAWK_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "clawk_dashboard_plugin_evil"
