"""Tests for the startup security posture audit (clawk_cli.security_audit_startup)."""
...
import clawk_cli.security_audit_startup as audit
...
def test_container_no_mount_flags(monkeypatch, tmp_path):
    ...
    msg = audit._container_no_volume_mount(tmp_path / ".clawk")
...
def test_container_with_mount_silent(monkeypatch, tmp_path):
    ...
    assert audit._container_no_volume_mount(tmp_path / ".clawk") is None
...
def test_log_startup_security_warnings_emits_and_is_idempotent(
    ...
    with caplog.at_level(logging.WARNING, logger="clawk.security_audit"):
        first = audit.log_startup_security_warnings(clawk_home=tmp_path, config={})
    assert len(first) == 1
