...
def clawk_home(tmp_path, monkeypatch):
    ...
    # /etc/clawk on the dev/CI box can't influence the test.
    monkeypatch.setenv("CLAWK_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(clawk_home, monkeypatch):
...
def test_env_expansion_in_user_config(clawk_home, monkeypatch):
    from clawk_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(clawk_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_clawk_dotenv(clawk_home=str(home))
