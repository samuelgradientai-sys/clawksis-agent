...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # clawk model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in CLAWK_OVERLAYS

        overlay = CLAWK_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        assert headers.get("User-Agent", "").startswith("ClawksisAgent/")

    def test_gmi_profile_declares_clawk_user_agent(self):
        """The GMI plugin sets a ClawksisAgent/<ver> User-Agent on its profile."""
        ...
        assert ua.startswith("ClawksisAgent/"), (
            f"expected GMI profile User-Agent to start with 'ClawksisAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("clawk_cli.config.get_container_exec_info", lambda: None)

        monkeypatch.setattr(
            "clawk_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )

        monkeypatch.setattr(sys, "argv", ["clawk", "chat", "--provider", "gmi"])
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        config = yaml.safe_load((get_clawk_home() / "config.yaml").read_text()) or {}
