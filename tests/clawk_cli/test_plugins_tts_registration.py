...
``$CLAWK_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
...
``tests/clawk_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        clawk_home = Path(os.environ["CLAWK_HOME"])
        ...
        _enable(clawk_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(clawk_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(clawk_home, "shadow-tts-plugin")
