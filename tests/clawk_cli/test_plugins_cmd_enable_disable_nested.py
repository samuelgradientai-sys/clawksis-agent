...
the *mutation* side: `clawk plugins enable/disable` must resolve a bare name
...
class TestEnableDisableNested:
    @patch("clawk_cli.plugins.get_bundled_plugins_dir")
    @patch("clawk_cli.plugins_cmd._plugins_dir")
    @patch("clawk_cli.plugins_cmd._save_disabled_set")
    @patch("clawk_cli.plugins_cmd._save_enabled_set")
    ...
    def test_enable_clears_manifest_name_alias_from_disabled(
        ...
        ``clawk plugins disable web-firecrawl`` gets ``web-firecrawl`` in
        ...
        with patch(
            "clawk_cli.plugins_cmd._get_disabled_set",
...
class TestEnableToolOverrideConsent:
    ...
    @patch("clawk_cli.plugins.get_bundled_plugins_dir")
    @patch("clawk_cli.plugins_cmd._plugins_dir")
    @patch("clawk_cli.plugins_cmd._set_plugin_entry_flag")
    @patch("clawk_cli.plugins_cmd._save_disabled_set")
    @patch("clawk_cli.plugins_cmd._save_enabled_set")
    @patch("clawk_cli.plugins_cmd._get_disabled_set", return_value=set())
    @patch("clawk_cli.plugins_cmd._get_enabled_set", return_value=set())
    ...
    def test_bundled_plugin_never_prompts_or_writes_entry(
        ...
        with patch(
            "rich.console.Console.input", side_effect=AssertionError("prompted")
        ):
...
class TestCompositeMenuWritesCanonicalKey:
    """#40190 follow-up: the interactive `clawk plugins` menu must persist
    ...
    def test_fallback_unchecked_plugin_disables_by_key_not_name(
        ...
        from clawk_cli.plugins_cmd import _run_composite_fallback
