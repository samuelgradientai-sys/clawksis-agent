"""Tests for SIGHUP protection and stdout mirroring in ``clawk update``.



Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and

``_finalize_update_output`` in ``clawk_cli/main.py``.  These exist so

that ``clawk update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "clawk update started" in contents
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_clawk_home() raises, stdio must be left untouched but SIGHUP still handled."""
