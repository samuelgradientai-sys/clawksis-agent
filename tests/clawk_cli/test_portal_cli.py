"""Tests for `clawk portal` dispatch.



`clawk portal` (no subcommand) is the human-readable alias for the Nous Portal

one-shot onboarding (`clawk auth add nous --type oauth` / `clawk setup

--portal`). The prior status default moved to `clawk portal info`, with
...
from clawk_cli import portal_cli
...
def test_bare_portal_and_login_run_one_shot(monkeypatch, sub):
    """`clawk portal`, `clawk portal login` -> one-shot onboarding."""
    ...
    monkeypatch.setattr("clawk_cli.setup._run_portal_one_shot", fake_one_shot)
...
def test_info_and_status_alias_run_status(monkeypatch, sub):
    """`clawk portal info` and the `status` back-compat alias -> status."""

    calls = {"login": 0, "status": 0}

    monkeypatch.setattr(
        "clawk_cli.setup._run_portal_one_shot",
...
def test_login_cancelled_returns_one(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.setup._run_portal_one_shot", boom)
...
def test_one_shot_delegates_to_model_flow_nous(monkeypatch):
    """`clawk portal` must run the quick-setup Nous flow (login + MODEL PICK +
    ...
    import clawk_cli.setup as setup_mod
    ...
    # _model_flow_nous lives in clawk_cli.main and is imported lazily inside

    # _run_portal_one_shot, so patch it at the source module.

    monkeypatch.setattr("clawk_cli.main._model_flow_nous", fake_model_flow)
    ...
    assert calls["model_flow"] == 1, (
        "`clawk portal` must route through _model_flow_nous so the model "
...
def test_one_shot_swallows_cancel_and_systemexit(monkeypatch, exc):
    ...
    monkeypatch.setattr("clawk_cli.main._model_flow_nous", boom)
