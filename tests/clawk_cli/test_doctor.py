...
from clawk_cli import doctor as doctor_mod
...
class TestDoctorToolAvailabilitySummary:
    ...
    def test_missing_api_key_summary_falls_back_when_config_unavailable(
        self, monkeypatch
    ):
        unavailable = [
            {"name": "rl", "missing_vars": ["TINKER_API_KEY"]},
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("CLAWK_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("CLAWK_INTERACTIVE")
        raise SystemExit(0)
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp CLAWK_HOME.

        ``run_doctor`` reads the module-level ``CLAWK_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("CLAWK_HOME")`` alone
        leaves doctor probing the REAL ~/.clawksis. On a dev machine with a
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("clawk_cli.auth.get_xai_oauth_auth_status", raising=False)
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(
        ...
        monkeypatch.setattr(doctor_mod, "CLAWK_HOME", clawk_home)
        monkeypatch.setattr(doctor_mod, "get_clawk_home", lambda: clawk_home)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (clawk_home / ".env").read_text(encoding="utf-8")
        assert "CLAWK_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`clawk doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Clawksis install.
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_collect_deprecated_env_vars(self):
        ...
        assert "CLAWK_TOOL_PROGRESS" in names
        assert "TERMINAL_CWD" in names
        assert "QQ_HOME_CHANNEL" in names
        assert "OPENAI_API_KEY" not in names
        by_name = dict(findings)
        assert "display.tool_progress" in by_name["CLAWK_TOOL_PROGRESS"]
    ...
    def _run_doctor_with_config(
        ...
        clawk_home = tmp_path / ".clawksis"
        clawk_home.mkdir(parents=True)
        (clawk_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (clawk_home / ".env").write_text(env_body, encoding="utf-8")
...
"""
        out, clawk_home = self._run_doctor_with_config(
            monkeypatch, tmp_path, config_yaml=cfg
        )
        ...
        on_disk = (clawk_home / "config.yaml").read_text(encoding="utf-8")
...
"""
        ...
        assert "Deprecated: CLAWK_TOOL_PROGRESS" in out
...
"""
        ...
        assert "Deprecated: CLAWK_TOOL_PROGRESS" not in out
    ...
    def test_report_does_not_count_as_blocking_issue(
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"CLAWK_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: CLAWK_TOOL_PROGRESS_MODE" in out
