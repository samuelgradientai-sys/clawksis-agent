"""Unit tests for clawk_cli.security_audit — parsers + OSV plumbing.
...
from clawk_cli import security_audit as sa
...
class TestRunAudit:
    def test_no_components_returns_empty(self, tmp_path: Path):
        ...
        findings = sa.run_audit(
            skip_venv=True, skip_plugins=True, skip_mcp=True, clawk_home=tmp_path
        )
    ...
    def test_findings_sorted_by_severity_desc(self, tmp_path: Path):
        ...
        with (
            ...
            findings = sa.run_audit(
                skip_venv=True, skip_plugins=False, skip_mcp=True, clawk_home=tmp_path
            )
...
class TestExitCodes:
    ...
    def test_clean_audit_exits_zero(self, tmp_path: Path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(sa, "get_clawk_home", lambda: str(tmp_path))
