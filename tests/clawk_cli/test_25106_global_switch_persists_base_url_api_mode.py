...
`clawk` launch re-read the stale base_url and routed the new model at the
old host.
...
def _run_switch(monkeypatch, result, cmd="/model MiniMax-M3 --global"):
    ...
    cli_mod.ClawksisCLI._handle_model_switch(_StubCLI(), cmd)
    return saved
...
def test_session_only_switch_does_not_touch_config(monkeypatch):
    ...
    cli_mod.ClawksisCLI._handle_model_switch(_StubCLI(), "/model MiniMax-M3 --session")
...
def _run_apply(monkeypatch, result, persist_global=True):
    ...
    monkeypatch.setattr(cli_mod, "save_config_value", _fake_save)
    cli_mod.ClawksisCLI._apply_model_switch_result(_StubCLI(), result, persist_global)
    return saved
