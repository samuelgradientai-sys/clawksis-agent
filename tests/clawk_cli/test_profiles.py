"""Comprehensive tests for clawk_cli.profiles module.
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.clawk/profiles)
    * CLAWK_HOME  -> tmp_path/.clawk  (so get_clawk_home() agrees)
    * Creates the bare-minimum ~/.clawksis directory.
    """
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        ...
        (default_home / "clawksis-agent" / ".git").mkdir(parents=True)
        (default_home / "clawksis-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "clawksis-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "clawksis-agent").exists()
...
class TestBackfillProfileEnvs:
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".clawk" / ".env").write_text("KEY=root\n")
...
class TestGetActiveProfileName:
    ...
    def test_default_clawk_home_returns_default(self, profile_env):
        # CLAWK_HOME points to tmp_path/.clawk which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        ...
        # With Docker-aware roots, a custom CLAWK_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("clawk")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        ...
        assert "exec /opt/clawksis/bin/clawk -p mybot" in content
...
class TestWrapperScriptSecurity:
    ...
    def test_legit_alias_stays_inside_wrapper_dir(self, profile_env, monkeypatch):
        ...
        assert 'clawk -p coder "$@"' in wrapper.read_text()
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "clawk.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["clawk_heimdall"]["aiPeer"] == "ssi_health"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["clawk.ssi_health"]["aiPeer"] == "ssi_health"
...
class TestExportImport:
    ...
    def test_export_default_uses_allowlist_for_unrelated_dirs(self, profile_env, tmp_path):
        ...
        Clawksis artifacts end up in the archive. Replaces the old
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_clawk_home()

    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with CLAWK_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("CLAWK_HOME", str(get_profile_dir("coder")))
        serve = profiles_to_serve(multiplex=False)
        assert len(serve) == 1
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_clawk_home()
