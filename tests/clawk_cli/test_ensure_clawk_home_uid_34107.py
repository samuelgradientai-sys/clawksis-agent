"""Regression tests for #34107 — Docker UID/GID handling in ensure_clawk_home.



When Clawksis runs in Docker with ``CLAWK_UID=1000`` / ``CLAWK_GID=911``,

the entrypoint chowns the top-level ``CLAWK_HOME`` once at startup. But
...
The fix is a ``_chown_to_clawk_uid`` helper that reads the env vars and
...
# _resolve_clawk_uid_gid

# ---------------------------------------------------------------------------


class TestResolveClawksisUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):

        monkeypatch.setenv("CLAWK_UID", "1000")

        monkeypatch.setenv("CLAWK_GID", "911")

        from clawk_cli.config import _resolve_clawk_uid_gid
    ...
    def test_returns_none_when_unset(self, monkeypatch):
        ...
        monkeypatch.delenv("CLAWK_UID", raising=False)

        monkeypatch.delenv("CLAWK_GID", raising=False)
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_UID", "not-a-number")
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_UID", "")

        monkeypatch.setenv("CLAWK_GID", "")
...
class TestChownToClawksisUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(
            ...
            cfg._chown_to_clawk_uid(d)  # must not raise
