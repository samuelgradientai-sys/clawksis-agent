...
CLAWK_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
from clawk_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Clawksis-Session-Token": _SESSION_TOKEN}
...
@pytest.fixture
def clawk_home(monkeypatch, tmp_path):
    """Fresh CLAWK_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("CLAWK_HOME", str(home))
    from clawk_cli.config import invalidate_env_cache
...
def test_delete_env_key_prunes_env_seeded_pool_entry(clawk_home):
    _write_env(clawk_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(clawk_home, _zai_pool_fixture())
...
def test_delete_env_key_removes_provider_pool_key_when_emptied(clawk_home):
...
def test_delete_survives_pool_reload(clawk_home):
    """#59761: the pool loader must not resurrect the entry after 'restart'."""
    _write_env(clawk_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(clawk_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
...
def test_delete_clears_provider_models_cache(clawk_home):
    _write_env(clawk_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(clawk_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = clawk_home / "provider_models_cache.json"
...
def test_delete_unknown_key_404s(clawk_home):
...
def test_delete_does_not_touch_other_providers(clawk_home):
    _write_env(clawk_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    other_key = "dk-" + "e" * 24
...
def test_update_rotates_config_yaml_model_mirror(clawk_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(clawk_home, OPENAI_API_KEY=old)
    _write_config(
        clawk_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = clawk_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
def test_update_rotates_custom_provider_mirror(clawk_home):
    old = "sk-cp-" + "h" * 24
    new = "sk-cp-" + "i" * 24
    _write_env(clawk_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_update_leaves_unrelated_config_keys_alone(clawk_home):
    """A DIFFERENT key configured inline must not be rewritten by value-match."""
    old = "sk-un-" + "j" * 24
    unrelated = "sk-un-" + "k" * 24
    _write_env(clawk_home, OPENAI_API_KEY=old)
    _write_config(clawk_home, f"model:\n  provider: custom\n  api_key: {unrelated}\n")
...
def test_delete_scrubs_config_yaml_mirror(clawk_home):
    old = "sk-dl-" + "m" * 24
    _write_env(clawk_home, OPENAI_API_KEY=old)
    _write_config(clawk_home, f"model:\n  provider: custom\n  api_key: {old}\n")
...
def test_delete_then_resave_round_trip(clawk_home):
    _write_env(clawk_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(clawk_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    ...
    assert not is_source_suppressed("zai", "env:ZAI_API_KEY"), (
        "an explicit re-save must lift the suppression (like `clawk auth add`)"
    )
