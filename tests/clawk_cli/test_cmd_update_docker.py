"""Tests for ``clawk update`` / ``--check`` inside the Docker container.
...
fix, ``clawk update`` would fall through to ``"✗ Not a git repository.

Please reinstall: curl ... install.sh"`` — that script installs a *new*

host-side Clawksis, not an update to the running container, so the message
...
from clawk_cli.main import _cmd_update_check, cmd_update
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``clawk update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``clawk update --check`` inside Docker → same message + exit 1, no fetch."""
...
def test_format_docker_update_message_contents():
    ...
    assert "CLAWK_HOME" in msg or "/opt/data" in msg, (
