...
- Bare `clawk setup` drops straight into the full reconfigure wizard

  (every prompt shows the current value as its default).

- `clawk setup --quick` runs the narrower "fill in missing items" flow.

- `clawk setup --reconfigure` is a backwards-compat alias for the
