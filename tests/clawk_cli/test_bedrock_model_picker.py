...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CLAWK_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CLAWK_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CLAWK_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from clawk_cli.providers import CLAWK_OVERLAYS
        assert "bedrock" in CLAWK_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from clawk_cli.providers import CLAWK_OVERLAYS
        assert CLAWK_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from clawk_cli.providers import CLAWK_OVERLAYS
        assert CLAWK_OVERLAYS["bedrock"].auth_type == "aws_sdk"
