...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (clawk_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_clawk_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:clawk_pkce")
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """clawk auth add qwen-oauth must set active_provider in auth.json.
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clawk_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `clawk auth add nous` must also populate providers.nous so the
    ...
    # raises "Clawksis is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clawk_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`clawk auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``clawk auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``clawk auth list`` showed two labels sharing
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from clawk_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    ...
    config_text = (clawk_home / "config.yaml").read_text()
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((clawk_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from clawk_cli.auth import _reset_config_provider
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("clawk_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("clawk_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("clawk_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "clawk_cli.auth_commands.time.time",
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from clawk_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `clawk auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("CLAWK_HOME", str(tmp_path / "clawk"))
    clawk_home = tmp_path / "clawk"
    clawk_home.mkdir(parents=True, exist_ok=True)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((clawk_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    ...
    (clawk_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (clawk_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `clawk auth add <provider>` clears any
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `clawk auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Clawksis reads from has a
...
def test_seed_from_singletons_respects_clawk_pkce_suppression(tmp_path, monkeypatch):
    """anthropic clawk_pkce must not re-seed from ~/.clawksis/.anthropic_oauth.json when suppressed."""
    clawk_home = tmp_path / "clawk"
    clawk_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("CLAWK_HOME", str(clawk_home))
    ...
    assert entries == []
    assert "clawk_pkce" not in active
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `clawk auth add <provider>` clears ALL
