...
Issue [#26990] reported that ``clawk auth add xai-oauth`` succeeds at the
...
The fix in ``clawk_cli/auth.py`` extracts the token POST into
...
def post_recorder(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.auth.httpx.post", recorder)
...
def test_transport_error_wraps_as_auth_error(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.auth.httpx.post", _boom)
...
def test_wire_format_is_form_urlencoded_with_all_pkce_fields(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.auth.httpx.post", _post)
