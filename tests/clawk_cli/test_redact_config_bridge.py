...
`CLAWK_REDACT_SECRETS` at module-import time. `clawk_cli/main.py` at
...
in .env), the toggle was silently ignored in both `clawk chat` and

`clawk gateway run`.



Fix: bridge `security.redact_secrets` from config.yaml → `CLAWK_REDACT_SECRETS`

env var in `clawk_cli/main.py` BEFORE the `setup_logging()` call.
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    probe = textwrap.dedent(
        ...
        os.environ.pop("CLAWK_REDACT_SECRETS", None)

        sys.path.insert(0, %r)

        import clawk_cli.main  # triggers the bridge + setup_logging
