"""Regression tests for #60328: --yolo must set CLAWK_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing CLAWK_YOLO_MODE at the
    ...
    from clawk_cli.main import main as cli_main
...
def test_chat_subcommand_yolo_flag_sets_env_before_startup(monkeypatch):
    """clawk chat --yolo must also set CLAWK_YOLO_MODE before
    _prepare_agent_startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["clawk", "chat", "--yolo"]
    )
    assert result == "1", (
        "CLAWK_YOLO_MODE was not '1' when _prepare_agent_startup was "
