...
The Python launcher (`clawk --tui …`) has a ~100–300ms cold-start window

where stdin is still in cooked + echo mode. If a previous Clawksis session
...
from clawk_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["clawk", "--tui", "-c", "abc"])

        monkeypatch.delenv("CLAWK_TUI", raising=False)

        monkeypatch.delenv("CLAWK_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_clawk_tui_env_set(self, monkeypatch):
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        ...
        # `clawk --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
