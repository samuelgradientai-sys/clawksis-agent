...
def test_show_status_includes_tavily_key(monkeypatch, capsys, tmp_path):
    """Fork-only: the Tavily row still shows the *redacted* key, not nothing."""
    monkeypatch.setenv("CLAWK_HOME", str(tmp_path))
...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    ...
    assert "Start with:   clawk gateway" in output
...
def test_show_status_reports_nous_auth_error(monkeypatch, capsys, tmp_path):
    ...
    output = capsys.readouterr().out
    assert "Nous Portal   ✗ not logged in (run: clawk portal)" in output
...
def test_show_status_reports_nous_inference_key_without_portal_login(monkeypatch, capsys, tmp_path):
    ...
    # Fork: el hint "Nous inference credentials are configured" no existe en
    # clawk_cli/status.py (Nous quedo dormante, sin copy de onboarding).
...
class TestShowStatusXaiOAuth:
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        assert "Auth file:  /home/u/.clawk/auth.json" in out
    ...
    def test_not_logged_in_shows_login_command(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: clawk auth add xai-oauth)" in out
