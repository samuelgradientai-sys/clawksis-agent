...
Covers three additions to ``clawk update``:
...
def test_venv_health_missing_venv_unhealthy_on_managed_install(tmp_path):
    ...
    (tmp_path / ".clawk-bootstrap-complete").write_text("done")
    with patch.object(cli_main, "PROJECT_ROOT", tmp_path):
        healthy, detail = cli_main._venv_core_imports_healthy()
...
def test_format_venv_holders_message_flags_desktop_backend(tmp_path):
    matches = [
        (101, "python.exe", "python.exe -m clawk_cli.main serve --host 127.0.0.1"),
        (102, "pythonw.exe", "pythonw.exe -m clawk_cli.main gateway run"),
    ]
    msg = cli_main._format_venv_python_holders_message(matches)
    assert "101" in msg
    assert "desktop app" in msg.lower()
    assert "gateway" in msg
    assert "clawk update" in msg
...
def test_detect_venv_python_catches_outside_venv_trampoline(_winp, tmp_path):
    ...
    clearly runs Clawksis from this install → must still be flagged as a holder
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_venv_clawk_cli_cmdline_outside_install_not_matched(_winp, tmp_path):
    """A clawk_cli.main process belonging to a DIFFERENT install (neither
