"""
Tests for clawk_cli.mcp_config — ``clawk mcp`` subcommands.
...
class TestMcpAdd:
    ...
    def test_add_connection_failure_save_disabled(
        ...
        monkeypatch.setattr(
            "clawk_cli.mcp_config._probe_single_server", mock_probe_fail
        )
...
class TestMcpTest:
    ...
    def test_probe_uses_configured_connect_timeout(self, monkeypatch):
        ...
        import asyncio
        from clawk_cli import mcp_config
...
class TestProbeEnvResolution:
    ...
    where `clawk mcp add --auth header` sent a literal
    ``Authorization: Bearer ${MCP_X_API_KEY}`` and got 401."""

    def test_resolve_interpolates_header(self, monkeypatch):
        from clawk_cli.mcp_config import _resolve_mcp_server_config
    ...
    def test_probe_resolves_before_connect(self, monkeypatch):
        ...
        import clawk_cli.mcp_config as mc
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from clawk_cli.mcp_config import _strip_bearer_prefix
...
class TestBearerAuthPersistence:
    def test_secret_and_header_are_persisted_separately(self):
        ...
        from clawk_cli.mcp_config import _save_bearer_auth_token
...
class TestConfigHelpers:
    ...
    def test_remove_mcp_server(self, tmp_path):
        from clawk_cli.mcp_config import (
            _save_mcp_server,
            _remove_mcp_server,
            _get_mcp_servers,
        )
    ...
    def test_env_key_for_server(self):
        from clawk_cli.mcp_config import _env_key_for_server
