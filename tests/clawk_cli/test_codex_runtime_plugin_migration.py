...
class TestMigrate:
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        migrate(
            {"mcp_servers": {"clawk-mcp": {"command": "npx"}}},
            codex_home=tmp_path,
            discover_plugins=False,
            expose_clawk_tools=False,
        )
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        ...
        report = migrate(
            {"mcp_servers": "notadict"}, codex_home=tmp_path, expose_clawk_tools=False
        )
...
class TestClawkHomeLeakGuard:
    ...
    def test_tempdir_detector_recognizes_pytest_paths(self):
        ...
        assert _looks_like_test_tempdir("/tmp/pytest-of-user/pytest-12/test_X/clawk")
