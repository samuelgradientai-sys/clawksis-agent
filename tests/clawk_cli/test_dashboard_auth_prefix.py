...
honour it too:
  ...
  1. The gate's ``Location:`` redirect to /login (in
     ...
     ``_unauth_response``) needs to be ``/clawksis/login`` so the browser
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        ...
        assert r.headers["location"].startswith("/clawksis/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        assert body["login_url"].startswith("/clawksis/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(self, gated_app_proxied):
        ...
        ``https://mission-control.tilos.com/clawksis/auth/callback`` — the
        ...
        assert parsed.path == "/clawksis/auth/callback", (
...
class TestPublicUrlOverride:
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        assert redirect_uri == "https://example.com/clawksis/auth/callback"
...
class TestCookiePathRespectsPrefix:
    ...
    def test_cookies_read_back_round_trip_through_prefix(self, gated_app_proxied):
        ...
        sent by a real browser hitting ``/clawksis/auth/callback``. To
