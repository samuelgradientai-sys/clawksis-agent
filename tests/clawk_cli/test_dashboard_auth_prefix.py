...
prefix (e.g. ``mission-control.tilos.com/clawk/*`` -> local Caddy ->
...
URLs and the bootstrap ``__CLAWK_BASE_PATH__``). The OAuth gate must
honour it too:

  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/clawk/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/clawk/auth/callback`` and the user gets 404.
...
from clawk_cli.dashboard_auth import prefix as prefix_mod
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        ...
        assert r.headers["location"].startswith("/clawk/auth/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/clawk/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(self, gated_app_proxied):
        ...
        ``https://mission-control.tilos.com/clawk/auth/callback`` — the
        ...
        assert parsed.path == "/clawk/auth/callback", (
...
# ---------------------------------------------------------------------------
# CLAWK_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``CLAWK_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    ...
    def patch_config(self, monkeypatch):
        """Replace ``clawk_cli.config.load_config`` with a stub
    ...
    def test_public_url_env_overrides_request_reconstruction(
        ...
        """``CLAWK_DASHBOARD_PUBLIC_URL`` wins over the URL the
    ...
    def test_public_url_config_yaml_used_when_env_unset(
        ...
        monkeypatch.delenv("CLAWK_DASHBOARD_PUBLIC_URL", raising=False)
    ...
    def test_empty_public_url_env_treated_as_unset(
        ...
        entry."""
        monkeypatch.setenv("CLAWK_DASHBOARD_PUBLIC_URL", "")
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://from-config.example/auth/callback"
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        of "I set CLAWK_DASHBOARD_PUBLIC_URL but the callback is still
        ...
        patch_config(None)
        monkeypatch.setenv("CLAWK_DASHBOARD_PUBLIC_URL", "clawk.domain.com")
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "clawk_session_pkce" in c)
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from clawk_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("clawk_session_pkce=") for c in cookies), (
