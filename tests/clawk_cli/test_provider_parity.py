...
the SAME provider universe as ``clawk model`` (the CLI/TUI picker).
...
def test_every_clawk_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `clawk model` universe."""
    ...
    assert not missing, (
        "providers shown in `clawk model` but not configurable in the desktop "
