...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        The dashboard spawns `clawk gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `clawk gateway restart`)."""
        unit_path = tmp_path / "clawk-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``CLAWK_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../clawk_test`` CLAWK_HOME — silently breaking
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E CLAWK_HOME like
        ``/tmp/clawk-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``clawk gateway restart`` with a /tmp CLAWK_HOME exported; the
        ...
        must refuse ANY temp-dir CLAWK_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "clawk-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="CLAWK_HOME=/tmp/clawk-e2e-41264"\n'
            "WorkingDirectory=/tmp/clawk-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="CLAWK_HOME=/var/tmp/clawk-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="CLAWK_HOME={tmp_path}/clawk-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None

    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>CLAWK_HOME</key>\n"
            "  <string>/tmp/clawk-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_accepts_unit_without_clawk_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m clawk_cli.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_clawk_home_for_target_user", lambda home: "/home/alice/.clawk")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        ...
        # CLAWK_HOME resolves under the (pytest tmp) test CLAWK_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test CLAWK_HOME) stays out of
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup clawk gateway run" in out
    ...
    def test_launchd_unsupported_marker_write_and_clear(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_clawk_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup clawk gateway" in out
...
class TestSystemUnitClawkHome:
    ...
    def test_system_unit_preserves_custom_clawk_home(self, monkeypatch):
        # Custom CLAWK_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
    ...
    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's CLAWK_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)
...
class TestSystemUnitRefreshSyncsClawkHome:
    ...
    def test_refresh_adopts_unit_clawk_home_before_rewriting(self, tmp_path, monkeypatch):
        ...
        (alice_clawk / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's CLAWK_HOME + drain timeout).
        monkeypatch.setenv("CLAWK_HOME", str(alice_clawk))
        ...
        # Simulate sudo without inherited CLAWK_HOME (falls back to root).
        monkeypatch.setenv("CLAWK_HOME", str(root_clawk))
        ...
        assert os.environ["CLAWK_HOME"] == str(alice_clawk)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned CLAWK_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_clawk_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_clawk_home_from_systemd_unit",
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "clawk gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_clawk_home_returns_empty(self, tmp_path, monkeypatch):
        ...
        result = gateway_cli._profile_arg(str(clawk_home))
        assert result == ""
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_clawk_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        # WorkingDirectory is anchored at the target user's CLAWK_HOME (stable,
...
class TestLegacyClawksisUnitDetection:
    ...
    older install left a ``clawk.service`` unit behind when the service was
    renamed to ``clawk-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``clawk.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``clawk-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_detects_legacy_clawk_service_in_system_scope(self, tmp_path, monkeypatch):
        ...
        legacy = system_dir / "clawk.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_clawk_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``clawk-gateway-coder.service`` is our standard
    ...
    def test_ignores_unrelated_clawk_service(self, tmp_path, monkeypatch):
        ...
        If a user has some other package named ``clawk`` installed as a
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m clawk_cli.main gateway run
          - python path/to/clawk_cli/main.py gateway run
          - clawk gateway run   (direct binary)
        ...
        for i, execstart in enumerate(variants):
            name = "clawk.service" if i == 0 else "clawk.service"  # same name
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "clawk.service"
...
class TestRemoveLegacyClawksisUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop clawk.service" in c for c in cmds_joined)
        assert any("--user disable clawk.service" in c for c in cmds_joined)
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "clawk.service"
        system_legacy = system_dir / "clawk.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        profile_unit = user_dir / "clawk-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "clawk-gateway.service"
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `clawk gateway --help` shows it.
        import subprocess
        import sys
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl start clawk-gateway" in out
        assert "sudo clawk gateway uninstall --system" in out
        assert "clawk gateway install" in out
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.clawk.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.clawk.gateway"
