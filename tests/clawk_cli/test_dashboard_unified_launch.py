...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "clawk_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # CLAWK_HOME.  For a standard install (CLAWK_HOME unset) that root is
    ...
    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (CLAWK_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop CLAWK_HOME.

        Dropping it makes the child fall back to $HOME/.clawk
        (= /opt/data/.clawk), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("CLAWK_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "clawk_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
        ...
        # get_default_clawk_root() strips the trailing profiles/<name>, so the
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        monkeypatch.delenv("CLAWK_WEB_DIST", raising=False)
        ...
        calls = []
        monkeypatch.setattr(
            "clawk_cli.mcp_startup.start_background_mcp_discovery",
