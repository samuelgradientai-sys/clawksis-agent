"""Behavior tests for clawk_cli.inventory.
...
from clawk_cli.inventory import (
    ConfigContext,
...
def test_pricing_uses_cached_nous_tier_by_default():
    ...
    with (
        _list_auth_returning(rows),
        patch(
            "clawk_cli.models.get_pricing_for_provider",
            return_value={
                "openai/gpt-5.5": {
                    "prompt": "0.000001",
                    "completion": "0.000002",
        ...
        patch("clawk_cli.models.check_nous_free_tier", return_value=False) as mock_free,
    ):
...
def test_explicit_only_keeps_moa_when_raw_config_has_enabled_preset():
    ...
    with (
        _list_auth_returning(rows),
        patch("clawk_cli.config.load_config", return_value=raw_config),
        patch("clawk_cli.config.read_raw_config", return_value=raw_config),
        patch("clawk_cli.auth.is_provider_explicitly_configured", return_value=False),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_build_models_payload_keeps_static_provider_models_from_providers_dict():
    ...
    with (
        ...
        patch("clawk_cli.providers.CLAWK_OVERLAYS", {}),
        patch(
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from clawk_cli import model_switch

    with patch("clawk_cli.models.clear_provider_models_cache") as clear:
