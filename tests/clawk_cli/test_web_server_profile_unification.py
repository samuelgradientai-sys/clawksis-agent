...
profile switcher can target any profile's CLAWK_HOME. These tests pin:
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_clawk_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_clawk_home", lambda: default_home)
...
class TestProfileScopedModel:
    ...
    def test_model_options_hides_unconfigured_providers_by_default(
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same CLAWK_HOME the rest
        ...
        monkeypatch.setattr(
            "clawk_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_clawk_home(self, isolated_profiles, monkeypatch):
        import clawk_cli.web_server as web_server

        monkeypatch.setattr(
            "clawk_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["CLAWK_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "CLAWK_TUI_GATEWAY_URL" not in env
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("CLAWK_HOME") != str(isolated_profiles["worker_beta"])
