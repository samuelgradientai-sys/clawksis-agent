...
class TestSlackNativeSlashes:
    ...
    def test_clawk_catchall_is_first(self):
        """``/clawk`` must be reserved as the first slot so the legacy
        ``/clawk <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "clawk"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /clawk <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/clawk <alias>``.
        """
        slashes = slack_native_slashes()
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `clawk slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from clawk_cli import commands as commands_mod
        ...
        monkeypatch.setattr(
            "clawk_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from clawk_cli.commands import discord_skill_commands_by_category  # noqa: E402
