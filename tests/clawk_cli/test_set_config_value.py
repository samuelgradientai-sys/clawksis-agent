...
from clawk_cli.config import set_config_value, config_command


@pytest.fixture(autouse=True)
def _isolated_clawk_home(tmp_path):
    """Point CLAWK_HOME at a temp dir so tests never touch real config."""
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_clawk_home):
        ...
        assert key not in _read_config(_isolated_clawk_home)
...
class TestCatchAllPatterns:
    ...
    def test_case_insensitive(self, _isolated_clawk_home):
    ...
    def test_terminal_ssh_prefix_routes_to_env(self, _isolated_clawk_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_clawk_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_clawk_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_clawk_home)

    def test_nested_key(self, _isolated_clawk_home):
        set_config_value("terminal.backend", "docker")
        config = _read_config(_isolated_clawk_home)
        assert "docker" in config
        assert "terminal" not in _read_env(_isolated_clawk_home)

    def test_terminal_image_goes_to_config(self, _isolated_clawk_home):
...
class TestFalsyValues:
    ...
    def test_empty_string_routes_to_env(self, _isolated_clawk_home):
    ...
    def test_empty_string_routes_to_config(self, _isolated_clawk_home):
    ...
    def test_config_command_accepts_empty_string(self, _isolated_clawk_home):
...
class TestConfigGetUnset:
    ...
    def test_config_get_prints_structured_json(self, _isolated_clawk_home, capsys):
        set_config_value("terminal.backend", "docker")
        capsys.readouterr()
    ...
    def test_config_unset_removes_env_key(self, _isolated_clawk_home, capsys):
        set_config_value("OPENROUTER_API_KEY", "sk-test")
        assert "OPENROUTER_API_KEY=sk-test" in _read_env(_isolated_clawk_home)
        capsys.readouterr()
...
class TestStringTypedConfigValues:
    ...
    def test_unknown_keys_keep_existing_coercion(self, _isolated_clawk_home):
...
class TestSecretRedactionInDisplay:
    ...
    def test_set_echo_masks_secret_value(self, _isolated_clawk_home, capsys):
    ...
    def test_set_echo_keeps_nonsecret_value(self, _isolated_clawk_home, capsys):
...
class TestSchemaValidation:
    ...
    a post-write notice warns that Clawksis may never read it and suggests the
    ...
    def test_unknown_top_level_key_written_with_notice(
        ...
        assert "totally_made_up_key" in _read_config(_isolated_clawk_home)

    def test_unknown_subkey_written_with_notice(self, _isolated_clawk_home, capsys):
    ...
    def test_platforms_container_is_accepted(self, _isolated_clawk_home, capsys):
        ...
        content = _read_config(_isolated_clawk_home)
        assert "enabled: true" in content
        assert "not a recognized config key" not in capsys.readouterr().out

    def test_gateway_platforms_nested_is_accepted(self, _isolated_clawk_home, capsys):
    ...
    def test_unknown_approvals_subkey_warns_but_writes(
        ...
        out = capsys.readouterr().out
        assert "not a recognized config key" in out
        assert "notarealkey" in _read_config(_isolated_clawk_home)

    def test_known_approvals_subkey_is_accepted(self, _isolated_clawk_home, capsys):
    ...
    def test_close_typo_suggests_correct_key(self, _isolated_clawk_home, capsys):
    ...
    def test_typoed_subkey_suggests_sibling(self, _isolated_clawk_home, capsys):
        """``agent.max_turn`` should suggest ``agent.max_turns``."""
        set_config_value("agent.max_turn", "100")
        out = capsys.readouterr().out
        assert "agent.max_turns" in out

    def test_force_suppresses_notice(self, _isolated_clawk_home, capsys):
    ...
    def test_known_top_level_key_accepted(self, _isolated_clawk_home):
        """Sanity check: real config keys still work."""
        set_config_value("terminal.backend", "docker")
        content = _read_config(_isolated_clawk_home)
        assert "backend: docker" in content

    def test_known_platform_config_accepted(self, _isolated_clawk_home):
    ...
    def test_open_dict_mcp_servers_accepts_any_subkey(self, _isolated_clawk_home):
