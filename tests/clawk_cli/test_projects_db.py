"""Tests for the per-profile Projects store (clawk_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Clawksis", folders=["/tmp/clawk"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "clawksis-agent"
    assert proj.name == "Clawksis"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/clawk"
    assert [f.path for f in proj.folders] == ["/tmp/clawk"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "clawksis-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Clawksis")
    pdb.create_project(conn, name="Clawksis")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["clawksis-agent", "clawksis-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' CLAWK_HOME.
...
def test_db_path_under_clawk_home():
    # Resolves under CLAWK_HOME (set by the autouse isolation fixture).
