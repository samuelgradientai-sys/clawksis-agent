...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    with patch("clawk_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    CLAWK_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `clawk update` correctly refusing to run there. The guard
