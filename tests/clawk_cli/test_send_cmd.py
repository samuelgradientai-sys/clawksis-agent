"""Tests for the ``clawk send`` CLI subcommand.



Covers the argument parsing / stdin / file / list behavior of

``clawk_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
def test_load_clawk_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``clawk send`` from a fresh shell cannot resolve the home channel

    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``clawk config set``) lives
