...
def isolated_kanban_home(monkeypatch):
    ...
    # Force-reimport so the fresh CLAWK_HOME is picked up.
    ...
    # gives each test its own monkeypatched CLAWK_HOME so no cross-test
...
def test_dry_run_with_default_assignee_reports_without_mutating(isolated_kanban_home):
    ...
    `clawk kanban dispatch --dry-run` see the routing decision before
