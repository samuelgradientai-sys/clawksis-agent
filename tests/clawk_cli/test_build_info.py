"""Tests for clawk_cli.build_info — baked-in build SHA resolution.



The build SHA is written by the Dockerfile's ``CLAWK_GIT_SHA`` build-arg

into ``<project_root>/.clawk_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    from clawk_cli import build_info



    missing = tmp_path / ".clawk_build_sha"  # never created
...
def test_get_build_sha_reads_baked_file(tmp_path):
    ...
    sha_file = tmp_path / ".clawk_build_sha"
