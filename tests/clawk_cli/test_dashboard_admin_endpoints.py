...
def _client():
    ...
    # Keep the state DB under the isolated CLAWK_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_clawk_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        env_text = (clawk_home / ".env").read_text()
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.clawksis/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `clawk auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `clawk auth add`, which clears every suppression for the
    ...
    def test_delete_does_not_clobber_other_providers(self):
        ...
        from agent.credential_pool import load_pool
        from clawk_cli.auth import _load_auth_store, read_credential_pool
        from clawk_cli.config import save_env_value
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_clawk_home):
        ...
        (get_clawk_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestOpsEndpoints:
    ...
    def test_backup_output_uses_output_flag(self, monkeypatch):
        ...
        r = self.client.post(
            "/api/ops/backup",
            json={"output": "  /tmp/clawk-test.zip  "},
        )

        assert r.status_code == 200
        assert captured == {
            "subcommand": ["backup", "-o", "/tmp/clawk-test.zip"],
            "name": "backup",
        }
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "clawk_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("clawk-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # clawk-index source advertises availability + featured search.
            idx = _Src("clawk-index")
        ...
        assert {"official", "github", "clawk-index"} <= ids
...
class TestUpdateCheckEndpoint:
    """``GET /api/clawk/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/clawk/update`` runs ``clawk update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/clawk/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/clawk/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        ...
        monkeypatch.setattr("clawk_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `clawk tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_clawk_action", _fake_spawn)
        r = self.client.post(
...
def test_spawn_clawk_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    ...
    _CLAWK_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    ws._spawn_clawk_action(["gateway", "restart"], "gateway-restart")

    assert "_CLAWK_GATEWAY" not in captured["env"]
    assert captured["env"]["CLAWK_NONINTERACTIVE"] == "1"
