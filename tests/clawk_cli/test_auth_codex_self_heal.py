...
Clawksis keeps its OWN copy of the Codex OAuth token (per profile + top-level),
separate from the Codex CLI's ``~/.codex/auth.json``. OAuth refresh_tokens are
single-use, so when the Codex CLI (or another Clawksis process) rotates the shared
...
def test_self_heals_on_stale_refresh_token(monkeypatch):
    ...
    # the recovered token was persisted to the Clawksis auth store
...
def test_self_heals_missing_singleton_access_token_from_codex_cli(
    ...
    assert resolved["source"] == "clawk-auth-store"
    stored = json.loads((clawk_home / "auth.json").read_text())
