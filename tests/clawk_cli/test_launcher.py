"""Tests for the top-level `./clawk` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./clawk` should use `clawk_cli.main`, not the legacy Fire wrapper."""

    launcher_path = Path(__file__).resolve().parents[2] / "clawk"

    called = []

    fake_main_module = types.ModuleType("clawk_cli.main")

    def fake_main():

        called.append("clawk_cli.main")

    fake_main_module.main = fake_main

    monkeypatch.setitem(sys.modules, "clawk_cli.main", fake_main_module)
