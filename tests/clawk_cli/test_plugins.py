"""Tests for the Clawksis plugin system (clawk_cli.plugins)."""
...
from clawk_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(
    ...
    *,
    ...
    ``<clawk_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<clawk_home>/plugins/``; we derive
    ``<clawk_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <CLAWK_HOME>/config.yaml.
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.clawksis/plugins/ are discovered."""
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        """Plugins in ./.clawk/plugins/ are discovered."""
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under clawk_plugins.<name>."""
        plugins_dir = tmp_path / "clawk_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("CLAWK_HOME", str(tmp_path / "clawk_test"))

        # Clean up any prior namespace module
        sys.modules.pop("clawk_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "clawk_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="clawk_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("clawk_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(
        ...
        """
        from tools.registry import registry
        from clawk_cli.plugins import PluginToolOverrideError
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `clawk plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("clawk_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "clawk_test" / "plugins"
        # Set CLAWK_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDebugLogging:
    """CLAWK_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("CLAWK_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With CLAWK_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("CLAWK_PLUGINS_DEBUG", "1")
        from clawk_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from CLAWK_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """CLAWK_HOME at the root resolves to 'default'."""
        home = tmp_path / ".clawk"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """CLAWK_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".clawk" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWK_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".clawk" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWK_HOME", str(prof))
        ctx = self._ctx()
