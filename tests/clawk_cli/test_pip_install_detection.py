...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $CLAWK_HOME.

    Models a host git install whose $CLAWK_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'clawk update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("clawk_cli.config.get_managed_system", return_value=None), \
         patch("clawk_cli.config.get_clawk_home", return_value=home):
        from clawk_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $CLAWK_HOME. With no code-scoped stamp, a host
    ...
    with patch("clawk_cli.config.get_managed_system", return_value=None), \
         patch("clawk_cli.config.get_clawk_home", return_value=home), \
         patch("clawk_cli.config._running_in_container", return_value=False):
        from clawk_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("clawk_cli.config.get_managed_system", return_value=None), \
         patch("clawk_cli.config.get_clawk_home", return_value=home), \
         patch("clawk_cli.config._running_in_container", return_value=True):
        from clawk_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $CLAWK_HOME keeps
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $CLAWK_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``clawk update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("clawk_cli.config.get_managed_system", return_value=None), \
