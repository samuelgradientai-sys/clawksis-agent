"""Tests for clawk_cli configuration management."""
...
class TestGetClawkHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            home = get_clawk_home()
            assert home == Path.home() / ".clawksis"

    def test_env_override(self):
        with patch.dict(os.environ, {"CLAWK_HOME": "/custom/path"}):
            home = get_clawk_home()
            assert home == Path("/custom/path")


class TestEnsureClawkHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from clawk_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from clawk_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        profile_home = tmp_path / ".clawksis" / "profiles" / "coder"
        profile_home.mkdir(parents=True)
        with patch.dict(os.environ, {"CLAWK_HOME": str(profile_home)}):
            ensure_clawk_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``clawk logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from clawk_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path)}):
            ...
            with caplog.at_level(logging.WARNING, logger="clawk_cli.config"):
                config = load_config()
            ...
            # stands out at clawk startup before logging is configured)
            captured = capsys.readouterr()
            assert "clawk config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path)}):
            ...
            assert "clawk config:" in after_edit, "edited file should re-warn"
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from clawk_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path)}, clear=False):
    ...
    def test_save_env_value_quotes_values_with_internal_spaces(self, tmp_path):
        ...
        path = "/Users/paulo/Library/Application Support/clawk/keys/id_ed25519"
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path)}, clear=False):
        ...
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path)}, clear=False):
            ...
            r = subprocess.run(
                [
                    ...
                    "sh",
                    "-c",
                    f'set -a; . \'{env_path}\'; set +a; printf "%s" "$TABBY_KEY"',
                ],
                capture_output=True,
                text=True,
            )
    ...
    def test_save_env_value_readback_resave_is_idempotent(self, tmp_path):
        """clawk setup path: dotenv unquotes, then re-save must not grow quotes."""
        from dotenv import dotenv_values
    ...
    def test_save_env_value_does_not_requote_untouched_spaced_lines(self, tmp_path):
        ...
        """
        env_path = tmp_path / ".env"
        legacy = "TERMINAL_SSH_KEY=/Users/me/Application Support/key\nPLAIN=ok\n"
        ...
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path)}, clear=False):
            ...
            assert "TERMINAL_SSH_KEY=/Users/me/Application Support/key\n" in content
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(
            os.environ, {"CLAWK_HOME": str(tmp_path), "MY_KEY": "my_value"}
        ):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(
            os.environ, {"CLAWK_HOME": str(tmp_path), "GHOST_KEY": "ghost"}
        ):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(
            os.environ, {"CLAWK_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}
        ):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from clawk_cli.config import ENV_VARS_BY_VERSION
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """CLAWK_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `clawk setup`) lets a
        ...
        via config.yaml; CLAWK_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from clawk_cli.config import OPTIONAL_ENV_VARS

        assert "CLAWK_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `clawk setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``CLAWK_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Clawksis
    process on next startup via the dotenv → ``os.environ`` chain in
    ``clawk_cli/env_loader.py``.
    ...
    def _clawk_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "CLAWK_SPOTIFY_CLIENT_ID",
            "CLAWK_QWEN_BASE_URL",
            "CLAWK_MAX_ITERATIONS",
        ],
    )
    def test_clawk_integration_keys_still_writable(self, allowed_key):
        """``CLAWK_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``CLAWK_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``CLAWK_*``) keep working. The denylist
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from clawk_cli.config import _persist_migration, read_raw_config
...
class TestCodexAppServerAutoConfig:
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"CLAWK_HOME": str(tmp_path)}):
            self._write(
                tmp_path,
                "_config_version: 31\ncompression:\n  codex_app_server_auto: clawk\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "clawk"
