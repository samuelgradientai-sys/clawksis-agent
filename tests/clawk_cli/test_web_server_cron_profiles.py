...
def test_call_cron_for_profile_routes_storage_without_mutating_globals(
    ...
    assert job["clawk_home"] == str(isolated_profiles["worker_alpha"])
...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_clawk_home", None)
    captured = {}

    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            captured["job_id"] = job_id
            captured["runtime_home"] = scheduler._get_clawk_home()
    ...
    try:
        ...
        assert scheduler._get_clawk_home() == default_home
    finally:
        reset_clawk_home_override(outer_token)
...
async def test_update_cron_job_refreshes_snapshots_when_unpinning(
    ...
    from clawk_cli import runtime_provider, web_server
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.clawksis`` when the request carries no explicit ``profile`` (the
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """CLAWK_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from clawk_cli import web_server

    monkeypatch.setenv("CLAWK_HOME", str(isolated_profiles["default"]))
