...
def temp_pyproject(tmp_path, monkeypatch):
    ...
    pyproject.write_text(
        ...
        [project.scripts]
        clawk = "clawk_cli.main:main"
        clawksis-agent = "run_agent:main"
        clawk-acp = "acp_adapter.entry:main"
    """
        )
    )
...
class TestVerifyConsoleScriptsInstalled:
    ...
    def test_triggers_reinstall_when_clawk_exe_missing(
    ...
    def test_load_console_script_names_reads_pyproject(self, temp_pyproject):
        from clawk_cli.main import _load_console_script_names

        names = _load_console_script_names()
        assert names == ["clawk", "clawksis-agent", "clawk-acp"]
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        with patch("clawk_cli.main._is_windows", return_value=True):
            names = {path.name for path in main_mod._clawk_exe_shims(fake_scripts_dir)}

        assert {"clawk.exe", "clawksis-agent.exe", "clawk-acp.exe"} <= names
        assert "clawk-gateway.exe" in names
