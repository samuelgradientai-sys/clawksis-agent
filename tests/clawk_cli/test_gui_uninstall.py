"""Tests for clawk_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import clawk_cli.gui_uninstall as gu


def _make_agent(clawk_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = clawk_home / "clawksis-agent"
    (agent_root / "clawk_cli").mkdir(parents=True)
    (agent_root / "clawk_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(clawk_home: Path) -> None:
    """Create the source-built GUI artifacts a `clawk desktop` run produces."""
    desktop = clawk_home / "clawksis-agent" / "apps" / "desktop"
    ...
    (clawk_home / "clawksis-agent" / "node_modules").mkdir(parents=True)
    (clawk_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(clawk_home: Path) -> None:
    (clawk_home / "config.yaml").write_text("x: 1\n")
    (clawk_home / ".env").write_text("KEY=secret\n")
    (clawk_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    clawk_home = tmp_path / ".clawk"
    clawk_home.mkdir()
    assert gu.agent_is_installed(clawk_home) is False
    _make_agent(clawk_home)
    assert gu.agent_is_installed(clawk_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    clawk_home = tmp_path / ".clawk"
    (clawk_home / "clawksis-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(clawk_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    clawk_home = tmp_path / ".clawk"
    _make_gui_build(clawk_home)
    artifacts = gu.source_built_gui_artifacts(clawk_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(clawk_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(clawk_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    clawk_home = tmp_path / ".clawk"
    agent_root = _make_agent(clawk_home)
    _make_gui_build(clawk_home)
    _make_user_data(clawk_home)
    ...
    removed = gu.uninstall_gui(clawk_home)
    ...
    assert not (clawk_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "clawk_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (clawk_home / "config.yaml").exists()
    assert (clawk_home / ".env").exists()
    assert (clawk_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    clawk_home = tmp_path / ".clawk"
    _make_agent(clawk_home)
    userdata = tmp_path / "Clawksis-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(clawk_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(clawk_home)
    ...
    assert summary["clawk_home"] == str(clawk_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Clawksis"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Clawksis"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Clawksis"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Clawksis"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    CLAWK_HOME with all the destructive externals stubbed out.
    """
    ...
    from clawk_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole CLAWK_HOME non-interactively."""
    import clawk_cli.uninstall as uninstall

    clawk_home = tmp_path / ".clawk"
    (clawk_home / "clawksis-agent" / "clawk_cli").mkdir(parents=True)
    (clawk_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not clawk_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m clawk_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_clawk_home", lambda: clawk_home)
    ...
    assert not (desktop / "release").exists()
    assert not (clawk_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "clawk_cli").exists()
    assert (clawk_home / "config.yaml").exists()
