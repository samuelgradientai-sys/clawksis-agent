...
The terminal `clawk model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import clawk_cli.providers as providers_mod
import pytest
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "CLAWK_OVERLAYS", {})
    monkeypatch.setattr("clawk_cli.models.fetch_api_models", lambda *a, **k: [])
...
def test_list_authenticated_providers_can_skip_custom_provider_live_probe(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.models.fetch_api_models", fetch)
...
def test_list_authenticated_providers_can_probe_only_current_custom_provider(
    monkeypatch,
):
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "CLAWK_OVERLAYS", {})
...
def test_list_authenticated_providers_can_probe_active_bare_custom_endpoint(
    monkeypatch,
):
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "CLAWK_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(
        "clawk_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION
    )
    monkeypatch.setattr("clawk_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr(
        "clawk_cli.model_switch.get_model_capabilities", lambda *a, **k: None
    )
...
def test_custom_provider_no_key_singular_model_still_probes_live_models(monkeypatch):
    ...
    so /model matches the terminal ``clawk model`` flow.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "CLAWK_OVERLAYS", {})
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Clawksis's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_same_endpoint_different_extra_headers_not_collapsed(monkeypatch):
    ...
    rows = [p for p in providers if p.get("api_url") == "http://localhost:8081/v1"]
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from clawk_cli.providers import resolve_custom_provider
...
def test_discovered_models_auto_saved_to_cache(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.models.fetch_api_models", fake_fetch_api_models)
    monkeypatch.setattr(
        "clawk_cli.model_switch._save_discovered_models_to_config",
...
def test_save_discovered_models_skips_unchanged(monkeypatch):
    ...
    from clawk_cli.model_switch import _save_discovered_models_to_config
    ...
    monkeypatch.setattr("clawk_cli.config.save_config", fake_save)
    monkeypatch.setattr(
...
def test_save_discovered_models_noop_on_empty_args(monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.config.load_config", fake_load)
...
def test_save_discovered_models_preserves_dict_form(monkeypatch):
    ...
    assert save_calls == [], "Dict-form models must not be replaced with a flat list"
