...
def _install_example_plugin(_isolate_clawk_home):
    ...
    ``CLAWK_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, clawk-achievements, model providers
    ...
    # plugin in exactly as a real operator would with `clawk plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down CLAWK_HOME.
    app = web_server.app
...
class TestWebServerEndpoints:
    ...
    def test_get_status(self):
        ...
        assert "clawk_home" in data
        assert "active_sessions" in data
        assert data["can_update_clawk"] is True
    ...
    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        ...
        fake_db_path.touch()
        monkeypatch.setattr(clawk_state, "DEFAULT_DB_PATH", fake_db_path)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(
        ...
        monkeypatch.setattr(clawk_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")
        ...
        monkeypatch.setattr("clawk_state.SessionDB", _boom)
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        ...
        resp = self.client.get("/api/status")
        assert resp.status_code == 200
        assert resp.json()["can_update_clawk"] is False
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in clawksis-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "clawk"
    ...
    def test_instance_schema_serves_providers_without_declared_schema(
        self, monkeypatch
    ):
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        ...
        config_path = get_clawk_home() / "hindsight" / "config.json"
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide CLAWK_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from clawk_constants import get_clawk_home

        path = get_clawk_home() / "honcho.json"
    ...
    def test_get_honcho_config_returns_safe_defaults(self, monkeypatch, tmp_path):
        ...
        assert (
            data["docs_url"] == "https://docs.honcho.dev/v3/guides/integrations/clawk"
        )
        ...
        assert fields["workspace"]["placeholder"] == "clawk"
        assert fields["aiPeer"]["placeholder"] == "clawk"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_clawk_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["clawk"]["workspace"] == "myws"
        assert cfg["hosts"]["clawk"]["peerName"] == "eri"
        assert cfg["hosts"]["clawk"]["environment"] == "local"
        assert cfg["hosts"]["clawk"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["clawk"]["apiKey"] == "hch-test-key"
    ...
    def test_put_honcho_blank_text_clears_key(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_clawk_home() / "honcho.json").read_text(encoding="utf-8"))
        assert "workspace" not in cfg.get("hosts", {}).get("clawk", {})
    ...
    def test_put_honcho_partial_save_preserves_other_keys(self, monkeypatch, tmp_path):
        ...
        host = json.loads(
            (get_clawk_home() / "honcho.json").read_text(encoding="utf-8")
        )["hosts"]["clawk"]
    ...
    def test_put_honcho_bool_stored_natively_and_false_survives(
        self, monkeypatch, tmp_path
    ):
        monkeypatch.setenv("HOME", str(tmp_path))
        self._seed_local_honcho()
    ...
    def test_put_honcho_number_stored_as_native_number(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_clawk_home() / "honcho.json").read_text(encoding="utf-8"))
        assert cfg["hosts"]["clawk"]["dialecticMaxChars"] == 1200
        assert isinstance(cfg["hosts"]["clawk"]["dialecticMaxChars"], int)
    ...
    def test_put_honcho_first_save_merges_into_resolved_config(
        self, monkeypatch, tmp_path
    ):
        ...
        assert resp.status_code == 200
        assert not (get_clawk_home() / "honcho.json").exists()
        cfg = json.loads(global_path.read_text(encoding="utf-8"))
        assert cfg["baseUrl"] == "https://kept.example"
        assert cfg["hosts"]["clawk"] == {"workspace": "kept", "peerName": "eri"}
    ...
    def test_put_honcho_updates_legacy_dot_form_host_block(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("CLAWK_HONCHO_HOST", "clawk_work")

        path = self._seed_local_honcho({
            "hosts": {"clawk.work": {"workspace": "w", "peerName": "eri"}}
        })
        ...
        assert set(hosts) == {"clawk.work"}
        assert hosts["clawk.work"] == {
            "workspace": "w",
            "peerName": "eri",
            "sessionStrategy": "per-repo",
        }

    def test_put_honcho_api_key_never_overwrites_oauth_token(
        self, monkeypatch, tmp_path
    ):
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("HONCHO_API_KEY", "guard")
        ...
        path = self._seed_local_honcho({
            "hosts": {"clawk": {"apiKey": "hch-at-oauth-token"}}
        })
        ...
        assert cfg["hosts"]["clawk"]["apiKey"] == "hch-at-oauth-token"
    ...
    def test_put_honcho_tolerates_null_hosts(self, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert (
            json.loads(path.read_text(encoding="utf-8"))["hosts"]["clawk"]["workspace"]
            == "myws"
        )
    ...
    def test_memory_provider_config_honors_profile_param(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        # The suite pins CLAWK_HONCHO_HOST=clawk; this test exercises
        # profile-driven host resolution, so drop the override explicitly.
        monkeypatch.delenv("CLAWK_HONCHO_HOST", raising=False)
        from clawk_constants import get_clawk_home
        ...
        worker_cfg.write_text(
            json.dumps({"hosts": {"clawk_worker": {"workspace": "kept"}}}),
            encoding="utf-8",
        )
        ...
        own = json.loads((get_clawk_home() / "honcho.json").read_text(encoding="utf-8"))
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_clawk_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from clawk_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_chat_image_upload_writes_to_default_profile_images(self):
        ...
        assert target.parent == get_clawk_home() / "images"
    ...
    def test_import_sessions_endpoint_rejects_metadata_that_would_break_session_list(
        self,
    ):
        invalid = self.client.post(
            "/api/sessions/import",
    ...
    def test_import_sessions_endpoint_rejects_missing_or_null_message_role(
        self, message
    ):
        response = self.client.post(
            "/api/sessions/import",
    ...
    def test_update_clawk_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("clawk-update", None)
        web_server._ACTION_RESULTS.pop("clawk-update", None)

        resp = self.client.post("/api/clawk/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "clawk-update"
    ...
    def test_update_clawk_returns_managed_runtime_guidance_without_spawning(
        self, monkeypatch
    ):
        import clawk_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            ...
            spawned = True
            raise AssertionError(
                "managed runtime update guard should not spawn clawk update"
            )
    ...
    def test_update_clawk_spawns_on_non_docker_install(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_clawk_action", fake_spawn)
        web_server._ACTION_PROCS.pop("clawk-update", None)
        web_server._ACTION_RESULTS.pop("clawk-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "clawk-update"}
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("clawk-update", None)
        web_server._ACTION_RESULTS.pop("clawk-update", None)
        web_server._ACTION_PROCS["clawk-update"] = proc

        resp = self.client.get("/api/actions/clawk-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "clawk-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["clawk-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_action_status_tails_large_log_without_read_text(
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["clawk-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/clawk-update/status?lines=3")
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `clawk model` but
        invisible in the desktop Providers → API keys tab.
        """
        from clawk_cli.provider_catalog import provider_catalog
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Clawksis provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `clawk import` doesn't auto-abort at the overwrite prompt."""
        import clawk_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_clawk_action", fake_spawn)
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_clawk_home() / "backups"
        assert archive.name.startswith("clawk-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_clawk_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import clawk_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("CLAWK_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "clawk-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_clawk_action", fail_spawn)
    ...
    def test_get_messaging_platforms(self):
        ...
        assert (
            "Complete Telegram bot token" in fields["TELEGRAM_BOT_TOKEN"]["description"]
        )
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://clawksis-agent.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from clawk_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://clawksis-agent.nousresearch.com/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://clawksis-agent.nousresearch.com/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Clawksis"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("ClawksisDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "clawk_pair123_bot",
                "deep_link": "https://t.me/newbot/ClawksisSetupBot/clawk_pair123_bot",
                "qr_payload": "https://t.me/newbot/ClawksisSetupBot/clawk_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "clawk_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "clawk_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_clawk_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "clawk_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "clawk_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_clawk_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``clawk gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "clawk_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "clawk_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "clawk_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "clawk_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/ClawksisSetupBot/clawk_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get(
            "/assets/app.css", headers={"x-forwarded-prefix": "/clawk"}
        )
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`clawk serve` (CLAWK_SERVE_HEADLESS) must NOT serve the SPA even
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.setenv("CLAWK_SERVE_HEADLESS", "1")
        app_ = FastAPI()
        ws.mount_spa(app_)
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``clawk model`` custom flow. Regression for the desktop loop where a
    ...
    def test_custom_endpoint_upsert_persists_provider_and_sets_default(self):
        ...
        assert (
            cfg["providers"]["axet-proxy"]["models"]["gpt-5.4"]["context_length"]
            == 262144
        )
...
class TestBuildSchemaFromConfig:
    ...
    def test_memory_provider_options_cover_discovered_providers(self):
        ...
        assert missing == set(), (
            f"discovered providers missing from schema options: {missing}"
        )
    ...
    def test_approvals_mode_options_match_config_values(self):
        ...
        are 'manual', 'smart', and 'off' (see clawk_cli/config.py).
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from clawk_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from clawk_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("clawk://")
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(
        ...
        monkeypatch.setattr(
            profiles_mod.shutil, "which", lambda name: "/opt/clawksis/bin/clawk"
        )
        ...
        if is_windows:
            assert lines == ["@echo off", "clawk -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/clawksis/bin/clawk -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_clawk_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_clawk_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_clawk_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_clawk_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_clawk_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(
        ...
        prof_dir = get_clawk_home() / "profiles" / "builder"
        token = set_clawk_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        cfg_path = get_clawk_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import clawk_cli.skills_config as skills_config
        import clawk_cli.web_server as web_server
    ...
    def test_get_toolset_config_tts_rows_carry_provider_key(self):
        ...
        for prov in providers:
            assert prov.get("tts_provider"), (
                f"row {prov['name']!r} missing tts_provider"
            )
    ...
    def test_web_config_reports_per_capability_backends(self):
        ...
        assert rows_with_backend, (
            "expected at least one provider with a web backend key"
        )
        for prov in rows_with_backend:
            ...
            assert prov["capabilities"], (
                "a web provider must support at least one capability"
            )
    ...
    def test_terminal_docker_probe_daemon_down(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "run",
            lambda cmd, **kw: subprocess_mod.CompletedProcess(
                cmd, 1, stdout="", stderr="daemon down"
            ),
        )
    ...
    def test_terminal_docker_probe_daemon_ready(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "which",
            lambda name: (
                "/usr/bin/docker" if name in {"docker", "singularity"} else None
            ),
        )
        monkeypatch.setattr(
            web_server.subprocess,
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "clawk"
        save_config(config)

        body = self.client.get("/api/tools/terminal/backends").json()
        ssh = next(r for r in body["backends"] if r["name"] == "ssh")
        assert ssh["status"] == "ready"
        assert "clawk@devbox.example.com" in ssh["detail"]
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("CLAWK_RESTART_DRAIN_TIMEOUT", "90")
...
class TestDiscoverUserThemes:
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_clawk_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
            reset_clawk_home_override(token)
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(clawk_home, name="ocean"):
        themes_dir = clawk_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="clawk-theme-bootstrap">')
    ...
    def test_serve_index_no_bootstrap_for_builtin_theme(self, tmp_path, monkeypatch):
        ...
        resp = client.get("/chat")
        assert resp.status_code == 200
        assert "clawk-theme-bootstrap" not in resp.text
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``clawk_cli/web_server.py``).
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(
        ...
        installs it into the per-test ``CLAWK_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into CLAWK_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, clawk-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (clawk-achievements is loaded by default).
        resp = self.client.get("/api/plugins/clawk-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        CLAWK_HOME override scopes a request to another profile."""
        from clawk_constants import (
            reset_clawk_home_override,
            set_clawk_home_override,
        )
        ...
        monkeypatch.setenv("CLAWK_HOME", str(launch_home))
        from clawk_cli import web_server
...
# or ``sh -c 'printf …'``) instead of the real ``clawk --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["CLAWK_TUI_DASHBOARD"] == "1"
    ...
    def test_resolve_chat_argv_sets_tui_python_environment(self, monkeypatch):
        ...
        monkeypatch.delenv("CLAWK_PYTHON_SRC_ROOT", raising=False)
        monkeypatch.delenv("CLAWK_PYTHON", raising=False)
        monkeypatch.delenv("CLAWK_CWD", raising=False)
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (
        ...
        assert env["CLAWK_PYTHON_SRC_ROOT"] == str(main_mod.PROJECT_ROOT)
        assert env["CLAWK_PYTHON"] == sys.executable
        assert env["CLAWK_CWD"] == os.getcwd()

    def test_resolve_chat_argv_replaces_invalid_tui_python_environment(
        self, monkeypatch
    ):
        """Dashboard chat does not preserve unusable inherited TUI Python env."""
        import clawk_cli.main as main_mod

        monkeypatch.setenv("CLAWK_PYTHON_SRC_ROOT", "/definitely/missing/clawk-src")
        monkeypatch.setenv("CLAWK_PYTHON", "/definitely/missing/python")
        monkeypatch.setenv("CLAWK_CWD", "/definitely/missing/cwd")
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
            lambda project_root, tui_dev=False: (
    ...
    def test_resolve_chat_argv_keeps_relative_python_under_tui_cwd(
        ...
        monkeypatch.setenv("CLAWK_CWD", str(tmp_path))
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        env = {
            "CLAWK_CWD": str(tmp_path),
            "CLAWK_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["CLAWK_PYTHON"] == command

    def test_resolve_chat_argv_falls_back_when_getcwd_is_missing(
        self, monkeypatch, tmp_path
    ):
        ...
        monkeypatch.setattr(
            main_mod.os, "getcwd", lambda: (_ for _ in ()).throw(FileNotFoundError())
        )
        monkeypatch.setattr(
            main_mod,
            "_make_tui_argv",
        ...
        assert env is not None
        assert env["CLAWK_CWD"] == str(tmp_path)

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_clawk_home
    ):
        import clawk_cli.main as main_mod
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/clawk-tools:latest"
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    assert env is not None
    gateway_url = env.get("CLAWK_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(
        ...
        the per-test ``CLAWK_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_clawk_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under CLAWK_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_clawk_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("CLAWK_DESKTOP", raising=False)
