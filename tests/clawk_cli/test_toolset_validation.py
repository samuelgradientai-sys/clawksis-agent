"""Unit tests for clawk_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Clawksis.
"""

import pytest

from clawk_cli.toolset_validation import validate_platform_toolsets

# A representative set of real toolset names. `clawk` is deliberately absent —
# that is the corruption #38798 reported (`clawk-cli` rewritten to `clawk`).
_KNOWN = {
    "clawk-cli",
    "clawk-telegram",
    "clawk-discord",
...
def test_valid_config_produces_no_warnings():
    cfg = {"cli": ["clawk-cli"], "telegram": ["clawk-telegram"]}
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'clawk' instead of 'clawk-cli'.
    warnings = validate_platform_toolsets({"cli": ["clawk"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'clawk'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'clawk-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["clawk-cli"], "discord": ["bogus"]}
...
def test_unknown_without_valid_platform_default_omits_suggestion():
    # clawk-mystery is not a known toolset, so no "did you mean" hint.
...
@pytest.mark.parametrize("value", [None, {}, [], "clawk-cli", 42])
...
def test_scalar_toolset_value_is_accepted():
    ...
    assert validate_platform_toolsets({"cli": "clawk-cli"}, _is_valid) == []


def test_non_string_entries_are_skipped_not_counted_invalid():
    cfg = {"cli": [None, 123, "clawk-cli"]}
    # The junk entries are ignored; the valid one keeps it from being "zero".
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["clawk"], "discord": ["clawk"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_clawk_cli_valid_and_clawk_invalid():
    # Ties the helper to reality: the canonical registry check agrees that
    # `clawk-cli` is the real toolset and `clawk` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("clawk-cli") is True
    assert validate_toolset("clawk") is False
    warnings = validate_platform_toolsets({"cli": ["clawk"]}, validate_toolset)
    assert any("did you mean 'clawk-cli'?" in w for w in warnings)
