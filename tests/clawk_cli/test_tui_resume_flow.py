...
def test_exit_after_oneshot_flushes_stdio_and_calls_os_exit(monkeypatch, main_mod):
    flushed = []
    exits = []
...
def test_exit_after_oneshot_invokes_logging_shutdown_in_order(monkeypatch, main_mod):
    events = []
...
def test_run_and_exit_oneshot_prints_system_exit_message(monkeypatch, capsys, main_mod):
    exits = []
...
def test_run_and_exit_oneshot_cleans_global_runtime_before_hard_exit(
    ...
    monkeypatch.setitem(
        ...
        "clawk_cli.oneshot",
        types.SimpleNamespace(
            run_oneshot=lambda *_args, **_kwargs: events.append("run") or 0
        ),
    )
    monkeypatch.setitem(
        sys.modules,
        ...
        _mod(
            "tools.terminal_tool",
            cleanup_all_environments=lambda: events.append("terminal"),
        ),
    )
    monkeypatch.setitem(
        sys.modules,
        ...
        _mod(
            "tools.async_delegation",
            interrupt_all=lambda **kw: events.append("delegation"),
        ),
    )
    monkeypatch.setitem(
        sys.modules,
        ...
        _mod(
            "tools.browser_tool",
            _emergency_cleanup_all_sessions=lambda: events.append("browser"),
        ),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        _mod(
            "agent.auxiliary_client",
            shutdown_cached_clients=lambda: events.append("aux"),
        ),
    )
    monkeypatch.setattr(
...
def test_run_and_exit_oneshot_hard_exits_when_cleanup_is_interrupted(
    ...
    monkeypatch.setattr(main_mod, "_cleanup_oneshot_runtime", _raise_keyboard_interrupt)
...
def test_run_and_exit_oneshot_routes_keyboard_interrupt_to_130(monkeypatch, main_mod):
    exits = []
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/clawk-test", provider="nous", toolsets="web, terminal"
        )
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_clawk_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/clawk-tools:latest"
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    assert "CLAWK_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["CLAWK_TUI_RESUME"] == "20260518_000000_goodid"
