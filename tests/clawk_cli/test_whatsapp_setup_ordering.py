...
Before the fix, ``clawk whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``clawk gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    clawk.mkdir(parents=True)

    monkeypatch.setattr(Path, "home", lambda: home)

    monkeypatch.setenv("CLAWK_HOME", str(clawk))
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("clawk_cli.main._require_tty", lambda *_a, **_kw: None)
