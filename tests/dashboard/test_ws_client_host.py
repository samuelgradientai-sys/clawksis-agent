...
The contract these tests pin down:
  ...
  * Explicit ``CLAWK_DASHBOARD_WS_HOST`` env var → wins always, regardless
    of the bind host.
...
def clear_ws_host_env(monkeypatch):
    """Ensure no ``CLAWK_DASHBOARD_WS_HOST`` leaks in from the test shell."""
    monkeypatch.delenv("CLAWK_DASHBOARD_WS_HOST", raising=False)
    yield monkeypatch
...
class TestResolveClientWsHost:
    ...
    def test_explicit_env_wins_over_wildcard(self, saved_app_state, monkeypatch):
        monkeypatch.setenv("CLAWK_DASHBOARD_WS_HOST", "10.0.0.7")
        _set_bound(saved_app_state, "0.0.0.0")
        assert web_server._resolve_client_ws_host() == "10.0.0.7"

    def test_explicit_env_wins_over_lan_bind(self, saved_app_state, monkeypatch):
    ...
    def test_blank_env_falls_back_to_bind(self, saved_app_state, monkeypatch):
        """An explicitly empty override (e.g. ``CLAWK_DASHBOARD_WS_HOST=``)
        ...
        monkeypatch.setenv("CLAWK_DASHBOARD_WS_HOST", "   ")
        _set_bound(saved_app_state, "0.0.0.0")
        assert web_server._resolve_client_ws_host() == "127.0.0.1"

    def test_no_bound_host_returns_none(self, saved_app_state, clear_ws_host_env):
...
class TestGatewayWsUrlHost:
    def test_wildcard_bind_dials_loopback(self, saved_app_state, clear_ws_host_env):
    ...
    def test_loopback_bind_uses_loopback(self, saved_app_state, clear_ws_host_env):
    ...
    def test_explicit_env_overrides_wildcard(self, saved_app_state, monkeypatch):
        monkeypatch.setenv("CLAWK_DASHBOARD_WS_HOST", "10.0.0.7")
        _set_bound(saved_app_state, "0.0.0.0", port=9119)
        url = web_server._build_gateway_ws_url()
        assert url is not None
    ...
    def test_explicit_env_overrides_lan(self, saved_app_state, monkeypatch):
        monkeypatch.setenv("CLAWK_DASHBOARD_WS_HOST", "10.0.0.7")
        _set_bound(saved_app_state, "192.168.1.5", port=9120)
        url = web_server._build_gateway_ws_url()
        assert url is not None
    ...
    def test_wildcard_keeps_query_string(self, saved_app_state, clear_ws_host_env):
...
def test_netloc_helper_handles_ipv6_bracket_form():
    ...
    branch when ``CLAWK_DASHBOARD_WS_HOST`` points at an IPv6 address.
