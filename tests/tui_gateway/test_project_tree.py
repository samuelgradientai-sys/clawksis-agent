...
def test_non_git_cwd_can_group_inside_a_junk_repo_subtree():
    ...
    workspace = _session("/home/test/.clawk/workspaces/notes")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_root=lambda path: path.startswith("/home/test/.clawk"),
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.clawk"},
    )

    assert [p["id"] for p in tree["projects"]] == ["/home/test/.clawk/workspaces/notes"]
    assert tree["scoped_session_ids"] == [workspace["id"]]


def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.clawk")
