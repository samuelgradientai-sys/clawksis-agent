    ...
    with patch.dict(
        "sys.modules",
        {
            "clawk_constants": MagicMock(
                get_clawk_home=MagicMock(return_value="/tmp/clawk_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'clawk-release' patched")
    ...
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'clawk-release' patched"
    }
