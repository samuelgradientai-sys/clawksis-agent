...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `CLAWK_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("CLAWK_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("CLAWK_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    from clawk_cli.active_sessions import (
...
def test_session_resume_live_payload_uses_current_history_with_ancestors(
    ...
    monkeypatch.setattr(
        ...
        lambda _sid, key, session_id=None, session_db=None, **_kwargs: (
            types.SimpleNamespace(model="test/model", session_id=session_id or key)
        ),
    )
    monkeypatch.setattr(server, "_SlashWorker", lambda _key, _model: _Worker())
    monkeypatch.setattr(
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "clawksis-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "clawk_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with (
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "clawksis-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "clawksis-agent-dev"
