...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves clawk_cli.kanban_db on its own
  ...
  - worker logs are captured to CLAWK_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "CLAWK_KANBAN_TASK": task.id,
            "CLAWK_KANBAN_WORKSPACE": workspace,
...
def main():
    ...
    home = tempfile.mkdtemp(prefix="clawk_e2e_")
    ...
    # Point the `clawk` CLI child processes will run at the worktree

    # clawk_cli.main. We do this by putting a shim on PATH.

    shim_dir = os.path.join(home, "bin")

    os.makedirs(shim_dir, exist_ok=True)

    shim_path = os.path.join(shim_dir, "clawk")

    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh

exec {PY} -m clawk_cli.main "$@"
...
""")
    ...
    # Scenario A workers wrote to /tmp/clawk_e2e_*/worker_*.log
