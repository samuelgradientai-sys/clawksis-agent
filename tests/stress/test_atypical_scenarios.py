...
normal tests assume away:
  ...
  - Filesystem: CLAWK_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own CLAWK_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"clawk_atyp_{name}_")
            os.environ["CLAWK_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("clawk_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from clawk_cli import kanban_db as kb  # noqa: F401
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "CLAWK_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from clawk_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["CLAWK_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith(
                ...
                print(f"  ⚠ workspace resolved OUTSIDE clawk_home: {resolved}")
                print(
                    "    (not necessarily a bug — dir: workspaces are intentionally arbitrary, but worth documenting)"
                )
        except Exception as e:
...
@scenario("clawk_home_with_spaces")
def _(home, kb):
    """CLAWK_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="clawk with spaces ")
    os.environ["CLAWK_HOME"] = weird
    ...
    try:
        ...
        print(f"  CLAWK_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("clawk_home_with_unicode")
def _(home, kb):
    """CLAWK_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/clawk_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  CLAWK_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("clawk_home_via_symlink")
def _(home, kb):
    """CLAWK_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="clawk_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["CLAWK_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same CLAWK_HOME share DB correctly")
    finally:
...
def _idempotency_race_worker(
    clawk_home: str, key: str, result_file: str, barrier_path: str
) -> None:
    """Subprocess body for the idempotency race test."""
    os.environ["CLAWK_HOME"] = clawk_home
    os.environ["HOME"] = clawk_home
    sys.path.insert(0, str(WT))
...
def _(home, kb):
    ...
    try:
        from clawk_cli import web_server as ws  # noqa
    except Exception:
        pass
