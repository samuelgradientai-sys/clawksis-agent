...
The recovery path lives in ``ClawksisCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A ClawksisCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(ClawksisCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise ClawksisCLI._recover_terminal_after_interrupt()."""
    ...
    def test_drains_stdin_then_redraws(self, bare_cli):
        ...
        with patch("clawk_cli.curses_ui.flush_stdin") as mock_flush:
    ...
    def test_flush_runs_before_redraw(self, bare_cli):
        ...
        with patch(
            "clawk_cli.curses_ui.flush_stdin",
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        src = inspect.getsource(ClawksisCLI.run)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(ClawksisCLI, "_recover_terminal_after_interrupt")
        assert callable(ClawksisCLI._recover_terminal_after_interrupt)
