...
Regression: the old implementation wrote ``clawk_conversation_<ts>.json``

to the current working directory (CWD). Users who ran /save expected the

file to be discoverable via ``clawk sessions browse``, but CWD-resident

snapshots are not indexed in the state DB and are generally invisible.

The fix writes snapshots under ``~/.clawksis/sessions/saved/`` and prints
...
def clawk_home(tmp_path, monkeypatch):
    ...
    # Clear any cached clawk_home computation
...
def test_save_conversation_writes_under_clawk_home(clawk_home, tmp_path, monkeypatch, capsys):
    ...
    files = list(saved_dir.glob("clawk_conversation_*.json"))
