...
@pytest.fixture
def bare_cli():
    """A ClawksisCLI with no __init__ — we only exercise the redraw helper."""
    cli = object.__new__(ClawksisCLI)
    return cli
...
class TestForceFullRedraw:
    ...
    def test_missing_app_attr_is_safe(self, bare_cli):
        # Simulate ClawksisCLI before the TUI has ever been constructed.
        bare_cli._force_full_redraw()  # must not raise
    ...
    def test_resize_recovery_uses_prompt_toolkit_original_resize_before_reset(
        ...
        erase_down(). If Clawksis resets the renderer first, that cursor position
