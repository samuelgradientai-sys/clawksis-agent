...
The guard exists to protect the developer's live ``clawk-gateway`` process
...
def test_subprocess_run_systemctl_restart_blocked():
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["systemctl", "--user", "restart", "clawk-gateway"])
...
def test_subprocess_run_full_path_systemctl_blocked():
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["/usr/bin/systemctl", "--user", "stop", "clawk-gateway"])
...
def test_subprocess_run_sudo_systemctl_blocked():
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["sudo", "systemctl", "restart", "clawk-gateway"])
...
def test_subprocess_run_env_systemctl_blocked():
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["env", "systemctl", "--user", "restart", "clawk-gateway"])
...
def test_subprocess_run_bash_c_systemctl_blocked():
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["bash", "-c", "systemctl --user restart clawk-gateway"])


def test_subprocess_run_sh_c_systemctl_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["sh", "-c", "systemctl --user stop clawk-gateway"])


def test_subprocess_run_setsid_systemctl_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["setsid", "systemctl", "kill", "clawk-gateway"])
...
def test_subprocess_popen_systemctl_blocked():
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.Popen(["systemctl", "--user", "stop", "clawk-gateway"])


def test_subprocess_call_systemctl_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.call(["systemctl", "--user", "restart", "clawk-gateway"])


def test_subprocess_check_call_systemctl_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.check_call(["systemctl", "--user", "restart", "clawk-gateway"])


def test_subprocess_check_output_systemctl_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.check_output(["systemctl", "--user", "restart", "clawk-gateway"])


def test_subprocess_getoutput_systemctl_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.getoutput("systemctl --user restart clawk-gateway")


def test_subprocess_getstatusoutput_systemctl_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.getstatusoutput("systemctl --user restart clawk-gateway")
...
def test_os_system_systemctl_blocked():
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        os.system("systemctl --user restart clawk-gateway")


def test_os_popen_systemctl_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        os.popen("systemctl --user restart clawk-gateway")
...
def test_pty_spawn_systemctl_blocked():
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        pty.spawn(["systemctl", "--user", "restart", "clawk-gateway"])
...
def test_asyncio_create_subprocess_shell_systemctl_blocked():
    ...
    async def _attempt():
        ...
        await asyncio.create_subprocess_shell("systemctl --user restart clawk-gateway")
...
def test_subprocess_pkill_clawk_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "clawk"])


def test_subprocess_pkill_clawk_gateway_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "clawk-gateway"])


def test_subprocess_pkill_python_dash_f_blocked():
    """``pkill -f python`` matches the gateway's "python -m clawk_cli.main"."""

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "python"])


def test_subprocess_killall_clawk_blocked():

    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["killall", "clawk"])
...
def test_systemctl_status_passes_through():
    ...
    r = subprocess.run(
        ["systemctl", "--user", "status", "clawk-gateway", "--no-pager"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_show_passes_through():
    ...
    r = subprocess.run(
        ["systemctl", "--user", "show", "clawk-gateway", "--no-pager"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_unrelated_unit_passes_through():
    """systemctl restart of a non-clawk unit is allowed (we only protect clawk)."""
...
def test_subprocess_pkill_with_unrelated_pattern_passes_through():
    """``pkill -f some-unrelated-pattern`` (no clawk/python) is fine."""
