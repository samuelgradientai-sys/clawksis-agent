...
// (e.g. https://mission-control.tilos.com/clawksis/). The Python backend

// injects ``window.__CLAWK_BASE_PATH__`` into index.html based on the
...
function readBasePath(): string {
  ...
  const raw = window.__CLAWK_BASE_PATH__ ?? "";
...
export const CLAWK_BASE_PATH = readBasePath();

const BASE = CLAWK_BASE_PATH;
...
declare global {
  ...
  interface Window {
    ...
    __CLAWK_SESSION_TOKEN__?: string;

    __CLAWK_BASE_PATH__?: string;
    ...
    __CLAWK_AUTH_REQUIRED__?: boolean;
...
const SESSION_HEADER = "X-Clawksis-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__CLAWK_SESSION_TOKEN__;
  ...
  if (res.status === 401) {
    ...
    // (``clawk update``, ``clawk gateway restart``, etc.). A tab kept

    // open across the restart holds the OLD token in

    // ``window.__CLAWK_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__CLAWK_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        ...
        alreadyReloaded =
          ...
          sessionStorage.getItem("clawk.tokenReloadAttempted") === "1";
      ...
      if (!alreadyReloaded) {
        ...
        try {
          ...
          sessionStorage.setItem("clawk.tokenReloadAttempted", "1");
  ...
  if (res.ok) {
    ...
    // current ``window.__CLAWK_SESSION_TOKEN__`` is valid, so the next

    // 401 — if any — should be allowed to trigger its own reload cycle.

    try {

      sessionStorage.removeItem("clawk.tokenReloadAttempted");
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__CLAWK_SESSION_TOKEN__;
  ...
  throw new Error("Session token not available — page must be served by the Clawksis dashboard server");
 ...
 * The dashboard's gated-mode WS auth (``clawk_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  ...
  if (window.__CLAWK_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Clawksis-Session-Token`` header.

 *  - gated OAuth: no token header (it's absent by design); the

 *    ``clawk_session_at`` cookie rides along via ``credentials: 'include'``.
 ...
 * ``window.__CLAWK_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateClawksis: () =>
    ...
    fetchJSON<ActionResponse>("/api/clawksis/update", { method: "POST" }),

  checkClawksisUpdate: (force = false) =>

    fetchJSON<UpdateCheckResponse>(

      `/api/clawksis/update/check${force ? "?force=true" : ""}`,
