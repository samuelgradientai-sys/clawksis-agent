...
import { CLAWK_BASE_PATH, getWsTicket } from "@/lib/api";
...
export class GatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    if (token) {
    ...
    } else if (window.__CLAWK_AUTH_REQUIRED__) {
      ...
      authParamValue = window.__CLAWK_SESSION_TOKEN__ ?? "";
      ...
      if (!authParamValue) {
        ...
        throw new Error(
          ...
          "Session token not available — page must be served by the Clawksis dashboard",
    ...
    const ws = new WebSocket(
      ...
      `${scheme}//${location.host}${CLAWK_BASE_PATH}/api/ws?${authParamName}=${encodeURIComponent(authParamValue)}`,
