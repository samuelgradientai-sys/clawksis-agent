 ...
 * ChatPage — embeds `clawk --tui` inside the dashboard.
 ...
 *     FastAPI pty_ws  (clawk_cli/web_server.py)                          .
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${CLAWK_BASE_PATH}/api/pty?${qs.toString()}`;
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__CLAWK_SESSION_TOKEN__ &&

    !window.__CLAWK_AUTH_REQUIRED__

      ? "Session token unavailable. Open this page through `clawk dashboard`, not directly."
  ...
  useEffect(() => {
    ...
    const gated = !!window.__CLAWK_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Clawksis TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      ...
      try {
        ...
        console.warn(
          ...
          "[clawk-chat] WebGL renderer unavailable; falling back to default",
    ...
    // WebSocket. In gated mode (``window.__CLAWK_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Clawksis. Keyboard input, paste, and resize still
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3 lg:flex-row lg:gap-4">
        ...
        >
          ...
          <div
            ...
            className="clawk-chat-xterm-host min-h-0 min-w-0 flex-1"
