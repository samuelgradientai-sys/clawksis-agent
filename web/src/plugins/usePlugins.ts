...
import { api, CLAWK_BASE_PATH } from "@/lib/api";
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        ...
        const cssUrl = `${CLAWK_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${CLAWK_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;

      const scriptSrc = import.meta.env.DEV

        ? `${baseUrl}?clawk_dv=${Date.now()}`
      ...
      script.setAttribute("data-clawk-plugin", manifest.name);
