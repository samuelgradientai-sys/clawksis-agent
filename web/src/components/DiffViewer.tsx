import { useMemo, useState } from "react";
import { ChevronDown, ChevronRight, FileDiff } from "lucide-react";

/**
 * Git-style side-by-side diff viewer for the agent's file edits.
 *
 * Parses the raw unified-diff string the backend already produces
 * (`PatchResult.diff` via Python `difflib`, surfaced as `tool.inline_diff`)
 * and renders it like Claude Code / a GitHub PR: per-file sections with a
 * change summary, old/new line numbers, and aligned red (removed) / green
 * (added) columns. Falls back to a plain colorized block if parsing yields
 * nothing (e.g. a non-unified diff string).
 *
 * Pure + dependency-free — no diff/highlighter libraries — to stay light in
 * the chat transcript and match the repo's hand-rolled renderer style.
 */

type LineKind = "context" | "del" | "add";

interface DiffLine {
  kind: LineKind;
  oldN?: number;
  newN?: number;
  text: string;
}

interface Hunk {
  heading: string;
  lines: DiffLine[];
}

interface FileDiff {
  path: string;
  hunks: Hunk[];
  adds: number;
  dels: number;
}

/** Strip the `a/`|`b/` prefix and any trailing tab-metadata from a diff path. */
function cleanPath(raw: string): string {
  return (
    raw
      .replace(/^[ab]\//, "")
      .replace(/\t.*$/, "")
      .trim() || "(archivo)"
  );
}

export function parseUnifiedDiff(diff: string): FileDiff[] {
  const files: FileDiff[] = [];
  let cur: FileDiff | null = null;
  let hunk: Hunk | null = null;
  let oldN = 0;
  let newN = 0;

  const startFile = (path: string) => {
    cur = { path, hunks: [], adds: 0, dels: 0 };
    files.push(cur);
    hunk = null;
  };

  for (const raw of diff.split("\n")) {
    if (raw.startsWith("diff --git")) {
      startFile("");
      continue;
    }
    if (raw.startsWith("--- ")) {
      // A `---` with no preceding `diff --git` (plain difflib) starts a file.
      if (!cur || cur.hunks.length > 0) startFile("");
      continue;
    }
    if (raw.startsWith("+++ ")) {
      if (!cur) startFile("");
      if (cur) cur.path = cleanPath(raw.slice(4));
      continue;
    }
    const m = raw.match(/^@@ -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@(.*)$/);
    if (m) {
      if (!cur) startFile("");
      oldN = parseInt(m[1], 10);
      newN = parseInt(m[2], 10);
      hunk = { heading: m[3].trim(), lines: [] };
      cur!.hunks.push(hunk);
      continue;
    }
    if (!hunk || !cur) continue;

    if (raw.startsWith("-")) {
      hunk.lines.push({ kind: "del", oldN, text: raw.slice(1) });
      oldN++;
      cur.dels++;
    } else if (raw.startsWith("+")) {
      hunk.lines.push({ kind: "add", newN, text: raw.slice(1) });
      newN++;
      cur.adds++;
    } else if (raw.startsWith("\\")) {
      // "\ No newline at end of file" — metadata, skip.
    } else {
      const text = raw.startsWith(" ") ? raw.slice(1) : raw;
      hunk.lines.push({ kind: "context", oldN, newN, text });
      oldN++;
      newN++;
    }
  }

  return files;
}

interface Row {
  left?: DiffLine; // removed / context (old side)
  right?: DiffLine; // added / context (new side)
  context: boolean;
}

/** Align a hunk's lines into side-by-side rows: paired del↔add, context spans both. */
function alignHunk(lines: DiffLine[]): Row[] {
  const rows: Row[] = [];
  let dels: DiffLine[] = [];
  let adds: DiffLine[] = [];

  const flush = () => {
    const n = Math.max(dels.length, adds.length);
    for (let i = 0; i < n; i++) {
      rows.push({ left: dels[i], right: adds[i], context: false });
    }
    dels = [];
    adds = [];
  };

  for (const ln of lines) {
    if (ln.kind === "del") {
      dels.push(ln);
    } else if (ln.kind === "add") {
      adds.push(ln);
    } else {
      flush();
      rows.push({ left: ln, right: ln, context: true });
    }
  }
  flush();
  return rows;
}

function Num({ n }: { n?: number }) {
  return (
    <td className="select-none border-r border-border/40 px-1.5 text-right align-top tabular-nums text-text-tertiary">
      {n ?? ""}
    </td>
  );
}

function Cell({ line, side }: { line?: DiffLine; side: "left" | "right" }) {
  if (!line) {
    return <td className="bg-muted/10" />;
  }
  const isChange = line.kind !== "context";
  const tone = !isChange
    ? ""
    : side === "left"
      ? "bg-destructive/10 text-destructive"
      : "bg-success/10 text-success";
  const sign = !isChange ? " " : side === "left" ? "-" : "+";
  return (
    <td className={"whitespace-pre px-1.5 align-top " + tone}>
      <span className="select-none opacity-50">{sign}</span>
      {line.text || " "}
    </td>
  );
}

function FileBlock({ file }: { file: FileDiff }) {
  const [open, setOpen] = useState(true);
  const Chevron = open ? ChevronDown : ChevronRight;
  const rowsPerHunk = useMemo(
    () => file.hunks.map((h) => alignHunk(h.lines)),
    [file],
  );

  return (
    <div className="overflow-hidden rounded-md border border-border">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center gap-1.5 bg-muted/40 px-2 py-1 text-left text-[0.7rem]"
      >
        <Chevron className="size-3 shrink-0 text-muted-foreground" />
        <FileDiff className="size-3 shrink-0 text-muted-foreground" />
        <span className="min-w-0 flex-1 truncate font-mono font-medium text-foreground">
          {file.path}
        </span>
        {file.adds > 0 && (
          <span className="shrink-0 font-mono text-success">+{file.adds}</span>
        )}
        {file.dels > 0 && (
          <span className="shrink-0 font-mono text-destructive">
            −{file.dels}
          </span>
        )}
      </button>

      {open && (
        <div className="overflow-x-auto">
          <table className="w-full border-collapse font-mono text-[0.7rem] leading-snug">
            <tbody>
              {file.hunks.map((h, hi) => (
                <HunkRows key={hi} heading={h.heading} rows={rowsPerHunk[hi]} />
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}

function HunkRows({ heading, rows }: { heading: string; rows: Row[] }) {
  return (
    <>
      <tr className="bg-primary/5">
        <td
          colSpan={4}
          className="select-none px-2 py-0.5 text-[0.65rem] text-primary/80"
        >
          {"⋯ " + (heading || "cambios")}
        </td>
      </tr>
      {rows.map((r, i) => (
        <tr key={i} className={r.context ? "" : "bg-foreground/[0.015]"}>
          <Num n={r.left?.oldN} />
          <Cell line={r.left} side="left" />
          <Num n={r.right?.newN} />
          <Cell line={r.right} side="right" />
        </tr>
      ))}
    </>
  );
}

export function DiffViewer({ diff }: { diff: string }) {
  const files = useMemo(() => parseUnifiedDiff(diff), [diff]);

  const totals = useMemo(
    () =>
      files.reduce(
        (acc, f) => ({ adds: acc.adds + f.adds, dels: acc.dels + f.dels }),
        { adds: 0, dels: 0 },
      ),
    [files],
  );

  // Parsing produced nothing usable (not a unified diff) — fall back to a
  // plain colorized block so we never show an empty panel.
  if (files.length === 0 || files.every((f) => f.hunks.length === 0)) {
    return (
      <pre className="overflow-x-auto whitespace-pre text-[0.7rem] leading-snug">
        {diff.split("\n").map((line, i) => (
          <div
            key={i}
            className={
              line.startsWith("+") && !line.startsWith("+++")
                ? "text-success"
                : line.startsWith("-") && !line.startsWith("---")
                  ? "text-destructive"
                  : line.startsWith("@@")
                    ? "text-primary"
                    : "text-text-secondary"
            }
          >
            {line || " "}
          </div>
        ))}
      </pre>
    );
  }

  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-2 text-[0.7rem] text-text-tertiary">
        <span>
          {files.length} {files.length === 1 ? "archivo" : "archivos"}
        </span>
        {totals.adds > 0 && <span className="text-success">+{totals.adds}</span>}
        {totals.dels > 0 && (
          <span className="text-destructive">−{totals.dels}</span>
        )}
      </div>
      {files.map((f, i) => (
        <FileBlock key={i} file={f} />
      ))}
    </div>
  );
}
