...
interface ClawksisConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ClawksisConsoleModal({ open, onClose }: ClawksisConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("clawk> ");
  const inputPromptRef = useRef("clawk> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "clawk> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Clawksis Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="clawk-console-title"
    >
      <div
        className={cn(
          themedBody,
