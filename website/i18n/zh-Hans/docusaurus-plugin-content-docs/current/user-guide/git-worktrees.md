...
description: "使用 git worktrees 和隔离检出在同一仓库中安全运行多个 Clawksis"
...
本页介绍如何将 worktrees 与 Clawksis 结合使用，使每个会话拥有干净、隔离的工作目录。



## 为什么在 Clawksis 中使用 Worktrees？



Clawksis 将**当前工作目录**视为项目根目录：



- CLI：运行 `clawk` 或 `clawk chat` 时所在的目录
...
- 新分支：`feature/clawk-experiment`，已在该目录中检出



现在可以 `cd` 进入新 worktree 并在其中运行 Clawksis：
...
- 在各自的分支上工作（`feature/clawk-a` 与 `feature/clawk-b`）。
...
- `~/.clawksis/checkpoints/` 下的 Clawksis checkpoint 数据在移除 worktree 时不会自动清理，但通常体积很小。
...
- **以实验内容命名分支**
  ...
  - 例如：`feature/clawk-checkpoints-docs`、`feature/clawk-refactor-tests`。
...
- **使用 worktrees 时避免从裸仓库根目录运行 Clawksis**

  - 优先使用 worktree 目录，使每个 agent 拥有明确的作用范围。



## 使用 `clawk -w`（自动 Worktree 模式）



Clawksis 内置 `-w` 标志，可**自动创建一个一次性 git worktree** 及其独立分支。无需手动配置 worktree——只需 `cd` 进入仓库并运行：
...
如需并行运行多个 agent，在多个终端中分别运行 `clawk -w`——每次调用都会自动获得独立的 worktree 和分支。
...
- 使用 **git worktrees** 为每个 Clawksis 会话提供独立的干净检出。
