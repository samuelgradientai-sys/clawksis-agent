...
| 来源 | 可选 — 通过 `clawk skills install official/mcp/fastmcp` 安装 |
...
- 在接入 Clawksis 或其他客户端之前，使用 FastMCP CLI 对服务器进行冒烟测试
...
若服务器已存在且只需连接到 Clawksis，请使用 `native-mcp`。若目标是对现有 MCP 服务器进行临时 CLI 访问而非构建新服务器，请使用 `mcporter`。
...
若目标是集成到 Clawksis，可选择：



- 使用 `native-mcp` skill，在 `~/.clawksis/config.yaml` 中配置服务器，或
...
### Clawksis 无法看到已部署的服务器



服务器构建部分可能正确，但 Clawksis 配置有误。加载 `native-mcp` skill 并在 `~/.clawksis/config.yaml` 中配置服务器，然后重启 Clawksis。
