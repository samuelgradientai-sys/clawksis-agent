...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    ...
    if [ -f ~/.clawksis/.env ] && grep -q "^GITHUB_TOKEN=" ~/.clawksis/.env; then

      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.clawksis/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    ...
    \"body\": \"Code review from Clawksis\",
...
source "${CLAWK_HOME:-$HOME/.clawksis}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Clawksis. Code looks clean — good test coverage, no security concerns."
