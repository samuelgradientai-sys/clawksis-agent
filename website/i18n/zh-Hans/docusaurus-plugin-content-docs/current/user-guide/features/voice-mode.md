...
description: "与 Clawksis 进行实时语音对话 — CLI、Telegram、Discord（私信、文字频道和语音频道）"
...
Clawksis 支持在 CLI 和消息平台上进行完整的语音交互。通过麦克风与 Agent 对话，听取语音回复，并在 Discord 语音频道中进行实时语音对话。



如需包含推荐配置和实际使用模式的实践指南，请参阅 [使用 Clawksis 的语音模式](/guides/use-voice-mode-with-clawk)。
...
1. **已安装 Clawksis** — `pip install clawksis-agent`（参见 [安装](/getting-started/installation)）

2. **已配置 LLM 提供商** — 运行 `clawk model` 或在 `~/.clawksis/.env` 中设置首选提供商的凭据

3. **基础设置正常** — 运行 `clawk` 验证 Agent 能够响应文字消息，再启用语音功能



:::tip

`~/.clawksis/` 目录和默认的 `config.yaml` 会在首次运行 `clawk` 时自动创建。只需手动创建 `~/.clawksis/.env` 来存放 API 密钥。
...
付费的 [Nous Portal](/user-guide/features/tool-gateway) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`clawk setup --portal` 可一次性完成两项配置。
...
语音模式在**经典 CLI**（`clawk chat`）和 **TUI**（`clawk --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.clawksis/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
...
1. 使用 `clawk` 启动 CLI，并通过 `/voice on` 启用语音模式
...
录音键可通过 `~/.clawksis/config.yaml` 中的 `voice.record_key` 配置（默认：`ctrl+b`）。
...
clawk gateway        # 启动 gateway（连接到已配置的平台）

clawk gateway setup  # 首次配置的交互式设置向导
...
**服务器频道：** Bot 仅在被 @提及时响应（例如 `@clawkbyt4 你好`）。请确保从提及弹窗中选择 **Bot 用户**，而非同名角色。



:::tip

如需在服务器频道中禁用提及要求，在 `~/.clawksis/.env` 中添加：
...
3. 或在 `~/.clawksis/.env` 中设置 `DISCORD_REQUIRE_MENTION=false`
...
- 查看 gateway 日志：`tail -f ~/.clawksis/logs/gateway.log`
