...
Clawksis 以后台子进程方式运行完整的语言服务器——pyright、gopls、rust-analyzer、
...
这与顶级编码 agent 所采用的架构相同。Clawksis 将其作为自包含组件提供：
...
Clawksis 会自动检测 PATH 上或 `<CLAWK_HOME>/lsp/bin/` 中的二进制文件。
...
目录树中导入——当你运行 `clawk lsp install typescript` 或首次使用时触发自动安装时，
...
clawk lsp status          # 服务状态 + 各服务器安装状态

clawk lsp list            # 注册表，可选 --installed-only

clawk lsp install <id>    # 主动安装单个服务器

clawk lsp install-all     # 尝试安装所有已知安装方式的服务器

clawk lsp restart         # 关闭正在运行的客户端

clawk lsp which <id>      # 打印解析后的二进制路径

```



`clawk lsp status` 是最佳起点——它显示哪些语言当前可获得语义诊断，
...
lsp:
  ...
  #   auto    — 通过 npm/pip/go install 安装到 <CLAWK_HOME>/lsp/bin
...
当 `install_strategy: auto` 时，Clawksis 将二进制文件安装到 `<CLAWK_HOME>/lsp/bin/`。

NPM 包安装到 `<CLAWK_HOME>/lsp/node_modules/`，bin 符号链接位于上一级目录。
...
服务器在 Clawksis 进程的整个生命周期内保持运行。没有空闲超时回收机制——每次写入都
...
**`clawk lsp status` 显示某服务器为"missing"**



该二进制文件不在 PATH 上，也不在 `<CLAWK_HOME>/lsp/bin/` 中。运行

`clawk lsp install <server_id>` 尝试自动安装，或通过该语言的常规工具链手动安装。



**`clawk lsp status` 中出现 `Backend warnings` 部分**
...
它将诊断委托给 `shellcheck`。当 `clawk lsp status` 显示 `Backend warnings` 部分时，
...
同样的警告会在服务器启动时记录一次到 `~/.clawksis/logs/agent.log`。
...
检查 `~/.clawksis/logs/agent.log` 中的 `[agent.lsp.client]` 条目——语言服务器的
...
`clawk lsp restart` 清除该集合；下次编辑时会重新启动。
