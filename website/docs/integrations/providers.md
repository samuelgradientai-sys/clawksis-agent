...
| **Google Gemini (OAuth)** | `clawk model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
...
# → done — Clawksis auto-provisions your free tier on first request
```

Clawksis ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.clawksis/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `CLAWK_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Clawksis works unchanged
...
| Workspace / Standard / Enterprise account | Set `CLAWK_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Clawksis detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Clawksis shows
...
```bash
CLAWK_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
CLAWK_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
Clawksis will automatically load a LM Studio model with 64K context length
