...
| | |
|---|---|
| Source | Optional — install with `clawk skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Clawksis |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Clawksis terminal calls (desktop app flow only)
...
### Service Account (recommended for Clawksis)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${CLAWK_HOME:-~/.clawksis}/.env` (the skill will prompt for this on first load).
...
## Clawksis Execution Pattern (desktop app flow)

Clawksis terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/clawk-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/clawk-op.sock"
