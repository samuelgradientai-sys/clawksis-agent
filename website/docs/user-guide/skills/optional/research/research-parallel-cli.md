...
| Source | Optional — install with `clawk skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Clawksis core capability.
...
- It overlaps with Clawksis native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Clawksis native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Clawksis native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Clawksis usage patterns
...
- Do not silently choose Parallel when Clawksis native tools are already sufficient.
