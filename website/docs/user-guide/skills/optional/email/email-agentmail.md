...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. clawksis-agent@agentmail.to).
...
| Source | Optional — install with `clawk skills install official/email/agentmail` |
...
Add to `~/.clawksis/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
...
1. Create a dedicated inbox:
   ...
   - Use `create_inbox` with a username (e.g. `clawksis-agent`)

   - The agent gets address: `clawksis-agent@agentmail.to`
...
1. create_inbox (username: "clawk-outreach")
...
clawk --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
