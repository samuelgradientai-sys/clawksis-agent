...
| | |
|---|---|
| Source | Optional — install with `clawk skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Clawksis or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Clawksis. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.clawksis/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.clawksis/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Clawksis integration, either:

- configure the server in `~/.clawksis/config.yaml` using the `native-mcp` skill, or
...
### Clawksis cannot see the deployed server

The server-building part may be correct while the Clawksis config is not. Load the `native-mcp` skill and configure the server in `~/.clawksis/config.yaml`, then restart Clawksis.
