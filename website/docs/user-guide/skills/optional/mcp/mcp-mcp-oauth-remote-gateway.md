...
| | |
|---|---|
| Source | Optional — install with `clawk skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp/mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Clawksis |
| License | MIT |
...
# MCP OAuth on a Remote Clawksis Gateway

## Overview

Clawksis' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Clawksis process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Clawksis runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Clawksis.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Clawksis' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Clawksis is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Clawksis** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
Clawksis' native MCP OAuth client (`tools/mcp_oauth.py`):

1. Picks a free local port `P`.
2. Registers a dynamic OAuth client with the AS, sending `redirect_uri = http://127.0.0.1:P/callback`.
3. Starts an HTTP server on `127.0.0.1:P` **inside the Clawksis process**.
4. Prints the authorize URL and waits for the code at its local endpoint.

When Clawksis runs remotely, the `127.0.0.1` in the `redirect_uri` is the remote
...
laptop and fails to connect. The callback never reaches the Clawksis process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the clawk user, an
empty or missing tokens directory (`$CLAWK_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Clawksis detects a remote session it prints two
...
Both require an interactive terminal to the Clawksis host. The rest of this skill
is for when there is NO interactive TTY — Clawksis running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Clawksis Dashboard (try this BEFORE manual token surgery)

A remote Clawksis gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `clawk dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'clawk dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "CLAWK_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`CLAWK_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$CLAWK_HOME/.env`. Those
...
Clawksis' `ClawksisTokenStorage` would have written, so on `/reload-mcp` Clawksis finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "CLAWKSIS|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if Clawksis' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find CLAWK_HOME and the config path

```bash
CLAWK_HOME=$(python3 -c 'from clawk_constants import get_clawk_home; print(get_clawk_home())')
echo "config: $CLAWK_HOME/config.yaml"
echo "tokens: $CLAWK_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Clawksis (manual OAuth)",
...
### 8. Write tokens in Clawksis' exact schema

`tools/mcp_oauth.py::ClawksisTokenStorage` expects two files under
`$CLAWK_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "clawk-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though Clawksis (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Clawksis sees `auth: oauth`, calls `ClawksisTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Clawksis process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Clawksis needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Clawksis does natively).
...
- **Don't claim Clawksis doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Clawksis. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Clawksis' config.
