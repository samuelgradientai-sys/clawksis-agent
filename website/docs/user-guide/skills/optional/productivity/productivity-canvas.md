...
| | |
|---|---|
| Source | Optional — install with `clawk skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Clawksis"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${CLAWK_HOME:-~/.clawksis}/.env`:
...
```bash
CANVAS="python $CLAWK_HOME/skills/productivity/canvas/scripts/canvas_api.py"
