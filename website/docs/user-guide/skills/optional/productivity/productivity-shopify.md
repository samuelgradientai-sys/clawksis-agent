...
| | |
|---|---|
| Source | Optional — install with `clawk skills install official/productivity/shopify` |
...
4. Save to `${CLAWK_HOME:-~/.clawksis}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Clawksis","productType":"Apparel","tags":["test"]}}'
```
