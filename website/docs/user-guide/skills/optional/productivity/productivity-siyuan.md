...
| | |
|---|---|
| Source | Optional — install with `clawk skills install official/productivity/siyuan` |
...
3. Store it in `${CLAWK_HOME:-~/.clawksis}/.env`:
   ```
...
```yaml
# In ~/.clawksis/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
