...
| Source | Optional — install with `clawk skills install official/productivity/here-now` |
...
- Skill helper paths:
  ...
  - `${CLAWK_SKILL_DIR}/scripts/publish.sh` for publishing sites

  - `${CLAWK_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
PUBLISH="${CLAWK_SKILL_DIR}/scripts/publish.sh"

bash "$PUBLISH" {file-or-dir} --client clawk
...
bash "$PUBLISH" {file-or-dir} --slug {slug} --client clawk
...
DRIVE="${CLAWK_SKILL_DIR}/scripts/drive.sh"
...
| `--client {name}`      | Agent name for attribution (e.g. `clawk`)    |
