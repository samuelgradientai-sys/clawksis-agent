...
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Clawksis"

sidebar_label: "Openclaw Migration"

description: "Migrate a user's OpenClaw customization footprint into Clawksis"
...
Migrate a user's OpenClaw customization footprint into Clawksis. Imports Clawksis-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Source | Optional — install with `clawk skills install official/migration/openclaw-migration` |

| Path | `optional-skills/migration/openclaw-migration` |

| Version | `1.0.0` |

| Author | Clawksis (Nous Research) |

| License | MIT |

| Platforms | linux, macos, windows |

| Tags | `Migration`, `OpenClaw`, `Clawksis`, `Memory`, `Persona`, `Import` |

| Related skills | [`clawksis-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clawksis-agent) |
...
# OpenClaw -> Clawksis Migration



Use this skill when a user wants to move their OpenClaw setup into Clawksis with minimal manual cleanup.
...
clawk claw migrate              # Full interactive migration
...
clawk claw migrate --source /custom/path/.openclaw  # Custom source
...
**First-time setup:** The `clawk setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
...
It uses `scripts/openclaw_to_clawk.py` to:



- import `SOUL.md` into the Clawksis home directory as `SOUL.md`

- transform OpenClaw `MEMORY.md` and `USER.md` into Clawksis memory entries

- merge OpenClaw command approval patterns into Clawksis `command_allowlist`

- migrate Clawksis-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Clawksis working-directory configuration

- copy OpenClaw skills into `~/.clawksis/skills/openclaw-imports/`

- optionally copy the OpenClaw workspace instructions file into a chosen Clawksis workspace

- mirror compatible workspace assets such as `workspace/tts/` into `~/.clawksis/tts/`

- archive non-secret docs that do not have a direct Clawksis destination
...
- `~/.clawksis/skills/migration/openclaw-migration/scripts/openclaw_to_clawk.py`



Do not guess a shorter path like `~/.clawksis/skills/openclaw-migration/...`.
...
1. Prefer the installed path under `~/.clawksis/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Clawksis-compatible secrets, currently:
...
Clawksis CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Clawksis. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Clawksis workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Clawksis skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
python3 ~/.clawksis/skills/migration/openclaw-migration/scripts/openclaw_to_clawk.py
...
{"command":"python3 /home/USER/.clawksis/skills/migration/openclaw-migration/scripts/openclaw_to_clawk.py","workdir":"/home/USER"}
...
python3 ~/.clawksis/skills/migration/openclaw-migration/scripts/openclaw_to_clawk.py --preset user-data
...
python3 ~/.clawksis/skills/migration/openclaw-migration/scripts/openclaw_to_clawk.py --execute --preset user-data --skill-conflict skip
...
python3 ~/.clawksis/skills/migration/openclaw-migration/scripts/openclaw_to_clawk.py --execute --preset full --migrate-secrets --skill-conflict skip
...
python3 ~/.clawksis/skills/migration/openclaw-migration/scripts/openclaw_to_clawk.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Clawksis unless the user explicitly asks for secret migration.

3. Do not silently overwrite non-empty Clawksis targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Clawksis destination. Unsupported auth blobs must still be reported as skipped.
...
- Clawksis persona state imported

- Clawksis memory files populated with converted OpenClaw knowledge

- OpenClaw skills available under `~/.clawksis/skills/openclaw-imports/`
