...
description: "Configure and use Honcho memory with Clawksis -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Clawksis -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `clawk skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Clawksis

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Clawksis profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/clawk#running-honcho-locally-with-clawk

### Verify

```bash
clawk honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Clawksis profile's AI peer
...
Honcho models conversations as interactions between **peers**. Clawksis creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Clawksis instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Clawksis run |
| `global` | Single session across all directories |

Manual override: `clawk honcho map my-project-name`
...
Each Clawksis profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block clawk.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `clawk.coder` host block in `honcho.json`
...
```bash
clawk honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Clawksis when Honcho memory is active.
...
Config file: `$CLAWK_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `clawk honcho setup`. Ensure `memory.provider: honcho` is in `~/.clawksis/config.yaml`.

### Memory not persisting across sessions
Check `clawk honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `clawk profile create <name> --clone`. For existing profiles: `clawk honcho sync`.
...
| Command | Description |
|---------|-------------|
| `clawk honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `clawk honcho status` | Show resolved config, connection test, peer info for active profile |
| `clawk honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `clawk honcho disable` | Disable Honcho for the active profile |
| `clawk honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `clawk honcho peers` | Show peer identities across all profiles |
| `clawk honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `clawk honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `clawk honcho sessions` | List known directory-to-session-name mappings |
| `clawk honcho map <name>` | Map current working directory to a Honcho session name |
| `clawk honcho identity` | Seed AI peer identity or show both peer representations |
| `clawk honcho sync` | Create host blocks for all Clawksis profiles that don't have one yet |
| `clawk honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Clawksis + Honcho |
| `clawk memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `clawk memory status` | Show active memory provider and config |
| `clawk memory off` | Disable external memory provider |
