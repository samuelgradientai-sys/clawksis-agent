...
| Related skills | [`systematic-debugging`](/docs/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/docs/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), [`debugging-clawk-tui-commands`](/docs/user-guide/skills/bundled/software-development/software-development-debugging-clawk-tui-commands) |
...
- A long-running process (clawk gateway, tui_gateway) misbehaves and you can't restart it
...
The clawk test runner and pytest both support this:
...
For long-lived processes: Clawksis gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
source /home/bb/clawksis-agent/.venv/bin/activate
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Clawksis you have two practical options:
...

{
  ...
  "pathMappings": [
    ...
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/clawksis-agent" }
...
## Debugging Clawksis-specific Processes
...
Easiest: add `breakpoint()` near the suspect line, then run `clawk` normally. Control returns to your terminal at the pause point.



### `tui_gateway` subprocess (spawned by `clawk --tui`)
...
Start `clawk --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Clawksis subagents, debug one process at a time.
