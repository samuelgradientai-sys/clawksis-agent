...
4. Store the token in `~/.clawksis/.env` (or via `clawk setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.clawksis/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
