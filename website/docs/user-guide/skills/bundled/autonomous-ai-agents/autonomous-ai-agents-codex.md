...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`clawksis-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clawksis-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Clawksis terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Clawksis itself, `model.provider: openai-codex` uses Clawksis-managed Codex
OAuth from `~/.clawksis/auth.json` after `clawk auth add openai-codex`. For the
...
## Clawksis Gateway Caveat

When invoking the Codex CLI from a Clawksis gateway/service context (for example,
