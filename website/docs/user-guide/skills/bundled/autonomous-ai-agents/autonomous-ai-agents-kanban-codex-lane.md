...
description: "Use when a Clawksis Kanban worker wants to run Codex CLI as an isolated implementation lane while Clawksis keeps ownership of task lifecycle, reconciliation, tes..."
...
Use when a Clawksis Kanban worker wants to run Codex CLI as an isolated implementation lane while Clawksis keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker), [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`clawksis-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clawksis-agent) |
...
This skill defines the lightweight Clawksis+Codex dual-lane convention for Kanban workers. Clawksis is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.



The convention exists so a Clawksis worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn Clawksis workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by Clawksis in one run.

- The repo can be copied or checked out in an isolated git worktree/branch.

- Clawksis can run the relevant tests itself after Codex exits.
...
1. Clawksis owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Clawksis board CLI as a substitute for the worker.

2. Clawksis owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.

3. Clawksis owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Clawksis with the repo's canonical wrapper.

4. Clawksis owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.

5. Clawksis owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
TASK_ID="${CLAWK_KANBAN_TASK:-t_manual}"
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Clawksis can do the task directly.
...
Clawksis owns the Kanban lifecycle; do not call Clawksis kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Clawksis reconciliation.
...
- Explicit statement: Clawksis owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Clawksis will run afterward.
...
Clawksis must perform this checklist before accepting any Codex lane result:



- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.

- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Clawksis.
...
- [ ] Clawksis ran the canonical tests itself, using `scripts/run_tests.sh` for Clawksis or the repo's documented wrapper for other repos.

- [ ] Any Codex-run tests are listed separately from Clawksis-run tests.

- [ ] Accepted commits/diffs were applied to the Clawksis-owned workspace/branch.
...

{
  ...
  "codex_lane": {
    ...
    "tests_run": [
      ...
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "clawk"},
...

{
  ...
  "codex_lane": {
    ...
    "rejected_reason": "Direct Clawksis edit was smaller and safer than spawning Codex.",
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Clawksis.

2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.

3. Letting Codex own Kanban. Codex may summarize progress, but Clawksis writes board state.
...
- [ ] Clawksis reviewed `git diff` and safety-sensitive files.

- [ ] Clawksis ran canonical tests independently.
