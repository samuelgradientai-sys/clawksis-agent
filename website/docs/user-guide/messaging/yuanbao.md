...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Clawksis to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Clawksis to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.clawksis/.env`:
...
All standard Clawksis commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.clawksis/logs/gateway.log`
...
4. Consider enabling verbose logging: `CLAWK_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Clawksis automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
CLAWK_LOG_LEVEL=debug clawk gateway
```
...
```bash
clawk chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
