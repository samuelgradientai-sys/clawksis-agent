...
description: "Chat with Clawksis from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Clawksis from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Clawksis](/guides/use-voice-mode-with-clawk).
...
```mermaid
flowchart TB
    subgraph Gateway["Clawksis Gateway"]
...
For group chats, hooks, and automation flows, Clawksis supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Clawksis keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Clawksis does not hide failures just because the text resembles a silence token.
...
```bash
clawk gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
clawk gateway              # Run in foreground
clawk gateway setup        # Configure messaging platforms interactively
clawk gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo clawk gateway install --system   # Linux only: install a boot-time system service
clawk gateway start        # Start the default service
clawk gateway stop         # Stop the default service
clawk gateway status       # Check default service status
clawk gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.clawksis/config.yaml"
gateway:
...
```bash
clawk gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Clawksis sends heartbeats
...
| `/update` | Update Clawksis to the latest version |
...
with the `session_reset` section in `~/.clawksis/config.yaml`:
...
Configure per-platform overrides in `~/.clawksis/gateway.json`:
...
clawk pairing approve telegram XKGH5N7P

# Other pairing commands:
clawk pairing list          # View pending + approved users
clawk pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Clawksis appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.clawksis/config.yaml`:
...
Off by default. When enabled, Clawksis prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.clawksis/config.yaml`:
...
```bash
CLAWK_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
clawk gateway install               # Install as user service
clawk gateway start                 # Start the service
clawk gateway stop                  # Stop the service
clawk gateway status                # Check status
journalctl --user -u clawk-gateway -f  # View logs
...
sudo clawk gateway start --system
sudo clawk gateway status --system
journalctl -u clawk-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Clawksis installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit clawk-gateway` (or `sudo systemctl edit clawk-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `clawk update`. When `clawk update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart clawk-gateway` command (it never blocks on an interactive password prompt).
...
```bash
clawk gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `clawk update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo clawk update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/clawk-gateway`:

```
clawk ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed clawk-gateway*, /usr/bin/systemctl --no-ask-password start clawk-gateway*, /usr/bin/systemctl --no-ask-password restart clawk-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Clawksis will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Clawksis installations on the same machine (with different `CLAWK_HOME` directories), each gets its own systemd service name. The default `~/.clawksis` uses `clawk-gateway`; other installations use `clawk-gateway-<hash>`. The `clawk gateway` commands automatically target the correct service for your current `CLAWK_HOME`.
:::

### macOS (launchd)

```bash
clawk gateway install               # Install as launchd agent
clawk gateway start                 # Start the service
clawk gateway stop                  # Stop the service
clawk gateway status                # Check status
tail -f ~/.clawksis/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.clawk.gateway.plist`. It includes three environment variables:
...
- **CLAWK_HOME** — scopes the gateway to your Clawksis installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `clawk gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `CLAWK_HOME` directory gets its own launchd label. The default `~/.clawksis` uses `ai.clawk.gateway`; other installations use `ai.clawk.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `clawk-cli` | Full access |
| Telegram | `clawk-telegram` | Full tools including terminal |
| Discord | `clawk-discord` | Full tools including terminal |
| WhatsApp | `clawk-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `clawk-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `clawk-slack` | Full tools including terminal |
| Google Chat | `clawk-google_chat` | Full tools including terminal |
| Signal | `clawk-signal` | Full tools including terminal |
| SMS | `clawk-sms` | Full tools including terminal |
| Email | `clawk-email` | Full tools including terminal |
| Home Assistant | `clawk-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `clawk-mattermost` | Full tools including terminal |
| Matrix | `clawk-matrix` | Full tools including terminal |
| DingTalk | `clawk-dingtalk` | Full tools including terminal |
| Feishu/Lark | `clawk-feishu` | Full tools including terminal |
| WeCom | `clawk-wecom` | Full tools including terminal |
| WeCom Callback | `clawk-wecom-callback` | Full tools including terminal |
| Weixin | `clawk-weixin` | Full tools including terminal |
| BlueBubbles | `clawk-bluebubbles` | Full tools including terminal |
| QQBot | `clawk-qqbot` | Full tools including terminal |
| Yuanbao | `clawk-yuanbao` | Full tools including terminal |
| Microsoft Teams | `clawk-teams` | Full tools including terminal |
| API Server | `clawk-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `clawksis-webhook` | Full tools including terminal |
| Raft | `clawk-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.clawksis/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
