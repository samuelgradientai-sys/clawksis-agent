...
Connect Clawksis to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Clawksis and the BlueBubbles server
...
Or set environment variables directly in `~/.clawksis/.env`:
...
By default, Clawksis responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Clawksis uses conservative defaults for `Clawksis` and `@Clawksis` variants.
...
When someone messages your iMessage, Clawksis automatically sends them a pairing code. Approve it with:

```bash

clawk pairing approve bluebubbles <CODE>

```

Use `clawk pairing list` to see pending codes and approved users.



**Pre-authorize specific users** (in `~/.clawksis/.env`):
...
Clawksis will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.
...
iMessage → Messages.app → BlueBubbles Server → Webhook → Clawksis

Clawksis → BlueBubbles REST API → Messages.app → iMessage
...
- **Outbound:** Clawksis sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Clawksis wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |



Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.clawksis/config.yaml` (default: `true`). There is no corresponding environment variable.
...
You can address chats by email or phone number — Clawksis resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `clawk logs gateway` for webhook errors (or `clawk logs -f` to follow in real-time)
