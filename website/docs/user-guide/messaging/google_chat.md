...
description: "Set up Clawksis as a Google Chat bot using Cloud Pub/Sub"
...
Connect Clawksis to Google Chat as a bot. The integration uses Cloud Pub/Sub

pull subscriptions for inbound events and the Chat REST API for outbound messages.

Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Clawksis
...
> Run `clawk gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Clawksis can read (e.g.,

`~/.clawksis/google-chat-sa.json`, `chmod 600`).
...
- Message retention: **7 days** (so backlog survives a clawk restart)
...
- Principal: `clawk-chat-bot@<your-project>.iam.gserviceaccount.com`

- Role: `Pub/Sub Subscriber`



Also grant `Pub/Sub Viewer` on the same subscription — Clawksis calls
...
- **App name**: whatever you want users to see ("Clawksis" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  ...
  `projects/<your-project>/topics/clawk-chat-events`.
...
`ADDED_TO_SPACE` event that Clawksis uses to cache the bot's own `users/{id}` for
...
## Step 9: Configure Clawksis



Add the Google Chat section to `~/.clawksis/.env`:
...
Install the dependencies the Google Chat adapter needs (no Clawksis extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Clawksis is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Clawksis detects the

`thread.name` and posts its reply in the same thread, so each thread gets a

separate Clawksis session.
...
3. Download the JSON. Move it onto the host that runs Clawksis.

4. On the host, register the client with Clawksis:
...
That writes `~/.clawksis/google_chat_user_client_secret.json`. This is shared
...
This file lives at the default Clawksis root, so a gateway running under a named

profile (`clawk -p <name> gateway …`) finds the same host-wide secret — you do
...
profile's `CLAWK_HOME` and it takes precedence. Per-user tokens always stay
...
The token lands at `~/.clawksis/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.clawksis/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   ...
   If it does, Clawksis isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `clawk gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Clawksis will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.clawksis/google_chat_user_tokens/<sanitized_email>.json` (filesystem
