...
description: Control your smart home with Clawksis via Home Assistant integration.
...
Clawksis integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Clawksis"
5. Copy the token
...
Clawksis registers four tools for smart home control:
...
Configure which events the agent sees in `~/.clawksis/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Clawksis".
...
The adapter reads credentials from `~/.clawksis/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Clawksis profile
...
running Clawksis — `curl -H "Authorization: Bearer <token>" <url>/api/` should
