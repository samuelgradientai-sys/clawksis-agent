...
ntfy makes a great lightweight push channel for Clawksis: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.



> Run `clawk gateway setup` and pick **ntfy** for a guided walk-through.
...
- A topic name (any unique string — `clawk-myname-2026` works fine)
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already a Clawksis dependency.
...
Add these to `~/.clawksis/.env`:



```

NTFY_TOPIC=clawk-myname-2026

NTFY_ALLOWED_USERS=clawk-myname-2026

NTFY_HOME_CHANNEL=clawk-myname-2026
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Clawksis adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
- Or **pick a long, unguessable topic name** (`clawk-7d4f9c8b-2026`) and treat it as the shared secret. This is the lightest setup but the topic name leaks via any logs or screenshots.
...
1. Pick a topic name: `clawk-myname-2026`

2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `clawk-myname-2026`

3. On the host:

   ```bash

   echo 'NTFY_TOPIC=clawk-myname-2026' >> ~/.clawksis/.env

   echo 'NTFY_ALLOWED_USERS=clawk-myname-2026' >> ~/.clawksis/.env
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Clawksis replies:
...
- **Message size**: ntfy caps message bodies at 4096 chars. Clawksis truncates with a warning when this is exceeded.
