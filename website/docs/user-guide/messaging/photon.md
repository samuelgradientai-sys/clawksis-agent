...
Connect Clawksis to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Clawksis — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Clawksis runs it in a
...
clawk photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Clawksis` project on your account.
...
Runtime credentials are written to `~/.clawksis/.env`
...
(device token, dashboard project id) lives in `~/.clawksis/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Clawksis
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Clawksis replies with a pairing code. Approve it with:

```bash
clawk pairing approve photon <CODE>
```

Use `clawk pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.clawksis/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.clawksis/.env`):
...
By default Clawksis responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Clawksis` and
`@Clawksis agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Clawksis will reply.
...
assigned iMessage line Clawksis uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run clawk photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `clawk photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `clawk photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `clawk photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Clawksis sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Clawksis wake words  | JSON list / comma / newline regex patterns for group mentions |
