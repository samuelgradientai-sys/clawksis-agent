...
description: "Set up Clawksis as a Matrix bot"
...
Clawksis integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Clawksis pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).



Clawksis works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.
...
| **DMs** | Clawksis responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |

| **Rooms** | By default, Clawksis requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |

| **Threads** | Clawksis supports Matrix threads (MSC3440). If you reply in a thread, Clawksis keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |

| **Auto-threading** | By default, Clawksis auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |

| **Commands** | Clawksis accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Clawksis normalizes known `!command` aliases to `/command`. |

| **Shared rooms with multiple users** | By default, Clawksis isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `clawk` — the full user ID will be `@clawk:your-server.org`.
...
2. Pick a username for your bot (e.g., `clawk-bot`).
...
You can also run Clawksis as your own user. This means the bot posts as you — useful for personal assistants.
...
Clawksis needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Clawksis the bot's user ID and password. Clawksis will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.



```bash

MATRIX_USER_ID=@clawk:your-server.org
...
Clawksis uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Add the following to your `~/.clawksis/.env` file:
...
# MATRIX_USER_ID=@clawk:matrix.example.org
...
Optional behavior settings in `~/.clawksis/config.yaml`:
...
You can run `clawk gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
...
Clawksis supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with clawk extras

pip install 'clawksis-agent[matrix]'
...
When E2EE is enabled, Clawksis:



- Stores encryption keys in `~/.clawksis/platforms/matrix/store/` (legacy installs: `~/.clawksis/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Clawksis imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.



:::warning[Deleting the crypto store]

If you delete `~/.clawksis/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.



Clawksis detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     ...
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@clawk:your-server';

     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@clawk:your-server';

     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@clawk:your-server';

     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@clawk:your-server';
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     ...
     'https://your-server/_synapse/admin/v2/users/%40clawk%3Ayour-server/devices/DEVICE_ID'
...
2. Delete the local crypto store and restart Clawksis:

   ```bash

   rm -f ~/.clawksis/platforms/matrix/store/crypto.db*
...
Clawksis supports the same gateway commands in Matrix that it supports on other
...
Clawksis only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/samuelgradientai-sys/clawksis-agent/issues/12614).
...
If you previously used Clawksis with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       ...
       "identifier": {"type": "m.id.user", "user": "@clawk:your-server.org"},

       "password": "***",

       "initial_device_display_name": "Clawksis"
   ...
   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.clawksis/.env`.
...
2. **Delete old encryption state**:
   ...
   rm -f ~/.clawksis/platforms/matrix/store/crypto_store.*

   ```



3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.clawksis/.env`:
...
a working E2EE setup with a previous version of Clawksis and are upgrading.
...
services:
  ...
  clawk-matrix:
    ...
    volumes:
      ...
      - ./matrix-store:/root/.clawksis/platforms/matrix/store
...
RUN pip install 'clawksis-agent[matrix]'
...
| `GATEWAY_PROXY_URL` | URL of the remote Clawksis API server (e.g., `http://192.168.1.100:8642`) |
...
Session continuity is maintained via the `X-Clawksis-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Clawksis logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.
...
**Cause**: The Clawksis gateway isn't running, or it failed to connect.



**Fix**: Check that `clawk gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.
...
**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.clawksis/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your Clawksis deployment, see the [Security Guide](../security.md).
...
- **Media support**: Clawksis can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
