...
Connect Clawksis to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.



:::info WeCom Bot vs WeCom Callback

Clawksis supports two WeCom integration modes:
...
> Run `clawk gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Clawksis decrypts the message, queues it for the agent
...
(Use `clawk gateway start` only after `clawk gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Clawksis and the token the
...
in `~/.clawksis/.env` values around `=` will also break signature checks. After

fixing, restart `clawk gateway run`.
...
Check `clawk gateway run` logs for the bound host/port. If the adapter bound to
