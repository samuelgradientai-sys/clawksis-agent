...
description: "Set up Clawksis as a LINE Messaging API bot"
...
Run Clawksis as a [LINE](https://line.me/) bot via the official LINE Messaging API. The adapter lives as a bundled platform plugin under `plugins/platforms/line/` — no core edits, just enable it like any other platform.
...
> Run `clawk gateway setup` and pick **LINE** for a guided walk-through.
...
devtunnel create clawk-line --allow-anonymous

devtunnel port create clawk-line -p 8646 --protocol https

devtunnel host clawk-line
...
**Bot receives nothing in groups.** Check `LINE_ALLOWED_GROUPS` includes the `C...` group ID. To find a group ID, send a test message and grep `~/.clawksis/logs/gateway.log` for `LINE: rejecting unauthorized source` — the rejected source dict has the IDs.
...
**"already in use by another profile".** The same channel access token is bound to another running Clawksis profile. Stop the other gateway or use a separate channel.
