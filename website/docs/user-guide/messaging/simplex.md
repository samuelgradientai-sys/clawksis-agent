...
`simplex:group:12` as a cron `deliver=` target or in a `clawk send` call.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`clawk send` CLI](/guides/pipe-script-output):

```bash
clawk send simplex:<contact-id> "Done!"
```
