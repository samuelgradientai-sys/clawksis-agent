...
description: "Customize the Clawksis CLI with built-in and user-defined skins"
...
Skins control the **visual presentation** of the Clawksis CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.clawksis/skins/mytheme.yaml

```



Or set the default skin in `~/.clawksis/config.yaml`:
...
| `default` | Classic Clawksis — gold and kawaii | `Clawksis` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Clawksis` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |

| `slate` | Cool blue — developer-focused | `Clawksis` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |

| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Clawksis` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |

| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Clawksis` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
| `agent_name` | Name shown in banner title and status display | `Clawksis` |

| `welcome` | Welcome message shown at CLI startup | `Welcome to Clawksis! Type your message or /help for commands.` |

| `goodbye` | Message shown on exit | `Goodbye! ∇` |

| `response_label` | Label on the response box header | ` ∇ Clawksis ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default CLAWK_AGENT banner) | `""` |
...
Create YAML files under `~/.clawksis/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.
...
## Clawksis Mod — Visual Skin Editor



[Clawksis Mod](https://github.com/cocktailpeanut/clawk-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.



![Clawksis Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/clawk-mod/master/nous.png)
...
- Opens any skin into a visual editor with all Clawksis skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.clawksis/skins/`

- Activates a skin by updating `~/.clawksis/config.yaml`
...
git clone https://github.com/cocktailpeanut/clawk-mod.git
...
6. Click **Save** to write the skin YAML to `~/.clawksis/skins/`.

7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).



Clawksis Mod respects the `CLAWK_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.
...
- Built-in skins load from `clawk_cli/skin_engine.py`.

- Unknown skins automatically fall back to `default`.

- `/skin` updates the active CLI theme immediately for the current session.

- User skins in `~/.clawksis/skins/` take precedence over built-in skins with the same name.
