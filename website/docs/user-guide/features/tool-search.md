...
**Tool Search** is Clawksis' opt-in progressive-disclosure layer for that
...
:::info Built-in Clawksis tools never defer

The tools that make up Clawksis' core capability set (`terminal`,
...
`_CLAWK_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Clawksis **unwraps the bridge** and
...
- **No JS sandbox.** Clawksis uses the simpler "structured tools" mode
