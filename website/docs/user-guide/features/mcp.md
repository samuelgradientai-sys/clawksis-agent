...
description: "Connect Clawksis to external tool servers via MCP — and control exactly which MCP tools Clawksis loads"
...
MCP lets Clawksis connect to external tool servers so the agent can use tools that live outside Clawksis itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.



If you have ever wanted Clawksis to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.
...
- Access to external tool ecosystems without writing a native Clawksis tool first
...
- Per-server filtering so you can expose only the MCP tools you actually want Clawksis to see
...
2. Add an MCP server to `~/.clawksis/config.yaml`:
...
4. Ask Clawksis to use the MCP-backed capability.
...
Clawksis will discover the MCP server's tools and use them like any other tool.
...
Clawksis ships a curated catalog of MCP servers that Nous staff has reviewed
...
clawk mcp                # interactive picker (default)

clawk mcp catalog        # plain-text list, scriptable

clawk mcp install n8n    # install a catalog entry by name
...
n8n          available              Manage and inspect n8n workflows from Clawksis
...
`optional-mcps/` in the clawksis-agent repo — presence in that directory means
...
- **API key** — Clawksis prompts at install time and writes the value to

  `~/.clawksis/.env`. Non-secret values (base URLs) go to the same file.

- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP

  client opens a browser on first connection.

- **OAuth** (third-party provider like Google/GitHub) — Clawksis points you at

  `clawk auth <provider>` if you haven't authenticated already.
...
After credentials are configured, Clawksis probes the MCP server to list every
...
written (if not). Re-run `clawk mcp configure <name>` once the server is
...
the clawksis-agent repo, so Nous has reviewed each entry before it shipped —
...
[`optional-mcps/<name>/manifest.yaml`](https://github.com/samuelgradientai-sys/clawksis-agent/tree/main/optional-mcps)
...
from environment variables (which include everything in `~/.clawksis/.env`).
...
MCPs are never auto-updated. Re-run `clawk mcp install <name>` to refresh

after a Clawksis update if a manifest version changed.



To add an MCP to the catalog, open a PR against

[`optional-mcps/`](https://github.com/samuelgradientai-sys/clawksis-agent/tree/main/optional-mcps).
...
HTTP MCP servers are remote endpoints Clawksis connects to directly.
...
- you do not want Clawksis spawning a local subprocess for that integration
...
Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Clawksis handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
On first connect, Clawksis prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.clawksis/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.



**Remote / headless hosts.** When Clawksis runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:



- **Paste-back (no setup):** on an interactive terminal Clawksis prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.

- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.



See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `clawk mcp login <server>`.



**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `clawk mcp login` can list the tools and look like it worked, but every real tool call later times out. `clawk mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `clawk mcp login googledrive` — with the pre-registered client, Clawksis skips registration and runs the normal browser authorization flow.



**Pitfall — config auto-reload race.** When you edit `~/.clawksis/config.yaml` from inside a running Clawksis session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `clawk mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.
...
Clawksis reads MCP config from `~/.clawksis/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Clawksis skips the server entirely |
...
For well-known MCP servers, `clawk mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
clawk mcp add codex --preset codex
...
You can pick any local name (`clawk mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.



## How Clawksis registers MCP tools



Clawksis prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Clawksis sees the tool and chooses it during normal reasoning.
...
When supported, Clawksis also registers utility tools around MCP resources and prompts:
...
- Clawksis only registers resource utilities if the MCP session actually supports resource operations

- Clawksis only registers prompt utilities if the MCP session actually supports prompt operations
...
You can control which tools each MCP server contributes to Clawksis, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Clawksis skips the server completely and does not even attempt a connection.
...
You can also separately disable Clawksis-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Clawksis does not create an empty runtime MCP toolset for that server.
...
Clawksis discovers MCP servers at startup and registers their tools into the normal tool registry.
...
MCP servers can notify Clawksis when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Clawksis receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Clawksis does not blindly pass your full shell environment.
...
Then verify your config and restart Clawksis.
...
Because Clawksis now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Clawksis may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Clawksis via the `sampling/createMessage` protocol. This allows an MCP server to ask Clawksis to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Clawksis as an MCP server



In addition to connecting **to** MCP servers, Clawksis can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Clawksis's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.
...
- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Clawksis

- You want a single MCP server that bridges to all of Clawksis's connected messaging platforms at once

- You already have a running Clawksis gateway with connected platforms
...
Add Clawksis to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Clawksis in a specific location:
...

{
  ...
  "mcpServers": {
    ...
    "clawk": {
      ...
      "command": "/home/user/.clawksis/clawksis-agent/venv/bin/clawk",
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Clawksis-specific channel browser:
...
The MCP server includes a live event bridge that polls Clawksis's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
clawk mcp serve              # Normal mode

clawk mcp serve --verbose    # Debug logging on stderr
...
The MCP server reads conversation data directly from Clawksis's session store (`~/.clawksis/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Clawksis itself.
...
- The embedded `clawk mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Clawksis, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
