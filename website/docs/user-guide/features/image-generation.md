...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `clawk tools`.
...
Clawksis generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `clawk tools` and persists in `config.yaml`.
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Clawksis materializes them to the local cache so delivery still works after expiry
