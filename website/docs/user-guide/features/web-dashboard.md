...
The web dashboard is a browser-based UI for managing your Clawksis installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `clawk setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
clawk dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
clawk dashboard --host 0.0.0.0
...
The default `clawksis-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'clawksis-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install clawksis-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `clawk dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `clawk dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- The server spawns `clawk --tui` behind a POSIX pseudo-terminal
...
- Node.js (same requirement as `clawk --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'clawksis-agent[web,pty]'`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.clawksis/.env
ExecStart=/path/to/venv/bin/python -m clawk_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.clawksis/.env` containing:

```bash
CLAWK_DASHBOARD_BASIC_AUTH_USERNAME=admin
CLAWK_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
CLAWK_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `clawk config set` and the gateway read from.
:::
...
Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.clawksis/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `clawk skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `clawk mcp` reads from.
...
`clawk mcp catalog` / `clawk mcp install` use.
...
Connect Clawksis to any messaging platform from the browser — full parity with
`clawk setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.clawksis/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Clawksis versions, CPU core count + utilization, memory, disk usage of the Clawksis home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Clawksis version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `clawk update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `clawk portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `clawk curator`.
...
This re-reads `~/.clawksis/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/clawksis/update/check` | Report update availability (commits behind, install method) without applying. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Clawksis engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Two providers ship in the box:

- **[Username/password](#usernamepassword-provider-no-oauth-idp)** — the simplest way to put auth on a self-hosted / on-prem / homelab dashboard (and the recommended path for a [remote Clawksis Desktop connection](#connecting-clawk-desktop-to-a-remote-backend)). No external identity provider.
...
|-------|-----------|----------|
| `clawk dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `clawk dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `clawk dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
|---------|-----------|--------|----------------|
| `CLAWK_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | Nous Portal at Fly.io provisioning time |
| `CLAWK_DASHBOARD_PORTAL_URL` | `dashboard.oauth.portal_url` | URL (default: `https://portal.nousresearch.com`) | Portal — override only for staging or a custom deployment |

Per the Clawksis convention (`~/.clawksis/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so Fly.io's platform-secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
|---------|-----------|-------|
| `CLAWK_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `CLAWK_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `CLAWK_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `CLAWK_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `CLAWK_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/clawksis/   # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `CLAWK_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `CLAWK_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `CLAWK_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-clawk.json` — it defines a `clawk` realm, a **public PKCE client** (`clawksis-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "clawksis-dashboard",
      "name": "Clawksis Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-clawk.json:/opt/keycloak/data/import/realm-clawk.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/clawksis/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/clawk`). The admin console is at
...
```bash
export CLAWK_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/clawk"
export CLAWK_DASHBOARD_OIDC_CLIENT_ID="clawksis-dashboard"
export CLAWK_DASHBOARD_PUBLIC_URL="http://localhost:9119"
clawk dashboard --host 0.0.0.0 --port 9119 --no-open
```

`CLAWK_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → clawksis-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain Fly deploys with partial proxy chains), set `dashboard.public_url` (or `CLAWK_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/clawk"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `CLAWK_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `CLAWK_DASHBOARD_PUBLIC_URL` env var | — | Fly.io platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `clawk_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `clawk_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `clawk_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `clawk_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$CLAWK_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```bash
# Quick env-var path (Fly.io shape). CLAWK_DASHBOARD_PORTAL_URL is
# optional — defaults to production.
CLAWK_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  clawk dashboard --host 0.0.0.0
...
Clawksis Desktop can drive a Clawksis backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Clawksis Desktop](/user-guide/desktop) page.)
...
```bash
# 1. Set the dashboard login credentials in ~/.clawksis/.env (secrets file, 0600).
cat >> ~/.clawksis/.env <<'EOF'
CLAWK_DASHBOARD_BASIC_AUTH_USERNAME=admin
CLAWK_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
# Recommended: a stable signing secret so sessions survive restarts.
CLAWK_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.clawksis/.env

# 2. Run the dashboard bound to a reachable address. The non-loopback bind
#    engages the auth gate; the username/password provider handles login.
clawk dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `CLAWK_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.clawksis/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.clawksis/.env`, so the credentials are in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/clawk` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `CLAWK_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `CLAWK_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `CLAWK_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `CLAWK_DASHBOARD_BASIC_AUTH_USERNAME` / `CLAWK_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `CLAWK_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `clawk_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `clawk update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `clawk dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Clawksis Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Clawksis Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
