...
When Clawksis runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.clawksis/SOUL.md`, or as a named preset under

`agent.personalities` in `~/.clawksis/config.yaml` (switchable per session
...
Install MCP servers via `~/.clawksis/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Clawksis's deliverable
