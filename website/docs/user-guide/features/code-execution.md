...
The `execute_code` tool lets the agent write Python scripts that call Clawksis tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Clawksis over a Unix domain socket RPC.
...
1. The agent writes a Python script using `from clawk_tools import ...`

2. Clawksis generates a `clawk_tools.py` stub module with RPC functions

3. Clawksis opens a Unix domain socket and starts an RPC listener thread
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.clawksis/config.yaml`:
...
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Clawksis's own python |

| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Clawksis's own python) |
...
### `CLAWK_*` variables in the child



The child process receives only a small, fixed set of operational `CLAWK_*`
...
(plus `CLAWK_RPC_DIR` / `CLAWK_RPC_SOCKET` / `TZ` / `HOME`, which Clawksis
...
Earlier versions passed **any** variable whose name began with `CLAWK_`

through to the child. That broad prefix was removed for security hardening: it

could leak `CLAWK_*`-named configuration that doesn't match a secret substring

(for example `CLAWK_BASE_URL`, `CLAWK_KANBAN_DB`, or a `CLAWK_*_WEBHOOK`
...
— relied on a `CLAWK_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Clawksis-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `CLAWK_*`

variables, Clawksis emits a one-line `debug` log naming them and pointing at the

`env_passthrough` escape hatch. Run with debug logging (`clawk logs --level

DEBUG`, or check `~/.clawksis/logs/agent.log`) and look for

`execute_code: dropped N non-allowlisted CLAWK_* var(s)` if a script behaves

as though a `CLAWK_*` variable is missing.



Clawksis always writes the script and the auto-generated `clawk_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Clawksis tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.
