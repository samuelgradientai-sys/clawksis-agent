...
description: "Project context files — .clawk.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
...
Clawksis automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Clawksis instance and is loaded from `CLAWK_HOME` only.
...
| **.clawk.md** / **HERMES.md** | Project instructions (highest priority) | Walks to git root |

| **AGENTS.md** | Project instructions, conventions, architecture | CWD at startup + subdirectories progressively |

| **CLAUDE.md** | Claude Code context files (also detected) | CWD at startup + subdirectories progressively |

| **SOUL.md** | Global personality and tone customization for this Clawksis instance | `CLAWK_HOME/SOUL.md` only |
...
Only **one** project context type is loaded per session (first match wins): `.clawk.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
At session start, Clawksis loads the `AGENTS.md` from your working directory into the system prompt. As the agent navigates into subdirectories during the session (via `read_file`, `terminal`, `search_files`, etc.), it **progressively discovers** context files in those directories and injects them into the conversation at the moment they become relevant.
...
- or `$CLAWK_HOME/SOUL.md` if you run Clawksis with a custom home directory
...
- Clawksis seeds a default `SOUL.md` automatically if one does not exist yet

- Clawksis loads `SOUL.md` only from `CLAWK_HOME`

- Clawksis does not probe the working directory for `SOUL.md`
...
Clawksis is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.clawk.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.



This means your existing Cursor conventions automatically apply when using Clawksis.
...
1. **Scan working directory** — checks for `.clawk.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
