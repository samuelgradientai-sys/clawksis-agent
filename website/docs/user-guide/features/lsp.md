...
Clawksis runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Clawksis
...
1. Clawksis captures a baseline of current diagnostics for the file.
...
…). Clawksis auto-detects the binary on PATH or in
`<CLAWK_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Clawksis at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<CLAWK_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`clawk lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Clawksis installs both packages together when you
run `clawk lsp install typescript` or auto-install fires on first
use.

## CLI

```
clawk lsp status          # service state + per-server install status
clawk lsp list            # registry, optionally --installed-only
clawk lsp install <id>    # eagerly install one server
clawk lsp install-all     # try every server with a known recipe
clawk lsp restart         # tear down running clients
clawk lsp which <id>      # print resolved binary path
```

`clawk lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CLAWK_HOME>/lsp/bin
...
When `install_strategy: auto`, Clawksis installs binaries into
`<CLAWK_HOME>/lsp/bin/`. NPM packages land in
`<CLAWK_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Clawksis-owned and is
...
Servers are kept alive for the life of the Clawksis process. There's
...
**`clawk lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CLAWK_HOME>/lsp/bin/`. Run
`clawk lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `clawk lsp status`**
...
When `clawk lsp status` shows a `Backend warnings` section, install
...
`~/.clawksis/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.clawksis/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `clawk lsp restart` to clear the set;
