...
Clawksis can drive your desktop — clicking, typing, scrolling,
...
```
clawk computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `clawk computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `clawk tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Clawksis app). `clawk computer-use doctor` will tell you which permission is missing. |
...
```
clawk -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.clawksis/config.yaml`.

## `clawk computer-use doctor` — your first triage stop

`clawk computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ clawk computer-use doctor
...
agent is acting here." Each Clawksis run declares its own cua-driver
...
Clawksis intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Clawksis-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Clawksis skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Clawksis
...
Clawksis applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.clawksis/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Clawksis applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Clawksis agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Clawksis agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Clawksis warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Clawksis from inside the RDP / console
...
```
CLAWK_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
CLAWK_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Clawksis disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Clawksis sets
...
When it's on, `clawk computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Clawksis at a binary you built from source instead
of the published release. Clawksis resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`CLAWK_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Clawksis testing (see notes).
...
### Option B — point Clawksis straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`CLAWK_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
CLAWK_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
CLAWK_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Clawksis is using your build

- `clawk computer-use status` prints the resolved binary path and
  version.
- `clawk computer-use doctor` confirms the binary is reachable and
...
- **Clawksis spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Clawksis
  change needed either way.
- **Local builds skip the version check.** Clawksis warns when the
...
**First action when anything's off: run `clawk computer-use doctor`.**
...
Run `clawk computer-use install` to fetch the cua-driver binary, or
run `clawk tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `clawk computer-use doctor` will
...
- **Clawksis-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Clawksis `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Clawksis (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Clawksis among them)
