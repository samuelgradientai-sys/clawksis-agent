...
Clawksis can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `clawk computer-use status` to verify the install.
...
1. Run `clawk tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Clawksis refreshes the binary in two

places so you don't get stuck on a stale release:



- **`clawk update`** — when you update Clawksis itself, if `cua-driver` is
...
- **`clawk computer-use install --upgrade`** — manual force-refresh.
...
`clawk computer-use status` shows the installed version next to the
...
Pair with `approvals.mode: manual` in `~/.clawksis/config.yaml` if you want every action confirmed.
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  ...
  OS update. Pin the driver version with the `CLAWK_CUA_DRIVER_VERSION`
...
`clawk computer-use install` to fetch the cua-driver binary, or run

`clawk tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/samuelgradientai-sys/clawksis-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
