...
description: "Extend Clawksis with custom tools, hooks, and integrations via the plugin system"
...
Clawksis has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Clawksis

core tools that live in `tools/` and `toolsets.py`.



**→ [Build a Clawksis Plugin](/guides/build-a-clawk-plugin)** — step-by-step guide with a complete working example.
...
Drop a directory into `~/.clawksis/plugins/` with a `plugin.yaml` and Python code:
...
Start Clawksis — your tools appear alongside built-in tools. The model can call them immediately.
...
**`~/.clawksis/plugins/hello-world/plugin.yaml`**
...
Project-local plugins under `./.clawksis/plugins/` are disabled by default. Enable them only for trusted repositories by setting `CLAWK_ENABLE_PROJECT_PLUGINS=true` before starting Clawksis.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `clawk <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `clawk plugins install` |

| Distribute via pip | `[project.entry-points."clawk_agent.plugins"]` |
...
| Bundled | `<repo>/plugins/` | Ships with Clawksis — see [Built-in Plugins](/user-guide/features/built-in-plugins) |

| User | `~/.clawksis/plugins/` | Personal plugins |

| Project | `.clawksis/plugins/` | Project-specific plugins (requires `CLAWK_ENABLE_PROJECT_PLUGINS=true`) |

| pip | `clawk_agent.plugins` entry_points | Distributed packages |

| Nix | `services.clawksis-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |
...
Within each source, Clawksis also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.clawksis/plugins/model-providers/<name>/` and `~/.clawksis/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.
...
**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `clawk plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.clawksis/config.yaml`. This stops third-party code from running without your explicit consent.
...
clawk plugins                    # interactive toggle (space to check/uncheck)

clawk plugins enable <name>      # add to allow-list

clawk plugins disable <name>     # remove from allow-list + add to disabled

```



After `clawk plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.
...
Several categories of plugin bypass `plugins.enabled` — they're part of Clawksis' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.clawksis/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |



In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.clawksis/plugins/`.
...
Clawksis has four kinds of plugins:
...
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.clawksis/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Clawksis also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:
...
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Clawksis Plugin](/guides/build-a-clawk-plugin) · [Adding Tools](/developer-guide/adding-tools) |

| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Clawksis Plugin](/guides/build-a-clawk-plugin) |

| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Clawksis Plugin](/guides/build-a-clawk-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |

| A **subcommand** for `clawk <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `CLAWK_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |

| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Clawksis auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |

| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `clawk skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |

| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.clawksis/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `clawk plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
clawk plugins                               # unified interactive UI

clawk plugins list                          # table: enabled / disabled / not enabled

clawk plugins install user/repo             # install from Git, then prompt Enable? [y/N]

clawk plugins install user/repo --enable    # install AND enable (no prompt)

clawk plugins install user/repo --no-enable # install but leave disabled (no prompt)

clawk plugins update my-plugin              # pull latest

clawk plugins remove my-plugin              # uninstall

clawk plugins enable my-plugin              # add to allow-list

clawk plugins disable my-plugin             # remove from allow-list + add to disabled
...
Running `clawk plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `clawk plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/guides/build-a-clawk-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
