...
description: "Customize Clawksis's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Clawksis's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CLAWK_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Clawksis is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Clawksis now seeds a default `SOUL.md` automatically in:

```text
~/.clawksis/SOUL.md
```

More precisely, it uses the current instance's `CLAWK_HOME`, so if you run Clawksis with a custom home directory, it will use:
...
- Clawksis creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Clawksis loads `SOUL.md` only from `CLAWK_HOME`
- Clawksis does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Clawksis falls back to a built-in default identity
...
If Clawksis loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CLAWK_HOME`, the personality belongs to the Clawksis instance itself.

That also makes it easier to teach users:
- "Edit `~/.clawksis/SOUL.md` to change Clawksis' default personality."
...
- how Clawksis should handle uncertainty, disagreement, or ambiguity
...
## What Clawksis injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Clawksis falls back to a built-in default identity ("You are Clawksis, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Clawksis ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Clawksis, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Clawksis its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.clawksis/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.clawksis/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-clawk)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Clawksis speaks
- `display.skin` and `/skin` affect how Clawksis looks in the terminal
