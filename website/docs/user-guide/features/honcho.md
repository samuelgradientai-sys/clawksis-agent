...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Clawksis's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Clawksis instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
clawk memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `clawk` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Clawksis syncs them back at session init.
...
The `clawk honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `clawk memory setup honcho` (or run `clawk memory setup` and pick it from the list); the `clawk honcho` subcommand then appears on the next invocation.

```bash
clawk memory setup honcho    # Configure Honcho directly (works before activation)
clawk honcho status          # Connection status, config, and key settings
clawk honcho setup           # Redirects to `clawk memory setup` (post-activation alias)
clawk honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
clawk honcho peer            # Show or update peer names + dialectic reasoning level
clawk honcho mode            # Show or set recall mode (hybrid/context/tools)
clawk honcho tokens          # Show or set token budget for context and dialectic
clawk honcho identity        # Seed or show the AI peer's Honcho identity
clawk honcho sync            # Sync Honcho config to all existing profiles
clawk honcho peers           # Show peer identities across all profiles
clawk honcho sessions        # List known Honcho session mappings
clawk honcho map             # Map current directory to a Honcho session name
clawk honcho enable          # Enable Honcho for the active profile
clawk honcho disable         # Disable Honcho for the active profile
clawk honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `clawk honcho`

If you previously used the standalone `clawk honcho setup`:
...
No re-login or re-setup needed. Run `clawk memory setup` and select "honcho" — the wizard detects your existing config.
