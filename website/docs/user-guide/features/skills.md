...
All skills live in **`~/.clawksis/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.



You can also point Clawksis at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `clawk update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:



**At install time** (applies to the default `~/.clawksis` profile):



```bash

curl -fsSL https://github.com/samuelgradientai-sys/clawksis-agent | bash -s -- --no-skills
...
clawk profile create research --no-skills
...
clawk skills opt-out            # stop future seeding — nothing on disk is touched

clawk skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)

clawk skills opt-in --sync      # undo: remove the marker and re-seed now

```



All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `clawk update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `clawk skills opt-in`) to re-enable.



:::note Safe by default

`clawk skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Clawksis installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Clawksis to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.clawksis/plans/` relative to the active workspace/backend working directory.
...
clawk chat --toolsets skills -q "What skills do you have?"

clawk chat --toolsets skills -q "Show me the axolotl skill"
...
/home/user/.clawksis/cache/chart-q4-2025.png
...
When a missing value is encountered, Clawksis asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `clawk setup` or `~/.clawksis/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `clawk config migrate` prompts for unconfigured settings, and `clawk config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
~/.clawksis/skills/                  # Single source of truth
...
If you maintain skills outside of Clawksis — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Clawksis to scan those directories too.



Add `external_dirs` under the `skills` section in `~/.clawksis/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.clawksis/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.

- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Clawksis process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.

- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.

- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.

- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Clawksis ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
~/.clawksis/skills/               # Local (primary, read-write)
...
Bundles live in **`~/.clawksis/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `clawk bundles list`.
...
clawk bundles create research



# Overwrite an existing bundle

clawk bundles create backend-dev --skill ... --force
...
# Re-scan ~/.clawksis/skill-bundles/ and report changes

clawk bundles reload
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.clawksis/skill-bundles/`.



A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.clawksis/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
clawk skills browse                              # Browse all hub skills (official first)

clawk skills browse --source official            # Browse only official optional skills

clawk skills search kubernetes                   # Search all sources

clawk skills search react --source skills-sh     # Search the skills.sh directory

clawk skills search https://mintlify.com/docs --source well-known

clawk skills inspect openai/skills/k8s           # Preview before installing

clawk skills install openai/skills/k8s           # Install with security scan

clawk skills install official/security/1password

clawk skills install skills-sh/vercel-labs/json-render/json-render-react --force

clawk skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
clawk skills list --source hub                   # List hub-installed skills

clawk skills check                               # Check installed hub skills for upstream updates

clawk skills update                              # Reinstall hub skills with upstream changes when needed

clawk skills audit                               # Re-scan all hub skills for security

clawk skills uninstall k8s                       # Remove a hub skill

clawk skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)

clawk skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits

clawk skills publish skills/my-skill --to github --repo owner/repo

clawk skills snapshot export setup.json          # Export skill config

clawk skills tap add myorg/skills-repo           # Add a custom GitHub source
...
| `official` | `official/security/1password` | Optional skills shipped with Clawksis. |

| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `clawk skills search <query> --source skills-sh`. Clawksis resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Clawksis currently integrates with these skills ecosystems and discovery sources:
...
These are maintained in the Clawksis repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Clawksis can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
clawk skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
Clawksis can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
clawk skills install openai/skills/k8s
...
[Skills Hub](https://github.com/samuelgradientai-sys/clawksis-agent page — instead of a

tag-derived guess. This is generic: any tap that ships the file gets real

categorization, no Clawksis-side changes required.
...
Clawksis supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.
...
Clawksis source id: `claude-marketplace`
...
Clawksis can search and convert agent entries from LobeHub's public catalog into installable Clawksis skills.
...
Clawksis integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Clawksis' browser tools and any browser-automation skills you already have installed.
...
clawk skills search airbnb --source browse-sh

clawk skills inspect browse-sh/airbnb.com/search-listings-ddgioa

clawk skills install browse-sh/airbnb.com/search-listings-ddgioa
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Clawksis fetches the URL, parses the YAML frontmatter, security-scans it, and installs.



- Clawksis source id: `url`
...
clawk skills install https://sharethis.chat/SKILL.md

clawk skills install https://example.com/my-skill/SKILL.md --category productivity
...
clawk skills install https://example.com/SKILL.md --name sharethis-chat
...
Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `clawk skills update` re-fetches from the same URL automatically when you want to refresh.
...
`clawk skills inspect ...` now also surfaces upstream metadata when available:
...
| `builtin` | Ships with Clawksis | Always trusted |
...
clawk skills check          # Report which installed hub skills changed upstream

clawk skills update         # Reinstall only the skills with updates available

clawk skills update react   # Update one specific installed hub skill
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Clawksis users add with `clawk skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.clawk.tags`, `version`, `author`, `platforms`, `metadata.clawk.config`).
...
Clawksis discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Clawksis user can subscribe and install:



```bash

clawk skills tap add my-org/clawk-skills

clawk skills search deploy

clawk skills install my-org/clawk-skills/deploy-runbook
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.clawksis/.hub/taps.json`:
...
The `clawk skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `clawk skills tap list` shows the effective path per tap.
...
clawk skills install owner/repo/skills/my-workflow
...
clawk skills tap list                                # show all configured taps

clawk skills tap add myorg/skills-repo               # add (default path: skills/)

clawk skills tap remove myorg/skills-repo            # remove
...
Taps are stored in `~/.clawksis/.hub/taps.json` (created on demand).



## Bundled skill updates (`clawk skills reset`)



Clawksis ships with a set of bundled skills in `skills/` inside the repo. On install and on every `clawk update`, a sync pass copies those into `~/.clawksis/skills/` and records a manifest at `~/.clawksis/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).



On each sync, Clawksis recomputes the hash of your local copy and compares it to the origin hash:
...
clawk skills reset google-workspace --restore
...
Each profile has its own `.bundled_manifest` under its own `CLAWK_HOME`, so `clawk -p coder skills reset <name>` only affects that profile.
