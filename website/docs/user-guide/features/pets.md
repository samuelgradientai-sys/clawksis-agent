...
Clawksis can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<CLAWK_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Clawksis renders the sprite at full fidelity when your
...
clawk pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `clawk pets list [query] [--limit N]` |
| List installed pets | `clawk pets list --installed` |
| Install a pet | `clawk pets install <slug> [--select] [--force]` |
| Set the active pet | `clawk pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `clawk pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `clawk pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `clawk pets off` |
| Remove an installed pet | `clawk pets remove <slug>` |
| Diagnose setup | `clawk pets doctor` |

`clawk pets show` flags:
...
Beyond installing pre-made pets from the gallery, Clawksis can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `clawk pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `clawk tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `CLAWK_PET_IMAGE_PROVIDER` env var (e.g. `CLAWK_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Clawksis is
...
Run `clawk pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Clawksis uses its own
  profile-scoped `<CLAWK_HOME>/pets/` instead — install through `clawk pets`.
