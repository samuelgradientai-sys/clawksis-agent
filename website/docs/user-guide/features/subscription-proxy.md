...
chat completions — use your Clawksis-managed provider subscription as their
...
| Use case | "Use Clawksis as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Clawksis stores
the refresh token in `~/.clawksis/auth.json` — the same place all Clawksis
...
interface in `clawk_cli/proxy/adapters/`.
...
If you see `not logged in`, run `clawk portal`. If you see
...
`clawk_cli/proxy/adapters/<provider>.py` and registering it in
