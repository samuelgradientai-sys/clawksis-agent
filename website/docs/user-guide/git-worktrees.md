...
description: "Run multiple Clawksiss safely on the same repository using git worktrees and isolated checkouts"
...
Clawksis is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Clawksis so each session has a clean, isolated working directory.



## Why Use Worktrees with Clawksis?



Clawksis treats the **current working directory** as the project root:



- CLI: the directory where you run `clawk` or `clawk chat`

- Messaging gateways: the directory set by `terminal.cwd` in `~/.clawksis/config.yaml`
...
git worktree add ../repo-feature feature/clawk-experiment
...
- A new branch: `feature/clawk-experiment` checked out in that directory



Now you can `cd` into the new worktree and run Clawksis there:
...
# Start Clawksis in the worktree
...
git worktree add ../repo-experiment-a feature/clawk-a

git worktree add ../repo-experiment-b feature/clawk-b
...
Each Clawksis process:



- Works on its own branch (`feature/clawk-a` vs `feature/clawk-b`).
...
- Clawksis checkpoint data under `~/.clawksis/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.
...
- **One worktree per Clawksis experiment**
...
- **Name branches after the experiment**
  ...
  - e.g. `feature/clawk-checkpoints-docs`, `feature/clawk-refactor-tests`.
...
- **Avoid running Clawksis from the bare repo root when using worktrees**

  - Prefer the worktree directories instead, so each agent has a clear scope.



## Using `clawk -w` (Automatic Worktree Mode)



Clawksis has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `clawk/clawk-<hash>`).
...
clawk -w -q "Fix issue #123"

```



For parallel agents, open multiple terminals and run `clawk -w` in each — every invocation gets its own worktree and branch automatically.
...
- Use **git worktrees** to give each Clawksis session its own clean checkout.
