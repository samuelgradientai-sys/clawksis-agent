...
Clawksis can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.
...
clawk chat --checkpoints

```



Or enable globally in `~/.clawksis/config.yaml`:
...
| `clawk checkpoints` | Show total size, project count, per-project breakdown |

| `clawk checkpoints status` | Same as bare `checkpoints` |

| `clawk checkpoints list` | Alias for `status` |

| `clawk checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |

| `clawk checkpoints clear` | Nuke the entire checkpoint base (asks first) |

| `clawk checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |
...
- Clawksis detects when tools are about to **modify files** in your working tree.

- Once per conversation turn (per directory), it:

  - Resolves a reasonable project root for the file.

  - Initialises or reuses the **single shared shadow store** at `~/.clawksis/checkpoints/store/`.

  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/clawksis/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.clawksis/checkpoints/ at startup
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `clawk checkpoints prune` manually.
...
Clawksis responds with a formatted list showing change statistics:
...
Checkpoint base: /home/you/.clawksis/checkpoints
...
Projects:        12
  ...
  /home/you/code/clawksis-agent                                        20       2h ago  live
...
Clear with: clawk checkpoints clear-legacy
...
clawk checkpoints prune --retention-days 3 --max-size-mb 200
...
- **Directory scope** — Clawksis skips overly broad directories (root `/`, home `$HOME`).
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `clawk checkpoints status` / `prune` / `clear` instead.
...
Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.clawksis/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.



On first v2 run, any pre-v2 shadow repos are moved into `~/.clawksis/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:



```bash

clawk checkpoints clear-legacy
...
- **Enable checkpoints only when you need them** — `clawk chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `clawk checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.

- **Combine with Git worktrees** for maximum safety — keep each Clawksis session in its own worktree/branch, with checkpoints as an extra layer.
