...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Clawksis terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Clawksis's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `clawk setup --portal` — and you're ready to `clawk chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Clawksis also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `clawk --tui` — see the [TUI](tui.md) guide.
:::
...
clawk chat --model "anthropic/claude-sonnet-4"

# With a specific provider
clawk chat --provider nous        # Use Nous Portal
clawk chat --provider openrouter  # Force OpenRouter
...
clawk chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
clawk --continue             # Resume the most recent CLI session (-c)
clawk --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
clawk chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
clawk -w                         # Interactive mode in worktree
clawk -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Clawksis CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Clawksis CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `CLAWK_YOLO_MODE` is on (either `clawk --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`clawk -c` or `clawk --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Clawksis to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Clawksis supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Clawksis replies |
...
```bash
clawk -s clawksis-agent-dev,github-auth
clawk chat -s github-pr-workflow -s github-auth
```

Clawksis loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.clawksis/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.clawksis/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Clawksis..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Clawksis recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Clawksis — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
Resume this session with:
  clawk --resume 20260225_143052_a1b2c3
...
```bash
clawk --continue                          # Resume the most recent CLI session
clawk -c                                  # Short form
clawk -c "my project"                     # Resume a named session (latest in lineage)
clawk --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
clawk --resume "refactoring auth"         # Resume by title
clawk -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `clawk sessions rename <id> <title>` from the command line. Use `clawk sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Clawksis's SQLite state database under `~/.clawksis/state.db`. The database keeps:
...
Clawksis immediately confirms the task and gives you back the prompt:
