...
A **profile distribution** packages a complete Clawksis — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.
...
Before distributions, sharing a Clawksis meant sending someone:
...
clawk profile install github.com/you/my-research-agent --alias

```



…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `clawk profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Clawksis in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`clawk profile export` / `import`](../reference/profile-commands.md#clawk-profile-export) — that's what those are for.
...
clawk profile create research-bot

research-bot setup                    # configure model, API keys

# Edit ~/.clawksis/profiles/research-bot/SOUL.md
...
Create `~/.clawksis/profiles/research-bot/distribution.yaml`:
...
cd ~/.clawksis/profiles/research-bot
...
Recipients who run `clawk profile update research-bot` will pull the latest.
...
clawk profile install github.com/you/research-bot --alias
...
5. Copies distribution-owned files into `~/.clawksis/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
clawk profile install github.com/you/research-bot



# Full HTTPS

clawk profile install https://github.com/you/research-bot.git



# SSH

clawk profile install git@github.com:you/research-bot.git



# Self-hosted, GitLab, Gitea, Forgejo — any Git host

clawk profile install https://git.example.com/team/research-bot.git



# Private repo using your configured git auth

clawk profile install git@github.com:your-org/internal-bot.git



# Local directory during development (no git push needed)

clawk profile install ~/my-profile-in-progress/
...
clawk profile install github.com/acme/support-bot --name support-us --alias

# Bob (same distribution, different local name)

clawk profile install github.com/acme/support-bot --name support-eu --alias
...
# Environment variables required by this Clawksis distribution.
...
cp ~/.clawksis/profiles/research-bot/.env.EXAMPLE ~/.clawksis/profiles/research-bot/.env
...
clawk profile info research-bot
...
`clawk profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
clawk profile update research-bot
...
clawk profile delete research-bot
...
Path:    ~/.clawksis/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `clawk profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.clawksis/profiles/pr-reviewer
...
clawk profile install git@github.com:your-org/pr-reviewer.git --alias
...
When the lead ships v1.1 (better SOUL, new skill), engineers run `clawk profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.clawksis/profiles/polymarket-trader
...
git remote add origin https://github.com/you/clawk-polymarket-trader.git
...
clawk profile install github.com/you/clawk-polymarket-trader --alias
...
You built Clawksis-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
clawk profile install git@github.com:your-org/incident-2026-q2.git --alias



# Incident resolved — tear it down

clawk profile delete incident-2026-q2
...
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `clawk profile info <name>` and hold off on updates until you're ready.
...
clawk profile info research-bot | grep Version
...
# ~/.clawksis/profiles/research-bot/local/my-overrides.yaml
...
clawk profile delete research-bot --yes
...
clawk profile update research-bot --force-config --yes
...
clawk profile install github.com/yourname/forked-research-bot --alias



# Iterate locally in ~/.clawksis/profiles/forked-research-bot/
...
clawk profile install ~/.clawksis/profiles/research-bot --name research-bot-test --alias



# Tweak, delete, re-install until it's right

clawk profile delete research-bot-test --yes

clawk profile install ~/.clawksis/profiles/research-bot --name research-bot-test
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `clawk -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `clawk profile` — not a parallel command tree.
...
- Reserved profile names (`clawk`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`clawk profile export` / `import`](../reference/profile-commands.md#clawk-profile-export) — local backup / restore (not distribution)

- [Using SOUL with Clawksis](../guides/use-soul-with-clawk.md) — authoring personalities
