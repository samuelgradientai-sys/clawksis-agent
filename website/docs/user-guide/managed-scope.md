...
`~/.clawksis/config.yaml`, `~/.clawksis/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/clawk`:

```text
/etc/clawk/
├── config.yaml     # managed config layer (wins over ~/.clawksis/config.yaml)
└── .env            # managed env layer (wins over ~/.clawksis/.env + shell)
```
...
The location can be relocated with the `CLAWK_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `CLAWK_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Clawksis.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export CLAWK_MANAGED_DIR=/opt/org/clawk-policy
```

:::warning
A user who can set `CLAWK_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `clawk doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/clawk/config.yaml` (managed) | `/etc/clawk/.env` (managed) |
| 2 | `~/.clawksis/config.yaml` (user) | `~/.clawksis/.env` (user) |
...
```bash
clawk config        # shows a header naming the managed source + the pinned keys
clawk doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Clawksis refuses and names the source:

```bash
$ clawk config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/clawk/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `clawk config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/clawk

# Pin some config values for every user on this machine
sudo tee /etc/clawk/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/clawk/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/clawk
sudo chmod 0644 /etc/clawk/config.yaml /etc/clawk/.env
```

Changes take effect on the next Clawksis start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`clawk doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Clawksis as `root`), managed scope is advisory.
