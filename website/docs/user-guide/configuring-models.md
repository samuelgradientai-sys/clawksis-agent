...
Every auxiliary task defaults to `auto` — meaning Clawksis uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
`provider: auto` with `model: ''` tells Clawksis to use the main model for that task.
...
`clawk model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.clawksis/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `clawk config show | grep '^model\.'` and `clawk status`.
