...
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `clawk update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.clawksis`), use `clawk skills opt-out` / `clawk skills opt-in`. |
...
clawk profile create work2 --clone-from work

# Clone everything from a specific profile
clawk profile create work2-backup --clone-from work --clone-all
```
