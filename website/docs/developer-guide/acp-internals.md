...
The ACP adapter wraps Clawksis' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
clawk acp / clawk-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'clawksis-agent[acp]==<version>' clawk-acp`, pointed at the `clawksis-agent` PyPI release.
...
- `allow_once` -> Clawksis `once`

- `allow_always` -> Clawksis `always`

- reject options -> Clawksis `deny`
...
`acp_adapter/tools.py` maps Clawksis tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  ...
  -> create AIAgent(platform="acp", enabled_toolsets=["clawk-acp"])
...
Instead it reuses Clawksis' runtime resolver:



- `acp_adapter/auth.py`

- `clawk_cli/runtime_provider.py`



So ACP advertises and uses the currently configured Clawksis provider/credentials. It also always advertises a terminal setup auth method (`clawk-setup`, args `--setup`) so first-run registry clients can open Clawksis' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.clawksis/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `clawk-acp` toolset definition

- `clawk_cli/main.py` — `clawk acp` CLI subcommand

- `pyproject.toml` — `[acp]` optional dependency + `clawk-acp` script
