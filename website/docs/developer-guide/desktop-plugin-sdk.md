...
title: "Desktop Plugin SDK (@clawk/plugin-sdk)"
description: "Extend the native Clawksis Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Clawksis Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `ClawksisPlugin`.
It imports one module — `@clawk/plugin-sdk` — and gets everything: the app's
...
`$CLAWK_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Clawksis. This page is the **native
desktop app** (`clawk desktop`) SDK — the `@clawk/plugin-sdk` module and
`$CLAWK_HOME/desktop-plugins/`. The **web dashboard** (`clawk dashboard`) has
its own, unrelated plugin system on `window.__CLAWK_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Clawksis Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$CLAWK_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `ClawksisPlugin` contract, appear in **Settings → Plugins**, and
...
Create `$CLAWK_HOME/desktop-plugins/hello/plugin.js` (that's `~/.clawksis/...`
by default, or `~/.clawksis/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.clawksis/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@clawk/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Clawksis' }),
      jsx('div', {
...
The only importable specifiers are `@clawk/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `ClawksisPlugin`:

```ts
interface ClawksisPlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `clawk.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@clawk/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@clawk/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@clawk/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@clawk/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@clawk/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@clawk/plugin-sdk'
...
```javascript
import { queryClient } from '@clawk/plugin-sdk'
...
`dashboard/` subfolder of a regular Clawksis plugin and declare it in a
`manifest.json`:
...
clawksis-agent codebase directly (`clawk_state`, `clawk_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.clawk/`)
...
(`clawk.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `ClawksisPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@clawk/plugin-sdk` alias.
2. It's still lint-fenced to `@clawk/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@clawk/plugin-sdk`, `react`,
...
| Plugin contract | `ClawksisPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `clawk-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`clawk-desktop-plugins`** skill — it carries the same contract as this page in
...
`$CLAWK_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `clawk logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@clawk/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.clawksis/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.clawksis/logs/errors.log` for
