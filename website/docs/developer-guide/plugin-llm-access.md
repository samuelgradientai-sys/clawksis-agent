...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Clawksis already has applies. The plugin never sees a
...
[`clawk-example-plugins`](https://github.com/NousResearch/clawk-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
clawksis-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/clawk-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Clawksis provider
        # (must be one Clawksis already knows about — same names as
        # `clawk model` and config.yaml model.provider).
        ...
        # plugin sends — Clawksis does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.clawksis/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Clawksis' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Clawksis subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/samuelgradientai-sys/clawksis-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/samuelgradientai-sys/clawksis-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/clawk-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/clawk-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
