...
description: "How Clawksis detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
...
The CDP supervisor closes two long-standing gaps in Clawksis' browser tooling:
...
(`clawk-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Clawksis `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `clawk-acp`, `clawk-api-server`, and core toolsets (gated on CDP reachability)

- `clawk_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
