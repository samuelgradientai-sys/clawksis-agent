...
| Variable | Meaning |
|----------|---------|
| `CLAWK_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `CLAWK_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `CLAWK_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Clawksis itself — they're private to these helpers. The variables Clawksis *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `clawk --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_clawk_root)" || { echo "htui: not in a Clawksis checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$CLAWK_MAIN_CHECKOUT/.venv/bin/python" -m clawk_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `CLAWK_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_clawk_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `CLAWK_TUI_DIR` are mutually exclusive
`CLAWK_TUI_DIR` points Clawksis at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `clawk --tui --dev` exits with an error. Run `unset CLAWK_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_clawk_root)" || { echo "hgui: not in a Clawksis checkout" >&2; return 1; }
  deps="${CLAWK_GUI_DEPS_CHECKOUT:-$CLAWK_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _clawk_link_deps "$desktop" "$deps/apps/desktop"
    _clawk_link_deps "$root" "$deps"
  else
  ...
  trap '_clawk_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    CLAWK_DESKTOP_CLAWK_ROOT="$root" \
    CLAWK_DESKTOP_PYTHON="$CLAWK_MAIN_CHECKOUT/.venv/bin/python" \
    CLAWK_DESKTOP_IGNORE_EXISTING=1 \
    CLAWK_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `CLAWK_DESKTOP_CLAWK_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `clawk`. |
| `CLAWK_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `CLAWK_DESKTOP_IGNORE_EXISTING` | Ignores any `clawk` on `PATH` so it can't shadow the worktree. |
| `CLAWK_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Clawksis checkout.
_clawk_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/clawk_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_clawk_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'clawk_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `clawk --tui --dev` and the `CLAWK_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/samuelgradientai-sys/clawksis-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `CLAWK_*` variable Clawksis reads
