...
description: "How Clawksis stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
...
| `clawk_cli/cron.py` | CLI `clawk cron` subcommands |
...
Jobs are stored in `~/.clawksis/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `clawk cron` commands are run or during active CLI sessions.
...
2. **Environment variable** — `CLAWK_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.clawksis/cron/output/` |
...
By default (`cron.wrap_response: true`), cron deliveries are prefixed with a small `⏰ <job name>` tag so the recipient can tell the message came from a scheduled task. The agent's output then follows directly — no job id, divider, or footer. Set `cron.wrap_response: false` to deliver the raw output with no tag.
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `clawk cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.
...
The `clawk cron` CLI provides direct job management:



```bash

clawk cron list                    # Show all jobs

clawk cron create                  # Interactive job creation (alias: add)

clawk cron edit <job_id>           # Edit job configuration

clawk cron pause <job_id>          # Pause a running job

clawk cron resume <job_id>         # Resume a paused job

clawk cron run <job_id>            # Trigger immediate execution

clawk cron remove <job_id>         # Delete a job
