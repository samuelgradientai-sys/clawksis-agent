...
Clawksis uses a SQLite database (`~/.clawksis/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `clawk_state.py`


## Architecture Overview

```
~/.clawksis/state.db (SQLite, WAL mode)
...
Multiple clawk processes (gateway + CLI sessions + worktree agents) share one
...
Default path: `~/.clawksis/state.db`

This is derived from `clawk_constants.get_clawk_home()` which resolves to
`~/.clawksis/` by default, or the value of `CLAWK_HOME` environment variable.
