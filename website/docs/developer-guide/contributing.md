...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Clawksis — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Clawksis! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Clawksis core? Start with [Build a Clawksis Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Clawksis itself? Start with [Adding Tools](./adding-tools.md)
...
The installer creates the Clawksis venv, wires the `clawk` command, stamps the
install method for `clawk update`, and clones the full git project into
`$CLAWK_HOME/clawksis-agent` (usually `~/.clawksis/clawksis-agent`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://clawksis-agent.nousresearch.com/install.sh | bash
cd "${CLAWK_HOME:-$HOME/.clawk}/clawksis-agent"
...
You can also run a fully isolated Clawksis instance (throwaway CLAWK_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m clawk_cli.main
scripts/dev-sandbox.sh --persistent python -m clawk_cli.main desktop  # state survives restarts, but lives in the worktree :)
```

### Manual clone fallback

Use this only if you intentionally do not want Clawksis' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `clawk` entrypoint from this venv; running the
system `python3 -m clawk_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.clawksis/venvs/clawk-dev --python 3.11
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.clawksis/.env
```

### Run

```bash
# The standard installer already put `clawk` on PATH.
clawk doctor
clawk chat -q "Hello"
```

If you used the manual clone fallback, run `./clawk` from the checkout or
...
- **Profile-safe paths**: Never hardcode `~/.clawksis` — use `get_clawk_home()` from `clawk_constants` for code paths and `display_clawk_home()` for user-facing messages. See [AGENTS.md](https://github.com/samuelgradientai-sys/clawksis-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/samuelgradientai-sys/clawksis-agent/blob/main/LICENSE).
