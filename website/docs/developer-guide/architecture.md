...
sidebar_position: 1
title: "Architecture"
description: "Clawksis internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Clawksis internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ clawk_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ClawksisCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── clawk_state.py           # SQLite session/state database with FTS5
├── clawk_constants.py       # CLAWK_HOME, profile-aware paths
...
│
├── clawk_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `clawk` subcommands (large file)
...
│   ├── skills_config.py      # clawk skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # clawk tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # clawk gateway start/stop
│
...
```text
User input → ClawksisCLI.process_input()
...
Three discovery sources: `~/.clawksis/plugins/` (user), `.clawksis/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `clawk plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-clawk-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Clawksis as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`clawk -p <name>`) gets its own CLAWK_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
