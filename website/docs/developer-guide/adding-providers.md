...
description: "How to add a new inference provider to Clawksis — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Clawksis can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `clawk model` menu entries
...
1. `clawk_cli/auth.py` decides how credentials are found.
2. `clawk_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `clawk_cli/models.py` and `clawk_cli/main.py` make the provider show up in the CLI. (`clawk_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `clawk_cli/auth.py`
2. `clawk_cli/models.py`
3. `clawk_cli/runtime_provider.py`
4. `clawk_cli/main.py`
...
:::tip
`clawk_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `clawk setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$CLAWK_HOME/plugins/model-providers/` both get picked up.
...
7. `clawk model` menu includes the provider
8. `clawk setup` wizard delegates to `main.py` automatically
...
User plugins at `$CLAWK_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `clawk model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `clawk_cli/auth.py`
- `_PROVIDER_LABELS` in `clawk_cli/models.py`
- `_PROVIDER_ALIASES` in both `clawk_cli/auth.py` and `clawk_cli/models.py`
- CLI `--provider` choices in `clawk_cli/main.py`
...
## Step 2: Add auth metadata in `clawk_cli/auth.py`
...
- What env vars should Clawksis check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `clawk_cli/models.py`
...
## Step 4: Resolve runtime data in `clawk_cli/runtime_provider.py`
...
Be careful with API-key precedence. Clawksis already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `clawk_cli/main.py`

A provider is not discoverable until it shows up in the interactive `clawk model` flow.

Update these in `clawk_cli/main.py`:
...
:::tip
`clawk_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `clawk model` and `clawk setup` automatically.
:::
...
When you add a native provider, double-check that Clawksis is only sending fields that provider actually understands.
...
- `tests/clawk_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/clawk_cli/test_model_switch_custom_providers.py` (and adjacent `tests/clawk_cli/test_model_switch_*.py`)
- `tests/clawk_cli/test_setup_model_provider.py`
...
python -m pytest tests/clawk_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/clawk_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m clawk_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `clawk_cli/auth.py`
- [ ] aliases added in `clawk_cli/auth.py` and `clawk_cli/models.py`
- [ ] model catalog added in `clawk_cli/models.py`
- [ ] runtime branch added in `clawk_cli/runtime_provider.py`
- [ ] CLI wiring added in `clawk_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `clawk model` but not `clawk setup`
