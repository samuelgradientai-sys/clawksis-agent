...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Clawksis — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Clawksis setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Clawksis Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Clawksis Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Clawksis for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Clawksis working on my machine | `clawk setup` | Run a real chat and verify it responds |
| I already know my provider | `clawk model` | Save the config, then start chatting |
| I want a bot or always-on setup | `clawk gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `clawk model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `clawk model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Clawksis cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
```bash
pip install clawksis-agent
clawk postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
- **Desktop downloads:** [GitHub Releases](https://github.com/samuelgradientai-sys/clawksis-agent/releases/latest)
...
The single most important setup step. Use `clawk model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `clawk model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `clawk model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `clawk model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `clawk_cli/models.py` may change between releases) | `clawk model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `clawk model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `clawk model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `clawk model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `clawk model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `clawk model` (requires `copilot` CLI + `copilot login`) |
...
Clawksis requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `clawk model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Clawksis separates secrets from normal config:

- **Secrets and tokens** → `~/.clawksis/.env`
- **Non-secret settings** → `~/.clawksis/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
clawk config set model anthropic/claude-opus-4.6
clawk config set terminal.backend docker
clawk config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
clawk            # classic CLI
clawk --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Clawksis ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `clawk` vs `clawk --tui`.
:::
...
- Clawksis replies without error
...
```bash
clawk --continue    # Resume the most recent session
clawk -c            # Short form
```
...
```bash
clawk gateway setup    # Interactive platform configuration
```
...
- `clawk tools` — tune tool access per platform
- `clawk skills` — browse and install reusable workflows
...
```bash
clawk config set terminal.backend docker    # Docker isolation
clawk config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Clawksis install directory (the curl installer placed it at
# ~/.clawksis/clawksis-agent on Linux/macOS or %LOCALAPPDATA%\clawk\clawksis-agent on Windows):
...
Skills are on-demand instruction documents that teach Clawksis how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Clawksis ships with a catalog of bundled skills already installed in `~/.clawksis/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
clawk skills browse                      # list everything available
clawk skills search kubernetes           # find skills by keyword
clawk skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `clawk skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Clawksis ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.clawksis/clawksis-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Clawksis opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `clawk model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `clawk gateway setup` and check `clawk gateway status` |
| `clawk --continue` can't find old session | Switched profiles or session never saved | Check `clawk sessions list` and confirm you're in the right profile |
...
| `clawk doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `clawk model`
3. `clawk setup`
4. `clawk sessions list`
5. `clawk --continue`
6. `clawk gateway status`
...
| `clawk tools` | Configure which tools are enabled per platform |
| `clawk setup` | Full setup wizard (configures everything at once) |
| `clawk doctor` | Diagnose issues |
| `clawk update` | Update to latest version |
| `clawk gateway` | Start the messaging gateway |
| `clawk --continue` | Resume last session |
