...
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Clawksis — no API key required"
...
Clawksis supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Clawksis automatically refreshes your session.
...
# → Clawksis opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.clawksis/auth.json` and are refreshed automatically before each session.
...
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.clawksis/.env
...
clawk auth add minimax-oauth --no-browser

```



Clawksis will print the verification URL and user code — open the URL on any device and enter the code when prompted.
...
Clawksis implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:



1. Clawksis generates a PKCE verifier / challenge pair and a random state value.
...
4. Clawksis polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).

5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.clawksis/auth.json` under the `minimax-oauth` key.
...
clawk config set model.default MiniMax-M2.7

clawk config set model.provider minimax-oauth
...
After login, `~/.clawksis/config.yaml` will contain entries similar to:
...
clawk --provider minimax-oauth    # canonical

clawk --provider minimax-portal   # alias

clawk --provider minimax-global   # alias

clawk --provider minimax_oauth    # alias (underscore form)
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `clawk setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:



```bash

clawk --provider minimax-oauth
...
Clawksis refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Clawksis marks the session as requiring re-login.



When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Clawksis marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.



**Fix:** run `clawk auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.
...
The device-code flow has a finite expiry window. If you don't approve the login in time, Clawksis raises a timeout error.



**Fix:** re-run `clawk auth add minimax-oauth` (or `clawk model`). The flow starts fresh.
...
Clawksis detected that the `state` value returned by the authorization server does not match what it sent.
...
If `clawk` cannot open a browser window, use `--no-browser`:
...
Clawksis prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `clawk model` and select MiniMax (OAuth), or run `clawk auth add minimax-oauth`.
...
clawk auth remove minimax-oauth
...
- [clawk doctor](../reference/cli-commands.md)
