...
title: "Run Clawksis with Nous Portal"
...
- Clawksis installed ([Quickstart](/getting-started/quickstart))
...
3. Sets `model.provider: nous` in `~/.clawksis/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Clawksis is running. Two options:
...
clawk setup --portal                              # on the remote, open the printed URL in your local browser



# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)

clawk auth add nous --type oauth --manual-paste

# Then re-run `clawk setup --portal` to wire the provider + gateway
...
Hey, search the web for "Clawksis release notes" and summarize the top 3 hits.

```



You should see Clawksis call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.
...
`clawk setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
clawk config set model.default anthropic/claude-sonnet-4.6
...
The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Clawksis-side opinion.
...
These rows appear in `clawk tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Clawksis runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Clawksis will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
clawk cron create "every day at 9am" \
...
If you use [Clawksis profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.



For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.clawksis/auth.json` → no token sharing across users. This is the right boundary.
...
### `clawk portal info` shows "not logged in" after `clawk setup --portal`
...
clawk config set model.provider nous
...
Re-verify with `clawk portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Clawksis doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/samuelgradientai-sys/clawksis-agent/issues) — most gaps are routing config we can update.
...
`clawk portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Clawksis profiles where you're using the wrong one (check `clawk profile current`)
...
clawk auth remove nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Clawksis tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Clawksis configurations
