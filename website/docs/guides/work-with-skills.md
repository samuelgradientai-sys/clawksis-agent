...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Clawksis new workflows"
...
Skills are on-demand knowledge documents that teach Clawksis how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Clawksis installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Clawksis to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Clawksis but aren't active by default. Install them explicitly:
...
clawk skills install official/research/arxiv
...
1. The skill directory is copied to `~/.clawksis/skills/`
...
For how to ship skills in your own plugin, see [Build a Clawksis Plugin → Bundle skills](/guides/build-a-clawk-plugin#bundle-skills).
...
When a skill with config is first loaded, Clawksis prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.
...
clawk skills config gif-search



# View all skill config

clawk config show | grep '^skills\.config'
...
mkdir -p ~/.clawksis/skills/my-category/my-skill
...
```markdown title="~/.clawksis/skills/my-category/my-skill/SKILL.md"
...
clawk chat -q "/my-skill help me with the thing"

```



The skill appears automatically — no registration needed. Drop it in `~/.clawksis/skills/` and it's live.



:::info

The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Clawksis may offer to save the approach as a skill for next time.
...
**Let the agent create skills.** After a complex multi-step task, Clawksis will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.



**Use categories.** Organize skills into subdirectories (`~/.clawksis/skills/devops/`, `~/.clawksis/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.



**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Clawksis to update the skill with what you learned. Skills that aren't maintained become liabilities.
