...
Clawksis calls this **no-agent mode**. It's the cron system minus the LLM.
...
- **Bash or Python.** `.sh` / `.bash` files run under `/bin/bash`; any other extension runs under the current Python interpreter. Anything in `~/.clawksis/scripts/` is accepted.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Clawksis writes the script, schedules it, and tells you when it'll fire.
...
> **Clawksis:** *(writes `~/.clawksis/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
...
> **Clawksis:** *(calls `cronjob(action='pause', job_id='abc123')`)*

>

> Paused. Resume with "turn it back on" or via `clawk cron resume abc123`.
...
> **Clawksis:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
cat > ~/.clawksis/scripts/memory-watchdog.sh <<'EOF'
...
chmod +x ~/.clawksis/scripts/memory-watchdog.sh
...
clawk cron run <job_id>    # fire it once to test
...
Scripts must live in `~/.clawksis/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
clawk cron create "every 5m"        # interval

clawk cron create "every 2h"

clawk cron create "0 9 * * *"       # standard cron: 9am daily

clawk cron create "30m"             # one-shot: run once in 30 minutes
...
--deliver local                          # just save to ~/.clawksis/cron/output/

```



No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.clawksis/.env` / `~/.clawksis/config.yaml`.
...
clawk cron list                                    # see all jobs

clawk cron pause <job_id>                          # stop firing, keep definition

clawk cron resume <job_id>

clawk cron edit <job_id> --schedule "every 10m"    # adjust cadence

clawk cron edit <job_id> --agent                   # flip to LLM mode

clawk cron edit <job_id> --no-agent --script …     # flip back

clawk cron remove <job_id>                         # delete it
...
cat > ~/.clawksis/scripts/disk-alert.sh <<'EOF'
...
chmod +x ~/.clawksis/scripts/disk-alert.sh



clawk cron create "*/15 * * * *" \
...
| `cronjob --no-agent` (this page) | Your script on Clawksis' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |

| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |

| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When Clawksis might be unhealthy (the thing you're monitoring) |



For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Clawksis webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Clawksis being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
