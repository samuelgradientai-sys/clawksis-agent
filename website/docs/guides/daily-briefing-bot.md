...
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `clawk setup --portal`. See [Nous Portal](/integrations/nous-portal).
...
2. **Clawksis spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.clawksis/cron/output/` and you can read them anytime.
...
Clawksis will search the web, read through results, and produce something like:
...
3 stories • Sources searched: 8 • Generated by Clawksis
...
Before creating cron jobs, ensure Clawksis has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.
...
Just tell Clawksis what you want:
...
Clawksis will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Clawksis to delegate each topic to a sub-agent:
...
Clawksis will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
