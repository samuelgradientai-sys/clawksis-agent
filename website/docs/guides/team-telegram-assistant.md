...
This tutorial walks you through setting up a Telegram bot powered by Clawksis that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Clawksis installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.

- **A Telegram account** for yourself (the bot owner)

- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.clawksis/.env`



:::tip

A $5/month VPS is plenty for running the gateway. Clawksis itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
...
2. **Send `/newbot`** — BotFather will ask you two things:
   ...
   - **Display name** — what users see (e.g., `Team Clawksis Assistant`)

   - **Username** — must end in `bot` (e.g., `myteam_clawk_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   Team AI assistant powered by Clawksis. DM me for help with code, research, debugging, and more.
...
Add these lines to `~/.clawksis/.env`:
...
[Gateway] Starting Clawksis Gateway...
...
journalctl --user -u clawk-gateway -f
...
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `clawk gateway install` to update the plist.
...
clawk gateway stop && clawk gateway start
...
clawk pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.clawksis/.env`:
...
Control how much detail the bot shows when using tools. In `~/.clawksis/config.yaml`:
...
Customize how the bot communicates by editing `~/.clawksis/SOUL.md`:



For a full guide, see [Use SOUL.md with Clawksis](/guides/use-soul-with-clawk).
...
clawk cron list          # View all scheduled jobs

clawk cron status        # Check if scheduler is running
...
| Gateway logs | `journalctl --user -u clawk-gateway` (Linux) or `~/.clawksis/logs/gateway.log` (macOS) |

| Cron job output | `~/.clawksis/cron/output/{job_id}/{timestamp}.md` |

| Cron job definitions | `~/.clawksis/cron/jobs.json` |

| Pairing data | `~/.clawksis/pairing/` |

| Session history | `~/.clawksis/sessions/` |
