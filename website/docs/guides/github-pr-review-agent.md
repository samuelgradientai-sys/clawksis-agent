...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Clawksis  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Clawksis instantly when PRs are opened or updated.
:::
...
- **Clawksis installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  clawk gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.clawksis/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Clawksis can access GitHub. Start a chat:
...
```
Run: gh pr list --repo samuelgradientai-sys/clawksis-agent --state open --limit 3
```
...
Still in the chat, ask Clawksis to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo samuelgradientai-sys/clawksis-agent
```
...
A skill gives Clawksis consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.clawksis/skills/code-review
```

Create `~/.clawksis/skills/code-review/SKILL.md`:
...
Verify it loaded — start `clawk` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Clawksis your team's standards:
...
2. Teach Clawksis your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
clawk gateway status    # Is the gateway running?
clawk cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-clawk-plugin)** — wrap the review logic into a shareable plugin
