...
clawk teams-pipeline token-health

clawk teams-pipeline token-health --force-refresh
...
clawk teams-pipeline subscriptions
...
clawk teams-pipeline maintain-subscriptions

clawk teams-pipeline maintain-subscriptions --dry-run
...
#### Option 1: Clawksis cron (recommended if you already run the Clawksis gateway)



Clawksis ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.clawksis/scripts/`. First create the script:
...
cat > ~/.clawksis/scripts/maintain-teams-subscriptions.sh <<'EOF'

#!/usr/bin/env bash

exec clawk teams-pipeline maintain-subscriptions

EOF

chmod +x ~/.clawksis/scripts/maintain-teams-subscriptions.sh
...
Create `/etc/systemd/system/clawk-teams-pipeline-maintain.service`:
...
Description=Clawksis Teams pipeline subscription maintenance
...
EnvironmentFile=/etc/clawksis/env

ExecStart=/usr/local/bin/clawk teams-pipeline maintain-subscriptions

```



And `/etc/systemd/system/clawk-teams-pipeline-maintain.timer`:
...
Description=Run Clawksis Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now clawk-teams-pipeline-maintain.timer

systemctl list-timers clawk-teams-pipeline-maintain.timer
...
0 */12 * * * /usr/local/bin/clawk teams-pipeline maintain-subscriptions >> /var/log/clawksis/teams-pipeline-maintain.log 2>&1

```



Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.clawksis/.env` at the top of a wrapper script that crontab calls.
...
clawk teams-pipeline subscriptions   # should show expirationDateTime advanced

clawk teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
...
clawk teams-pipeline list

clawk teams-pipeline list --status failed

clawk teams-pipeline show <job-id>
...
clawk teams-pipeline run <job-id>
...
clawk teams-pipeline fetch --meeting-id <meeting-id>

clawk teams-pipeline fetch --join-web-url "<join-url>"
...
- run `clawk teams-pipeline maintain-subscriptions --dry-run`

- inspect `clawk teams-pipeline list --status failed`
...
- run `clawk teams-pipeline validate`
...
- whether you manually replayed a job with `clawk teams-pipeline run`
...
- [ ] `clawk teams-pipeline validate` returns an OK snapshot

- [ ] `clawk teams-pipeline token-health --force-refresh` succeeds

- [ ] **`maintain-subscriptions` is scheduled** (Clawksis cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
