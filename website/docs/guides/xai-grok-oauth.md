...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Clawksis — no API key required"
...
Clawksis supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Clawksis automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Clawksis — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Clawksis opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.clawksis/auth.json` and refreshed automatically before they expire.
...
On servers, containers, or SSH sessions where no browser is available, Clawksis detects the remote environment and prints the authorization URL instead of opening a browser.
...
clawk auth add xai-oauth --no-browser
...
If you don't have a regular SSH client (e.g. you're running Clawksis inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Clawksis skips the loopback listener and lets you paste the failed callback URL straight from your browser:



```bash

clawk auth add xai-oauth --manual-paste

# Or via the model picker:

clawk model --manual-paste

```



See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/samuelgradientai-sys/clawksis-agent/issues/26923).



If the consent page renders the authorization code directly on the page (xAI's current behavior on browser-based consoles) instead of redirecting to your `127.0.0.1:56121/callback`, paste **just the bare code value** at the `Callback URL:` prompt — Clawksis accepts the full URL, a bare `?code=...&state=...` query fragment, or a bare code interchangeably.
...
1. Clawksis opens your browser to `accounts.x.ai`.

2. You sign in (or confirm your existing session) and approve access.

3. xAI redirects back to Clawksis and the tokens are saved to `~/.clawksis/auth.json`.

4. From then on, Clawksis refreshes the access token in the background — you stay signed in until you `clawk auth remove xai-oauth` or revoke access from your xAI account settings.
...
clawk config set model.default grok-4.3

clawk config set model.provider xai-oauth
...
After login, `~/.clawksis/config.yaml` will contain:
...
clawk --provider xai-oauth        # canonical

clawk --provider grok-oauth       # alias

clawk --provider x-ai-oauth       # alias

clawk --provider xai-grok-oauth   # alias
...
The `video_gen` toolset is disabled by default. Enable it in `clawk tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `clawk setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.
...
Clawksis refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Clawksis surfaces a typed re-auth message instead of crashing.



When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Clawksis marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.



**Fix:** run `clawk auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.
...
The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Clawksis raises a timeout error.



**Fix:** re-run `clawk auth add xai-oauth` (or `clawk model`). The flow starts fresh.
...
Clawksis detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Clawksis prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `clawk model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/samuelgradientai-sys/clawksis-agent/issues/26847)).
...
clawk config set model.provider xai
...
**Fix:** run `clawk model` and pick the xAI Grok OAuth provider, or run `clawk auth add xai-oauth`.
...
clawk auth logout xai-oauth

```



This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `clawk auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `clawk auth list xai-oauth` to see them).
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Clawksis is on a different machine than your browser
