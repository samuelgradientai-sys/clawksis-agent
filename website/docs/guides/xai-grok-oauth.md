...
# → Clawksis opens or prints an accounts.x.ai verification URL
...
On servers, containers, browser-only consoles (Cloud Shell, Codespaces, EC2 Instance Connect), or SSH sessions where Clawksis cannot open a browser locally, Clawksis prints the xAI verification URL and user code. Open the URL in any browser on your laptop or in the cloud console, enter the code if prompted, and Clawksis will keep polling until xAI approves the login. No SSH tunnel or local callback listener is required.
...
The same device-code flow applies when you sign in from the web dashboard or the desktop app: Clawksis shows the verification URL and user code, then polls in the background until you approve access.

## How the Login Works

1. Clawksis requests a device code from `auth.x.ai`.
2. You open the verification URL, sign in, enter the displayed code if prompted, and approve access.
3. Clawksis polls xAI until approval, then saves tokens to `~/.clawksis/auth.json`.
4. From then on, Clawksis refreshes the access token in the background — you stay signed in until you `clawk auth logout xai-oauth` or revoke access from your xAI account settings.
...
```bash
clawk config set model.default grok-build-0.1
...
Device-code approval has a finite expiry window (xAI sets `expires_in` on the device-code response, typically on the order of tens of minutes). If you do not approve the login in time, Clawksis raises a timeout error.

**Fix:** re-run `clawk auth add xai-oauth` (or `clawk model`). The flow starts fresh.

### Logging in from a remote server

On SSH or container sessions Clawksis prints the verification URL and user code instead of opening a browser. Open that URL in a browser on your laptop or in a cloud console — no SSH port forward is needed for xAI Grok OAuth.
