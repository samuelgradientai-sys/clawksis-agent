...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Clawksis — what gets migrated, how config maps, and what to check after."
...
`clawk claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Clawksis. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.



:::tip

If your OpenClaw setup was multi-provider, `clawk setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
...
clawk claw migrate --preset full --migrate-secrets --yes
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.clawksis/` (by default a single restore-point archive is written before apply, under `~/.clawksis/backups/pre-migration-*.zip`; restorable with `clawk import`). |
...
| What | OpenClaw source | Clawksis destination | Notes |

|------|----------------|-------------------|-------|

| Persona | `workspace/SOUL.md` | `~/.clawksis/SOUL.md` | Direct copy |

| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |

| Long-term memory | `workspace/MEMORY.md` | `~/.clawksis/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |

| User profile | `workspace/USER.md` | `~/.clawksis/memories/USER.md` | Same entry-merge logic as memory. |

| Daily memory files | `workspace/memory/*.md` | `~/.clawksis/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Clawksis destination |

|--------|------------------|-------------------|

| Workspace skills | `workspace/skills/` | `~/.clawksis/skills/openclaw-imports/` |

| Managed/shared skills | `~/.openclaw/skills/` | `~/.clawksis/skills/openclaw-imports/` |

| Personal cross-project | `~/.agents/skills/` | `~/.clawksis/skills/openclaw-imports/` |

| Project-level shared | `workspace/.agents/skills/` | `~/.clawksis/skills/openclaw-imports/` |



Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Clawksis skill, `overwrite` replaces it, `rename` creates a `-imported` copy.
...
| What | OpenClaw config path | Clawksis destination | Notes |
...
| Provider API keys | `models.providers.*.apiKey` | `~/.clawksis/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |
...
| What | OpenClaw config path | Clawksis config path | Mapping |
...
| OpenClaw config path | Clawksis config path | Notes |
...
| OpenClaw field | Clawksis field | Notes |
...
| TTS assets | `~/.clawksis/tts/` (file copy) |
...
| Platform | OpenClaw config path | Clawksis `.env` variable | Notes |
...
| What | OpenClaw path | Clawksis path | Notes |
...
| Gateway auth token | `gateway.auth.token` | `.env` → `CLAWK_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Clawksis equivalent)



These are saved to `~/.clawksis/migration/openclaw/<timestamp>/archive/` for manual review:



| What | Archive file | How to recreate in Clawksis |

|------|-------------|--------------------------|

| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |

| `TOOLS.md` | `archive/workspace/TOOLS.md` | Clawksis has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `clawk cron create` |

| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |

| Hooks/webhooks | `archive/hooks-config.json` | Use `clawk webhook` or gateway hooks |

| Memory backend | `archive/memory-backend-config.json` | Configure via `clawk honcho` |

| Skills registry | `archive/skills-registry-config.json` | Use `clawk skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Clawksis profiles |
...
2. **Review archived files** — anything in `~/.clawksis/migration/openclaw/<timestamp>/archive/` needs manual attention.
...
4. **Verify API keys** — run `clawk status` to check provider authentication.



5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart clawk-gateway`



6. **Check session policies** — run `clawk config show` and verify the `session_reset` value matches your expectations.



7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `clawk whatsapp` to pair.



8. **Archive cleanup** — after confirming everything works, run `clawk claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Imported skills land in `~/.clawksis/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.
...
OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `clawk config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
