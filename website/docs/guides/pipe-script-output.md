...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `clawk send`."
...
`clawk send` is a small, scriptable CLI that pushes a message to any

messaging platform Clawksis is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | clawk send --to slack:#builds`)



The command reuses the same credentials and platform adapters that `clawk
...
clawk send --to slack:#eng --subject "[CI] build.log" --file build.log



# Thread target (Telegram topic, Discord thread)

clawk send --to telegram:-1001234567890:17585 "threaded reply"
...
Any platform Clawksis ships adapters for works as a target:
...
`clawk send` resolves the message body in this order:



1. **Positional argument** — `clawk send --to telegram "hi"`

2. **`--file PATH`** — `clawk send --to telegram --file msg.txt`

3. **Piped stdin** — `echo hi | clawk send --to telegram`



When stdin is a TTY (no pipe), Clawksis does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  ...
  clawk send --to telegram --subject "⚠ MEMORY WARNING" \
...
Because `clawk send` reuses your Clawksis config, the same script works on

any host where Clawksis is installed — no need to export bot tokens into
...
`clawk send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  ...
  clawk send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"

else

  tail -n 100 deploy.log | clawk send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  ...
  | /home/me/.clawksis/bin/clawk send \
...
./train.py --epochs 200 && \
  ...
  clawk send --to telegram "training done" || \

  clawk send --to telegram "training failed (exit $?)"
...
clawk send --to telegram --quiet "keepalive" || {
...
msg_id=$(clawk send --to discord:#ops --json "build started" \
...
## Does `clawk send` Need the Gateway Running?



**Usually no.** For any bot-token platform — Telegram, Discord, Slack,

Signal, SMS, WhatsApp Cloud API, and most others — `clawk send` calls

the platform's REST endpoint directly using credentials from

`~/.clawksis/.env` and `~/.clawksis/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `clawk gateway start` and retry.
...
clawk send --list --json

```



The listing is built from `~/.clawksis/channel_directory.json`, which the

gateway refreshes every few minutes while it's running. If you see

"no channels discovered yet", start the gateway once (`clawk gateway
...
| Approach | Multi-platform | Reuses Clawksis creds | Needs gateway | Best for |

|----------|----------------|---------------------|---------------|----------|

| `clawk send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`clawk send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `clawk send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  ...
  the delivery router that `clawk send` shares with cron delivery.
