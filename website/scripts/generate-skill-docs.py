...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/samuelgradientai-sys/clawksis-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    clawk_meta = (fm.get("metadata") or {}).get("clawk") or {}

    tags = clawk_meta.get("tags") or []

    related = clawk_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append((
            "Source",
            "Optional — install with `clawk skills install official/"
            + meta["category"]
            + "/"
            + meta["slug"]
            + "`",
...
def build_catalog_md_bundled(
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Clawksis"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Clawksis ships with a large built-in skill library copied into `~/.clawksis/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Clawksis also syncs bundled skills on `clawk update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.clawksis/skills/` tree, it is still shipped with Clawksis; restore it with `clawk skills reset <name> --restore`.",
        "",
