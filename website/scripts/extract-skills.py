...
2. The unified Clawksis Skills Index at ``website/static/api/skills-index.json``,
...
def _install_command(source: str, identifier: str, name: str) -> str:
    """Build the ``clawk skills install …`` command for a unified-index entry.
    ...
    if not identifier:
        return f"clawk skills install {name}"
    ...
    if src in {"official", "built-in", "optional"}:
        ...
        return f"clawk skills install {identifier}"
    ...
    if src == "clawhub":
        return f"clawk skills install clawhub/{identifier}"
