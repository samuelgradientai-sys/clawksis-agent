...
load_clawk_dotenv(
    clawk_home=_clawk_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.clawksis/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_clawk_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("CLAWK_TUI_SLASH_TIMEOUT_S") or "45")
...
_LONG_HANDLERS = frozenset({
    # session.usage does a DB read for the durable per-session fallback; keep
    # it off the main stdin loop so a slow disk can't stall approval.respond /
    # session.interrupt / other RPCs.
    "session.usage",
    "browser.manage",
    "cli.exec",
...
try:
    _rpc_pool_workers = max(2, int(os.environ.get("CLAWK_TUI_RPC_POOL_WORKERS") or "8"))
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Clawksis agent → needs provider credentials.
        # Tier-1 secrets (gateway/GitHub/infra) are still stripped (#29157).
        env = clawk_subprocess_env(inherit_credentials=True)
        if profile_home:
            ...
            # gateway's launch CLAWK_HOME (#40677).
            env["CLAWK_HOME"] = str(profile_home)
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from clawk_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from clawk_cli.plugins import invoke_hook
...
def _close_session_by_id(sid: str, *, end_reason: str = "tui_close") -> bool:
    ...
    """
    return _teardown_popped_session(_pop_session_by_id(sid), end_reason=end_reason)
...
def _ws_session_is_orphaned(session: dict | None) -> bool:
    ...
    if not session or session.get("_finalized"):
        return False
    # Requisito duro: el trabajo del agente sobrevive a la navegación. Un turno
    # EN CURSO o encolado (inflight_turn) nunca cuenta como huérfano.
...
def _schedule_ws_orphan_reap(sid: str) -> None:
    ...
    def _reap() -> None:
        ...
        with _session_resume_lock:
            ...
            if not _ws_session_is_orphaned(live):
                # Sigue detached pero con un turno EN CURSO/encolado: NO se
                # finaliza; se re-agenda el reap para que, al terminar el turno
                # (cuyo resultado queda persistido), se limpie tras la gracia.
                if (
                    live is not None
                    and not live.get("_finalized")
                    and live.get("transport") is _detached_ws_transport
                    and (live.get("running") or live.get("inflight_turn"))
                ):
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `clawk --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `clawk --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from clawk_cli.active_sessions import coerce_max_concurrent_sessions
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s CLAWK_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_clawk_home``).
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``clawk dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("CLAWK_COMPUTE_HOST_CHILD") == "1"
...
def _submit_prompt_to_compute_host(
    rid: str, sid: str, session: dict, text: Any
) -> dict:
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `clawk` shows the prompt before constructing AIAgent; the TUI used
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from clawk_cli.runtime_provider import canonical_custom_identity
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from clawk_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    overlay; it does not deep-merge ``clawk_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _clawk_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_clawk_home_override()
        home = override if isinstance(override, str) and override else _clawk_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    clawk_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _block(event: str, sid: str, payload: dict, timeout: int = 300) -> str:
    ...
    if (
        ...
        and event in {"secret.request", "sudo.request"}
    ):
        _emit(
...
def resolve_skin() -> dict:
    try:
        from clawk_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("CLAWK_MODEL", "") or os.environ.get("CLAWK_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from clawk_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``CLAWK_DESKTOP=1`` and ``CLAWK_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``CLAWK_DESKTOP_TERMINAL=1`` → "tui"
        (``clawk --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``clawk --tui``.)
    """
    if is_truthy_value(os.environ.get("CLAWK_DESKTOP")) and not is_truthy_value(
        os.environ.get("CLAWK_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads CLAWK_MODEL /
    CLAWK_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`clawk --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads CLAWK_MODEL /
    # CLAWK_INFERENCE_MODEL, which `clawk --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("CLAWK_TUI_PROVIDER", "").strip()
    ...
    try:
        from clawk_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("CLAWK_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_approval_mode() -> str:
    from clawk_cli.config import DEFAULT_CONFIG, _deep_merge
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`clawk_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("CLAWK_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("CLAWK_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Clawksis): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `clawk --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] CLAWK_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown CLAWK_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in CLAWK_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid CLAWK_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _CLAWK_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off clawk-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid CLAWK_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    cfg = None
    try:
        from clawk_cli.config import get_compatible_custom_providers, load_config
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `clawk --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # CLAWK_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("CLAWK_DEV_CREDITS")):
        try:
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from clawk_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        if is_unsupported_install_method(_install_method):
            info["install_warning"] = format_unsupported_install_warning(
                _install_method
            )
    except Exception:
        pass
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from clawk_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from clawk_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from clawk_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from clawk_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("CLAWK_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("CLAWK_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``ClawksisCLI.__init__`` and ``gateway/run.py``:
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from clawk_cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses clawk_cli.mcp_startup; TUI stdio keeps
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `clawk skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("CLAWK_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("CLAWK_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("CLAWK_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("CLAWK_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # Chat project this session belongs to (Clawksis projects feature). Stamped on
    # the session so _ensure_session_db_row persists it and prompt.submit applies
    # the project's instruction overlay to the live agent.
    ...
    # and each turn re-bind CLAWK_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `CLAWK_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_clawk_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_clawk_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_clawk_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds CLAWK_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``clawk --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    if target in active:
        # Una sesión VIVA pero ociosa no debe bloquear el delete: el usuario está
        # borrando la conversación a propósito. La finalizamos y la soltamos antes
        # del delete. Solo un turno EN CURSO (o encolado) mantiene el bloqueo —
        # borrar debajo de un agente trabajando corrompería el turno.
        live = None
        try:
            live = _find_live_session_by_key(target)
        except Exception:
        ...
        _close_session_by_id(live_sid, end_reason="deleted_by_user")
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``clawk gateway`` process, whose
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Clawksis has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from clawk_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from clawk_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from clawk_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from clawk_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from clawk_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_clawk_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("CLAWK_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
def _(rid, params: dict) -> dict:
    ...
    if _session_uses_compute_host(session):
        ...
        return _ok(
            rid, {"status": "compressed", "turn_isolation": True, "host_ack": ack}
        )
    session, err = _sess(params, rid)
    if err:
        return err
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Clawksis profile home
    # (~/.clawksis/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $CLAWK_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_clawk_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _(rid, params: dict) -> dict:
    from clawk_cli.input_sanitize import sanitize_user_prompt_text
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn CLAWK_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_clawk_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from clawk_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from clawk_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: clawk_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from clawk_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _clawk_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".clawk" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.clawk/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.clawk/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Clawksis Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Clawksis/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Clawksis Desktop. If it serves Clawksis/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from clawk_cli.model_switch import parse_model_flags_detailed
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("CLAWK_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under CLAWK_HOME (~/.clawksis by default) — config/sessions/skills,
    ...
    clawk_home = os.path.realpath(str(get_clawk_home()))

    return real == home or real == clawk_home or real.startswith(clawk_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    CLAWK_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    clawk_home = os.path.normcase(os.path.realpath(str(get_clawk_home())))
    return real == home or real == clawk_home
...
def _discover_repos_payload(db, *, conn=None, backfill: bool = True) -> list[dict]:
    ...
    repos even with zero clawk sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (clawk home subtree + bare
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from clawk_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_clawk_home), "display": display_clawk_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from clawk_cli.runtime_provider import resolve_runtime_provider
        from clawk_cli.auth import has_usable_secret
        from clawk_cli.main import _has_any_provider_configured
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Clawksis provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.clawksis/.env`` into the gateway process via
    ``clawk_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `clawk` is interactive — use `/clawk chat -q …` or run `clawk` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`clawk setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`clawk gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`clawk sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`clawk config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m clawk_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "clawk_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=min(int(params.get("timeout", 240)), 600),
            cwd=os.getcwd(),
            # cli.exec runs `python -m clawk_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from clawk_cli.moa_config import moa_usage, normalize_moa_config
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `clawk model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.clawksis/.env via the unified credential lifecycle
        ...
        from clawk_cli.credential_lifecycle import save_provider_env_credential
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from clawk_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from clawk_cli.credential_lifecycle import remove_provider_env_credential
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("CLAWK_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("CLAWK_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["CLAWK_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from clawk_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["CLAWK_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from clawk_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from clawk_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from clawk_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("CLAWK_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("CLAWK_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_clawk_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``clawk journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from clawk_cli.skills_hub import do_install
        ...
        if action == "browse":
            from clawk_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from clawk_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``clawk plugins`` / the dashboard, so the three surfaces
