...
try:
    _rpc_pool_workers = max(2, int(os.environ.get("CLAWK_TUI_RPC_POOL_WORKERS") or "4"))
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from clawk_cli.browser_connect import try_launch_chrome_debug
