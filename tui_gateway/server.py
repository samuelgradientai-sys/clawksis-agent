...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("CLAWK_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
def _resolve_model() -> str:
    env = (
        os.environ.get("CLAWK_MODEL", "")
        or os.environ.get("CLAWK_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
                ...
                try:
                    ...
                    if sid_key:
                        ...
                        if goal_mgr.is_active():
                            try:
                                from clawk_cli.goals import gather_background_processes as _gather_bg
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Clawksis" if role == "assistant" else role.title()
