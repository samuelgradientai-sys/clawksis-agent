...
description: "Emil Kowalski's design engineering philosophy — UI polish, component design, animation/easing decisions, springs, performance, accessibility, and the invisible details that make software feel great. USE THIS when creating or editing pages, landing pages, UI, components, or animations. Trigger on ES+EN: 'crear/hacer una landing', 'editar una página', 'mejorar/pulir la UI', 'diseño de interfaz', 'animaciones', 'build/edit a landing page', 'polish the UI', 'make it feel better'. Pair it with a coding CLI (codex_exec / claude_code / opencode_run) to implement, applying these design rules and reviewing with the Before/After table."
...
metadata:
  ...
  clawksis:
    emoji: "🎨"
    attribution: "Design-engineering philosophy by Emil Kowalski (https://emilkowal.ski/skill). Bundled as-is; course at https://animations.dev/."
---
...
## Implementing in Clawksis (use a coding CLI)

> Attribution: the design philosophy above is **Emil Kowalski's**, bundled as-is
> from <https://github.com/emilkowalski/skill> (<https://emilkowal.ski/skill>).
> Go deeper with his course: [animations.dev](https://animations.dev/).

When you actually need to **create or edit the code** of a page / landing /
component, do not hand-write it blindly. Delegate the implementation to an
available coding CLI and feed it these design rules as the brief; then review the
result against the **Before/After** table above. This skill is the design taste;
the coding CLI is the hands.

Available coding-CLI tools (each appears only when its CLI is installed — see
`clawk tools`):

- **Codex** — tool `codex_exec`
- **Claude Code** — tool `claude_code`
- **OpenCode** — tool `opencode_run`

Flow:
1. Decide what to build/change and the design constraints from the rules above
   (easing, durations, spring vs transition, transform/opacity only, a11y, etc.).
2. Call the coding CLI tool with a precise prompt that includes the relevant
   rules (e.g. "use `transition: transform 200ms cubic-bezier(0.23,1,0.32,1)`,
   `scale(0.97)` on `:active`, origin-aware popover, respect
   `prefers-reduced-motion`").
3. Read back the diff and review it with the Before/After table; iterate.

If no coding CLI is installed, implement directly with the file/edit tools,
applying the same rules — the design contract does not change.
