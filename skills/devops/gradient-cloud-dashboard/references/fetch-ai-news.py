...
for q in [
    ...
    data = fetch(
        f"https://news.google.com/rss/search?q={q}&hl=en-US&gl=US&ceid=US:en&date=1d"
    )
    if not data:
        continue
    try:
...
# Hacker News
data = fetch(
    f"https://hn.algolia.com/api/v1/search?query=AI&tags=story&hitsPerPage=10&numericFilters=created_at_i>{int(datetime.now().timestamp()) - 172800}"
)
if data:
    try:
        for hit in json.loads(data).get("hits", []):
            ...
            if t and t not in seen and len(t) > 15:
                seen.add(t)
                all_news.append(("", t, f"Hacker News (▲{hit.get('points', 0)})"))
    except:
        pass

recent = [
    n for n in all_news if any(w in n[0].lower() for w in ["jun", "may", "2026"])
][:12]
