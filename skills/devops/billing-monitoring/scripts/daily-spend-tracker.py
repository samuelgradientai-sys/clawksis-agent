...
def compute_projections(history):
    ...
    projection["deepseek"] = {
        ...
        "days_remaining": round(
            sorted_hist[-1].get("deepseek_balance", 0) / ds_daily, 1
        )
        if ds_daily > 0
        else float("inf"),
    }
...
def print_report(projection, history):
    ...
    print(
        f"     Recommended:   ${comb['recommended_topup_amount']:.0f} every {comb['recommended_topup_every_months']} months"
    )
    print()
    ...
    for entry in sorted(history, key=lambda x: x["date"])[-10:]:
        ds_b = (
            f"${entry.get('deepseek_balance', '?'):<8}"
            if isinstance(entry.get("deepseek_balance"), (int, float))
            else "?"
        )
