...
"""Google Workspace OAuth2 setup for Clawksis.
...
# Ensure sibling modules (_clawk_home) are importable when run standalone.
...
from _clawk_home import display_clawk_home, get_clawk_home


CLAWK_HOME = get_clawk_home()

TOKEN_PATH = CLAWK_HOME / "google_token.json"

CLIENT_SECRET_PATH = CLAWK_HOME / "google_client_secret.json"

PENDING_AUTH_PATH = CLAWK_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Clawksis profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Clawksis Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Clawksis Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Clawksis home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_clawk_home()}/google_token.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Google Workspace OAuth setup for Clawksis"
    )
