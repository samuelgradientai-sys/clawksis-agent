...
dataset = wandb.Artifact(
    ...
    metadata={"size": "1.2M images", "format": "JPEG", "resolution": "224x224"},
)
...
# Create model artifact
model_artifact = wandb.Artifact(
    name="sentiment-classifier", type="model", metadata={"accuracy": 0.94, "f1": 0.92}
)
...
for idx, (img, label, pred) in enumerate(zip(images, labels, predictions)):
    table.add_data(idx, wandb.Image(img), label, pred)
...
train_run.link_artifact(
    model_artifact, "model-registry/nlp-models", aliases=["candidate"]
)
