...
# Clawksis Desktop Plugins Skill

Write plugins for the Clawksis desktop app: statusbar items, layout panes,
...
system (`~/.clawksis/plugins/`) is otherwise documented separately.
...
- The Clawksis desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$CLAWK_HOME/desktop-plugins/` (usually
  `~/.clawksis/desktop-plugins/`).

## How to Run

1. Create `$CLAWK_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (relative to this skill directory) — that's
   `~/.clawksis/...` by default, or `~/.clawksis/profiles/<profile>/...` under a
...
The ONLY import surface is `@clawk/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.clawksis/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@clawk/plugin-sdk`, `react`, and
