...
`$CLAWK_HOME/config.yaml` (defaults to `~/.clawksis/config.yaml` when `CLAWK_HOME` is unset)
...
Restart the Clawksis session for changes to take effect. The MCP connection is
