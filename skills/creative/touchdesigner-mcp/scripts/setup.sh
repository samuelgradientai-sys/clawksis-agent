...
CLAWK_HOME_DIR="${CLAWK_HOME:-$HOME/.clawksis}"
CLAWK_CFG="${CLAWK_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Clawksis config has twozero_td MCP entry ──
if [[ ! -f "$CLAWK_CFG" ]]; then
    echo -e " ${FAIL} Clawksis config not found at ${CLAWK_CFG}"
    manual_steps+=("Create ${CLAWK_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CLAWK_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Clawksis config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Clawksis config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CLAWK_CFG} manually"); }
    manual_steps+=("Restart Clawksis session to pick up config change")
fi
