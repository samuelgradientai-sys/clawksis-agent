...
Uses a file-based lock (~/.clawksis/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `clawk update` reloads
# the module) fail with ModuleNotFoundError for clawk_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from clawk_constants import get_clawk_home
from clawk_cli._subprocess_compat import windows_hide_flags
from clawk_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy clawk_cli import at cron module load (matches
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``clawk tools`` config for the ``cron`` platform.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``clawk update`` / ``clawk gateway stop`` / systemd restart, or an
...
_clawk_home: Path | None = None


def _get_clawk_home() -> Path:
    """Resolve Clawksis home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active CLAWK_HOME at call time
    ...
    """
    return _clawk_home or get_clawk_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    clawk_home = _get_clawk_home()
    lock_dir = clawk_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Clawksis profile.
    ...
    the scheduler's test/override hook and a context-local Clawksis home override
    both point at the resolved profile directory so _get_clawk_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_clawk_home() callers agree on the same home.
    ...
    prior_override = _clawk_home
    env_snapshot = os.environ.copy()

    from clawk_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    override_token = None
    try:
        override_token = set_clawk_home_override(profile_home)
        _clawk_home = profile_home
        logger.info(
            "Job '%s': using Clawksis profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _clawk_home = prior_override
        if override_token is not None:
            reset_clawk_home_override(override_token)
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Clawksis — {task_name}"
    try:
...
# Max characters of ~/.clawksis/user.md injected into a cron prompt.
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within CLAWK_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Clawksis-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against CLAWK_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_clawk_home() / "scripts"
    ...
    # escape — scripts MUST reside within CLAWK_HOME/scripts/.
    try:
    ...
    try:
        ...
        # Profile awareness: point the child at the home this run resolved to
        # (a per-job ``profile`` swaps it via _job_profile_context), so scripts
        # that read CLAWK_HOME / ~ agree with the scheduler.
        env["CLAWK_HOME"] = str(_get_clawk_home())
        try:
...
def _run_job_impl(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _clawk_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    #
    # Bounded with its own timeout (separate from CLAWK_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("CLAWK_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid CLAWK_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_clawk_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    os.environ["CLAWK_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed CLAWK_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads CLAWK_SESSION_PLATFORM
    #     and CLAWK_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via CLAWK_CRON_AUTO_DELIVER_* below.
    ...
    #     send_message gate read CLAWK_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the CLAWK_CRON_AUTO_DELIVER_* vars set
    # below, so clearing CLAWK_SESSION_* here does not affect delivery.
    _ctx_tokens = set_session_vars(
        platform="",
        chat_id="",
        chat_name="",
        # A cron job cannot receive a completion after its turn ends. We clear the
        # CLAWK_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ CLAWK_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "CLAWK_CRON_AUTO_DELIVER_PLATFORM",
        "CLAWK_CRON_AUTO_DELIVER_CHAT_ID",
        "CLAWK_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        # load_clawk_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this CLAWK_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_clawk_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from clawk_cli.env_loader import (
            load_clawk_dotenv,
        ...
        # Model resolution precedence: per-job override > CLAWK_MODEL env >
        ...
        model = job.get("model") or os.getenv("CLAWK_MODEL") or ""
        ...
        try:
            # Do not inject CLAWK_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        fallback_model = _job_fallback or get_fallback_chain(_cfg) or None
        ...
        agent = AIAgent(
            ...
            # CLAWK_HOME. When a workdir is configured, also inject project
        ...
        # override via CLAWK_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("CLAWK_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid CLAWK_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        try:
            ...
            #
            # When the agent goes silent (condition not met / nothing to
            # report), the default is to deliver a short "nothing new"
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # Set CLAWK_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("CLAWK_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
        ...
        #
        # Jobs with a per-job ``profile`` are serialized for the same reason:
        # _job_profile_context installs a context-local Clawksis home override,
        # rewrites the scheduler's _clawk_home hook, and snapshots/restores
        # os.environ around the profile's .env load — all process-global.
        # Jobs with neither field stay parallel-safe.
        def _needs_sequential(j: dict) -> bool:
            return bool(
                (j.get("workdir") or "").strip() or (j.get("profile") or "").strip()
            )
