...
``clawk gateway restart`` (or ``launchctl kickstart ai.clawk.gateway``
or ``systemctl restart clawk-gateway``).  When the cron fires, the
...
``clawk cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``clawk gateway``, ``launchctl ... clawk-gateway``,
``systemctl ... clawk-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_CLAWK_GATEWAY=1``, and
``clawk gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `clawk gateway restart|stop` — the canonical foot-gun.
    ...
    # Branch C: systemctl ops on a clawk-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bclawk[.\-]?gateway)"
    # Branch D: pkill / kill targeting the clawk gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<CLAWK_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.clawksis/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_clawk_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `clawk gateway restart` from a shell outside "
