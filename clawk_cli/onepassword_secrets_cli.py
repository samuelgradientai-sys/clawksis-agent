"""CLI handlers for ``clawk secrets onepassword ...``.
...
the CLI through OS package managers and signed installers, so Clawksis expects
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Clawksis resolves [cyan]op://vault/item/field[/cyan] references through your\n"
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]clawk secrets onepassword setup[/cyan] to enable.")
        return 0
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Clawksis will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]clawk secrets onepassword setup[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "`clawk secrets onepassword setup` first.[/yellow]"
        )
        return 1
    ...
    console.print(
        ...
        "[cyan]clawk[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export "
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "next Clawksis invocation.\n"
        "  Your reference mappings are left in config.yaml — remove them with "
        "[cyan]clawk secrets onepassword remove ENV_VAR[/cyan] if you no longer "
        "need them."
    )
    return 0
