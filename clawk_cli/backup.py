...
# CLAWK_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under CLAWK_HOME) —
...
# File names that ``clawk import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting hosted instances from their
#     control plane.
...
# Reserved archive subtree for provider state that lives OUTSIDE CLAWK_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside CLAWK_HOME, resolved from config only (no network, no init).
...
def run_backup(args) -> None:
    ...
    # outside CLAWK_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_clawk_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under CLAWK_HOME.
            ...
            # reconciler on the target and disconnects hosted instances from
            # their control plane. Matched by basename so both the root profile and
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_clawk_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: clawk profile list  (after installing clawk)")
...
_QUICK_STATE_FILES = (
    ...
    # are at $CLAWK_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside CLAWK_HOME and the entry is silently
