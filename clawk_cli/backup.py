"""
Backup and import commands for clawk CLI.

`clawk backup` creates a zip archive of the entire ~/.clawksis/ directory
(excluding the clawksis-agent repo and transient files).

`clawk import` restores from a backup zip, overlaying onto the current
CLAWK_HOME root.
"""
...
from clawk_constants import get_default_clawk_root, get_clawk_home, display_clawk_home
...
# ``clawksis-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/clawksis-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "clawksis-agent",  # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to clawk root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``clawksis-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/clawksis-agent/`` — must be preserved.
        if part == "clawksis-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside CLAWK_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Clawksis home directory."""
    clawk_root = get_default_clawk_root()

    if not clawk_root.is_dir():
        print(f"Error: Clawksis home directory not found at {clawk_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"clawk-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"clawk-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_clawk_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(clawk_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(clawk_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``clawksis-agent`` is only pruned at the root level; nested dirs
        ...
        dirnames[:] = [
            d
            for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "clawksis-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(clawk_root)
    ...
    print(f"\nRestore with: clawk import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Clawksis backup.
    ...
    # Look for telltale files that a clawk home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Clawksis backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.clawksis/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_clawk_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (clawk_root / "config.yaml").exists()
        has_env = (clawk_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Clawksis configuration.")
        ...
        for member in members:
            ...
            target = clawk_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(clawk_root.resolve())
            except ValueError:
        ...
        profiles_dir = clawk_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # clawk_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: clawk profile list  (after installing clawk)")

        # Guidance
        print()
        if not (clawk_root / "clawksis-agent").is_dir():
            print("Note: The clawksis-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: clawk update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  clawk -p {pname} gateway install")

        print("Done. Your Clawksis configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and clawk backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to CLAWK_HOME).
...
# — `clawk update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(clawk_home: Optional[Path] = None) -> Path:
    home = clawk_home or get_clawk_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    clawk_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(clawk_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside CLAWK_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``clawk update``.
    ...
    Args:
        ...
        clawk_home: Override for the Clawksis home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(clawk_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for clawk backup --quick."""
    ...
    if snap_id:
        ...
        print(
            f"  {len(snaps)} snapshot(s) stored in {display_clawk_home()}/state-snapshots/"
        )
...
def _write_full_zip_backup(out_path: Path, clawk_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``clawk_root`` to ``out_path``.
...
def _pre_update_backup_dir(clawk_home: Optional[Path] = None) -> Path:
    home = clawk_home or get_clawk_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of CLAWK_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<CLAWK_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``clawk update``) should continue even if the backup fails.
    """
    clawk_root = clawk_home or get_default_clawk_root()
    if not clawk_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(clawk_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, clawk_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `clawk claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of CLAWK_HOME under ``backups/`` before a
    ``clawk claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``clawk import <archive>``.  Writes to
    ``<CLAWK_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `clawk import` and the
