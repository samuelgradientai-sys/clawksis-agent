"""CLI subcommand: ``clawk send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"clawk send: cannot read {file_path}: not a text file. --file reads the "
                ...
                f'  clawk send --to telegram "MEDIA:{file_path}"\n'
                f'  clawk send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  clawk send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"clawk send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"clawk send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"clawk send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"clawk send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"clawk send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `clawk gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.clawksis/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_clawk_env() -> None:
    """Populate ``os.environ`` from ``~/.clawksis/.env`` AND bridge top-level
    ...
    startup that ``clawk send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.clawksis/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.clawksis/config.yaml`` into
       ...
       via ``clawk config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``clawk send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.clawksis/.env and ~/.clawksis/config.yaml into os.environ so the
    ...
    if message is None or not message.strip():
        print(
            "clawk send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `clawk send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Clawksis "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.clawksis/.env + ~/.clawksis/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `clawk send --list telegram`.",
    )
