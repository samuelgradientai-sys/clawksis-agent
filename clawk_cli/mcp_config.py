"""
MCP Server Management CLI — ``clawk mcp`` subcommand.

Implements ``clawk mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.clawksis/config.yaml under the ``mcp_servers`` key.
"""
...
from clawk_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_clawk_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.clawksis/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``clawk mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    # clawk_cli/main.py for why the dest is renamed.
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_clawk_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: clawk mcp test " + name)
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``clawk mcp login`` and
    ``clawk mcp reauth`` so both behave identically for a single server.
    """
...
def cmd_mcp_reauth(args):
    ...
    ``clawk mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``clawk mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: clawk mcp reauth <name>   |   clawk mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def mcp_command(args):
    ...
    if handler:
        ...
        _info("clawk mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("clawk mcp add <name> --command <cmd>         Add a stdio server")
        _info("clawk mcp add <name> --preset <preset>       Add from a known preset")
        _info("clawk mcp remove <name>                      Remove a server")
        _info("clawk mcp list                               List configured servers")
