...
- ``clawk_cli.providers``        -- canonical provider identity + overlays
- ``clawk_cli.model_normalize``  -- per-provider name formatting
...
# happen to carry the family name in their tag but are fully tool-capable.
#
...
#
# NOTE: the model family is literally named "hermes" — this is an upstream LLM
# id, NOT the pre-fork product name, so it must never be rebranded to "clawk".
...
def is_nous_clawk_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_CLAWK_NON_AGENTIC_RE.search(model_name))
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``clawk config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from clawk_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'clawk model' for available providers, or define it "
    ...
    # =================================================================
        ...
        try:
            from clawk_cli.config import load_config
            from clawk_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    # clawk_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    clawk_warn = _check_clawk_model_warning(new_model)
    if clawk_warn:
        warnings.append(clawk_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from clawk_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from clawk_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from clawk_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from clawk_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by clawk provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://clawksis-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Clawksis release. Falls back to the in-repo
    ...
    # --- 1. Check Clawksis-mapped providers ---
    from clawk_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    ...
    for clawk_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(clawk_id)
        if (
            _alias_target
            and _alias_target != clawk_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Resolve the canonical provider profile name.  Skip clawk_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose clawk_id matches the canonical profile name so
        ...
        # same clawk_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if clawk_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(clawk_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (CLAWK_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Clawksis may not route yet.
            ...
            try:
                ...
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        clawk_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `clawk model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(clawk_id)
        if not model_ids:
            model_ids = curated.get(clawk_id, [])
            if clawk_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(clawk_id, model_ids)
    ...
    # --- 2. Check Clawksis-only providers (nous, openai-codex, copilot, opencode-go) ---
    from clawk_cli.providers import CLAWK_OVERLAYS
    from clawk_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Clawksis provider ID.
    # CLAWK_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Clawksis IDs ("copilot").
    _mdev_to_clawk = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in CLAWK_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Clawksis slug — e.g. "github-copilot" → "copilot"
        clawk_slug = _mdev_to_clawk.get(pid, pid)
        if clawk_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or clawk_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(clawk_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, clawk_slug):
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(clawk_slug):
                    has_creds = True
                elif for_picker:
                ...
                logger.debug("Credential pool check failed for %s: %s", clawk_slug, exc)
        ...
        # configured.
        if not has_creds and clawk_slug == "anthropic":
            try:
        ...
        if clawk_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(clawk_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `clawk model` picker deliberately shows ONLY the curated agentic
            ...
            if not model_ids:
                model_ids = curated.get(clawk_slug, []) or curated.get(pid, [])
                if clawk_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(clawk_slug, model_ids)
        total = len(model_ids)
        if clawk_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": clawk_slug,
            "name": get_label(clawk_slug),
            "is_current": clawk_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "clawk",
        ...
        seen_slugs.add(clawk_slug.lower())
        _record_builtin_endpoint(clawk_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CLAWK_OVERLAYS (keeps /model in sync
    # with `clawk model`).
    try:
        from clawk_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Clawksis writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Clawksis's own writer uses (mirrors section-4 grouping).
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Clawksis's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, clawk_cli/config.py) already consume that dict.
    ...
    # Ollama (local): if an Ollama daemon is running on this machine, surface
    # its pulled models as their own picker row (see `clawk cookbook`). Purely
    # local discovery — no credentials, best-effort, never fatal.
    if not any(str(r.get("slug", "")).strip().lower() == "ollama" for r in results):
        try:
            ...
            logger.debug("Ollama picker row discovery failed", exc_info=True)
...
def _prepend_moa_picker_provider(
    ...
    """
    try:
        from clawk_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`clawk_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from clawk_cli.models import fetch_openrouter_models
