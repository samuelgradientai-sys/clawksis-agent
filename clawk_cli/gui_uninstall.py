"""
Clawksis Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``clawk desktop`` / ``clawk gui``)
     Built inside the agent checkout under ``$CLAWK_HOME/clawksis-agent/``:
       ...
       - ``$CLAWK_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Clawksis.app`` or ``~/Applications/Clawksis.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Clawksis`` (NSIS per-user)
...
the app name ("Clawksis"), separate from ``$CLAWK_HOME``:
  - macOS:   ``~/Library/Application Support/Clawksis``
  - Windows: ``%APPDATA%\\Clawksis``
  - Linux:   ``$XDG_CONFIG_HOME/Clawksis`` (default ``~/.config/Clawksis``)
...
``clawk uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(clawk_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return clawk_home / "clawksis-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Clawksis"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Clawksis"
...
def source_built_gui_artifacts(clawk_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``clawk desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``clawksis-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(clawk_home)
    ...
    return [
        ...
        clawk_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Clawksis" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Clawksis.app"),
            home / "Applications" / "Clawksis.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Clawksis).
            local_base / "Programs" / "Clawksis",
            # Older / alternate layout some builds used.
            local_base / "clawk-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Clawksis")
    else:
        ...
        paths += [
            data_base / "applications" / "clawk.desktop",
            data_base / "applications" / "Clawksis.desktop",
        ]
    return paths


def agent_is_installed(clawk_home: Path) -> bool:
    """Return True when a usable Python agent install exists under CLAWK_HOME.
    ...
    if (agent_root / "clawk_cli").is_dir():
        return True
...
def gui_is_installed(clawk_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(clawk_home):
        if p.exists():
            return True
...
def gui_install_summary(clawk_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = clawk_home if clawk_home is not None else get_clawk_home()
...
def uninstall_gui(clawk_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``clawksis-agent/clawk_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$CLAWK_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove clawk' or "
            "'sudo dnf remove clawk'). AppImage builds are a single file you "
