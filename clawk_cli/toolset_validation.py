...
the tool registry or launching Clawksis (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``clawk-cli`` to the non-existent ``clawk``. ``resolve_toolset('clawk')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``clawk-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``clawk`` instead of ``clawk-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `clawk tools` to reconfigure."
        )
    return warnings
