"""``clawk portal`` — the human-readable entry point for Nous Portal.

Running ``clawk portal`` with no subcommand performs the one-shot Portal
...
``clawk auth add nous --type oauth`` (which still works), is identical to
``clawk setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
DOCS_URL = "https://clawksis-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `clawk model`)")
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `clawk auth add nous --type
    oauth`. It reuses the exact wiring behind `clawk setup --portal` (which in
    ...
    """
    from clawk_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `clawk portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `clawk portal` is the
        # human-readable alias for `clawk auth add nous --type oauth` /
        # `clawk setup --portal`.
    ...
    print("Run `clawk portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `clawk portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `clawk portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `clawk auth add "
            "nous --type oauth`, identical to `clawk setup --portal`). "
