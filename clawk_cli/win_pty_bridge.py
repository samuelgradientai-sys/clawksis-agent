"""Windows ConPTY bridge for the `clawk dashboard` chat tab.

Drop-in counterpart to ``clawk_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``clawk_cli.web_server`` consumes: ``spawn``, ``read``,
