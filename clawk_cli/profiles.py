"""
Profile management for multiple isolated Clawksis instances.

Each profile is a fully independent CLAWK_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.clawksis/profiles/<name>/`` by default.

The "default" profile is ``~/.clawksis`` itself — backward compatible,
zero migration needed.

Usage::

    clawk profile create coder          # fresh profile + bundled skills
    clawk profile create coder --clone  # also copy config, .env, SOUL.md, skills
    clawk profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    clawk -p coder chat                 # or via flag
    clawk profile use coder             # set as sticky default
    clawk profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See clawk_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.clawksis``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   clawksis-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# `clawk skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_clawk_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.clawksis) profile.
...
# Allow-list for ``export_profile("default")``: when CLAWK_HOME equals the
...
# into the export. The set below identifies the *known Clawksis profile
# artifacts* at the root of CLAWK_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``clawk_constants``.
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the clawk root, NOT to the current CLAWK_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where CLAWK_HOME points outside
    ``~/.clawksis``, profiles live under ``CLAWK_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_clawk_home() / "profiles"


def _get_default_clawk_home() -> Path:
    """Return the default (pre-profile) CLAWK_HOME path.

    In standard deployments this is ``~/.clawksis``.
    In Docker/custom deployments where CLAWK_HOME is outside ``~/.clawksis``
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_clawk_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``clawk``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``clawk`` profile inside ``~/.clawksis/``) or get refused
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Clawksis installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CLAWK_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_clawk_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, clawk subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _CLAWK_SUBCOMMANDS:
        return f"'{canon}' conflicts with a clawk subcommand"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Clawksis. If we
    ...
    try:
        ...
        from clawk_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `clawk doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``clawk -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``clawk profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``clawk -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``clawk profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``CLAWK_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Clawksis config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_clawk_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, clawk_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own CLAWK_HOME.
    ...
    The returned ``clawk_home`` is the path to pass to
    ``set_clawk_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_clawk_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``clawk update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.clawksis)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.clawksis/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Write the opt-out marker so seed_profile_skills() and `clawk update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`clawk profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `clawk profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `clawk -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CLAWK_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``clawk profile create
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_clawk_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Clawksis *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Clawksis backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``CLAWK_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `clawk -p <canon> profile
    ...
    clawk_markers = ("clawk_cli.main", "clawk-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Clawksis process: either an entrypoint marker in argv, or
            # a resolved executable named `clawk`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_clawk = (
                any(marker in joined for marker in clawk_markers)
                or exe_name == "clawk"
                or exe_name.startswith("clawk")
            )
            if not is_clawk:
                continue
            ...
            # ...or by CLAWK_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("CLAWK_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.clawksis).\n"
            "To remove everything, use: clawk uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_clawk_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``CLAWK_HOME`` and binds the port resolved by ``gateway/config.py``
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CLAWK_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CLAWK_HOME")
    try:
        os.environ["CLAWK_HOME"] = str(profile_dir)
        from clawk_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["CLAWK_HOME"] = old_home
        elif "CLAWK_HOME" in os.environ:
            del os.environ["CLAWK_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.clawksis/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: clawk profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CLAWK_HOME.

    Returns ``"default"`` if CLAWK_HOME is not set or points to ``~/.clawksis``.
    Returns the profile name if CLAWK_HOME points into ``~/.clawksis/profiles/<name>``.
    ...
    default_resolved = _get_default_clawk_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      CLAWK_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Clawksis file the
      user may have lying alongside CLAWK_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.clawksis itself — its parent is ~/ and its
        # directory name is ".clawk", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: clawk profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.clawksis itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.clawksis). "
            "Specify a different name: clawk profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="clawk_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    legacy_old_host = f"clawk.{old_name}"
    new_host = f"clawk_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_clawk_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any clawk modules
    are imported, to set the CLAWK_HOME environment variable.
    """
