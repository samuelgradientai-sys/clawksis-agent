...
def auth_add_command(args) -> None:
    ...
    if provider not in PROVIDER_REGISTRY and provider != "openrouter" and provider != "nous" and not provider.startswith(CUSTOM_POOL_PREFIX):
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_clawk_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:clawk_pkce",
    ...
    if provider == "nous":
        ...
        # <clawk-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `clawk --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``clawk auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``clawk auth add xai-oauth`` overwrote the
    ...
    if provider == "google-gemini-cli":
        # Fork-only provider (see clawk_cli/providers.py overlay + main.py
        # picker): upstream dropped its `auth add` branch in v2026.7.20, but
        # Clawksis still ships the Gemini CLI OAuth + Code Assist route, so the
        # branch is kept here.
    ...
    raise SystemExit(f"`clawk auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Clawksis reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `clawk auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `clawk auth` is called bare."""
