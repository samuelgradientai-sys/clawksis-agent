...
from clawk_cli.dashboard_auth.cookies import read_session_cookies

from clawk_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /clawk``, the

    ``login_url`` is prefixed (``/clawksis/login?next=...``) so the
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["clawk.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``clawk_session_rt``. If we bailed on ``not at`` here we'd
