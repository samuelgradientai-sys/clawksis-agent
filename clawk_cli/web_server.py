...
Usage:
    ...
    python -m clawk_cli.main web          # Start on http://127.0.0.1:9119

    python -m clawk_cli.main web --port 8080
...
# CLAWK_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# This list is defined in ``clawk_cli.dashboard_auth.public_paths`` so the
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from clawk_cli.profiles import _check_gateway_running, profiles_to_serve
    ...
    for name, home in homes:
        try:
            ...
            continue
        try:
            runtime = _read_runtime_status(home / "gateway_state.json")
        except Exception:
            runtime = None
...
async def get_status():
    ...
    # and which providers are registered so ``clawk status`` and the
    ...
    # Profile + gateway topology: which profiles exist, whether one multiplexed
    # gateway or several per-profile gateways serve them, and (gated) which host
    # ports the live gateways' port-binding platforms listen on.  Enumerating
    # profiles walks the filesystem and probes the process table, so keep it off
    # the event loop.
    #
    # Split by sensitivity: profile NAMES (``profiles``) and ``gateway_mode``
    # are low-sensitivity product surface (the portal renders the profile list
    # from this endpoint over a gated bind), so they survive the auth gate. The
    # per-gateway ``gateways[]`` detail carries host ports (deployment recon),
    # so it stays gated with the host paths / PID below.
    topology = await asyncio.get_running_loop().run_in_executor(
        None, _collect_profile_gateway_topology
    )

    body["profiles"] = topology["profiles"]

    body["gateway_mode"] = topology["gateway_mode"]
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the clawk home root).
...
# streamed to a per-action log file under ``~/.clawksis/logs/<action>.log`` so
...
def _spawn_clawk_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``clawk_cli.main`` module so the action
...
async def check_clawk_update(force: bool = False):
    ...
    ``POST /api/clawksis/update`` actually runs ``clawk update``.
    ...
    Returns:
        ...
        current_version: installed Clawksis version string
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.clawksis/config.yaml`` under ``tts.``.
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Clawksis supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `clawk model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.clawksis/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (clawk_cli/main.py
...
# the canonical ``clawk auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Clawksis resolves Anthropic creds in this order at runtime:

    1. ``~/.clawksis/.anthropic_oauth.json`` — Clawksis-managed PKCE flow
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Clawksis even

    when they also have a separate Clawksis-managed PKCE login.
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("clawk_pkce", "claude_code", ...)
...
#          → persists to ~/.clawksis/.anthropic_oauth.json AND credential pool
...
# Guarded so clawk web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(
    ...
    the system in the same state as ``clawk auth add anthropic``.
...
# the tokens exactly like `clawk auth add xai-oauth`.
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process CLAWK_HOME at import time. The dashboard is a single
...
# Wraps the same config data layer the CLI uses (clawk_cli.mcp_config), so

# servers managed here show up under `clawk mcp list` and vice versa.  Secrets
...
# Wraps the same JSON store the CLI uses (clawk_cli.webhook); the webhook
...
# spawn the real `clawk gateway <verb>` so behaviour matches the CLI exactly.
...
# `clawk memory setup`).  The dashboard covers the common admin actions:
...
async def list_checkpoints():
    ...
    cp_dir = get_clawk_home() / "checkpoints"
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``clawk profile use`` —

    the profile new CLI invocations pick up. ``current`` is the profile

    the running dashboard/gateway is scoped to (derived from CLAWK_HOME).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the CLAWK_HOME override.
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``clawk tools`` picker shows
...
# The endpoint spawns the same ``clawk --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``clawk --tui`` would run.  Tests monkeypatch this
    ...
    Session resume is propagated via the ``CLAWK_TUI_RESUME`` env var —

    matching what ``clawk_cli.main._launch_tui`` does for the CLI path.
    ...
    ``CLAWK_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    ...
    `sidecar_url` (when set) is forwarded as ``CLAWK_TUI_SIDECAR_URL`` so
    ...
    env.setdefault("CLAWK_TUI_DISABLE_MOUSE", "1")
...
# CLAWK_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`clawk_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    from clawk_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/clawksis/*`` -> local Caddy -> :9119), the

    proxy injects ``X-Forwarded-Prefix: /clawk`` on every request. We

    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)

    and the SPA's runtime ``__CLAWK_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/clawk``)
        ...
        ``__CLAWK_AUTH_REQUIRED__`` flag lets the SPA pick the right
    ...
    # under ``/clawk`` they'd hit ``mission-control.tilos.com/fonts/...``

    # (the MC Pages app), not the Clawksis backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.clawksis/, same trust level as the config file itself.
...
async def get_dashboard_themes():
    ...
    from `~/.clawksis/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as clawk_cli.plugins):

    1. User plugins:    ~/.clawksis/plugins/<name>/dashboard/manifest.json

    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)

    3. Project plugins: ./.clawksis/plugins/  (only if CLAWK_ENABLE_PROJECT_PLUGINS)
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.clawksis/plugins/``) ship with the CWD and are
...
from clawk_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
