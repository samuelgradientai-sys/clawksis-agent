"""Implementation of the ``clawk bundles`` CLI subcommand.



Mirrors the structure of ``clawk_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    ...
    # Bind to stderr so piping `clawk bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  clawk bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``clawk bundles`` argparse tree.



    Called from ``clawk_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        "-d",
        default="",
        help="Human-readable description shown in /help and `clawk bundles list`",
    )
...
def bundles_command(args) -> None:
    """Dispatch ``clawk bundles <subcommand>`` to the right handler."""
