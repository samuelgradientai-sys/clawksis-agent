...
Interactive setup wizard for Clawksis.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("Configure Clawksis using environment variables or config commands:")

    print_info("  clawk config set model.provider custom")

    print_info("  clawk config set model.base_url http://localhost:8080/v1")

    print_info("  clawk config set model.default your-model-name")

    print()

    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")

    print_info("Run 'clawk setup' in an interactive terminal to use the full wizard.")
...
def is_noninteractive() -> bool:
    ...
    ``CLAWK_NONINTERACTIVE=1`` (see ``clawk_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("CLAWK_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``CLAWK_NONINTERACTIVE=1`` or a closed/redirected
...
def _print_setup_summary(config: dict, clawk_home):
    ...
    print(f"   {color('clawk setup', Colors.GREEN)}          Re-run the full wizard")

    print(f"   {color('clawk setup model', Colors.GREEN)}    Change model/provider")

    print(f"   {color('clawk setup terminal', Colors.GREEN)} Change terminal backend")

    print(f"   {color('clawk setup gateway', Colors.GREEN)}  Configure messaging")

    print(f"   {color('clawk setup tools', Colors.GREEN)}    Configure tool providers")
    ...
    print(f"   {color('clawk', Colors.GREEN)}              Start chatting")

    print(f"   {color('clawk gateway', Colors.GREEN)}      Start messaging gateway")

    print(f"   {color('clawk doctor', Colors.GREEN)}       Check for issues")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``clawk model``)
    ...
    provider added to ``clawk model`` is automatically available here.
    ...
    # on demand via `clawk auth add`, `clawk setup` vision, and
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``clawk model`` -> xAI Grok OAuth (SuperGrok / Premium+).
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Clawksis runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `clawk setup terminal` / config.yaml.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CLAWK_MAX_ITERATIONS at startup. We no longer write
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CLAWK_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info(
        "  log     — Silent in chat; write every tool call to ~/.clawksis/logs/tool_calls.log (gateway only)"
    )
...
def _setup_telegram():
    ...
    print_info("📬 Home Channel: where Clawksis delivers cron job results,")
...
def _setup_webhooks():
    ...
    print_info("   Full guide: https://github.com/samuelgradientai-sys/clawksis-agent")
    ...
    print_info("   https://github.com/samuelgradientai-sys/clawksis-agent")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Clawksis from anywhere.")
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if (
                    ...
                    print_info(
                        "  Or as a boot-time service: clawk gateway install --system"
                    )
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `clawk setup tools` and `clawk tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      ...
      * ``PROVIDER_REGISTRY`` in ``clawk_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from clawk_cli.config
...
# Config values may have different semantics between OpenClaw and Clawksis.
...
def _offer_openclaw_migration(clawk_home: Path) -> bool:
    ...
    print_info("Clawksis can preview what would be imported before making any changes.")
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``clawk setup --portal`` and ``clawk portal``. This is the
    ...
    Clawksis session — model selected, provider set, and web/image/tts/browser

    tools routed via their Portal sub — without being told to run

    ``clawk setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``clawk model`` runs

    when you pick Nous. Routing through it (instead of hand-rolling the auth +

    provider write here) means ``clawk portal`` always offers a model picker,
    ...
    print_info("  Run `clawk portal info` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ...
      clawk setup           — full or quick (auto-detected)

      clawk setup model     — just model/provider

      clawk setup tts       — just text-to-speech

      clawk setup terminal  — just terminal backend

      clawk setup gateway   — just messaging platforms

      clawk setup tools     — just tool configuration

      clawk setup agent     — just agent settings
    ...
    if is_existing:
        ...
        print_success("You already have Clawksis configured.")
        ...
        # The only remaining fork is Full vs Blank Slate.
        ...
        if setup_mode == 1:
            _run_blank_slate_setup(config, clawk_home, is_existing)
    ...
    print_info(f"Data folder:  {clawk_home}")
...
def _run_first_time_quick_setup(config: dict, clawk_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``clawk setup <section>``

    or switch providers with ``clawk model``.
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``clawk tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``clawk setup agent`` /
    ``clawk config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, clawk_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `clawk update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        clawk tools")
        print_info("  Seed skills:         clawk skills opt-in --sync")
        print_info("  Add MCP servers:     clawk mcp add")
        print_info("  Enable plugins:      clawk plugins")
        print_info("  Tune agent settings: clawk setup agent")
        print()
        _print_setup_summary(config, clawk_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, clawk_home)


def _blank_slate_walkthrough(config: dict, clawk_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`clawk update` runs from re-injecting them. Opt back in any")
            print_info("time with `clawk skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `clawk tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info(
            "Manage plugins with `clawk plugins list` / `clawk plugins install`."
        )
    else:
        print_info("No plugins enabled. Add later with `clawk plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `clawk mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `clawk mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   clawk tools")
    print_info("  Seed skills:         clawk skills opt-in --sync")
    print_info("  Add MCP servers:     clawk mcp add")
    print_info("  Tune agent settings: clawk setup agent")
    print()
...
def _run_quick_setup(config: dict, clawk_home):
    ...
    if not has_anything_missing:
        ...
        print_info("Run 'clawk setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("Connect Clawksis to messaging apps to chat from anywhere.")
