...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "clawk CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``clawk gateway status`` from falsely counting the ``clawk`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``clawk gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. CLAWK_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``clawk update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Clawksis profile are returned.
    """
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m clawk_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``clawk update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Clawksis's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose CLAWK_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``clawk update`` users
    # with no running gateway until they re-invoke ``clawk gateway``
    ...
    if sys.platform == "win32":
        try:
            from clawk_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / CLAWK_HOME) the windowless
        # base interpreter needs to import clawk_cli.  Empty on POSIX, where
...
def _clawk_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``CLAWK_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_clawk_home_from_systemd_unit(system: bool) -> None:
    ...
    Under ``sudo``, ``CLAWK_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_clawk_home` falls back to ``/root/.clawk`` — the wrong
    profile. The unit file pins ``CLAWK_HOME`` for the actual gateway
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: clawk gateway stop")
    else:
        print(
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``clawk gateway status`` output so users with
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (clawk_cli/gateway.py restart
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Clawksis Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``clawk gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``CLAWK_GATEWAY_DETACHED=1``; older wrappers without the env marker are
...
def _profile_suffix() -> str:
    ...
    Works correctly in Docker (CLAWK_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
...
def _profile_arg(clawk_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        clawk_home: Optional explicit CLAWK_HOME path. Defaults to the current
            ``get_clawk_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Clawksis root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_clawk_root()``
...
def _profile_arg_for_target_user(clawk_home: str, target_home_dir: str) -> str:
    ...
    try:
        ...
        return _profile_arg(clawk_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this CLAWK_HOME.

    Default ``~/.clawksis`` returns ``clawk-gateway`` (backward compatible).
    Profile ``~/.clawksis/profiles/coder`` returns ``clawk-gateway-coder``.
    Any other CLAWK_HOME appends a short hash for uniqueness.
    """
...
# clawk-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (clawk-gateway-*.service) and unrelated third-party
# "clawk" units are never matched.
...
def _find_legacy_clawk_units() -> list[tuple[str, Path, bool]]:
    ...
    Detects unit files installed by older Clawksis versions that used a
    different service name (e.g. ``clawk.service`` before the rename to
    ``clawk-gateway.service``). When both a legacy unit and the current
    ``clawk-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``clawk-gateway-coder.service`` and unrelated third-party
      ``clawk-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``clawk.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("  These run alongside the current clawk-gateway service and")
...
def remove_legacy_clawk_units(
    ...
    Iterates over whatever ``_find_legacy_clawk_units()`` returns — which is
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    clawk gateway uninstall")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: clawk gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Clawksis often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.clawksis`` → ``ai.clawk.gateway.plist`` (backward compatible).
    Profile ``~/.clawksis/profiles/coder`` → ``ai.clawk.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.clawk.gateway-{suffix}" if suffix else "ai.clawk.gateway"
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m clawk_cli.main``, so module
    ...
    dir, or a clone that ``clawk update`` later relocates/removes — the path
    ...
    ``CLAWK_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if CLAWK_HOME
...
def _systemd_watchdog_seconds(clawk_home: str | Path | None = None) -> int:
    ...
    if clawk_home is not None:
        ...
        override_token = set_clawk_home_override(clawk_home)
        reset_home_override = reset_clawk_home_override
    try:
...
def _systemd_watchdog_service_fields(
    clawk_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(clawk_home)
    if seconds <= 0:
        return "simple", ""
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        profile_arg = _profile_arg_for_target_user(clawk_home, home_dir)
...
Group={group_name}
ExecStart={python_path} -m clawk_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="CLAWK_HOME={clawk_home}"
Restart=always
RestartSec=5
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m clawk_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="CLAWK_HOME={clawk_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned CLAWK_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo clawk gateway … --system``, CLAWK_HOME is often stripped
    # and falls back to ``/root/.clawk``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / CLAWK_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir CLAWK_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved CLAWK_HOME
    ...
    exported a throwaway ``CLAWK_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``clawk gateway restart``
    with ``CLAWK_HOME=/tmp/clawk-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="CLAWK_HOME=..."`` lines and launchd
    ``<key>CLAWK_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'CLAWK_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>CLAWK_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp CLAWK_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: CLAWK_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported CLAWK_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single CLAWK_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only CLAWK_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp CLAWK_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../clawk_test`` path into
    # ``Environment="CLAWK_HOME=..."``. Writing that to the developer's
    ...
    # CLAWK_HOME into the unit (manual E2E homes like /tmp/clawk-e2e-NNN
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo clawk gateway uninstall --system")
    print_info("         clawk gateway install")
...
def systemd_install(
    ...
    # before installing the new clawk-gateway.service. If both remain, they
    ...
    # Existing system units already pin CLAWK_HOME; adopt it before any
    ...
    # ``sudo clawk gateway install --system --force`` would bake /root/.clawk
...
def systemd_start(system: bool = False):
    ...
    # CLAWK_HOME sync happens inside refresh_systemd_unit_if_needed's
...
# process invocation (each ``clawk gateway start/stop/status`` call).
...
#      background process (the `nohup clawk gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_clawk_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_clawk_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m clawk_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active CLAWK_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "clawk_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup clawk gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup clawk gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.clawksis/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See clawk-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_clawk_home() / "logs" / "launchd-reload.log"
        try:
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  clawk gateway status             # Check status")
    from clawk_constants import display_clawk_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    # launchd PID and the Clawksis PID file).
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: clawk gateway stop")
            else:
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``CLAWK_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``clawk gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``clawk gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``clawk gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other CLAWK_HOME roots.
    """
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Service-style launchers set CLAWK_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `clawk gateway restart` which already
    ...
    # `clawk gateway start/restart` — leaving the gateway vulnerable to
    ...
    # CLAWK_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    # the env vars ``CLAWK_GATEWAY_MAX_STARTS`` /
    # ``CLAWK_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("CLAWK_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("CLAWK_GATEWAY_START_WINDOW_S")
...
def _all_platforms() -> list[dict]:
    ...
    ``clawk setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run clawk under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.clawksis/plugins/ still require
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Clawksis will open Tencent iLink QR login in this terminal.")
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "ClawksisAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``clawk_cli.setup`` (which
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.clawksis/plugins/
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`clawk_cli.service_manager` are caught and surfaced as
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``clawk gateway stop --all`` and ``... restart --all``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``clawk gateway run`` are unaffected.
      2. ``CLAWK_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``clawk gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``CLAWK_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    # `docker logs` and ${CLAWK_HOME}/logs/gateways/<profile>/current,
    ...
    #
    # Prefer `sleep infinity` (matches the static main-clawk service's
    # pattern in docker/s6-rc.d/main-clawk/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print()
            print("To run the gateway: clawk gateway run")
            sys.exit(0)
        else:
        ...
        # slot — start them individually via `clawk -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: clawk gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
