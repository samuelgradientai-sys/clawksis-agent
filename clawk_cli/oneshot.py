...
configured for "cli" in `clawk tools`.

Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.

Approvals = auto-bypassed (CLAWK_YOLO_MODE=1 is set for the call).

Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).



Model / provider selection mirrors `clawk chat`:
...
def _validate_explicit_toolsets(
    ...
    try:
        ...
        return None, f"clawk -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "clawk -z: --toolsets all enables every toolset; "
    ...
    if not valid:
        return None, "clawk -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to CLAWK_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("CLAWK_INFERENCE_MODEL", "").strip()

    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "clawk -z: --provider requires --model (or CLAWK_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"clawk -z: agent failed: {failure}\n")
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``clawk -z`` / oneshot mode.



    Oneshot bypasses ``ClawksisCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when clawk is invoked for

    # other commands (keeps top-level CLI startup cheap).

    from clawk_cli.config import load_config

    from clawk_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("CLAWK_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("CLAWK_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                CLAWK_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via CLAWK_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via CLAWK_YOLO_MODE=1
