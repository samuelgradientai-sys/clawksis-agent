"""CLI handlers for the ``clawk proxy`` subcommand."""
...
from clawk_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    ...
    print(
        "clawk proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"clawk auth add {adapter.name}")
    ...
    print(
        f"Starting Clawksis proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Clawksis proxy upstream adapters\n")
    ...
    print("\nStart the proxy with: clawk proxy start [--provider <name>]")
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``clawk proxy <subcommand>``."""
    ...
    print(
        "clawk proxy — local OpenAI-compatible proxy that attaches your\n"
