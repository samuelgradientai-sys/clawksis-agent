...
# xAI's public API is OpenAI-compatible for the endpoints Clawksis commonly

# uses. The Responses endpoint is included because Clawksis' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Clawksis-managed OAuth credentials."""

    auth_hint = "clawk auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`clawk auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`clawk auth reset xai-oauth` or re-authenticate with "
                    "`clawk auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `clawk auth add xai-oauth --type oauth`."
            )
