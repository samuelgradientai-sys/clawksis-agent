    ...
    if _contains_gateway_lifecycle_command(combined):
        print(
            color(
                ...
                "Use `clawk gateway restart` from a shell outside the gateway.",
                Colors.RED,
            )
        )
        return 1
...
def cron_command(args):
    ...
    print("Usage: clawk cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
