"""``clawk logs`` — view and filter Clawksis log files.
...
Usage examples::
    ...
    clawk logs                    # last 50 lines of agent.log

    clawk logs -f                 # follow agent.log in real time

    clawk logs errors             # last 50 lines of errors.log

    clawk logs gateway -n 100    # last 100 lines of gateway.log

    clawk logs gui -f            # follow gui.log (dashboard/pty/ws)

    clawk logs desktop -f        # follow desktop.log (Electron app boot/backend)

    clawk logs --level WARNING    # only WARNING+ lines

    clawk logs --session abc123   # filter by session ID substring

    clawk logs --component tools  # only tool-related lines

    clawk logs --since 1h         # lines from the last hour

    clawk logs --since 30m -f     # follow, starting 30 min ago
...
def tail_log(
    ...
    *,
    ...
    log_path = get_clawk_home() / "logs" / filename

    if not log_path.exists():
        print(f"Log file not found: {log_path}")

        print(f"(Logs are created when Clawksis runs — try 'clawk chat' first)")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_clawk_home()}/logs/")

        return

    print(f"Log files in {display_clawk_home()}/logs/:\n")
    ...
    if not found:
        print("  (no log files yet — run 'clawk chat' to generate logs)")
