"""Agent-construction and session-resume display methods for ``ClawksisCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.clawksis/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``ClawksisCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``ClawksisCLI``."""
        ...
        *,
        ...
        try:
            ...
            self.agent = AIAgent(
                ...
                # ``_on_reaction``) no esta portado en este fork, asi que el
                # callback es opcional. getattr evita el AttributeError y deja
                # el hook listo si algun dia se porta.
                reaction_callback=getattr(self, "_on_reaction", None),
            )
