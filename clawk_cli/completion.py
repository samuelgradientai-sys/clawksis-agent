"""Shell completion script generation for clawk CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f'                    COMPREPLY=($(compgen -W "$(_clawk_profiles)" -- "$cur"))\n'
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Clawksis bash completion
...
_clawk_profiles() {{
    ...
    local profiles_dir="$HOME/.clawksis/profiles"
...
_clawk_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        ...
        COMPREPLY=($(compgen -W "$(_clawk_profiles)" -- "$cur"))
...
complete -F _clawk_completion clawk
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef clawk

# Clawksis zsh completion
...
_clawk_profiles() {{
    ...
    if [[ -d "$HOME/.clawksis/profiles" ]]; then

        profiles+=($HOME/.clawksis/profiles/*(N/:t))
...
_clawk() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_clawk_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'clawk command' subcmds
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Clawksis fish completion",
        "# Add to your config:",
        "#   clawk completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __clawk_profiles",
        "    echo default",
        "    if test -d $HOME/.clawksis/profiles",
        "        for d in $HOME/.clawksis/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c clawk -f -s p -l profile"
        " -d 'Profile name' -xa '(__clawk_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__clawk_profiles)' -d 'Profile name'"
                )
