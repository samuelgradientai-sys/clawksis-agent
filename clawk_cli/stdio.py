...
on Japanese locales, etc.).  Clawksis's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``clawk_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Clawksis is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CLAWK_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CLAWK_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
    ...
    # Augment PATH with the Clawksis-managed Git install directories so
    ...
    # already-running shells don't see the change — which means clawk
    ...
    # install without Clawksis-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Clawksis's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Clawksis's

    ``clawk config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Clawksis (User env var in Windows Settings, or

    export in a PowerShell profile) and Clawksis picks it up automatically.
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Clawksis-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\clawk\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    user invokes ``clawk`` from right after install) retain their old PATH.



    Any subprocess Clawksis spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Clawksis-managed tool directories to our
    ...
    candidate_dirs = [
        os.path.join(local_appdata, "clawk", "git", "cmd"),
        os.path.join(local_appdata, "clawk", "git", "bin"),
        os.path.join(local_appdata, "clawk", "git", "usr", "bin"),
        # Clawksis venv Scripts directory — host of the clawk.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes clawk, but harmless to include.
        os.path.join(local_appdata, "clawk", "clawksis-agent", "venv", "Scripts"),
