"""MCP picker — interactive `clawk mcp picker` (also the default `clawk mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``clawk mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `clawk plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from clawk_cli.cli_output import prompt_yes_no
from clawk_cli.curses_ui import curses_single_select
from clawk_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Clawksis session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from clawk_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `clawk mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: clawk mcp install <name>    Picker: clawk mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Clawksis is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Clawksis — run `clawk update` "
...
def show_catalog() -> None:
    """`clawk mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`clawk mcp picker` (and default `clawk mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`clawk mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `clawk mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
