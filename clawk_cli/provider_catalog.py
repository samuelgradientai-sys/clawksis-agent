...
The provider list shown by ``clawk model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`clawk_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``clawk model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`clawk_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``clawk model`` shows.
"""
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``clawk model`` universe.
    ...
    # Clawksis overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = CLAWK_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Clawksis overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
