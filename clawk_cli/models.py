...
Add, remove, or reorder entries here — both `clawk setup` and
`clawk` provider-selection will pick up the change automatically.
"""
...
from clawk_cli.urllib_security import open_credentialed_url

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_CLAWK_USER_AGENT = f"clawk-cli/{_CLAWK_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `clawk model`
...
# $CLAWK_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Clawksis-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $CLAWK_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Clawksis refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Clawksis that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Clawksis that doesn't ship
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$CLAWK_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  clawk model, /model, list_authenticated_providers.
#
# Fields:
...
#
# Some vendors expose several Clawksis provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `clawk model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``clawk model``, the
...
# In-repo fallback for the model Clawksis silently lands on when the user never
...
# ``clawk model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in clawk_cli/web_server.py and
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    clawksis-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — clawksis-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Clawksis lands on when none is selected.
            desc = "default"
        else:
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``clawk model``, ``/model``, etc.).
    """
...
def _resolve_static_model_alias(
    ...
    try:
        from clawk_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Clawksis' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``clawk_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Clawksis should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``clawk auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.clawksis/.env``.
    ...
    """
    try:
        from clawk_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(
    ...
    on the platform appear in ``/model`` without a Clawksis release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Clawksis release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Clawksis release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``clawk model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $CLAWK_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$CLAWK_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _CLAWK_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Clawksis' default Claude IDs elsewhere use
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=clawk"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Clawksis will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Clawksis cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Clawksis actually ships.  (#46850)
