"""Slash command definitions and autocomplete for the Clawksis CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Clawksis config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Clawksis works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Clawksis is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.clawksis/skills/ for newly installed or removed skills",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`clawk_cli.plugins.PluginContext.register_command`. They behave

    like ``CommandDef`` entries for gateway surfacing: they appear in the

    Telegram command menu, in Slack's ``/clawk`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Clawksis
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``clawk skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``clawk skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``clawk skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/clawk <verb>`` subcommand.
    ...
    can still reach them via ``/clawk <command>``.



    Results are clamped to Slack's 50-command limit with duplicate-name

    avoidance. ``/clawk`` is always reserved as the first entry so the

    legacy ``/clawk <subcommand>`` form keeps working for anything that
    ...
    # Reserve /clawk as the catch-all top-level command.

    entries.append(("clawk", "Talk to Clawksis or run a subcommand", "[subcommand] [args]"))
...
def slack_app_manifest(request_url: str = "https://clawksis-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    ...
    """Return subcommand -> /command mapping for Slack /clawk handler.



    Maps both canonical names and aliases so /clawk bg do stuff works

    the same as /clawk background do stuff.



    Plugin-registered slash commands are included so ``/clawk <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            ...
            from clawk_cli.skin_engine import list_skins
