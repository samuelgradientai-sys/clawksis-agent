...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = (
        os.getenv("CLAWK_QWEN_BASE_URL", "").strip().rstrip("/")
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Fork-only provider (upstream deleted it); keep it dispatchable.
    ...
    *,
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print(
                "Clawksis will create its own session to avoid conflicts with Codex CLI / VS Code."
            )
            try:
                do_import = (
                    input(
