...
from clawk_cli.config import get_clawk_home, get_config_path, read_raw_config
...
SPOTIFY_DOCS_URL = "https://clawksis-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://clawksis-agent.nousresearch.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://clawksis-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.clawksis/.env file
...
#
# Tokens live in ~/.clawksis/auth/google_oauth.json (managed by agent.google_oauth).
...
def _sync_codex_pool_entries(
    ...
    * ``manual:device_code`` — entries created by ``clawk auth add openai-codex``
      ...
      ``clawk auth add`` workaround for #33000 would silently leave that
...
def resolve_codex_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("CLAWK_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("CLAWK_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
...
# mtime so that `clawk auth login/logout/add/remove` invalidate naturally on the next call.
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from clawk_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
