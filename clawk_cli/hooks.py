"""clawk hooks — inspect and manage shell-script hooks.
...
Usage::
    ...
    clawk hooks test <event> [--for-tool X] [--payload-file F]

    clawk hooks revoke <command>

    clawk hooks doctor



Consent records live under ``~/.clawksis/shell-hooks-allowlist.json`` and

hook definitions come from the ``hooks:`` block in ``~/.clawksis/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``clawk hooks`` — dispatches to the requested action."""

    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: clawk hooks {list|test|revoke|doctor}")

        print("Run 'clawk hooks --help' for details.")
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.clawksis/config.yaml.")

        print("See `clawk hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `clawk hooks doctor` to re-validate"
                        )
...
# stdin a script sees under `clawk hooks test` and `clawk hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Clawksis wire shape): {json.dumps(parsed)}")
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    # is already allowlisted.  Otherwise `clawk hooks doctor` would execute
