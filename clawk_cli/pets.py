"""CLI subcommand: ``clawk pets <subcommand>``.
...
def _cmd_list(args) -> int:
    ...
    if getattr(args, "installed", False):
        ...
        if not pets:
            _print("No pets installed. Try: clawk pets install boba")
            return 0
    ...
    _print("\nInstall one with: clawk pets install <slug>")
    return 0
...
def _cmd_select(args) -> int:
    ...
    if not slug:
        pets = store.installed_pets()
        if not pets:
            _err("✗ no pets installed — run: clawk pets install boba")
            return 1
    ...
    if pet is None or not pet.exists:
        _err(f"✗ '{slug}' is not installed — run: clawk pets install {slug}")
        return 1
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        _print("  → no pets installed. Run: clawk pets install boba")
        ok = False
    elif active is None:
        _print("  → active pet unresolved. Run: clawk pets select <slug>")
        ok = False
    elif not enabled:
        _print("  → pet display is disabled. Run: clawk pets select " + active.slug)
