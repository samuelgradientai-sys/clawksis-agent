...
def _config_base_url_trustworthy_for_bare_custom(
    ...
    try:
        from clawk_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from clawk_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Clawksis' tool dispatch. Gated behind config key
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from clawk_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from clawk_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("CLAWK_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/clawk_cli/test_runtime_provider_resolution.py
...
def canonical_custom_identity(
    *,
    ...
    2. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``CLAWK_INFERENCE_PROVIDER`` equivalent). When the agent
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("CLAWK_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'clawk model' or "
...
def resolve_runtime_provider(
    *,
    ...
    from clawk_cli.config import is_provider_enabled, load_config
    ...
    if requested_provider in (
        ...
        if not token or not base_url:
            raise AuthError(
                ...
                "in ~/.clawksis/.env, or run 'gcloud auth application-default "
                "login' for ADC. Set the GCP project/region under vertex: in "
                "config.yaml if they aren't embedded in the credentials. "
                "Install the extra with: pip install 'clawksis-agent[vertex]'."
            )
        return {
            "provider": "vertex",
            "api_mode": "chat_completions",
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "clawk-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    # Fork-only provider: upstream deleted google-gemini-cli, this fork keeps
    # the OAuth + Cloud Code Assist backend (agent/gemini_cloudcode_adapter.py,
    # clawk_cli/providers.py entry, clawk_cli/auth.py registry + resolver).
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from clawk_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Clawksis canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Clawksis-compatible importers).
