"""`clawk checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.clawksis/checkpoints/``.  Actions:

    clawk checkpoints               # same as `status`
    clawk checkpoints status        # total size, project count, breakdown
    clawk checkpoints list          # per-project checkpoint counts + workdir
    clawk checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    clawk checkpoints clear [-f]    # nuke the entire base (asks first)
    clawk checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    clawk checkpoints
    clawk checkpoints prune --retention-days 3 --max-size-mb 200
    clawk checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: clawk checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``clawk checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `clawk checkpoints` → status
