...
Clawksis owns its own uv binary at ``$CLAWK_HOME/bin/uv`` (or ``uv.exe`` on
...
at ``$CLAWK_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Clawksis keeps *its* uv binary.



    ``$CLAWK_HOME/bin/uv`` on POSIX, ``$CLAWK_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``clawk update`` runs
    the call site from the *old*, already-imported ``clawk_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``clawk update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$CLAWK_HOME/bin/`` instead of ``~/.local/bin/``.
...
def rebuild_venv(uv_bin: str, venv_dir: Path, python_version: str = "3.11") -> bool:
    ...
    ``clawk.exe`` (gateway/desktop) holds ``venv\\Scripts\\python.exe`` open;
    ...
    venv back so Clawksis is never left with no venv at all.
    ...
    if result.returncode == 0:
        # Windows venvs put the interpreter at ``Scripts\python.exe``; probing a

        # bare ``Scripts/python`` there never matches, so every rebuild reported

        # failure and restored the backup even when uv succeeded.
        ...
        # Rebuild failed — restore the old venv so we never leave Clawksis with no
