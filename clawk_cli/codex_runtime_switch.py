...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Clawksis'
...
The actual runtime resolution happens in clawk_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "clawk"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, clawk-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Clawksis' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Clawksis for
        ...
        try:
            ...
            if "clawk-tools" in mig_report.migrated:
                msg_lines.append(
                    "Clawksis tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Clawksis runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Clawksis tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Clawksis runtime.")
