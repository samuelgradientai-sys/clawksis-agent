...
manual BotFather token copy-paste. Clawksis talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.clawksis-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "ClawksisSetupBot"

DEFAULT_BOT_NAME = "Clawksis"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``clawk_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``clawk_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"clawk_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Clawksis Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Clawksis Telegram onboarding service.")
