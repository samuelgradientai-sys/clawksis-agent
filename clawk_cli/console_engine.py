"""Safe Clawksis Console command engine.

This module backs ``clawk console`` and is intentionally narrower than the
full Clawksis CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["ClawksisConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="clawk", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `clawk "):
        return ""
    return summary
...
# dashboard opens a fresh ClawksisConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("clawk_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["ClawksisConsoleEngine", list[str]], str]:
    def handler(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    ...
    *,
    ...
    for child_path in paths:
        ...
        engine.register(
            ...
            confirmation=confirmation or f"Run `clawk {usage}`?",
        )


class ClawksisConsoleEngine:
    """Curated line-command executor for Clawksis Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Clawksis Console does not run shell syntax. Use one supported "
                    "Clawksis command at a time."
                )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Clawksis Console."""

        extracted = {
            "version": (
                "clawk_cli.subcommands.version",
            ...
            "prompt-size": (
                "clawk_cli.subcommands.prompt_size",
            ...
            "insights": (
                "clawk_cli.subcommands.insights",
            ...
            "security": (
                "clawk_cli.subcommands.security",
            ...
            "import": (
                "clawk_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Clawksis configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "clawk_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("clawk_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "clawk_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("clawk_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("clawk_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Clawksis Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Clawksis Console."
        return ""
...
def _status(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Clawksis Console.")
    ...
    from clawk_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from clawk_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from clawk_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from clawk_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config migrate")

    def _run() -> None:
        from clawk_cli.config import migrate_config
...
def _sessions_export(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
...
def _profile_status(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="clawk_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from clawk_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from clawk_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from clawk console")
...
def _cron_resume(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: ClawksisConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``clawk console`` REPL."""
    ...
    while True:
        if interactive:
            print("clawk> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
