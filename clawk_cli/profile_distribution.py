"""Profile distributions — shareable, packaged Clawksis profiles via git.

A distribution is a Clawksis profile published as a git repository (or
...
* ``clawk profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``clawk skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``clawk profile``, not a parallel tree):

    clawk profile install <source> [--name N] [--alias] [--force] [--yes]
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            clawk_requires=str(data.get("clawk_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.clawk_requires:
            out["clawk_requires"] = self.clawk_requires
        if self.author:
...
def check_clawk_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Clawksis {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``clawk profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Clawksis profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Clawksis distribution."
        )

    # Version check up-front so we fail fast
    check_clawk_requires(manifest.clawk_requires, clawk_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.clawksis).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="clawk_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `clawk profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`clawk profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="clawk_dist_update_") as tmp:
        plan = plan_install(
