"""
Clawksis Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with clawksis-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.clawksis/plugins/<name>/``
3. **Project plugins** – ``./.clawksis/plugins/<name>/`` (opt-in via
   ``CLAWK_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``clawk_agent.plugins``
   entry-point group.
...
# ---------------------------------------------------------------------------
#
# Set ``CLAWK_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.clawksis/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When CLAWK_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Clawksis loggers.
    """
...
class PluginManifest:
    ...
    #              in ~/.clawksis/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``clawk plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``clawk honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``clawk <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`clawk_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_slack_action_handler(
        ...
        Clawksis' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``clawk model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: clawk_cli.main imports plugins indirectly
        from clawk_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.clawksis/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to clawk_cli/. Two layouts are
        ...
        # 2. User plugins (~/.clawksis/plugins/)
        user_dir = get_clawk_home() / "plugins"
        ...
        if _env_enabled("CLAWK_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set CLAWK_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with clawk and must
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``clawk_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``clawk_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="clawk-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``clawk tools`` TUI so plugin-provided toolsets appear
