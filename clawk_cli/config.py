...
Configuration management for Clawksis.



Config files are stored in ~/.clawksis/ for easy access:

- ~/.clawksis/config.yaml  - All settings (model, toolsets, terminal, etc.)

- ~/.clawksis/.env         - API keys and secrets



This module provides:

- clawk config          - Show current configuration

- clawk config edit     - Open config in editor

- clawk config set      - Set a specific value

- clawk config wizard   - Re-run setup wizard
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``clawk config set`` (which rewrites
    ...
    ``config.yaml`` in place: clawk never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.clawksis/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``clawk logs``
    ...
    try:
        sys.stderr.write(f"⚠️  clawk config: {msg}\n")
...
#   the next ``subprocess.run([...])`` Clawksis makes loads attacker code

#   before main().

# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /

#   ``PYTHONUSERBASE`` — Python interpreter init. Clawksis itself starts

#   from one of these on every restart.

# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,

#   ``clawk update``, the TUI build.
...
#   on every plugin install / ``clawk update``.
...
# * ``CLAWK_HOME`` / ``CLAWK_PROFILE`` / ``CLAWK_CONFIG`` /

#   ``CLAWK_ENV`` — Clawksis runtime location flags. Writing these into
...
# IMPORTANT: ``CLAWK_*`` overall is NOT blocked. Many legitimate

# integration credentials follow that prefix (CLAWK_GEMINI_CLIENT_ID,

# CLAWK_LANGFUSE_PUBLIC_KEY, CLAWK_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "GIT_EXEC_PATH",
    "GIT_SHELL",
    # Clawksis runtime location — never via dashboard env writer.
    # NOT a CLAWK_* blanket: integration credentials (CLAWK_GEMINI_*,
    # CLAWK_LANGFUSE_*, CLAWK_SPOTIFY_*, ...) ARE allowed.
    "CLAWK_HOME",
    "CLAWK_PROFILE",
    "CLAWK_CONFIG",
    "CLAWK_ENV",
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Clawksis runtime location "
            "(CLAWK_HOME, CLAWK_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.clawksis/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `clawk plugins enable
    # observability/langfuse` or `clawk tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "CLAWK_LANGFUSE_ENV",
    "CLAWK_LANGFUSE_RELEASE",
    "CLAWK_LANGFUSE_SAMPLE_RATE",
    "CLAWK_LANGFUSE_MAX_CHARS",
    "CLAWK_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CLAWK_MANAGED", "").strip()
    ...
    managed_marker = get_clawk_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Clawksis is running in package-manager-managed mode.



    Two signals: the CLAWK_MANAGED env var (set by the systemd service),

    or a .managed marker file in CLAWK_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Clawksis was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.



    Resolution order:

    1. Stamped ``~/.clawksis/.install_method`` file (written by installers)

    2. CLAWK_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nousresearch/clawksis-agent`` image stamps ``docker``
    ...
    so ``clawk update`` bailed with "doesn't apply inside the Docker
    ...
    stamp = get_clawk_home() / ".install_method"
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.clawksis/.install_method."""
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Clawksis lives in a ``uv tool`` layout.



    ``uv tool install clawksis-agent`` places the install at

    ``.../uv/tools/clawksis-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``clawksis-agent`` happens to be uv-tool-installed on the machine while

    the *active* Clawksis is a regular pip/venv install, causing

    ``clawk update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/clawksis-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade clawksis-agent"

        import shutil

        if shutil.which("uv"):
            return "uv pip install --upgrade clawksis-agent"

        return "pip install --upgrade clawksis-agent"
...
# (see the project README, "Unsupported"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``clawk update`` / ``clawk update --check`` so the wording — and the
...
# Long-form text for ``clawk update`` / ``--check`` when running inside the

# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in

# clawk_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Clawksis, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``clawk update`` inside Docker.
...
def format_managed_message(action: str = "modify this Clawksis installation") -> str:
    """Build a user-facing error for managed installs."""

    managed_system = get_managed_system() or "a package manager"

    raw = os.getenv("CLAWK_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"

        return (
            f"Cannot {action}: this Clawksis installation is managed by NixOS "
            f"(CLAWK_MANAGED={env_hint}).\n"
            "Edit services.clawksis-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Clawksis installation is managed by Homebrew "
            f"(CLAWK_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade clawksis-agent"
        )

    return (
        f"Cannot {action}: this Clawksis installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Clawksis."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CLAWK_HOME/.container-mode.



    Returns a dict with keys: backend, container_name, exec_user, clawk_bin

    or None if container mode is not active, we're already inside the

    container, or CLAWK_DEV=1 is set.
    ...
    container_mode_file = get_clawk_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "clawksis-agent")

    exec_user = info.get("exec_user", "clawk")

    clawk_bin = info.get("clawk_bin", "/data/current-package/bin/clawk")
...
# Re-export from clawk_constants — canonical definition lives there.

from clawk_constants import get_clawk_home  # noqa: F811,E402
...
def get_config_path() -> Path:
    ...
    return get_clawk_home() / "config.yaml"


def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""

    return get_clawk_home() / ".env"
...
def _resolve_clawk_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the CLAWK_UID / CLAWK_GID env vars set by Docker deployments.



    Docker containers running Clawksis commonly set these to map the in-container

    user to a host user so volume-mounted state files end up with the right

    ownership. The entrypoint chowns the top-level CLAWK_HOME once, but

    subdirectories created at runtime by ``ensure_clawk_home()`` (especially
    ...
    uid_str = os.environ.get("CLAWK_UID", "").strip()

    gid_str = os.environ.get("CLAWK_GID", "").strip()
...
def _chown_to_clawk_uid(path) -> None:
    """Chown ``path`` to ``CLAWK_UID:CLAWK_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_clawk_home` on Docker deployments.

    See #34107.

    """

    uid, gid = _resolve_clawk_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the clawk group can

    share state with the gateway service.



    The mode can be overridden via the CLAWK_HOME_MODE environment variable

    (e.g. CLAWK_HOME_MODE=0701) for deployments where a web server (nginx,

    caddy, etc.) needs to traverse CLAWK_HOME to reach a served subdirectory.
    ...
    Also applies ``CLAWK_UID``/``CLAWK_GID``-based ownership when those env
    ...
    try:
        mode_str = os.environ.get("CLAWK_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Clawksis runs in a container with volume-mounted config files, forcing
    ...
    if os.environ.get("CLAWK_CONTAINER") or os.environ.get("CLAWK_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CLAWK_SKIP_CHMOD=1 to force-skip on other systems.
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CLAWK_HOME if the user doesn't have one yet."""
...
def ensure_clawk_home():
    """Ensure ~/.clawksis directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_clawk_home_managed(home)
...
def _ensure_clawk_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""

    if not home.is_dir():
        raise RuntimeError(
            f"CLAWK_HOME {home} does not exist. Run 'sudo nixos-rebuild switch' first."
        )
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Clawksis-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Clawksis
        ...
        # identity slot (SOUL.md). Empty by default. The CLAWK_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Clawksis auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Clawksis sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Clawksis
        ...
        # bundled Clawksis image, which drops to the `clawk` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Clawksis sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Clawksis should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``clawk chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.clawksis/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: clawk sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Clawksis truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``clawk kanban decompose`` and the kanban
        ...
        # ``clawk profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `clawk model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `clawk` (and `clawk chat`) launches by default:
        ...
        # REPL and `--tui` (or CLAWK_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `clawk --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `clawk -c` muscle memory.  Default off so existing
        # users aren't surprised.  CLAWK_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `clawk --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/clawk`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``CLAWK_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``CLAWK_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # Public URL override (env: ``CLAWK_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/clawk``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.clawksis/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.clawksis/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Clawksis feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Clawksis auto-pauses the goal and
    ...
    # always goes to ~/.clawksis/skills/.
    "skills": {
        "external_dirs": [],  # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${CLAWK_SKILL_DIR} and ${CLAWK_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `clawk curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `clawk update`, so pruning them only
        ...
        # skipped), snapshot ~/.clawksis/skills/ into
        # ~/.clawksis/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `clawk curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for clawk-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (CLAWK_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.clawksis/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CLAWK_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `clawk doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``clawk_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Clawksis to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `clawk -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `clawk` launches with
        ...
        # decomposition is manual via `clawk kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     clawksis-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Clawksis tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.clawksis/logs/.
    ...
    # update model picker lists without shipping a clawksis-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/samuelgradientai-sys/clawksis-agent",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `clawk model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.clawksis/.env,
        ...
        # behavior: files must live under the Clawksis cache, under
        ...
        # user. Bridged to CLAWK_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Clawksis
        # cache (~/.clawksis/cache/{documents,images,audio,video,screenshots})
        ...
        # to CLAWK_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # CLAWK_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CLAWK_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.clawksis/state.db.
    ...
    "sessions": {
        ...
        # default of ``clawk sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.clawksis/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        # Run a full ``clawk backup``-style zip of CLAWK_HOME before every
        # ``clawk update``.  Backups land in ``<CLAWK_HOME>/backups/`` and
        # can be restored with ``clawk import <path>``.  Off by default —
        # on large CLAWK_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `clawk update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<CLAWK_HOME>/lsp/bin/`` on first use.
    ...
    # `clawk tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.clawksis/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.clawksis/.env (or your shell) and never in config.yaml.
            ...
            # ~/.clawksis/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'clawk model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Clawksis auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @clawk:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CLAWK_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #clawk)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: clawk-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'clawksis-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Clawksis API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Clawksis API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Clawksis API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CLAWK_TOOL_PROGRESS and CLAWK_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.clawk.config`` entries, then checks
    ...
    try:
        ...
        # should never break `clawk update`.  Skill-config prompting is a
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``clawk_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(
        ...
        try:
            ...
            return [
                ConfigIssue(
                    ...
                    "Run 'clawk setup' to create a valid config",
                )
            ]
    ...
    if cp and not model_cfg:
        issues.append(
            ConfigIssue(
                "warning",
                "custom_providers defined but no 'model' section — Clawksis won't know which provider to use",
                "Add a model section:\n"
                "  model:\n"
                "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'clawk doctor' for fix suggestions.\033[0m")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CLAWK_HOME", "~/.clawksis")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CLAWK_TOOL_PROGRESS")

            old_mode = get_env_value("CLAWK_TOOL_PROGRESS_MODE")

            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"

                results["config_added"].append(
                    "display.tool_progress=off (from CLAWK_TOOL_PROGRESS=false)"
                )
                ...
                results["config_added"].append(
                    f"display.tool_progress={old_mode.lower()} (from CLAWK_TOOL_PROGRESS_MODE)"
                )
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CLAWK_TIMEZONE", "")

            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()

                results["config_added"].append(
                    f"timezone={old_tz.strip()} (from CLAWK_TIMEZONE)"
                )
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CLAWK_HOME/plugins/`` for currently installed user plugins.
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `clawk plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `clawk curator status` has no
    ...
    #   3. Creates `~/.clawksis/logs/curator/` if missing (belt-and-suspenders

    #      on top of ensure_clawk_home() — old profiles that predate this

    #      migration still benefit).

    if current_ver < 23:
        try:
            curator_dir = get_clawk_home() / "logs" / "curator"
    ...
    # metadata.clawk.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _clawk_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.clawksis/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.clawksis/config.yaml.
    ...
    (which change ``CLAWK_HOME`` and therefore ``get_config_path()``)
...
#   openai-codex (OAuth — clawk auth) — OpenAI Codex

#   nous         (OAuth — clawk auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.clawksis/config.yaml."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.clawksis/.env.
    ...
    interactive menu render (`clawk tools`, `clawk setup`, status

    panels). Sanitisation is O(lines × known-keys), so re-parsing the

    same file on every call was burning ~300ms of CPU per `clawk tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Clawksis env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.clawksis/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.clawksis/.env."""
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.clawksis/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.clawksis/.env into os.environ. Returns count of vars updated.



    Adds/updates vars that changed and removes vars that were deleted from

    the .env file (but only vars known to Clawksis — OPTIONAL_ENV_VARS and
    ...
    # Remove known Clawksis vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.clawksis/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.clawksis/.env`` over ``os.environ``.

    Used for Clawksis-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(
        color(
            "│              ∇ Clawksis Configuration                    │", Colors.CYAN
        )
    )
    ...
    # Warn on stale CLAWK_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("CLAWK_MAX_ITERATIONS")
        ...
        if (
            ...
            print(
                color(
                    f"                ⚠ .env has stale CLAWK_MAX_ITERATIONS={_env_ghost} "
                    f"(run 'clawk doctor --fix' to remove)",
                    Colors.YELLOW,
                )
            )
    ...
    print(color("  clawk config edit     # Edit config file", Colors.DIM))

    print(color("  clawk config set <key> <value>", Colors.DIM))

    print(color("  clawk setup           # Run setup wizard", Colors.DIM))
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: clawk config set <key> <value>")

            print()

            print("Examples:")

            print("  clawk config set model anthropic/claude-sonnet-4")

            print("  clawk config set terminal.backend docker")
        ...
        if missing_config:
            ...
            print("    Run 'clawk config migrate' to add them")
        ...
        print("  clawk config           Show current configuration")
        ...
        print("  clawk config migrate   Update config with new options")

        print("  clawk config path      Show config file path")

        print("  clawk config env-path  Show .env file path")
...
# ``_inject_profile_env_vars`` surfaces them in ``clawk config`` UI so users
