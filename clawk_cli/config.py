...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "CLAWK_HOME", "CLAWK_PROFILE", "CLAWK_CONFIG", "CLAWK_ENV",
})
...
def _ensure_clawk_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"CLAWK_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://clawksis-agent.nousresearch.com/docs/api/model-catalog.json",
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'clawk setup' to create a valid config")]
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            ...
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                ...
                results["config_added"].append("display.tool_progress=off (from CLAWK_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CLAWK_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            ...
            if old_tz and old_tz.strip():
                ...
                results["config_added"].append(f"timezone={old_tz.strip()} (from CLAWK_TIMEZONE)")
            else:
                config["timezone"] = ""
