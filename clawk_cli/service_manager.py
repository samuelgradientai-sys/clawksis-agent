...
the existing module-level functions in clawk_cli.gateway and
clawk_cli.gateway_windows directly. This protocol is a thin facade
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``clawk gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged clawk
    ...
    detection always fails. Since every Clawksis runtime call inside the
    container drops to clawk via ``s6-setuidgid``, that silent failure
...
# in ``clawk_cli.gateway`` (systemd/launchd) and ``clawk_cli.gateway_windows``
...
# active (set via the ``clawk -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in clawk_cli.gateway.
...
class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in clawk_cli.gateway."""
...
class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``clawk_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `clawk profile create`
# runs inside the container (Phase 4). Static services (main-clawk, dashboard)
...
# UID/GID of the in-image ``clawk`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the clawk user.
    ...
    (the ``clawk`` user — UID 10000 — running every Clawksis runtime
    ...
    is dispatched as the clawk user.
    ...
    with clawk ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           clawk:clawk, 0755 (parent must already exist)
    ``svc_dir/event/``                     clawk:clawk, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 clawk:clawk, 0755
    ``svc_dir/supervise/event/``           clawk:clawk, 03730
    ``svc_dir/supervise/control``          clawk:clawk, 0660    (FIFO)
    ...
    slot's supervise/ was clawk-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _CLAWK_UID, _CLAWK_GID)
        except PermissionError:
            # Running as the clawk user already — directory is clawk-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _CLAWK_UID, _CLAWK_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is clawk-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _CLAWK_UID, _CLAWK_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``clawk -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`clawk profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-clawk, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources CLAWK_HOME (and any extra env) via with-contenv —
             so e.g. ``-e CLAWK_HOME=/data/clawk`` is honored at run
          ...
          4. Drops to the clawk user and exec's
             ``clawk -p <profile> gateway run`` (or just ``clawk
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``clawk gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root CLAWK_HOME profile" (the implicit profile that exists at
        the top of $CLAWK_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``clawk_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$CLAWK_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``CLAWK_HOME``,
        ...
        allocator in ``clawk_cli.profiles._allocate_gateway_port``).
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${CLAWK_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the CLAWK_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e CLAWK_HOME=/data/clawk``
        gets its logs under /data/clawksis/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``clawk logs`` reads and what
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``clawk gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with clawk ownership
            ...
            # the clawk user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is clawk-owned (see ``_seed_supervise_skeleton``), and on
