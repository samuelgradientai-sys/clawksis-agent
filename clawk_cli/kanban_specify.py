...
Used by ``clawk kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``clawk_cli/goals.py`` — same aux
...
CLAWK_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    int(os.getenv("CLAWK_KANBAN_SPECIFY_MAX_TOKENS", "6000")),
)
...
_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Clawksis board.
