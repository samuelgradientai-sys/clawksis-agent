"""CLI for the Clawksis Kanban board — ``clawk kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/clawk-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from clawk_cli import kanban_swarm as ks
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this CLAWK_HOME and its
    ...
    Used by ``clawk kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`clawk kanban daemon --force`).",
        )
    return (
        False,
...
def build_parser(
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Clawksis profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://github.com/samuelgradientai-sys/clawksis-agent "
            "or docs/clawk-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # CLAWK_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `clawk kanban boards switch <slug>` or the "
            "CLAWK_KANBAN_BOARD env var). Use `clawk kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument(
        ...
        "deterministic branch. See `clawk project list`.",
    )
    ...
    p_list.add_argument(
        "--mine", action="store_true", help="Filter by $CLAWK_PROFILE as assignee"
    )
    ...
    p_comment.add_argument(
        "--author", default=None, help="Author name (default: $CLAWK_PROFILE or 'user')"
    )
    p_comment.add_argument(
        "--max-len",
        type=int,
    ...
    p_attach.add_argument(
        "--author",
        default=None,
        help="uploaded_by label (default: $CLAWK_PROFILE or 'user')",
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `clawk gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        "(union of ~/.clawksis/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        "(default: $CLAWK_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
        "(default: $CLAWK_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``clawk kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: clawk kanban <action> [options]\n"
                "Run 'clawk kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # env-var pin for the duration of this call. Using CLAWK_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `clawk kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # CLAWK_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("CLAWK_PROFILE_NAME", "CLAWK_PROFILE"):
...
# ---------------------------------------------------------------------------
# Boards management (clawk kanban boards …)
# ---------------------------------------------------------------------------


def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``clawk kanban boards <action>``.
    ...
    DB, so a fresh CLAWK_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `clawk kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `clawk kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `clawk kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `clawk kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.clawksis/profiles/.")
        print("Create one with `clawk -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  clawk gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `clawk -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``clawk kanban show <id>``
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            # `clawk kanban complete <id>` from the terminal tool and
            ...
            if task and task.goal_mode:
                ...
                if judge_available:
                    ...
                    try:
                        ...
                        # clawk_cli/goals.py. Unpacking fewer raises
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "clawk kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    clawk gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`clawk kanban list --status ready` / "
                    f"`clawk kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Clawksis profile (i.e. one the
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from clawk_cli import kanban_decompose as decomp
