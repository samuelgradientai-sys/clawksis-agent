...
_DHH = (
    display_clawk_home()
)  # user-facing display path (e.g. ~/.clawksis or ~/.clawksis/profiles/coder)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    if not profiles:
        check_info(
            "No per-profile gateways registered yet — create one with `clawk profile create <name>`"
        )
...
def run_doctor(args):
        ...
        try:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "Run 'clawk doctor --fix' or 'clawk setup' to migrate config"
                    )
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "Stale root-level provider/base_url in config.yaml — run 'clawk doctor --fix'"
                    )
    ...
    if soul_path.exists():
        ...
        check_warn(
            ...
            "(create it to give Clawksis a custom personality)",
        )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append(
                        "Large WAL file — run 'clawk doctor --fix' to checkpoint"
                    )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        ...
                        check_ok(
                            f"Fixed symlink: {_cmd_link_display}/clawk → {_venv_bin}"
                        )
                        ...
                        issues.append(
                            f"Broken symlink at {_cmd_link_display}/clawk — run 'clawk doctor --fix'"
                        )
                ...
                if should_fix:
                    ...
                    check_ok(
                        f"Created symlink: {_cmd_link_display}/clawk → {_venv_bin}"
                    )
                    ...
                    issues.append(
                        f"Missing {_cmd_link_display}/clawk symlink — run 'clawk doctor --fix'"
                    )
    ...
    try:
        ...
        if api_disabled:
            issues.append(
                "Run 'clawk setup' to configure missing API keys for full tool access"
            )
    ...
    if should_fix and fixed_count > 0:
        ...
        if not should_fix:
            print(
                color(
                    "  Tip: run 'clawk doctor --fix' to auto-fix what's possible.",
                    Colors.DIM,
                )
            )
