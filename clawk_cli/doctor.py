...
Doctor command for clawk CLI.



Diagnoses issues with Clawksis setup.
...
from clawk_cli.config import get_project_root, get_clawk_home, get_env_path
...
_DHH = display_clawk_home()  # user-facing display path (e.g. ~/.clawksis or ~/.clawksis/profiles/coder)



# Load environment variables from ~/.clawksis/.env so API key checks work

_env_path = get_env_path()

load_clawk_dotenv(clawk_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")



from clawk_cli.colors import Colors, color

from clawk_cli.models import _CLAWK_USER_AGENT
...
from clawk_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    ...
    """Return True when ~/.clawksis/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("CLAWK_KANBAN_TASK"):
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """Verify pyproject.toml version matches clawk_cli.__version__.



    A git conflict resolution (reset/merge) can revert one file without the

    other, leaving ``clawk --version`` reporting a stale version while
    ...
    try:
        ...
        from clawk_cli import __version__ as init_version
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != clawk_cli/__init__.py {init_version})",

            "Re-sync version files (e.g. run 'clawk update', or set "

            "clawk_cli/__init__.py __version__ to match pyproject.toml)",
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      ...
      - Whether the main-clawk and dashboard static services are up
    ...
    for static in ("main-clawk", "dashboard"):
    ...
    if not profiles:
        ...
        check_info("No per-profile gateways registered yet — create one with `clawk profile create <name>`")
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            ...
            from clawk_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `clawk`.

    os.environ.setdefault("CLAWK_INTERACTIVE", "1")



    # Handle `clawk doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.clawksis/config.yaml is writable.",
        ...
        if fresh_hits:
            ...
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`clawk doctor --ack {hit.advisory.id}`."
    ...
    # Detect drift between pyproject.toml and clawk_cli/__init__.py versions
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'clawk setup' to configure API keys")
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'clawk setup' to configure API keys")

                fixed_count += 1

            else:

                check_info("Run 'clawk setup' to create one")

                issues.append("Run 'clawk setup' to create .env")

    

    # Check ~/.clawksis/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                from clawk_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'clawk config set model.provider <valid_provider>'"
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                ...
                try:
                    ...
                    if runtime_provider == "openrouter":
                        ...
                        from clawk_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        ...
                        _fail_and_issue(
                            ...
                            "(check ~/.clawksis/.env or run 'clawk setup')",

                            (

                                f"No credentials found for provider '{runtime_provider}'. "

                                f"Run 'clawk setup' or set the provider's API key in {_DHH}/.env, "

                                f"or switch providers with 'clawk config set model.provider <name>'"
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from clawk_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    ...
                    try:
                        ...
                        issues.append("Run 'clawk setup' to migrate config")

                else:

                    issues.append("Run 'clawk doctor --fix' or 'clawk setup' to migrate config")
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'clawk doctor --fix'")
        ...
        # Detect stale CLAWK_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives CLAWK_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            from clawk_cli.config import load_env, remove_env_value
            ...
            env_ghost = load_env().get("CLAWK_MAX_ITERATIONS")
            ...
            if drift:
                ...
                check_warn(
                    ...
                    f"CLAWK_MAX_ITERATIONS={env_ghost} in .env shadows "

                    f"agent.max_turns={cfg_max_turns} in config.yaml",

                    "(stale ghost from an earlier `clawk setup` run)",

                )

                if should_fix:

                    if remove_env_value("CLAWK_MAX_ITERATIONS"):

                        check_ok(

                            "Removed stale CLAWK_MAX_ITERATIONS from .env "
                        ...
                        check_warn("Could not remove CLAWK_MAX_ITERATIONS from .env")

                        manual_issues.append(

                            "Manually delete the CLAWK_MAX_ITERATIONS line from "
                    ...
                    issues.append(
                        ...
                        "Stale CLAWK_MAX_ITERATIONS in .env shadows config.yaml — "

                        "run 'clawk doctor --fix'"
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Clawksis' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        ...
        subdir_path = clawk_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Clawksis a custom personality)")
    ...
    memories_dir = clawk_home / "memories"
    ...
    state_db_path = clawk_home / "state.db"
    ...
    wal_path = clawk_home / "state.db-wal"
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'clawk doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            ...
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "clawk"
        ...
        if _venv_bin is None:
            ...
            check_warn(
                ...
                "(clawk not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    check_ok(f"{_cmd_link_display}/clawk → correct target")

                else:

                    check_warn(

                        f"{_cmd_link_display}/clawk points to wrong target",
                    ...
                    if should_fix:
                        ...
                        check_ok(f"Fixed symlink: {_cmd_link_display}/clawk → {_venv_bin}")

                        fixed_count += 1

                    else:

                        issues.append(f"Broken symlink at {_cmd_link_display}/clawk — run 'clawk doctor --fix'")

            elif _cmd_link.exists():

                # It's a regular file, not a symlink — possibly a wrapper script

                check_ok(f"{_cmd_link_display}/clawk exists (non-symlink)")

            else:

                check_fail(

                    f"{_cmd_link_display}/clawk not found",

                    "(clawk command may not work outside the venv)"
                ...
                if should_fix:
                    ...
                    check_ok(f"Created symlink: {_cmd_link_display}/clawk → {_venv_bin}")
                    ...
                    issues.append(f"Missing {_cmd_link_display}/clawk symlink — run 'clawk doctor --fix'")
    ...
    try:
        ...
        from clawk_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            ...
            try:
                ...
                # to eagerly load in every `clawk doctor` invocation.
            ...
            if r.status_code == 402:
                ...
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     ...
                     "Fix: run 'clawk config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``clawk doctor``.
    ...
    # never the legitimate source for `clawk doctor`.
    ...
    try:
        ...
        if api_disabled:
            ...
            issues.append("Run 'clawk setup' to configure missing API keys for full tool access")
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: clawk skills list)")
    ...
    try:
        ...
        _mem_cfg_path = CLAWK_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                check_warn("Honcho config not found", "run: clawk memory setup")
                ...
                _fail_and_issue(
                    ...
                    "run: clawk memory setup",

                    "No Honcho API key — run 'clawk memory setup'",
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run clawk memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: clawk memory status")

            else:

                check_warn(f"{_active_memory_provider} plugin not found", "run: clawk memory setup")
    ...
    try:
        ...
        from clawk_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "clawk -p" in content:
                            ...
                            _m = _re.search(r"clawk -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        if not should_fix:
            ...
            print(color("  Tip: run 'clawk doctor --fix' to auto-fix what's possible.", Colors.DIM))
