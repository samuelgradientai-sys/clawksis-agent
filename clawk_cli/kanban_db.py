...
def create_task(
    ...
    *,
    ...
    each name to ``clawk --skills ...`` alongside the built-in
...
def _kanban_worker_skill_available(clawk_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``clawk -p <name>``), its ``SKILLS_DIR``
...
def _default_spawn(
    ...
    *,
    ...
    if _kanban_worker_skill_available(env.get("CLAWK_HOME")):
