"""CLI handlers for ``clawk migrate ...``.



Currently exposes only ``clawk migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``clawk migrate <subtype>``."""
    ...
    print("usage: clawk migrate xai [--apply] [--no-backup]", file=sys.stderr)
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(
            color(
                "Re-run with `clawk migrate xai --apply` to rewrite "
    ...
    print(
        color(
            "Run `clawk doctor` to confirm no retired xAI models remain.",
            Colors.DIM,
        )
    )
