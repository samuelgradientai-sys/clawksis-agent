"""``clawk debug`` debug tools for Clawksis.

Currently supports:
    clawk debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.clawksis/logs/*.log`` are not leaked into
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``clawk debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``clawk debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.clawksis/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``clawk debug share`` also runs an opportunistic sweep as a fallback
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``clawk dump`` is run
        internally.
...
def collect_share_bundle(
    ...
    This is the single source of collection + redaction for every upload
    destination (the paste.rs path in :func:`build_debug_share`) and for the
    ``--local`` render.  Centralising it guarantees every consumer is built
    from the *same* force-redacted snapshots — redaction is the safety
    boundary, so no consumer ever sees raw logs.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``clawk debug share`` (CLI) and the
    ...
    # shared collector so the upload and ``--local`` paths build identical,
...
def run_debug_share(args):
    ...
    # Upstream ships an opt-in ``--nous`` destination (private Nous-internal
    # S3 storage). Clawksis is BYOK and has no Nous account service, so the
    # flag is refused loudly instead of silently falling through to the
    # PUBLIC paste service (which would leak data the user asked to keep
    # private).
    vendor_private_upload = getattr(args, "nous", False)
    ...
    if vendor_private_upload:
        print(
            "ERROR: --nous is not available in Clawksis (no vendor-internal "
            "diagnostics service).\n"
            "       Use `clawk debug share --local` to print the report "
            "without uploading,\n"
            "       or `clawk debug share` to upload to a public paste "
            "service.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print("To delete now:  clawk debug delete <url>")
...
def run_debug(args):
    ...
    # best-effort — any failure is swallowed so ``clawk debug`` stays
