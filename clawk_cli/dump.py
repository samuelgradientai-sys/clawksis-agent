"""
Dump command for clawk CLI.

Outputs a compact, plain-text summary of the user's Clawksis setup
...
from clawk_cli.config import get_clawk_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.clawksis/.env.
    ...
    exports. ``clawk debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.clawk_build_sha`` by the Dockerfile's
    ``CLAWK_GIT_SHA`` build-arg (see ``clawk_cli/build_info.py``).
...
def _gateway_status() -> str:
    ...
    try:
        from clawk_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(clawk_home: Path) -> int:
...
def _cron_summary(clawk_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = clawk_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"clawk_home:      {display_clawk_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.clawksis/.env: a managed
