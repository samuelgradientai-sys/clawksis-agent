...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or clawk -c.",
    ...
    "Ctrl+Z suspends Clawksis to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "clawk -c resumes your most recent CLI session. clawk -c \"project name\" resumes by title.",
    "clawk -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "clawk -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "clawk chat -t web,terminal enables only specific toolsets for a focused session.",
    "clawk chat -s github-pr-workflow preloads a skill at launch.",
    "clawk chat -q \"query\" runs a single non-interactive query and exits.",
    "clawk chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "clawk chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "clawk --yolo bypasses all dangerous command approval prompts for the entire session.",
    "clawk chat --source telegram tags the session for filtering in clawk sessions list.",
    "clawk -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "clawk doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "clawk dump outputs a compact setup summary — great for bug reports.",
    "clawk config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "clawk config edit opens config.yaml in your default editor.",
    "clawk config check scans for missing or stale configuration options.",
    "clawk sessions browse opens an interactive session picker with search.",
    "clawk sessions stats shows session counts by platform and database size.",
    "clawk sessions prune --older-than 30 cleans up old sessions.",
    "clawk skills search react --source skills-sh searches the skills.sh public directory.",
    "clawk skills check scans installed hub skills for upstream updates.",
    "clawk skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "clawk skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "clawk mcp add github --command npx adds MCP servers from the command line.",
    "clawk mcp serve runs Clawksis itself as an MCP server for other agents.",
    "clawk auth add lets you add multiple API keys for credential pool rotation.",
    "clawk completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "clawk logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "clawk backup creates a zip backup of your entire Clawksis home directory.",
    "clawk profile create coder creates an isolated profile that becomes its own command.",
    "clawk profile create work --clone copies your current config and keys to a new profile.",
    "clawk update syncs new bundled skills to ALL profiles automatically.",
    "clawk gateway install sets up Clawksis as a system service (systemd/launchd).",
    "clawk memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "clawk webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: clawk tools disables unused tools, clawk skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "clawk models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Clawksis tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'clawk profile create coder' creates the 'coder' command.",
    "clawk profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Clawksis prints a resume command with session ID and stats.",
    "clawk sessions export backup.jsonl exports all sessions for backup or analysis.",
    "clawk -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "clawk skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .clawksis/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: clawk cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Clawksis runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "clawk gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Clawksis via a local macOS server.",
    ...
    "Context files (.clawk.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Clawksis loads project context from .clawk.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "clawk mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "clawk mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "clawk mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — clawk tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.clawksis/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "clawk chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "clawk plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "clawk acp runs Clawksis as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "CLAWK_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "clawk auth add nous or clawk auth add openai-codex sets up OAuth-based providers.",
    ...
    "clawk status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.clawksis/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Clawksis your own.",
    ...
    "Before a gateway session reset, Clawksis auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.clawksis/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at CLAWK_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "CLAWK_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CLAWK_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.clawksis/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'clawk profile create ls' would be rejected.",
    "clawk profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "clawk claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "CLAWK_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via clawk tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.clawksis/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.clawksis/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Clawksis auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Clawksis config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Clawksis is working.',
    ...
    '/reload reloads ~/.clawksis/.env into the running session — pick up new API keys without restarting.',
    ...
    "CLAWK_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.clawksis/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.clawksis/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'clawk curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "clawk curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'clawk curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'clawk auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'CLAWK_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CLAWK_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'clawk dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'clawk dashboard embeds the full Clawksis TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.clawksis/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.clawksis/dashboard-plugins/ — no npm build required.',
    ...
    'CLAWK_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'CLAWK_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'CLAWK_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CLAWK_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.clawksis/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.clawksis/skills/ so drop-in skills appear without restarting the session.',
    ...
    'clawk -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'clawk chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'clawk chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'clawk chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "clawk chat --source tool tags programmatic chats so they don't clutter clawk sessions list.",
    'clawk dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'clawk sessions rename <ID> "new title" renames any past session; clawk sessions delete <ID> removes one.',
    'clawk import restores a session export or profile archive produced by sessions export or profile export.',
    'clawk fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'clawk pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'clawk setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'clawk status --deep runs the full health sweep across every component; plain clawk status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'CLAWK_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'CLAWK_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.clawksis/plugins/ — trust-gated by design.',
    "CLAWK_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CLAWK_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CLAWK_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CLAWK_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CLAWK_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CLAWK_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CLAWK_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CLAWK_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CLAWK_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CLAWK_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CLAWK_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Clawksis block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from clawk sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.clawksis/dashboard-plugins/.',
]
