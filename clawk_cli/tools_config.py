"""
Unified tool configuration for Clawksis.

`clawk tools` and `clawk setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.clawksis/config.yaml under
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``clawk tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`clawk_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from clawk_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _CLAWK_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `clawk tools` → Video Generation, which walks
...
# `clawk tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {
    "homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search",
    "codex_cli", "claude_code_cli", "opencode_cli", "mirofish", "agent_comms",
}
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from clawk_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `clawk tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``clawk tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from clawk_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    ...
    so ``clawk tools`` never claims to add or remove a toolset the user was
...
from clawk_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "desc": "Many hosted voice models on DeepInfra; needs a DeepInfra API key.",
                "env_vars": [
    ...
    "image_gen": {
        ...
        # Fork is BYOK: no managed-subscription row remains here.
        "providers": [],
    },
    "video_gen": {
        ...
        # ``_visible_providers``. Fork is BYOK: no managed-subscription row
        # remains here.
        "providers": [],
    },
    "x_search": {
        ...
        "setup_note": (
            "Clawksis routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set CLAWK_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "CLAWK_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "CLAWK_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "CLAWK_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "CLAWK_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "CLAWK_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    },
    # External coding CLIs — installed globally via npm by their post_setup
    # hooks (see _install_npm_cli). Auth is delegated to each vendor's own
    # login flow, so no env vars are prompted here.
    "codex_cli": {
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "MIROFISH_BASE_URL", "prompt": "MiroFish server URL", "default": "http://localhost:5001"},
                ],
                "post_setup": "mirofish",
            },
        ],
...
# provider+model picker like `clawk model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("CLAWK_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Clawksis telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``clawk update`` and
      by ``clawk computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `clawk update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `clawk computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `clawk update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `clawk update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.clawksis/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Clawksis process.")
            return True
...
def _npm_global_bin_dir(npm_bin: str) -> str:
    ...
    try:
        result = subprocess.run(
            [npm_bin, "prefix", "-g"], capture_output=True, text=True, timeout=15
        )
    except Exception:
        return ""
...
def _install_npm_cli(pkg: str, bin_name: str, label: str, *, login_hint: str = "") -> None:
    ...
    try:
        result = subprocess.run(
            [npm_bin, "install", "-g", pkg], capture_output=True, text=True, timeout=600
        )
    except subprocess.TimeoutExpired:
    ...
    if result.returncode == 0 and shutil.which(bin_name):
        ...
        _print_warning(
            f"    {os.path.dirname(installed_at)} is not on PATH — add it to your "
            f"shell profile so `{bin_name}` works everywhere."
        )
        if login_hint:
            _print_info(f"    {login_hint}")
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_clawk_home()}/clawksis-agent && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in clawksis-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "    Rebuild the image to get the bundled Chromium:"
            )
            _print_info(
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.clawksis/config.yaml")
        ...
        # Run the full `clawk auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.clawksis/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from clawk_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: clawk auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `clawk auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: clawk auth spotify")
        except Exception as exc:
        ...
        try:
            from clawk_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: clawk plugins enable observability/langfuse")
        _print_info("    Restart Clawksis for tracing to take effect.")
        _print_info("    Verify: clawk plugins list")
        ...
        try:
            ...
            _print_info("    Run later: clawk auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `clawk auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: clawk auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: clawk auth add xai-oauth"
                )
        else:
        ...
        if not shutil.which("docker"):
            _print_warning("    docker not found on PATH — install Docker to run MiroFish.")
...
def run_post_setup_command(args) -> int:
    """``clawk tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: clawk tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``clawk-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``clawk-discord``) is an opt-in to the platform's native default-off
    ...
    # "clawk-cli" (which include all _CLAWK_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[clawk-cli, spotify]`` after enabling Spotify via ``clawk
        ...
        # (e.g. "clawk-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``clawk tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `clawk tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"clawk-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("clawk-")}
    ...
    # shipped as a bundled plugin but user must opt in via `clawk tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `clawk tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by clawk tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `clawk` instead of
    # `clawk-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `clawk update`/`clawk doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `clawk tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (clawk-cli, clawk-telegram, etc.)
    ...
    # Opening `clawk tools` is the user's opt-in to reconfigure tools, so treat
...
def _visible_providers(
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # Fork (BYOK): managed-subscription / portal-auth rows are never
        # offered. Kept as a guard so a plugin re-introducing such a row
        # can't surface it in the picker.
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `clawk tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    no-op."""
    from clawk_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from clawk_cli.nous_subscription import _has_agent_browser
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `clawk model`).
    if managed_feature:
        from clawk_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``clawk model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `clawk model`, then re-run this."
        )
        return
...
def _configure_tool_category_for_reconfig(
    ...
    *,
    ...
    if len(providers) == 1:
        ...
        provider_idx = _prompt_choice(
            "  Select provider:", provider_choices, default_idx, descriptions=provider_descs
        )
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `clawk model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `clawk tools` and `clawk setup tools`.
    ...
    print(color("  Guide: https://github.com/samuelgradientai-sys/clawksis-agent", Colors.DIM))
    print()
    ...
    print()
    from clawk_constants import display_clawk_home
    print(color(f"  Tool configuration saved to {display_clawk_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'clawk' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, clawk mcp
