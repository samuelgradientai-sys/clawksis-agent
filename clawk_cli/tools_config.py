        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. CLAWK_CUA_DRIVER_VERSION is an
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `clawk update` calls this for every
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_clawk_home()}/clawksis-agent && npm install")
                if result.stderr:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/clawksis-agent:latest"
            )
            return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://clawksis-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
