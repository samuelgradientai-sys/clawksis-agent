        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {
                        "key": "CLAWK_LANGFUSE_PUBLIC_KEY",
                        "prompt": "Langfuse public key (pk-lf-...)",
                        "url": "https://cloud.langfuse.com",
                    },
                    {
                        "key": "CLAWK_LANGFUSE_SECRET_KEY",
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(
                    f"    npm install failed - run manually: cd {display_clawk_home()}/clawksis-agent && npm install --workspaces=false"
                )
                if result.stderr:
            ...
            _print_warning(
                "    Node.js not found - browser tools require: npm install (in clawksis-agent directory)"
            )
            return
        ...
        _print_info(
            "    Switch voices by setting tts.piper.voice in ~/.clawksis/config.yaml"
        )
        ...
        try:
            ...
            _print_info(
                "    Run later: clawk auth add xai-oauth   (or set XAI_API_KEY)"
            )
            return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(
        color(
            "  Guide: https://github.com/samuelgradientai-sys/clawksis-agent",
            Colors.DIM,
        )
    )
    print()
    ...
    print(
        color(
            f"  Tool configuration saved to {display_clawk_home()}/config.yaml",
            Colors.DIM,
        )
    )
    print(
        color("  Changes take effect on next 'clawk' or gateway restart.", Colors.DIM)
    )
    print()
