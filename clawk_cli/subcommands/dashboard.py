"""``clawk dashboard`` / ``clawk serve`` subcommand parsers.
...
``clawk_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `clawk dashboard` / `clawk serve` cmdlines and SIGTERM them directly —
    # the same path `clawk update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Clawksis web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Clawksis web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Clawksis desktop app shells (<= 0.15.x) spawn the
    # backend as `clawk dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Clawksis couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Clawksis Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Clawksis backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Clawksis backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports CLAWK_SERVE_HEADLESS=1). `dashboard` leaves it
