"""Migrate Clawksis' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Clawksis' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  ...
  Clawksis mcp_servers.<n>.command/args/env  → codex stdio transport

  Clawksis mcp_servers.<n>.url/headers       → codex streamable_http transport

  Clawksis mcp_servers.<n>.timeout           → codex tool_timeout_sec

  Clawksis mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec



What does NOT translate (warned + skipped):

  Clawksis-specific keys (sampling, etc.) — codex's MCP client has no
...
What's NOT migrated (intentional):
  ...
  AGENTS.md — codex respects this file natively in its cwd. Clawksis' own
...
MIGRATION_MARKER = "# managed by clawksis-agent — `clawk codex-runtime migrate` regenerates this section"

MIGRATION_END_MARKER = "# end clawksis-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Clawksis config.")
...
# Clawksis keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Clawksis' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, clawk_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Clawksis MCP server config to the codex inline-table dict
    ...
    if not isinstance(clawk_cfg, dict):
        return None, []
    ...
    has_command = bool(clawk_cfg.get("command"))

    has_url = bool(clawk_cfg.get("url"))
    ...
    if has_command:
        ...
        out["command"] = str(clawk_cfg["command"])

        args = clawk_cfg.get("args") or []

        if args:
            out["args"] = [str(a) for a in args]

        env = clawk_cfg.get("env") or {}
        ...
        out["url"] = str(clawk_cfg["url"])

        headers = clawk_cfg.get("headers") or {}

        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}

        # Clawksis' transport: sse hint is informational; codex auto-negotiates

        if clawk_cfg.get("transport") == "sse":
    ...
    if "timeout" in clawk_cfg:
        try:
            out["tool_timeout_sec"] = float(clawk_cfg["timeout"])

        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")

    if "connect_timeout" in clawk_cfg:
        try:
            out["startup_timeout_sec"] = float(clawk_cfg["connect_timeout"])
    ...
    if clawk_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning

    for key in clawk_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")

        elif key not in _KNOWN_CLAWK_KEYS:
            skipped.append(f"{key} (unknown Clawksis key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (CLAWK_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Clawksis)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Clawksis' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Clawksis' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Clawksis
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="clawk-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a CLAWK_HOME pointing

    at one of those paths is burned into ``~/.codex/config.toml``, every

    codex-routed clawk-tools call fails silently once the directory is GC'd.



    We err on the side of refusing — losing a (very unlikely) real

    ``~/.clawksis`` symlink that happens to live under ``/private/var/folders``
...
def _build_clawk_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Clawksis' own
    ...
    so a clawk installed under /opt/, /usr/local/, or a venv all work.

    CLAWK_HOME and PYTHONPATH are passed through so the spawned process
    ...
    # CLAWK_HOME passes through IF SET so the MCP subprocess sees the same

    # config / auth / sessions DB as the parent CLI. Read from os.environ

    # (not get_clawk_home()) on purpose: when the env var is unset we want

    # codex's subprocess to inherit whatever CLAWK_HOME its launcher sets
    ...
    # — that would override the launcher's CLAWK_HOME and pin the subprocess
    ...
    # a sibling test's monkeypatch.setenv("CLAWK_HOME", tmp_path) would
    ...
    clawk_home = os.environ.get("CLAWK_HOME") or ""

    if clawk_home and _looks_like_test_tempdir(clawk_home):
        clawk_home = ""

    if clawk_home:
        env["CLAWK_HOME"] = clawk_home

    # PYTHONPATH passes through so a worktree-launched clawk finds the
    ...
    env["CLAWK_REDACT_SECRETS"] = env.get("CLAWK_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_clawk_tools: bool = True,
) -> MigrationReport:
    """Translate Clawksis mcp_servers config + Codex curated plugins into
    ...
    Args:
        ...
        clawk_config: full ~/.clawksis/config.yaml dict
        ...
        expose_clawk_tools: when True (default), register Clawksis' own

            tool surface (web_search, browser_*, delegate_task, vision,

            memory, skills, etc.) as an MCP server in ~/.codex/config.toml

            so the codex subprocess can call back into Clawksis for tools
    ...
    clawk_servers = (clawk_config or {}).get("mcp_servers") or {}

    if not isinstance(clawk_servers, dict):
        report.errors.append(
            "mcp_servers in Clawksis config is not a dict; cannot migrate."
        )
    ...
    for name, cfg in clawk_servers.items():
    ...
    # Inject Clawksis' own tool surface as an MCP server so the spawned

    # codex subprocess can call back into Clawksis for the tools codex

    # doesn't ship with — web_search, browser_*, delegate_task, vision,

    # memory, skills, session_search, image_generate, text_to_speech.

    # The server itself is agent/transports/clawk_tools_mcp_server.py

    # and is launched on demand by codex (stdio MCP).

    if expose_clawk_tools:
        translated["clawk-tools"] = _build_clawk_tools_mcp_entry()

        if "clawk-tools" not in report.migrated:
            report.migrated.append("clawk-tools")
