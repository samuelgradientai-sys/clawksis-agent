...
# /opt/clawksis/bin/clawk — `docker exec` privilege-drop shim.
...
# `clawk` user (UID 10000). When an operator runs `docker exec <c> clawk ...`

# the default UID is root (0), and any file the command writes under

# $CLAWK_HOME — auth.json, .env, config.yaml — ends up root-owned and

# unreadable to the supervised gateway. The most common manifestation: the

# user runs `docker exec <c> clawk login`, this writes

# /opt/data/auth.json as root:root mode 0600, and from then on the gateway

# returns "Provider authentication failed: Clawksis is not logged into Nous

# Portal" on every incoming message — even though `docker exec <c> clawk
...
# This shim sits at /opt/clawksis/bin/clawk and is placed earliest on PATH.

# When invoked as root, it drops to the clawk user (via s6-setuidgid)

# before exec'ing the real venv binary, so anything that writes under

# $CLAWK_HOME is uid-aligned with the supervised processes. When invoked

# as any non-root UID — including the supervised processes themselves,

# `docker exec --user clawk`, kanban subagents, etc. — it short-circuits
...
# (/opt/clawksis/.venv/bin/clawk), so the second hop cannot re-enter this

# shim regardless of PATH state. No sentinel env var needed.

#

# Opt-out: set CLAWK_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the clawk CLI. Default is to drop.
...
REAL=/opt/clawksis/.venv/bin/clawk
...
if [ ! -x "$REAL" ]; then
    ...
    echo "clawk-shim: $REAL not found or not executable" >&2
...
# supervised processes (uid 10000) and for `docker exec --user clawk`.
...
case "${CLAWK_DOCKER_EXEC_AS_ROOT:-}" in
...
# Root, no opt-out. Drop to the clawk user.
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "clawk-shim: $S6_SUID not found; refusing to silently run as root." >&2

    echo "clawk-shim: re-run with --user clawk or set CLAWK_DOCKER_EXEC_AS_ROOT=1." >&2
...
# Reset HOME to the clawk user's home before dropping privileges. Without
