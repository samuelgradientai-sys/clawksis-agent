...
# Clawksis WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: CLAWK_TINI_SHIM_TARGET overrides the `/init` path (and
# CLAWK_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${CLAWK_TINI_SHIM_TARGET:-/init}"
WRAPPER="${CLAWK_TINI_SHIM_WRAPPER:-/opt/clawksis/docker/main-wrapper.sh}"
