...
RUN set -eu; \
    ...
    # 'Clawksis WebUI' catalog). The image has moved to s6-overlay /init\
...
# ---------- Permissions ----------
# Make install dir world-readable so any CLAWK_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the clawk user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# clawk_cli/main.py succeeds (see #18800). /opt/clawksis/web is build-time
# only (CLAWK_WEB_DIST points at clawk_cli/web_dist) and is intentionally
# not chowned here.
# /opt/clawksis/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the clawk UID is remapped at boot (#27221).
# The .venv MUST remain clawk-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/clawksis && \
    chown -R clawk:clawk /opt/clawksis/.venv /opt/clawksis/ui-tui /opt/clawksis/gateway /opt/clawksis/node_modules
...
# ---------- Link clawksis-agent itself (editable) ----------
...
RUN if [ -n "${CLAWK_GIT_SHA}" ]; then \
        ...
        chown clawk:clawk /opt/clawksis/.clawk_build_sha; \
    fi
...
COPY --chmod=0755 docker/clawk-exec-shim.sh /opt/clawksis/bin/clawk
