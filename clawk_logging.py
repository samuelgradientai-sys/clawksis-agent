"""Centralized logging setup for Clawksis.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.clawksis/logs/`` (profile-aware via ``get_clawk_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``clawk_cli.web_server``,
    ``clawk_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Clawksis (TUI, gateway, ``hy_memory`` server, MCP
...
from clawk_constants import get_config_path, get_clawk_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_clawk_session_injector", False):
    ...
    _session_record_factory._clawk_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``clawk logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``clawk logs --component gateway``.
    "gateway": ("gateway", "clawk_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("clawk_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "clawk_cli.web_server",
        "clawk_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Clawksis logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    clawk_home
        Override for the Clawksis home directory.  Falls back to
        ``get_clawk_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    handler._clawk_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the clawk group. However, both ``_open()`` (initial creation)
...
# the module header): when several Clawksis processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._clawk_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="clawk-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_clawk_queue", False):
