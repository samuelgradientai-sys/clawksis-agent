...
class Migrator:
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record(
                ...
                destination,
                "skipped",
                "Clawksis config.yaml does not exist yet",
            )
    ...
    def migrate_messaging_settings(
        ...
        if additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No Clawksis-compatible messaging settings found",
            )
            ...
            for src in sources:
                for oc_key, clawk_key in env_key_mapping.items():
                    ...
                    if (
                        isinstance(val, str)
                        and val.strip()
                        and clawk_key not in secret_additions
                    ):
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(
                    candidate,
                    reason="No direct Clawksis destination; archived for manual review",
                )
        ...
        partially_extracted = [
            (
                "openclaw.json",
                "Selected Clawksis-compatible values were extracted; raw OpenClaw config was not copied.",
            ),
            (
                "credentials/telegram-default-allowFrom.json",
                "Selected Clawksis-compatible values were extracted; raw credentials file was not copied.",
            ),
        ]
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if changes:
            ...
            self.record(
                ...
                "migrated",
                "Agent defaults mapped to Clawksis config",
            )
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record(
            ...
            "archived",
            "Gateway config archived. Use 'clawk gateway' to configure.",
        )
        ...
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var(
                "CLAWK_GATEWAY_TOKEN", auth["token"], "gateway.auth.token"
            )
    ...
    def generate_migration_notes(self) -> None:
        ...
        if has_cron_config_archive:
            notes.append(
                "- Run `clawk cron` to recreate scheduled tasks (see archive/cron-config.json)"
            )

        elif has_cron_store_archive:
            notes.append(
                "- Run `clawk cron` to recreate scheduled tasks (see archived cron-store)"
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Migrate OpenClaw user state into Clawksis."
    )
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing Clawksis targets after backing them up",
    )
