...
contact = client.messages.create(
    ...
    max_tokens=1024,
    messages=[
        {"role": "user", "content": "Extract: john@example.com, https://example.com"}
    ],
...
class Subscription(BaseModel):
    ...
    def validate_plan_limits(self):
        ...
        limits = {
            ...
            "enterprise": {
                "max_users": 999,
                "required_features": ["basic", "advanced", "premium"],
            },
        }
        ...
        if self.plan in limits:
            ...
            if self.max_users > limit["max_users"]:
                raise ValueError(
                    f"{self.plan} plan limited to {limit['max_users']} users"
                )
...
class DetailedModel(BaseModel):
    name: str = Field(
        min_length=2, max_length=100, description="Name between 2-100 characters"
    )
    age: int = Field(ge=0, le=120, description="Age between 0 and 120 years")
    ...
    def validate_name(cls, v):
        ...
        if not v.strip():
            raise ValueError(
                "Name cannot be empty. Please provide a valid name from the text."
            )
        return v
...
class Item(BaseModel):
    name: str = Field(
        min_length=1, max_length=200, description="Item name, 1-200 characters"
    )
```
