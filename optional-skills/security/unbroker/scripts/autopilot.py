...
def _digest(
    broker_row: dict, reason: str, steps: list[str], prep: list[str] | None = None
) -> dict:
    return {
        ...
        "reason": reason,
        "agent_prep": prep
        or [],  # commands the agent runs BEFORE handing this to the human
...
def _email_lane(row: dict) -> tuple[str | None, str]:
    ...
    if (
        row.get("tier") == "T3" or any(req.get(k) for k in _HUMAN_GATES)
    ) and deletion.get("email"):
        return deletion[
            "email"
        ], "rescue: primary flow is human-gated; deletion email restores autonomy"
    return None, ""
...
def _optout_action(
    ...
    can_email = (
        email_mode in ("programmatic", "alias") and smtp_ok
    ) or email_mode == "browser"
    if email_addr and can_email:
        kind = request_kind(dossier, deletion.get("kinds"))
        via = "browser" if email_mode == "browser" else "smtp"
        then = (
            "send-email records it + returns a recipient-locked payload; compose and send it in "
            "the operator's webmail via browser_*, then `verify-link` on the reply and open the link"
            if via == "browser"
            else "state auto-records as submitted; poll-verification picks up their verification reply, "
    ...
    if row.get("method") == "email":
        return None, _digest(
            row,
            "email opt-out (draft mode: a human must hit send)",
            [
            ...
            prep=[
                f"python3 scripts/pdd.py render-email {subject_id} {bid} --listing <confirmed-url>"
            ],
        )
    ...
    if tier == "T3":
        return None, _digest(
            row,
            "human-only opt-out (gov ID / fax / mail / voice phone)",
            [
    ...
    if req.get("phone_callback"):
        return None, _digest(
            row,
            "phone-callback verification (operator must be on the phone)",
            [
            ...
            prep=[
                f"python3 scripts/pdd.py plan {subject_id} --batch  # confirm fields first"
            ],
        )
    if req.get("account"):
        return None, _digest(
            row,
            "requires creating/holding an account with the broker",
            [
    ...
    steps = (
        (playbook.get(bid) or {}).get("steps")
        or list(row.get("optout_playbook") or [])
    ...
    if deletion:
        if deletion.get("prefer", True):
            action["prefer_deletion"] = (
                "this record has a right-to-delete lane -- complete the "
    ...
    if req.get("captcha"):
        action["note"] = (
            "CAPTCHA-gated: attempt with the configured browser backend once; if it "
...
def next_actions(
    ...
    for st in ("submitted", "verification_pending"):
        for bid, case in sorted(ledger.items()):
            ...
            if not ((broker.get("optout") or {}).get("requires") or {}).get(
                "email_verification"
            ):
                continue
            if mail["imap"]:
                actions.append({
                    "type": "poll_verification",
                    "via": "imap",
                    "broker_id": bid,
    ...
    for case in ledger_mod.due(subject_id, at=at, ledger=ledger):
        ...
        if st in ("awaiting_processing", "confirmed_removed"):
            actions.append({
                "type": "verify_removal",
                "broker_id": bid,
                "why": "processing window elapsed"
                if st == "awaiting_processing"
                else "periodic reappearance re-scan",
    ...
    for row in groups.get("found") or []:
        action, task = _optout_action(
            row, playbook, subject_id, dossier, email_mode, mail["smtp"], confirm_first
        )
        if action:
    ...
    for row in groups.get("indirect_exposure") or []:
        bid = row["broker_id"]
        has_email = bool(
            row.get("optout_email") or (row.get("deletion") or {}).get("email")
        )
        if not has_email and row.get("optout_url"):
            ...
            actions.append({
                ...
                "optout_url": row.get("optout_url"),
                "steps": [
                    f"browser_navigate {row.get('optout_url')}",
            ...
            email_mode in ("programmatic", "alias") and mail["smtp"]
        ) or email_mode == "browser":
            actions.append({
            ...
            digest.append(
                _digest(
                    row,
                    "indirect-exposure request (draft mode: a human must hit send)",
                    [
                    ...
                    prep=[
                        f"python3 scripts/pdd.py render-email {subject_id} {bid} "
        ...
        if bool(env.get("BROWSERBASE_API_KEY")):
            ...
            for r in blocked:
                digest.append(
                    _digest(
                        r,
                        "site blocks automated access (anti-bot); a human browser gets through",
                        [
    ...
    if registry_recs:
        coverage = {
            ...
            "worked_via": "CA DROP one-shot"
            if residency.startswith("US-CA")
            else "targeted CCPA/GDPR email",
        }
        if not residency.startswith("US-CA"):
            coverage["note"] = (
                "DROP is CA-only; for this subject the registry is covered by targeted "
                "CCPA/GDPR deletion emails (`registry --search` then `send-email`), "
                "not a single portal request."
            )
        elif drop_filed:
            coverage["note"] = (
                "DROP already filed; registry deletions are in the brokers' hands."
            )
    ...
    return {
        ...
        "note": (
            ...
            if confirm_first
            else "full autonomy: recorded intake consent authorizes these submissions; do not pause. "
