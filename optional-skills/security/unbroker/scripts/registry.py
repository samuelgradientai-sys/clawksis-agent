...
def _norm(s: str) -> str:
    ...
    return re.sub(
        r"\s+", " ", (s or "").replace("\ufeff", "").replace("\xa0", " ")
    ).strip()


def slug(name: str, website: str = "") -> str:
    base = re.sub(
        r"\.(com|org|net|io|ai|inc|co|us|info|llc)\b",
        "",
        (name or "").strip(),
        flags=re.I,
    )
...
def _build(
    ...
    if has_drop:
        notes = (
            "Registered CA data broker. One CA DROP request (privacy.ca.gov/drop) deletes from "
            "this and every registered broker at once; or send a CCPA deletion request to the "
            "contact email."
        )
    else:
        notes = (
            f"Registered {state} data broker (no one-shot delete portal in {state}). Send a "
            "CCPA/state-law deletion request to the contact email."
        )
    if fcra:
        notes += (
            " FCRA-regulated: some data is credit-reporting data with separate rules -- deletion "
    ...
    return {
        ...
        "optout": {
            ...
            "deletion": {
                ...
                "kinds": ["ccpa", "generic"],
                "notes": (
                    "Covered by the CA DROP one-shot (privacy.ca.gov/drop); CCPA email fallback."
                    if has_drop
                    else "CCPA/state-law deletion email (no one-shot portal)."
                ),
            },
            "fcra": fcra,
...
def parse(
    csv_text: str, jurisdiction: str = "US-CA", has_drop: bool = True
) -> list[dict]:
...
def refresh(
    cache_path: Path, url: str = DEFAULT_URL, csv_text: str | None = None
) -> dict:
    ...
    for key, src in SOURCES.items():
        ...
        try:
            if fetched is not None:
                ...
                recs = parse(
                    text, jurisdiction=src["jurisdiction"], has_drop=src["has_drop"]
                )
            elif key == "ca":
        ...
        if key == "ca" and len(recs) < MIN_EXPECTED_CA:
            entry["warning"] = (
                f"only {len(recs)} parsed (expected >{MIN_EXPECTED_CA}); the CA "
