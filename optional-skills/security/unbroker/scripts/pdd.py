...
def cmd_doctor(args) -> None:
    ...
    cfg = config_mod.load_config()
    caps = config_mod.detect_capabilities(config_mod.dotenv_env())  # see creds in $CLAWK_HOME/.env too
    ...
    if not crypto.is_engaged():
        L.append("  Storage: dossiers are PLAINTEXT JSON (0600 under CLAWK_HOME). "
...
def cmd_cdp(args) -> None:
    ...
    _out({"running": bool(live), "launched_pid": pid, "browser": browser,
          ...
          "next": ([f"point Clawksis's browser tools at 127.0.0.1:{port} (CDP)",
