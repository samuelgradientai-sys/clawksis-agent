...
The Clawksis agent orchestrates scanning and opt-out submission with native tools
...
Run it through the `terminal` tool (it can read PII files under CLAWK_HOME);
...
def cmd_setup(args) -> None:
    if getattr(args, "auto", False):
        ...
        # asking anyone. Read creds from $CLAWK_HOME/.env too (the terminal shell doesn't export
    ...
    if cfg.get("encryption") == "age":
        if not crypto.age_available():
            sys.exit(
                "error: encryption=age requested but `age`/`age-keygen` not found. "
    ...
    migrated = (
        _migrate_subjects()
    )  # rewrite existing dossiers/ledgers into the new at-rest format
    out = {
...
def cmd_doctor(args) -> None:
    ...
    caps = config_mod.detect_capabilities(
        config_mod.dotenv_env()
    )  # see creds in $CLAWK_HOME/.env too
    ...
    rows = [
        (
            "Cloud browser (Browserbase) *RECOMMENDED*",
            caps["browserbase"],
            "default backend: clears soft CAPTCHAs (Turnstile/hCaptcha) -> more T1",
            "set BROWSERBASE_API_KEY",
        ),
        (
            "Email auto (AgentMail)",
            caps["agentmail"],
            "send + auto-verify, per-broker aliases (Mode B/C)",
            "install agentmail skill / set AGENTMAIL_API_KEY",
        ),
        (
            "Email send (CLI SMTP)",
            caps["smtp_send"],
            "`send-email` delivers opt-outs itself (Mode B)",
            "set EMAIL_ADDRESS / EMAIL_PASSWORD (+ EMAIL_SMTP_HOST)",
        ),
        (
            "Verify-link poll (CLI IMAP)",
            caps["imap_read"],
            "`poll-verification` reads confirmation links itself",
            "set EMAIL_ADDRESS / EMAIL_PASSWORD (+ EMAIL_IMAP_HOST)",
        ),
        (
            "Google Sheets tracker",
            caps["google_workspace"],
            "shared status dashboard",
            "set up the google-workspace skill",
        ),
    ]
    ...
    L.append(
        f"  [{'ON ' if engaged else 'off'}] {'At-rest encryption (age)':<28} "
        "encrypts dossiers + ledgers on disk"
    )
    if engaged:
        L.append(
            f"        key: {crypto.identity_path()} (0600) - guards casual/backup/commit "
            "exposure, NOT a full-CLAWK_HOME read"
        )
    elif cfg["encryption"] == "age":
        L.append(
            "        WARNING: encryption=age is SET but NOT engaged (age binary or key missing);"
            " dossiers would be PLAINTEXT"
        )
    elif caps["age"]:
        L.append(
            "        off - dossiers are plaintext (0600). enable: `setup --encryption age`"
        )
    else:
        L.append(
            "        off - dossiers are plaintext (0600). install `age` first to enable"
        )

    L += [
        "",
        "Verdict:",
        "  Ready now in DRAFT mode (no setup needed): scan brokers, draft opt-out",
        "  emails for you to send, and track everything in the ledger.",
    ]
    if caps["browserbase"]:
        L.append(
            "  Cloud browser ON (recommended default): soft/managed CAPTCHAs "
            "(Turnstile/hCaptcha) clear automatically -> those brokers stay T1."
        )
    else:
        L.append(
            "  No cloud browser: set BROWSERBASE_API_KEY (the recommended default) so soft "
            "CAPTCHAs clear automatically; without it those brokers drop to T2 (human tasks)."
        )
    if cfg["email_mode"] == "draft_only":
        L.append(
            "  Email is draft-only: you send drafts + click verify links. For hands-off email "
        ...
        L.append(
            "  Email mode: browser (no password) - the agent sends opt-outs and opens verify "
            "links via the operator's logged-in webmail. This needs Clawksis pointed at the "
            "operator's OWN Chrome over CDP (launch with --remote-debugging-port=9222 "
            "--user-data-dir=~/.clawksis/chrome-debug, signed into the webmail once); else it falls "
        ...
        if cloud_scan:
            L.append(
                "  NOTE: your scan backend is a cloud browser (Browserbase). It is great for "
    ...
    if not crypto.is_engaged():
        L.append(
            "  Storage: dossiers are PLAINTEXT JSON (0600 under CLAWK_HOME). "
    ...
    fresh = []
    for label, p in [
        ("BADBOOL", paths_mod.brokers_cache_path()),
    ...
    for b in brokers_mod._load_curated():
        ...
        try:
            if (
                _time.time() - _time.mktime(_time.strptime(lv, "%Y-%m-%d"))
            ) / 86400.0 > STALE_VERIFY_DAYS:
                stale_curated += 1
    ...
    if stale_curated or documented:
        L.append(
            f"  Freshness: {stale_curated} curated broker(s) last-verified >{STALE_VERIFY_DAYS}d ago; "
...
def cmd_cdp(args) -> None:
    ...
    Clawksis at the operator's real Chrome on a dedicated debug profile (see methods.md).
    """
    import shlex
    import time
    ...
    if live:
        _out({
            ...
            "note": "a debuggable browser is already listening; point Clawksis's browser tools at "
    ...
    if not browser:
        _out({
            "running": False,
            "error": "no Chrome/Chromium-family browser found",
    ...
    _out({
        ...
        "next": (
            [
                f"point Clawksis's browser tools at 127.0.0.1:{port} (CDP)",
...
def cmd_intake(args) -> None:
    ...
    _out({
        ...
        "authorized": dossier_mod.is_authorized(d),
        "residency": residency,
        "email_mode": (prefs or {}).get("email_mode"),
...
def cmd_brokers(args) -> None:
    bl = (
        brokers_mod.by_priority(*(args.priority or []))
        if args.priority
        else brokers_mod.load_all()
    )
    _out([
        {
            "id": b.get("id"),
            ...
            "method": (b.get("optout") or {}).get("method"),
            "owns": b.get("owns") or [],
            "source": b.get("source"),
            "confidence": b.get("confidence", "curated"),
        }
        for b in bl
    ])
...
def cmd_registry(args) -> None:
    ...
    out = {
        "registered_brokers": len(recs),
        "fcra_regulated": fcra,
        "source": "CA Data Broker Registry (CPPA, 2025)",
    ...
    if args.search:
        ...
        hits = [
            ...
            or q in (r.get("id") or "")
            or q in ((r.get("optout") or {}).get("email") or "").lower()
        ]
        out["matches"] = [
            {
                "id": r["id"],
...
def cmd_drop(args) -> None:
    ...
    if args.filed:
        ...
        _out({
            ...
            "drop_filed_at": prefs["drop_filed_at"],
...
def cmd_fanout(args) -> None:
    ...
    mode = (
        "scan AND opt-out (operator authorized submissions)"
        if args.optout
...
def cmd_record(args) -> None:
    ...
    if args.disclosed:
        ledger_mod.log_disclosure(
            args.subject, args.broker, args.disclosed, args.channel or "unknown"
        )
    _out({
        "broker": args.broker,
        "state": case["state"],
...
def _email_request(
    d: dict, b: dict, kind: str, listings, identifiers
) -> tuple[dict, list[str]]:
    ...
    if kind == "ccpa_indirect":
        ...
        if not ids:
            ...
            ids.append(
                f'the name "{ident.get("full_name")}" where it appears as a relative/associated person'
            )
        fields = {
...
def cmd_render_email(args) -> None:
    ...
    fields, disclosed = _email_request(
        d, b, kind, args.listing, getattr(args, "identifier", None)
    )
    if kind == "generic":
    ...
    ledger_mod.log_disclosure(
        args.subject, args.broker, list(disclosed), f"email_draft:{kind}"
    )
...
def cmd_send_email(args) -> None:
    ...
    if mode not in ("programmatic", "alias", "browser"):
        sys.exit(
            "error: email_mode is draft_only; run `setup --email-mode browser` (no password; "
            "sends via your logged-in webmail) or `--email-mode programmatic`, or use "
            "`render-email` and send it yourself"
        )
    if not args.listing:
        sys.exit(
            "error: --listing <confirmed-url> is required (verify-before-disclose: never "
    ...
    body = (
        legal.render_optout_email(b, fields)
        if kind == "generic"
        else legal.render_request(kind, b, fields)
    )
    ...
    if mode == "browser":
        ...
        ledger_mod.log_disclosure(
            args.subject, args.broker, list(disclosed), f"email_browser:{kind}"
        )
        case = ledger_mod.transition(
            args.subject,
            args.broker,
            "submitted",
    ...
    ledger_mod.log_disclosure(
        args.subject, args.broker, list(disclosed), f"email_sent:{kind}"
    )
    case = ledger_mod.transition(
        args.subject,
        args.broker,
...
def cmd_verify_link(args) -> None:
    ...
    if not text:
        sys.exit(
            "error: provide --text '<email body>' (or --file) from the broker's confirmation email"
        )
    ...
    _out({
        ...
        "next": (
            "browser_navigate the link IN THE SAME browser (sessions are browser-bound), "
            f"complete the flow, then `record {args.subject} {args.broker} awaiting_processing`"
            if link
            else "no broker/opt-out-scoped link found in that text; confirm you opened the right email"
        ),
    })
...
def cmd_poll_verification(args) -> None:
    ...
    for bid, case in sorted(led.items()):
        ...
        if b and (((b.get("optout") or {}).get("requires")) or {}).get(
            "email_verification"
        ):
    ...
    for bid, case, b in targets:
        ...
        if hit:
            ...
            results.append({
                "broker": bid,
                "verification_link": hit["link"],
                "email_from": hit.get("from"),
                "email_subject": hit.get("subject"),
        ...
        "cases": [
            {
                ...
                "state": c.get("state"),
                "next_recheck_at": c.get("next_recheck_at"),
            }
            for c in cases
        ],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s = sub.add_parser(
        "setup",
        help="write install config (easiest-first defaults; --auto = most autonomous)",
    )
    s.add_argument(
        "--auto",
        action="store_true",
    ...
    s.add_argument(
        "--autonomy", dest="autonomy", choices=sorted(config_mod.VALID["autonomy"])
    )
    s.add_argument(
        "--email-mode",
        dest="email_mode",
        choices=sorted(config_mod.VALID["email_mode"]),
    )
    s.add_argument(
        "--browser-backend",
        dest="browser_backend",
        choices=sorted(config_mod.VALID["browser_backend"]),
    )
    s.add_argument(
        "--tracker-backend",
        dest="tracker_backend",
        choices=sorted(config_mod.VALID["tracker_backend"]),
    )
    s.add_argument(
        "--encryption",
        dest="encryption",
        choices=sorted(config_mod.VALID["encryption"]),
    )
    s.set_defaults(func=cmd_setup)

    s = sub.add_parser(
        "doctor", help="readiness check: config, brokers, available upgrades"
    )
    ...
    s.add_argument(
        "--port",
        type=int,
        default=cdp.DEFAULT_PORT,
        help="remote debugging port (default 9222)",
    )
    s.add_argument(
        "--profile",
        help="user-data-dir (default: $CLAWK_HOME/chrome-debug, a dedicated debug profile)",
    )
    s.add_argument(
        "--browser",
        help="path to (or PATH name of) a Chrome/Chromium/Brave/Edge binary",
    )
    s.add_argument(
        "--check",
        action="store_true",
    ...
    s.add_argument(
        "--street", help="current street line1 (enables reverse-address search)"
    )
    s.add_argument("--city")
    ...
    s.add_argument(
        "--dob", help="date of birth YYYY-MM-DD (only used if a broker requires it)"
    )
    s.add_argument(
        "--contact-email",
        dest="contact_email",
    ...
    s.add_argument(
        "--consent",
        action="store_true",
        help="subject authorizes removal on their behalf",
    )
    s.add_argument(
        "--consent-method",
        default="self",
        choices=["self", "written_authorization", "poa"],
    )
    s.add_argument(
        "--email-mode",
        dest="email_mode",
    ...
    s.add_argument(
        "--priority",
        action="append",
        choices=["crucial", "high", "standard", "long_tail"],
    )
    ...
    s.add_argument(
        "--search", help="find registered brokers by name / id / email substring"
    )
    s.add_argument(
        "--limit", type=int, default=25, help="max matches to print (default 25)"
    )
    ...
    s.add_argument(
        "--filed",
        action="store_true",
        help="mark DROP as filed (stops `next` surfacing it)",
    )
    s.set_defaults(func=cmd_drop)

    s = sub.add_parser(
        "plan", help="compute per-broker tier + next action for a subject"
    )
    s.add_argument("subject")
    s.add_argument(
        "--priority",
    ...
    s = sub.add_parser(
        "fanout",
        help="batch brokers into parallel delegate_task subagents (large runs)",
    )
    s.add_argument("subject")
    s.add_argument(
        "--priority",
    ...
    s.add_argument(
        ...
        type=int,
        default=5,
        help="brokers per subagent batch (default 5; 8+ times out)",
    )
    s.add_argument(
        "--optout",
        action="store_true",
    ...
    s = sub.add_parser(
        "record", help="record a ledger state transition after an agent action"
    )
    s.add_argument("subject")
    s.add_argument("broker")
    s.add_argument("state", choices=ledger_mod.STATES)
    s.add_argument(
        "--found", type=lambda v: v.strip().lower() in ("1", "true", "yes", "y")
    )
    s.add_argument("--evidence", help="JSON object stored as case.evidence")
    s.add_argument(
        "--disclosed", action="append", metavar="FIELD", help="field name disclosed"
    )
    s.add_argument("--channel", help="disclosure channel, e.g. web_form / email")
    s.add_argument(
        "--reason",
        help="for human_task_queued: why a human is needed (shown in `tasks`)",
    )
    s.set_defaults(func=cmd_record)

    s = sub.add_parser(
        "next", help="autonomous action queue: exactly what to do right now"
    )
    s.add_argument("subject")
    s.add_argument(
        "--priority",
    ...
    s = sub.add_parser(
        "send-email",
        help="Mode B: render AND send the opt-out/legal request (records it)",
    )
    s.add_argument("subject")
    s.add_argument("broker")
    s.add_argument(
    ...
    s.add_argument(
        "--kind",
        choices=["generic", "ccpa", "ccpa_agent", "ccpa_indirect", "gdpr"],
        default="generic",
    )
    s.add_argument(
        "--identifier",
    ...
    s.add_argument(
        "--to",
        help="override recipient (must be an address the broker record declares)",
    )
    s.add_argument(
        "--force",
        action="store_true",
        help="re-send even if already submitted (default: idempotent skip)",
    )
    ...
    s.add_argument("subject")
    s.add_argument(
        "--broker", help="only this broker (default: every in-flight verification case)"
    )
    ...
    s.add_argument("broker")
    s.add_argument(
        "--text", help="the confirmation email body (read from the operator's webmail)"
    )
    s.add_argument(
        "--file", help="path to a file with the email body (alternative to --text)"
    )
    s.set_defaults(func=cmd_verify_link)

    s = sub.add_parser(
        "tasks", help="ONE consolidated human-task digest (present at end of run)"
    )
    s.add_argument("subject")
    s.set_defaults(func=cmd_tasks)

    s = sub.add_parser(
        "show", help="read a case's state + evidence (for parent re-verification)"
    )
    s.add_argument("subject")
    s.add_argument("broker")
    ...
    s = sub.add_parser(
        "due", help="cases whose recheck window has arrived (cron re-scan queue)"
    )
    s.add_argument("subject")
    ...
    s = sub.add_parser(
        "render-email",
        help="render a Mode-A opt-out / legal-request draft (least-disclosure)",
    )
    s.add_argument("subject")
    s.add_argument("broker")
    s.add_argument(
        "--listing", action="append", metavar="URL", help="confirmed listing URL"
    )
    s.add_argument(
        "--kind",
        choices=["generic", "ccpa", "ccpa_agent", "ccpa_indirect", "gdpr"],
    ...
    s.add_argument("subject")
    s.add_argument(
        "--sheets", action="store_true", help="emit Google Sheets rows as JSON"
    )
