...
The design is **Clawksis-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
clawk skills install official/security/unbroker
```

Then start a new Clawksis session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Clawksis session:
...
- **PII handling.** Dossiers live under the Clawksis home directory (`0600`, optionally
