...
Usage (via cron with --no-agent):
    ...
    clawk cron create clawk-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CLAWK_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name clawk-issues --repo samuelgradientai-sys/clawksis-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Clawksis .env file
(``${CLAWK_HOME:-~/.clawksis}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Clawksis-Watcher/1.0",
    }
