...
Usage (via cron with --no-agent):
    ...
    clawk cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CLAWK_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Clawksis-Watcher/1.0"})
