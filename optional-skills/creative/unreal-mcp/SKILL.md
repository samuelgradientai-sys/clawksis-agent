...
Companion skill for the `unreal-engine` entry in the Clawksis MCP catalog. The
...
Two halves, in this order: the editor side must be up before Clawksis connects.
...
### One-time, Clawksis side

    clawk mcp install unreal-engine
...
`~/.clawksis/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Clawksis — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Clawksis connects
...
2. Start the Clawksis session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Clawksis
they appear as:

| Clawksis tool | Purpose |
|---|---|
...
then happens at `clawk mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Clawksis runs batched calls
...
- **Start order matters.** Editor + server up first, then the Clawksis
...
- **Timeouts on long operations.** Clawksis' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.clawksis/config.yaml` for
