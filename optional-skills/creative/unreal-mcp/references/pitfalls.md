...
### 1. Start order: editor first, Clawksis second

Clawksis probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Clawksis session.
...
restart the editor, restart the Clawksis session.
...
server fails to bind (Output Log) or Clawksis' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.clawksis/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Clawksis

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Clawksis' connection
lives in `~/.clawksis/config.yaml` via `clawk mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Clawksis — just don't
mistake it for the Clawksis setup step.
...
explicitly warns against overlapping calls. Clawksis executes same-turn tool
...
### 9. Timeouts: Clawksis gives up before Unreal does

Clawksis' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Clawksis runs elsewhere (SSH backend,
