...
Monitor a running video-production kanban. Polls `clawk kanban list` and
...
def clawk_available() -> bool:

    return shutil.which("clawk") is not None
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed clawk CLI."""

    try:

        out = subprocess.run(

            ["clawk", "kanban", "list", "--tenant", tenant, "--json"],
    ...
    # Fallback: textual parse of `clawk kanban list`

    out = subprocess.run(

        ["clawk", "kanban", "list", "--tenant", tenant],
...
def kanban_show(task_id: str) -> dict | None:
    ...
    out = subprocess.run(
        ...
        ["clawk", "kanban", "show", task_id, "--json"],
...
def main():
    ...
    if not clawk_available():

        print("ERROR: 'clawk' CLI not found in PATH", file=sys.stderr)
