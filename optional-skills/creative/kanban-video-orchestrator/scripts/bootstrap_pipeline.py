...
../assets/, and writes a setup.sh that creates Clawksis profiles and fires the
initial kanban task.

Profile-config patching, SOUL.md-per-profile, TEAM.md task-graph convention,
and the `clawk kanban create --workspace dir:` initial-task pattern are
                ...
                if "profile" in t:
                    if not PROFILE_NAME_RE.match(t["profile"]):
                        errors.append(
                            ...
                            f"[a-z0-9][a-z0-9_-]{{0,63}} per Clawksis profile rules"
                        )
