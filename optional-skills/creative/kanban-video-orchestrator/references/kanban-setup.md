...
`~/.clawksis/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    if grep -q "^${var}=" ~/.clawksis/.env 2>/dev/null && \

       [ -n "$(grep "^${var}=" ~/.clawksis/.env | cut -d= -f2-)" ]; then
    ...
    echo "ERROR: ${var} not set in ~/.clawksis/.env or Keychain (${kc_account}/${kc_service})"
