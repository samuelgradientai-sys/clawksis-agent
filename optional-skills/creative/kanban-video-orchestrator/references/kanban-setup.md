...
the actual `setup.sh` that creates the project workspace, configures Clawksis
...
2. **Create profiles** — `clawk profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.clawksis/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `clawk kanban create` assigned to the director
...
```bash
clawk profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.clawksis/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.clawksis/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Clawksis
...
Each profile gets a `SOUL.md` at `~/.clawksis/profiles/<name>/SOUL.md` that
...
```bash
clawk kanban create "Direct production of <video title>" \
    --assignee director \
...
the Clawksis `.env` (`${CLAWK_HOME:-$HOME/.clawk}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _clawk_env="${CLAWK_HOME:-$HOME/.clawk}/.env"
    if grep -q "^${var}=" "$_clawk_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_clawk_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_clawk_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY clawk ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY clawk OPENROUTER_API_KEY || exit 1
# ...
```
