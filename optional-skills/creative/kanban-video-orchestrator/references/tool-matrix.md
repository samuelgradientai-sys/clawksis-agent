...
Maps each role archetype to the Clawksis skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public clawksis-agent
...
## Clawksis skills relevant to video production

### Visual / rendering skills (`clawksis-agent/skills/creative/`)
...
### Design / pre-production skills (`clawksis-agent/skills/creative/`)
...
### Audio / media skills (`clawksis-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`clawksis-agent/skills/devops/`)
...
These are **not** Clawksis skills but external CLIs / APIs that profiles invoke.
...
## Built-in Clawksis tools for media review

These are native Clawksis tools — not invoked via terminal but through their own
...
key is present in `${CLAWK_HOME:-~/.clawksis}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Clawksis profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${CLAWK_HOME:-~/.clawksis}/.env` → environment variable.
...
When a new Clawksis-public video skill ships:
