...
clawk kanban watch --tenant <project-slug>

# Snapshot of the board
clawk kanban list --tenant <project-slug>
clawk kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
clawk kanban stats --tenant <project-slug>

# Visual dashboard (browser)
clawk dashboard

# Inspect a specific task (includes comments + events)
clawk kanban show <task-id>

# Follow a single task's event stream
clawk kanban tail <task-id>
```

Verify available subcommands with `clawk kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `clawk kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `clawk kanban block <id>` then `clawk kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `clawk kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `clawk kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `clawk profile list` (profile exists?), `clawk status` (gateway/dispatcher up?), and verify tenant. |
...
clawk kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
clawk kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(clawk kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
clawk kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
clawk kanban block <task-id>
clawk kanban archive <task-id>

# Diagnose what happened
clawk kanban show <task-id>      # task body, comments, recent events
clawk kanban tail <task-id>      # follow the live event stream
clawk kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `clawk kanban create` for the director
...
while true; do
    clear
    clawk kanban list --tenant <slug>
    echo "---"
    clawk kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `clawk kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `clawk profile list` and start
