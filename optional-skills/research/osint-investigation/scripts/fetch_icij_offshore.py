...
$CLAWK_OSINT_CACHE/icij/ (default: ~/.cache/clawk-osint/icij/) and search
...
def _cache_dir() -> Path:
    ...
    base = os.environ.get("CLAWK_OSINT_CACHE")

    if base:
        return Path(base) / "icij"

    return Path.home() / ".cache" / "clawk-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "clawksis-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $CLAWK_OSINT_CACHE/icij or ~/.cache/clawk-osint/icij)",
    )
