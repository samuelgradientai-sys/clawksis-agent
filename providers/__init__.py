...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with clawksis-agent)

2. User plugins: ``$CLAWK_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$CLAWK_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$CLAWK_HOME/plugins/model-providers/`` if it exists."""

    try:
        from clawk_constants import get_clawk_home

        d = get_clawk_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple CLAWK_HOME profiles don't alias each other.
    ...
    if source == "bundled":
        ...
        module_name = f"_clawk_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$CLAWK_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with clawksis-agent.
    ...
    # 2. User plugins — under $CLAWK_HOME/plugins/model-providers/<name>/.
