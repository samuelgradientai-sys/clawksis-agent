"""
Clawksis MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Clawksis-specific extra)

Usage:
    clawk mcp serve
    clawk mcp serve --verbose
...
logger = logging.getLogger("clawk.mcp_serve")
...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return Path(os.environ.get("CLAWK_HOME", Path.home() / ".clawk")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("CLAWK_HOME", Path.home() / ".clawk")
...
class EventBridge:
    ...
    This is the Clawksis equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("CLAWK_HOME", Path.home() / ".clawk")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    mcp = FastMCP(
        "clawk",
        instructions=(
            "Clawksis messaging bridge. Use these tools to interact with "
