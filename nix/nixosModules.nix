# nix/nixosModules.nix — NixOS module for clawksis-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: clawk runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CLAWK_HOME/.env
# and read by clawk at startup — no container recreation needed for env changes.
#
# Tool resolution: the clawk wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.clawksis-agent;
    effectivePackage =
    ...
    clawksis-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-clawksis-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "clawk-config-attrs";
      description = "Clawksis YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "clawk-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "clawk-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'CLAWK_DOC_EOF'\n${value}\nCLAWK_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "clawksis-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/clawk";
    ...
    # clawk user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "clawk-container-entrypoint" ''
      set -eu

      CLAWK_UID="''${CLAWK_UID:?CLAWK_UID must be set}"
      CLAWK_GID="''${CLAWK_GID:?CLAWK_GID must be set}"

      # ── Group: ensure a group with GID=$CLAWK_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$CLAWK_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="clawk"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$CLAWK_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$CLAWK_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$CLAWK_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$CLAWK_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="clawk"
        TARGET_HOME="/home/clawk"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$CLAWK_UID" -g "$CLAWK_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$CLAWK_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$CLAWK_UID:$CLAWK_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure CLAWK_HOME is owned by the target user.
      ...
      if [ -n "''${CLAWK_HOME:-}" ] && [ -d "$CLAWK_HOME" ]; then
        find "$CLAWK_HOME" \! -user "$CLAWK_UID" -exec chown "$CLAWK_UID:$CLAWK_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/clawk-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/clawk-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/clawk ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/clawk
        chmod 0440 /etc/sudoers.d/clawk
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$CLAWK_UID" --regid="$CLAWK_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $CLAWK_HOME/.env.
    ...
    # Default: /var/lib/clawksis/workspace → /data/workspace.
  ...
  in {
    options.services.clawksis-agent = with lib; {
      enable = mkEnableOption "Clawksis gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = clawksis-agent;
        description = "The clawksis-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/clawk";
        description = "State directory. Contains .clawksis/ subdir (CLAWK_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Clawksis config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $CLAWK_HOME/.env at activation time.
          Clawksis reads this file on every startup via load_clawk_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $CLAWK_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $CLAWK_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `clawk gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the clawk user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the clawk plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Clawksis discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          clawk_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as clawk (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in clawksis-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the clawk CLI to environment.systemPackages and export
          CLAWK_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.clawksis symlink to the service
            stateDir. These users are automatically added to the clawk group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.clawksis-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the clawk CLI to system PATH and export CLAWK_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.clawksis/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.CLAWK_HOME = "${cfg.stateDir}/.clawksis";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.clawksis-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the clawk user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.clawksis-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed clawk
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure clawk is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.clawksis        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawksis/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawksis/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawksis/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawksis/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawksis/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."clawksis-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.clawksis
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.clawksis ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.clawksis ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.clawksis -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.clawksis/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.clawksis/$_subdir"
            chmod 2770 "${cfg.stateDir}/.clawksis/$_subdir"
            find "${cfg.stateDir}/.clawksis/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.clawksis/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.clawksis/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clawksis/config.yaml
            chmod 0640 ${cfg.stateDir}/.clawksis/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.clawksis/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clawksis/.managed
          chmod 0644 ${cfg.stateDir}/.clawksis/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.clawksis/.container-mode <<'CLAWK_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    clawk_bin=${containerDataDir}/current-package/bin/clawk
    CLAWK_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clawksis/.container-mode
            chmod 0644 ${cfg.stateDir}/.clawksis/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.clawksis/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.clawksis";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.clawksis" ]; then
                  rm -f "${symlinkPath}"
                  echo "clawksis-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.clawksis -> stateDir/.clawksis for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.clawksis";
                target = "${cfg.stateDir}/.clawksis";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "clawksis-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.clawksis/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.clawksis/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.clawksis/auth.json
              fi
            ''}
          ''}
          ...
          # Clawksis reads $CLAWK_HOME/.env at startup via load_clawk_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.clawksis/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'CLAWK_NIX_ENV_EOF'
    ${envFileContent}
    CLAWK_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.clawksis/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.clawksis/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clawksis/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.clawksis-agent = {
          ...
          environment = {
            HOME = cfg.stateDir;
            CLAWK_HOME = "${cfg.stateDir}/.clawksis";
            CLAWK_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $CLAWK_HOME/.env by the activation script. load_clawk_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/clawk"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the clawk group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.clawksis-agent = {
          description = "Clawksis Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              CLAWK_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              CLAWK_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env CLAWK_UID="$CLAWK_UID" \
                --env CLAWK_GID="$CLAWK_GID" \
                --env CLAWK_HOME=${containerDataDir}/.clawksis \
                --env CLAWK_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/clawk gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
