...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      clawksis-agent = self'.packages.default;
      clawkVenv = clawksis-agent.clawkVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "clawk-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${clawkVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "clawk-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "clawk-build-package" { } ''
          echo "PASS: package built at ${clawksis-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "clawk-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "clawk-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${clawksis-agent}/bin/clawk || (echo "FAIL: clawk binary missing"; exit 1)
          test -x ${clawksis-agent}/bin/clawksis-agent || (echo "FAIL: clawksis-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${clawksis-agent}/bin/clawk version 2>&1 | grep -qi "clawk" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "clawk-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in clawk clawksis-agent clawk-acp; do
            test -x ${clawksis-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "clawk-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking clawk --help ==="
          ${clawksis-agent}/bin/clawk --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${clawksis-agent}/bin/clawk --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "clawk-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${clawksis-agent}/share/clawksis-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${clawksis-agent}/share/clawksis-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CLAWK_BUNDLED_SKILLS" ${clawksis-agent}/bin/clawk || \
            (echo "FAIL: CLAWK_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CLAWK_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "clawk-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${clawksis-agent}/share/clawksis-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${clawksis-agent}/share/clawksis-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CLAWK_BUNDLED_PLUGINS" ${clawksis-agent}/bin/clawk || \
            (echo "FAIL: CLAWK_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CLAWK_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "clawk-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${clawksis-agent}/share/clawksis-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${clawksis-agent}/share/clawksis-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${clawksis-agent}/share/clawksis-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "CLAWK_BUNDLED_LOCALES" ${clawksis-agent}/bin/clawk || \
            (echo "FAIL: CLAWK_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: CLAWK_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (CLAWK_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && CLAWK_BUNDLED_LOCALES=${clawksis-agent}/share/clawksis-agent/locales \
            ${clawkVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with CLAWK_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${clawkVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${clawkVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without CLAWK_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "clawk-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${clawksis-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${clawksis-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CLAWK_TUI_DIR" ${clawksis-agent}/bin/clawk || \
            (echo "FAIL: CLAWK_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CLAWK_TUI_DIR set in wrapper"
        ...
        # Verify CLAWK_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        clawk-node = pkgs.runCommand "clawk-node-version" { } ''
          set -e
          echo "=== Checking CLAWK_NODE in wrapper ==="
          grep -q "CLAWK_NODE" ${clawksis-agent}/bin/clawk || \
            (echo "FAIL: CLAWK_NODE not set in wrapper"; exit 1)
          echo "PASS: CLAWK_NODE present in wrapper"

          CLAWK_NODE=$(sed -n "s/^export CLAWK_NODE='\(.*\)'/\1/p" ${clawksis-agent}/bin/clawk)
          test -x "$CLAWK_NODE" || (echo "FAIL: CLAWK_NODE=$CLAWK_NODE not executable"; exit 1)
          echo "PASS: CLAWK_NODE executable at $CLAWK_NODE"

          NODE_MAJOR=$("$CLAWK_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All CLAWK_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CLAWK_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "clawk-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CLAWK_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CLAWK_MANAGED guards ==="
          check_blocked "config set" ${clawksis-agent}/bin/clawk config set model foo
          check_blocked "config edit" ${clawksis-agent}/bin/clawk config edit
        ...
        extra-python-packages = let
          ...
          clawkWithExtra = clawksis-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "clawk-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${clawkWithExtra}/bin/clawk || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${clawkWithExtra}/bin/clawk || \
          ...
          if grep -q "PYTHONPATH" ${clawksis-agent}/bin/clawk; then
        ...
        extra-dependency-groups = let
          clawkWithGroups = clawksis-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "clawk-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${clawkWithGroups}"
          echo "venv: ${clawkWithGroups.clawkVenv}"
        ...
        messaging-variant = pkgs.runCommand "clawk-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.clawkVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "clawk-config-roundtrip" {
          ...
          merge_and_load() {
            local clawk_home="$1"
            export CLAWK_HOME="$clawk_home"
            ${configMergeScript} ${nixSettings} "$clawk_home/config.yaml"
            ${clawkVenv}/bin/python3 -c '
import json, sys
