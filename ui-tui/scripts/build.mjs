...
await build({
  ...
  // Skip the prebuilt @clawk/ink bundle — esbuild's __esm helper doesn't

  // await nested async init, which breaks lazy-initialized exports like

  // `render`. Bundling from source sidesteps that.

  alias: { '@clawk/ink': resolve(root, 'packages/clawk-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The clawk_cli launcher
