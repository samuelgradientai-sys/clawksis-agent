import type { RunExternalProcess } from '@clawk/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`clawk ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching clawk: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`clawk ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
