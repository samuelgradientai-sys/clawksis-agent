...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  const override = (env.CLAWK_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Clawksis on the safer TERM-driven 256-color path unless

  // users explicitly opt back in via CLAWK_TUI_TRUECOLOR=1.
