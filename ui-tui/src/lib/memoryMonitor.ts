...
// Deferred @clawk/ink import: loading `@clawk/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@clawk/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const cooldownRaw = process.env.CLAWK_AUTO_HEAPDUMP_COOLDOWN_MS?.trim()
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@clawk/ink` off the cold-start critical path;
