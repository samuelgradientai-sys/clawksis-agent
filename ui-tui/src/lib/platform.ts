 ...
 * clawk-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``clawk_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but clawk-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS clawk-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    ...
    case 'alt':
      ...
      // Bare Escape on clawk-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is clawk-ink's Alt signal and accepting it would
