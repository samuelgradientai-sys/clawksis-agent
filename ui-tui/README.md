...
React + Ink terminal UI for Clawksis. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CLAWK_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `clawk setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.clawksis/.clawk_history` or under `CLAWK_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `clawk setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/clawk-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external clawk setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.clawksis/tui-parent.log
    ...
    types/
      clawk-ink.d.ts            type declarations for @clawk/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ClawksisCLI subprocess for slash commands
```
