...
What it does
------------
1. Spawns a SCRATCH dashboard (``clawk dashboard``) bound to loopback on a
   free port, with an ISOLATED ``CLAWK_HOME`` (temp dir, minimal seeded state).
   It NEVER touches the live :9119 dashboard / ai.clawk.dashboard / live
   state.db. Loopback bind ⇒ no auth gate (web_server.should_require_auth).
2. Arms the synthetic GIL-heavy turn seam (``CLAWK_ISO_CERTIFY_SYNTH_TURN=1``,
...
the scratch CLAWK_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
_READY_RE = re.compile(r"CLAWK_(?:DASHBOARD|BACKEND)_READY port=(\d+)")
