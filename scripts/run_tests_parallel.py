...
Environment:
    CLAWK_TEST_WORKERS  Override worker count (default: os.cpu_count())
    CLAWK_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
#                        nousresearch/clawksis-agent:test image, via
#                        ``CLAWK_TEST_IMAGE`` so the fixture skips
...
# Set to 0 to disable (env: CLAWK_TEST_FILE_RETRIES).
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/clawksis-agent/clawksis-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("CLAWK_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $CLAWK_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("CLAWK_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("CLAWK_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
    ...
    parser.add_argument(
        "--file-retries",
        type=int,
        default=int(os.environ.get("CLAWK_TEST_FILE_RETRIES", _DEFAULT_FILE_RETRIES)),
        help=(
            ...
            f"Default: {_DEFAULT_FILE_RETRIES}, env: CLAWK_TEST_FILE_RETRIES."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: CLAWK_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "--generate-slices",
    ...
    # Parse --slice (or CLAWK_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("CLAWK_TEST_SLICE")
    ...
    if _FLAKY_RESULTS:
        print()
        print(
            f"=== ⚠ {len(_FLAKY_RESULTS)} FLAKY file{'s' if len(_FLAKY_RESULTS) != 1 else ''} (failed once, passed on retry — fix these) ==="
        )
