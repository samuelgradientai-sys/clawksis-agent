...
# Bootstrap Open WebUI against Clawksis's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.clawksis/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-clawk.sh
...
#   OPEN_WEBUI_NAME='Johnny Clawksis'
...
#   CLAWK_API_PORT=8642
#   CLAWK_API_HOST=127.0.0.1
#   CLAWK_API_MODEL_NAME='Clawksis'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Clawksis WebUI}"
...
CLAWK_ENV_FILE="${CLAWK_ENV_FILE:-$HOME/.clawksis/.env}"
CLAWK_API_PORT="${CLAWK_API_PORT:-8642}"
CLAWK_API_HOST="${CLAWK_API_HOST:-127.0.0.1}"
CLAWK_API_CONNECT_HOST="${CLAWK_API_CONNECT_HOST:-127.0.0.1}"
CLAWK_API_MODEL_NAME="${CLAWK_API_MODEL_NAME:-Clawksis}"
CLAWK_API_BASE_URL="http://${CLAWK_API_CONNECT_HOST}:${CLAWK_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-clawk.sh"
LOG_DIR="$HOME/.clawksis/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$CLAWK_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$CLAWK_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.clawk.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.clawk</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.clawk"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.clawk"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-clawk.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Clawksis
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-clawk.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.clawksis/logs/openwebui.log
StandardError=append:%h/.clawksis/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-clawk.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$CLAWK_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Clawksis API server is configured...'
  upsert_env API_SERVER_ENABLED true "$CLAWK_ENV_FILE"
  upsert_env API_SERVER_HOST "$CLAWK_API_HOST" "$CLAWK_ENV_FILE"
  upsert_env API_SERVER_PORT "$CLAWK_API_PORT" "$CLAWK_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$CLAWK_API_MODEL_NAME" "$CLAWK_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$CLAWK_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Clawksis gateway so API server settings take effect...'
  clawk gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${CLAWK_API_CONNECT_HOST}:${CLAWK_API_PORT}/health" >/dev/null; then
    log 'Clawksis API server did not answer on the first check. Trying to start gateway in the background...'
    nohup clawk gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${CLAWK_API_CONNECT_HOST}:${CLAWK_API_PORT}/health" >/dev/null
  ...
  log "Clawksis API endpoint: ${CLAWK_API_BASE_URL}"
