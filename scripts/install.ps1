...
# Clawksis Installer for Windows
...
param(
    ...
    [string]$ClawkHome = $(if ($env:CLAWK_HOME) { $env:CLAWK_HOME } else { "$env:LOCALAPPDATA\clawksis" }),

    [string]$InstallDir = $(if ($env:CLAWK_HOME) { "$env:CLAWK_HOME\clawksis-agent" } else { "$env:LOCALAPPDATA\clawksis\clawksis-agent" }),
    ...
    # builds apps/desktop into a launchable Clawksis.exe.

    #

    # Why opt-in:

    #   * Clawksis-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Clawksis.exe; if THAT recursively

    #     built apps/desktop it would try to overwrite the live Clawksis.exe
    ...
    #     `clawk desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:samuelgradientai-sys/clawksis-agent.git"

$RepoUrlHttps = "https://github.com/samuelgradientai-sys/clawksis-agent.git"
...
function Write-Banner {
    ...
    Write-Host "|             * Clawksis Installer                    |" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ClawkHome)) {

        New-Item -ItemType Directory -Force -Path $ClawkHome | Out-Null

    }

    $envFile = Join-Path $ClawkHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ClawkHome "node"
...
function Install-Uv {
    ...
    # Clawksis owns its own uv at $ClawkHome\bin\uv.exe.  Always install there —

    # no PATH probing, no conda guards, no multi-location resolution chains.

    # The runtime update path (clawk_cli/managed_uv.py) looks in the same

    # place, so install.ps1 and `clawk update` stay in sync.

    $managedUv = Join-Path $ClawkHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ClawkHome\bin ..."

    New-Item -ItemType Directory -Path (Join-Path $ClawkHome "bin") -Force | Out-Null



    # UV_INSTALL_DIR tells the astral installer to place the binary

    # directly into $ClawkHome\bin instead of ~/.local/bin.
    ...
    try {
        ...
        $env:UV_INSTALL_DIR = Join-Path $ClawkHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and CLAWK_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Clawksis uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\clawk\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Clawksis needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Clawksis copy of Git ourselves is predictable and

    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\clawk\git``
    ...
    ``CLAWK_GIT_BASH_PATH`` (User scope) so Clawksis can find it in a fresh
    ...
    # Download PortableGit into $ClawkHome\git.  Always works as long as

    # we can reach github.com -- no admin, no winget, no reliance on the

    # user's possibly-broken system Git install.

    Write-Info "Git not found -- downloading PortableGit to $ClawkHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            ...
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Clawksis features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Clawksis needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CLAWK_GIT_BASH_PATH`` (User env scope) so Clawksis can find it even before
    ...
    #   PortableGit (our default): $ClawkHome\git\bin\bash.exe

    #   MinGit (32-bit fallback):  $ClawkHome\git\usr\bin\bash.exe

    $candidates += "$ClawkHome\git\bin\bash.exe"       # PortableGit layout (primary)

    $candidates += "$ClawkHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        ...
        if ($candidate -and (Test-Path $candidate)) {
            ...
            [Environment]::SetEnvironmentVariable("CLAWK_GIT_BASH_PATH", $candidate, "User")

            $env:CLAWK_GIT_BASH_PATH = $candidate

            Write-Info "Set CLAWK_GIT_BASH_PATH=$candidate"
    ...
    Write-Warn "Could not locate bash.exe -- Clawksis may not find Git Bash."

    Write-Info "If needed, set CLAWK_GIT_BASH_PATH manually to your bash.exe path."
...
function Test-Node {
    ...
    # Prefer a Clawksis-managed Node from a previous run over a too-old system one.

    $managedNode = "$ClawkHome\node\node.exe"
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        Write-Success "Node.js $version found (Clawksis-managed)"
    ...
    Write-Info "Installing Clawksis-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ClawkHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ClawkHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\clawk-node-extract"
            ...
            if ($extractedDir) {
                ...
                if (Test-Path "$ClawkHome\node") { Remove-Item -Recurse -Force "$ClawkHome\node" }

                Move-Item $extractedDir.FullName "$ClawkHome\node"
                ...
                Write-Success "Node.js $version installed to $ClawkHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            ...
            $log = "$env:TEMP\clawk-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running clawk processes) and try again."
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    ...
                    $zipUrl = "https://github.com/samuelgradientai-sys/clawksis-agent/archive/$Commit.zip"

                    $zipLabel = $Commit

                } elseif ($Tag) {

                    $zipUrl = "https://github.com/samuelgradientai-sys/clawksis-agent/archive/refs/tags/$Tag.zip"

                    $zipLabel = $Tag

                } else {

                    $zipUrl = "https://github.com/samuelgradientai-sys/clawksis-agent/archive/refs/heads/$Branch.zip"

                    $zipLabel = $Branch

                }

                $zipPath = "$env:TEMP\clawksis-agent-$zipLabel.zip"

                $extractPath = "$env:TEMP\clawksis-agent-extract"
    ...
    # next `clawk update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `clawk.exe` exists
...
try:
    ...
    for s in specs:
        ...
        m = re.search(r'clawksis-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        ...
        throw "Failed to install clawksis-agent package even with no extras. Inspect the uv pip install output above."
    ...
    # isn't set, leaving venv\ empty and clawk.exe broken with
    ...
    # here, not 30 seconds later when the user runs `clawk`.
    ...
    # users hit and lazy-import errors from `clawk dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            ...
            Write-Warn "fastapi/uvicorn not importable -- `clawk dashboard` will not work."
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Success "[web] extra installed; `clawk dashboard` should now work."
...
function Set-PathVariable {
    ...
    Write-Info "Setting up clawk command..."
    ...
    if ($NoVenv) {
        ...
        $clawkBin = "$InstallDir\venv\Scripts"

    }

    

    # Add the venv Scripts dir to user PATH so clawk is globally available

    # On Windows, the clawk.exe in venv\Scripts\ has the venv Python baked in

    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")

    

    if ($currentPath -notlike "*$clawkBin*") {

        [Environment]::SetEnvironmentVariable(

            "Path",

            "$clawkBin;$currentPath",

            "User"

        )

        Write-Success "Added to user PATH: $clawkBin"
    ...
    # Set CLAWK_HOME so the Python code finds config/data in the right place.

    # Only needed on Windows where we install to %LOCALAPPDATA%\clawk instead

    # of the Unix default ~/.clawksis

    $currentClawkHome = [Environment]::GetEnvironmentVariable("CLAWK_HOME", "User")

    if (-not $currentClawkHome -or $currentClawkHome -ne $ClawkHome) {

        [Environment]::SetEnvironmentVariable("CLAWK_HOME", $ClawkHome, "User")

        Write-Success "Set CLAWK_HOME=$ClawkHome"

    }

    $env:CLAWK_HOME = $ClawkHome

    

    # Update current session

    $env:Path = "$clawkBin;$env:Path"

    

    Write-Success "clawk command ready"
...
function Write-BootstrapMarker {
    ...
    # Writes $InstallDir\.clawk-bootstrap-complete which tells the Clawksis
    ...
    # Clawksis-Setup.exe) or fall back to whatever git resolves in the

    # checkout. The desktop validates schemaVersion + pinnedCommit

    # length but doesn't enforce that HEAD matches the pin (users

    # update via `clawk update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".clawk-bootstrap-complete"
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.clawksis directory structure

    New-Item -ItemType Directory -Force -Path "$ClawkHome\cron" | Out-Null

    New-Item -ItemType Directory -Force -Path "$ClawkHome\sessions" | Out-Null

    New-Item -ItemType Directory -Force -Path "$ClawkHome\logs" | Out-Null

    New-Item -ItemType Directory -Force -Path "$ClawkHome\pairing" | Out-Null

    New-Item -ItemType Directory -Force -Path "$ClawkHome\hooks" | Out-Null

    New-Item -ItemType Directory -Force -Path "$ClawkHome\image_cache" | Out-Null

    New-Item -ItemType Directory -Force -Path "$ClawkHome\audio_cache" | Out-Null

    New-Item -ItemType Directory -Force -Path "$ClawkHome\memories" | Out-Null

    New-Item -ItemType Directory -Force -Path "$ClawkHome\skills" | Out-Null
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.clawksis/.env from template"

        } else {

            New-Item -ItemType File -Force -Path $envPath | Out-Null

            Write-Success "Created ~/.clawksis/.env"

        }

    } else {

        Write-Info "~/.clawksis/.env already exists, keeping it"
    ...
    $configPath = "$ClawkHome\config.yaml"
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.clawksis/config.yaml from template"

        }

    } else {

        Write-Info "~/.clawksis/config.yaml already exists, keeping it"
    ...
    # (the default PS5 behaviour), and Clawksis's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.clawksis/SOUL.md (edit to customize personality)"

    }

    

    Write-Success "Configuration directory ready: ~/.clawksis/"

    

    # Seed bundled skills into ~/.clawksis/skills/ (manifest-based, one-time per skill)

    Write-Info "Syncing bundled skills to ~/.clawksis/skills/ ..."
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            ...
            Write-Success "Skills synced to ~/.clawksis/skills/"
            ...
            $userSkills = "$ClawkHome\skills"

            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {

                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue

                Write-Success "Skills copied to ~/.clawksis/skills/"
...
function Install-NodeDeps {
    ...
    if (-not $HasNode) {
        ...
        # Cross-process driver mode (Clawksis-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'clawk setup tools' later."
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\clawk-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\clawk-playwright-install-$(Get-Random).log"
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\clawk-npm-tui-$(Get-Random).log"
...
function Install-Desktop {
    ...
    # The Tauri bootstrap installer's launch_clawk_desktop command

    # resolves apps/desktop/release/win-unpacked/Clawksis.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Clawksis-managed
    ...
    # The --dir mode produces an unpacked Clawksis.exe in
    ...
    # are NOT trying to work around). The Clawksis icon + product name are

    # stamped onto Clawksis.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\clawk-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        ...
        "$desktopDir\release\win-unpacked\Clawksis.exe",

        "$desktopDir\release\win-arm64-unpacked\Clawksis.exe"
    ...
    if (-not $found) {
        ...
        throw "Desktop build completed but no Clawksis.exe was found under $desktopDir\release\*-unpacked\"

    }



    # 3b. The Clawksis icon + identity are stamped onto Clawksis.exe by the
    ...
    #    Clawksis.exe. We deliberately do NOT point them at `clawk desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            ...
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Clawksis.lnk'),

            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Clawksis.lnk')
        ...
        # with the Clawksis icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.clawksis/.env are importable.  Two problems this solves:
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'clawk setup'."
    ...
    # Run clawk setup using the venv Python directly (no activation needed)

    if (-not $NoVenv) {

        & ".\venv\Scripts\python.exe" -m clawk_cli.main setup

    } else {

        python -m clawk_cli.main setup
...
function Start-GatewayIfConfigured {
    ...
    $clawkCmd = "$InstallDir\venv\Scripts\clawk.exe"

    if (-not (Test-Path $clawkCmd)) {
    ...
    $whatsappSession = "$ClawkHome\whatsapp\session\creds.json"
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'clawk whatsapp' to pair via QR code..."
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: clawk gateway"
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            $logFile = "$ClawkHome\logs\gateway.log"

            Start-Process -FilePath $clawkCmd -ArgumentList "gateway" `

                -RedirectStandardOutput $logFile `

                -RedirectStandardError "$ClawkHome\logs\gateway-error.log" `
            ...
            Write-Warn "Failed to start gateway. Run manually: clawk gateway"

        }

    } else {

        Write-Info "Skipped. Start the gateway later with: clawk gateway"
...
function Write-Completion {
    ...
    Write-Host "$ClawkHome\config.yaml"

    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow

    Write-Host "$ClawkHome\.env"

    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow

    Write-Host "$ClawkHome\cron\, sessions\, logs\"

    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow

    Write-Host "$ClawkHome\clawksis-agent\"
    ...
    Write-Host "   clawk              " -NoNewline -ForegroundColor Green

    Write-Host "Start chatting"

    Write-Host "   clawk setup        " -NoNewline -ForegroundColor Green

    Write-Host "Configure API keys & settings"

    Write-Host "   clawk config       " -NoNewline -ForegroundColor Green

    Write-Host "View/edit configuration"

    Write-Host "   clawk config edit  " -NoNewline -ForegroundColor Green

    Write-Host "Open config in editor"

    Write-Host "   clawk gateway      " -NoNewline -ForegroundColor Green

    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"

    Write-Host "   clawk update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Clawksis repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Clawksis-Setup.exe), never via the irm|iex CLI one-liner.
...
$InstallStages += @(
    ...
    @{ Name = "path";             Title = "Adding Clawksis to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/samuelgradientai-sys/clawksis-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
