...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ClawkHome = $(if ($env:CLAWK_HOME) { $env:CLAWK_HOME } else { "$env:LOCALAPPDATA\clawk" }),
    [string]$InstallDir = $(if ($env:CLAWK_HOME) { "$env:CLAWK_HOME\clawksis-agent" } else { "$env:LOCALAPPDATA\clawk\clawksis-agent" }),
