#!/usr/bin/env python3
"""Live test harness for Clawksis's Tool Search feature.
...
# Force-isolate the test environment BEFORE any clawk imports.
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.clawksis/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.clawksis/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="clawk_ts_live_"))
    clawk_home = home_dir / ".clawk"
    clawk_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, clawk_home / "auth.json")
    ...
    (clawk_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return clawk_home
...
def reset_module_state():
    """Drop cached modules so the new CLAWK_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "clawk_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["CLAWK_HOME"] = str(home)
...
def main():
    ...
    # Restore original CLAWK_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CLAWK_HOME"] = ORIGINAL_HOME
    else:
