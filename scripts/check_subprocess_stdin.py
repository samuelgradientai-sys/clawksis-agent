...
When Clawksis runs in TUI mode, the gateway child process communicates with
...
# ``get_clawk_home() / "plugins"`` (user) and ``./.clawksis/plugins/`` (project,
# gated behind ``CLAWK_ENABLE_PROJECT_PLUGINS``) — see
# ``clawk_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``clawk_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import clawk_constants (this script
    ...
    for tui_dir in TUI_CONTEXT_DIRS:
        ...
        for py_file in dirpath.rglob("*.py"):
            # Normalize to forward slashes so KNOWN_SAFE entries (always written
            # with "/") match regardless of host OS path separator.
    ...
    # plugins in get_clawk_home()/plugins/ and project plugins in
    # ./.clawksis/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_clawk_home() / "plugins"]
    if os.environ.get("CLAWK_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".clawksis" / "plugins")
