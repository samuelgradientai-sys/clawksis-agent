...
When running Clawksis inside Docker, the default `network_mode: host` gives the
...
The Clawksis [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│   │ clawksis-agent │   │ clawksis-dashboard │   │
...
#   CLAWK_UID=$(id -u) CLAWK_GID=$(id -g) docker compose up -d
...
services:
  ...
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=clawk,clawksis-dashboard,localhost
...
docker compose exec gateway \
  ...
  curl -sf --max-time 5 http://clawksis-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Clawksis trust model and vulnerability reporting
