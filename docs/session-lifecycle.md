...
The canonical transcript store is SQLite via `SessionDB` (from `clawk_state`). The
...
Counts consecutive restarts via a JSON file (`{CLAWK_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `clawk update`, `clawk gateway restart`,
or `/restart`.
