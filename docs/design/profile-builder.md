...
Supersedes: PR #31781 (prompt_toolkit `clawk profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `clawk profile wizard` in the terminal.
...
needs, and a profile is just a CLAWK_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.clawksis/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — CLAWK_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with CLAWK_HOME override |
...
### Seam #1 — hub-skill install cannot use the CLAWK_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = CLAWK_HOME / "skills"` at **module
import time**. The context-local `set_clawk_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_clawk_action`
runs `python -m clawk_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_clawk_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's CLAWK_HOME bound
...
spawned detached (`_spawn_clawk_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchronous)
2. write model (synchronous, CLAWK_HOME override)
3. write selected MCP servers (synchronous, CLAWK_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `clawk -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
page afterward. Mirror `_write_profile_model`'s CLAWK_HOME-override helper for
...
- Backend E2E with isolated CLAWK_HOME: POST a full create body
