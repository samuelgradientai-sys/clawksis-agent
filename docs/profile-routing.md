...
`clawk config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.clawksis/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `CLAWK_HOME`, secret scope, and
