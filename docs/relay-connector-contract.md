...
This document is the formal interface between the **Clawksis gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `clawk gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`clawk gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `clawk` CLI.
