...
Clawksis middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Clawksis'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Clawksis calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Clawksis retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Clawksis guardrails,
...
- `telemetry_schema_version`: currently `clawk.observer.v1`
- `middleware_schema_version`: currently `clawk.middleware.v1`
...
Clawksis stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Clawksis runs
them as a nested chain in registration order. Middleware failures are fail-open:
Clawksis logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Clawksis applies middleware in this order:
...
For each tool call, Clawksis applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Clawksis pre-execution path against the effective arguments:
...
```bash
clawk plugins enable <plugin-name>
```

For isolated local testing, use one `CLAWK_HOME` for plugin enablement and the
agent run:

```bash
export CLAWK_HOME=/tmp/clawk-middleware-test
mkdir -p "$CLAWK_HOME"
clawk plugins enable <plugin-name>
clawk chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run clawk plugins enable <plugin-name>
uv run clawk chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["clawk_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/clawk-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Clawksis expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Clawksis treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Clawksis preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Clawksis does not convert downstream
