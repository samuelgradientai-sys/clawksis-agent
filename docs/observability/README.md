# Clawksis Observer Hooks

Clawksis observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Clawksis' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Clawksis catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Clawksis only builds
