...
Clawksis supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.clawksis/config.yaml`:
...
Or set the env var: `CLAWK_KANBAN_DISPATCH_IN_GATEWAY=false`
