...
`mcp_tool.py`, and `clawk_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ClawksisMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_clawk_provider_class() -> Optional[type]:
    ...
    class ClawksisMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._clawk_server_name = server_name
            self._clawk_home = ""
            ...
            self._clawk_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ClawksisTokenStorage` persisting an absolute
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``clawk mcp reauth``.
            ...
            back to ``clawk mcp reauth``.
            """
            try:
                if self._clawk_preregistered:
                    return
    ...
    return ClawksisMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_CLAWK_PROVIDER_CLS: Optional[type] = _make_clawk_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._clawk_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`ClawksisMCPOAuthProvider` directly using the helpers
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `clawk mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``clawk mcp remove <name>`` and (indirectly) by
        ``clawk mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, clawk_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, clawk_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, clawk_home))
