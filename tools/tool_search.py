"""Progressive tool disclosure ("tool search") for Clawksis.
...
``tool_call`` — and surfaced on demand. Core Clawksis tools never defer.
...
* Core tools defined in ``toolsets._CLAWK_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_CLAWK_CORE_TOOLS)
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_CLAWK_CORE_TOOLS``. Core tools are never deferred
