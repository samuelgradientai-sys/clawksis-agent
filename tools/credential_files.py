...
def register_credential_file(
    ...
    *relative_path* is relative to ``CLAWK_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside CLAWK_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because CLAWK_HOME is exactly where
    ...
    """
    clawk_home = _resolve_clawk_home()

    # Reject absolute paths — they bypass the CLAWK_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to CLAWK_HOME)",
            relative_path,
        )
        return False

    host_path = clawk_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape CLAWK_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, clawk_home)
    ...
    # inside CLAWK_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.clawksis/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="clawk-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching clawk_constants.get_clawk_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_clawk_dir()`` for backward compatibility with old directory layouts.
    """
    from clawk_constants import get_clawk_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_clawk_dir(new_subpath, old_name)
        if host_dir.is_dir():
