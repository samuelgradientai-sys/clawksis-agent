...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/clawk-results/{tool_use_id}.txt on

   standard Linux, or $TMPDIR/clawk-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/clawk-results"

HEREDOC_MARKER = "CLAWK_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/clawk-results"
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"CLAWK_PERSIST_{uuid.uuid4().hex[:8]}"
