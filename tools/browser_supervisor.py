...
One ``CDPSupervisor`` runs per Clawksis ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Clawksis logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "clawk-dialog-bridge.invalid"
...
(() => {
  if (window.__clawkDialogBridgeInstalled) return;
  window.__clawkDialogBridgeInstalled = true;
  const ENDPOINT = "http://clawk-dialog-bridge.invalid/";
