...
something different on each:
  ...
  * macOS — explicit TCC grants (Accessibility + Screen Recording). cua-driver
    ...
    the installed ``CuaDriver.app``), NOT Clawksis — so no Clawksis entitlement is
...
``clawk computer-use permissions`` CLI, and ``/api/tools/computer-use/status``.
"""
...
def request_permissions_grant(driver_cmd: Optional[str] = None) -> int:
    ...
    binary = shutil.which(_driver_cmd(driver_cmd))
    if not binary:
        print("cua-driver: not installed. Run: clawk computer-use install")
        return 2
