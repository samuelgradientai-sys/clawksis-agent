...
etc.) surface as specific blocked checks via `clawk computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and samuelgradientai-sys/clawksis-agent#67052.
    """
...
# fetches the latest release, so a `CLAWK_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `CLAWK_CUA_DRIVER_CMD` at a specific binary instead.
...
def _cua_telemetry_disabled() -> bool:
    """True when Clawksis should disable cua-driver telemetry for this user.
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of samuelgradientai-sys/clawksis-agent#47072: instead of hardcoding
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or CLAWK_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `clawk computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `clawk tools` and enable the Computer Use toolset to install it automatically."
    )
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of samuelgradientai-sys/clawksis-agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Clawksis-managed secrets out of the child env.
                env=_sanitize_subprocess_env(cua_driver_child_env()),
            )
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `clawk computer-use doctor` and check "
                f"{display_clawk_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    samuelgradientai-sys/clawksis-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of samuelgradientai-sys/clawksis-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Clawksis run, with
        ...
        #     concurrent Clawksis runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Clawksis run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"clawk-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Clawksis' minimal core.
    ...
    def capture(
        ...
        Maps clawk `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of samuelgradientai-sys/clawksis-agent#47072: read the canonical
    ...
    def _apply_delivery(
        ...
        to proceed. See samuelgradientai-sys/clawksis-agent#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # samuelgradientai-sys/clawksis-agent#47072). cua-driver-rs gained an explicit
