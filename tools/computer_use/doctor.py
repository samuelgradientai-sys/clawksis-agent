"""
`clawk computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Clawksis side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Clawksis provider secrets stripped.
...
def run_doctor(
    ...
    *,
    ...
    Honors `CLAWK_CUA_DRIVER_CMD` via the same `_cua_driver_cmd()` resolver
