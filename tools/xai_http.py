...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``clawk tools`` repaint, tool-registration scans,
    ...
    2. ``~/.clawksis/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``clawk auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_clawk_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.clawksis/.env`` first, then ``os.environ``.

    Wraps :func:`clawk_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from clawk_cli.config import get_env_value as _clawk_get_env_value

        value = _clawk_get_env_value(name)
...
def clawk_xai_user_agent() -> str:
    """Return a stable Clawksis-specific User-Agent for xAI HTTP calls."""
    try:
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Clawksis config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Clawksis home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_clawk_home() / "state"
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Clawksis-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``clawk_cli.config.get_env_value`` so keys
    stored in ``~/.clawksis/.env`` (the standard Clawksis location) are honored —
