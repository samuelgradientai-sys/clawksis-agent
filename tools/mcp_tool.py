...
transport, discovers their tools, and registers them into the clawksis-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.clawksis/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.clawksis/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Clawksis' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                os.path.join(clawk_home, "node", "bin", resolved_command),
                ...
                # image (which the Clawksis Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Clawksis MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Clawksis gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Clawksis document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Clawksis' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (CLAWK_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit CLAWK_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Clawksis' logging (agent.log via clawk_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'clawksis-agent[mcp]'\n"
                "or (full install):\n"
        ...
        # ungraceful exit of this Clawksis process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Clawksis' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Clawksis is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `clawk mcp login {server_name}` (or delete the tokens "
            f"file under ~/.clawksis/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local CLAWK_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_clawk_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_clawk_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local CLAWK_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_clawk_home()
...
def _filter_suspicious_mcp_servers(servers: Dict[str, dict]) -> Dict[str, dict]:
    ...
    try:
        from clawk_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
    except Exception:
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Clawksis config file.
    ...
    ``os.environ`` (which includes ``~/.clawksis/.env`` loaded at startup).
    """
    try:
        from clawk_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("CLAWK_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Clawksis' MEDIA: tag convention and out to messaging
            ...
            # Clawksis' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Clawksis's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Clawksis uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Clawksis registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``clawk tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Clawksis registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple clawk
