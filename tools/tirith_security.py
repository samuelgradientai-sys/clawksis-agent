...
it is automatically downloaded from GitHub releases to $CLAWK_HOME/bin/tirith.
...
def _get_clawk_home() -> str:
    """Return the Clawksis home directory, respecting CLAWK_HOME env var."""
...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_clawk_home(), ".tirith-install-failed")
...
def _clawk_bin_dir() -> str:
    """Return $CLAWK_HOME/bin, creating it if needed."""

    d = os.path.join(_get_clawk_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CLAWK_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_clawk_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CLAWK_HOME/bin/tirith (previously auto-installed)

    3. Auto-install from GitHub releases → $CLAWK_HOME/bin/tirith
    ...
    clawk_bin = os.path.join(_clawk_bin_dir(), "tirith")

    if os.path.isfile(clawk_bin) and os.access(clawk_bin, os.X_OK):
        _resolved_path = clawk_bin
