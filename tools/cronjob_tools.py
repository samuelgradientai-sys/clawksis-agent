"""
Cron job management tools for Clawksis.
...
#      saying "cat ~/.clawksis/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `clawksis-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.clawksis/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("CLAWK_SESSION_PLATFORM")
    origin_chat_id = get_session_env("CLAWK_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("CLAWK_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes CLAWK_SESSION_USER_ID to
...
def _local_delivery_notice(
    ...
    ``CLAWK_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via clawk model.
    ...
    if provider_name == "custom":
        try:
            from clawk_cli.runtime_provider import has_named_custom_provider
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within CLAWK_HOME/scripts/.
    ...
    # Only relative paths within ~/.clawksis/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.clawksis/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.clawksis/scripts/ and use just the filename."
        )
...
def cronjob(
    ...
    try:
        ...
        if normalized == "create":
            ...
            # Validate script path before storing. Inline script *content*
            # (shebang / multi-line) is materialized to a file first so the
            # common "agent pasted the whole script here" case still works.
            if script:
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("CLAWK_INTERACTIVE")
        or env_var_enabled("CLAWK_GATEWAY_SESSION")
        or env_var_enabled("CLAWK_EXEC_ASK")
    )
