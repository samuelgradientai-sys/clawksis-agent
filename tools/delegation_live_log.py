...
per child under::
    ...
    <clawk_home>/cache/delegation/live/<delegation_id>/task-<n>.log
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.clawksis)."""
    from clawk_constants import get_clawk_dir

    return get_clawk_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(
        ...
        try:
            ...
            header = [
                "=== Clawksis subagent live transcript ===",
