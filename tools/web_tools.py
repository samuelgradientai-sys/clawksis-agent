...
Backend is selected during ``clawk tools`` setup (web.backend in config.yaml).
When available, Clawksis can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    ...
    through Clawksis' config/.env layer (``clawk config set``, ``clawk tools``)
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``clawk tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "scrapegraph":
        # Extract-only in-house backend. Normally resolved through the plugin
        # registry above; this keeps it available when plugin discovery has not
        # run (e.g. unit tests that bypass _ensure_web_plugins_loaded()).
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `clawk plugins enable {disabled_key}` "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`clawk plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
