...
~/.clawksis/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `clawk config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.clawksis/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch CLAWK_HOME and
    ...
    live profile-scoped CLAWK_HOME on every call.
    """
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`clawk curator unpin {name}` if they want to delete it. "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`clawk curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        if isinstance(usage_rec, dict) and not skill_usage._is_curator_managed_record(
            usage_rec
        ):
            return {
                "success": False,
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.clawksis/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Clawksis profiles.
    ...
    # Default profile (~/.clawksis/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.clawksis/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`clawk -p "
            ...
            base += (
                ...
                f"Switch profiles (`clawk -p <name>`) to edit there, or "
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.clawksis/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `clawk curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `clawk curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_clawk_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `clawk curator unpin <name>`. "
