...
`clawk skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.clawk.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.clawk.blueprint`` is a mapping containing
    ...
    clawk = meta.get("clawk") if isinstance(meta, dict) else None
    blueprint = clawk.get("blueprint") if isinstance(clawk, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.clawk.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.clawk.blueprint`` block) they can hand
    to ``clawk skills publish`` to share. ``body`` is the plain-language
