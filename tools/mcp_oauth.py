...
This module provides the glue:
    - ``ClawksisTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Clawksis"
"""
...
def _get_token_dir(clawk_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses CLAWK_HOME so each profile gets its own OAuth tokens.
    Layout: ``CLAWK_HOME/mcp-tokens/``
    """
    try:
...
def _cached_redirect_port(storage: "ClawksisTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Clawksis restarts and chooses a
...
def _cached_redirect_uri(storage: "ClawksisTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``clawk mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `clawk mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# ClawksisTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------
...
class ClawksisTokenStorage:
    ...
    File layout::
        ...
        CLAWK_HOME/mcp-tokens/<server_name>.json         -- tokens
        CLAWK_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CLAWK_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, clawk_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._clawk_home = Path(clawk_home) if clawk_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._clawk_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._clawk_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._clawk_home) / f"{self._server_name}.meta.json"
    ...
    def restore(
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._clawk_home)
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = ClawksisTokenStorage(server_name, clawk_home=clawk_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "ClawksisTokenStorage | None" = None,
) -> int:
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`clawk mcp login {server_name}` interactively first to complete "
