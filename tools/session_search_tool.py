...
the get_anchored_view / get_messages_around primitives in clawk_state.
...
# Third-party integrations tag their sessions with CLAWK_SESSION_SOURCE=tool
...
def _resolve_profile_db(profile: str):
    """Open another profile's ``state.db``, or None for the current one.
    ...
    runs in profile A. Returns None when no profile is given (use the caller's
    ...
    return SessionDB(db_path=profiles_mod.get_profile_dir(canon) / "state.db")


def _locate_session_db(session_id: str):
    """Scan every profile's ``state.db`` for a session id.
...
def session_search(
    ...
    # Cross-profile read: swap in the named profile's DB for every shape below.

    # The current-session-lineage guards no longer apply across profiles, but

    # they key off ids that won't collide, so they stay inert.
