"""Central registry for all clawksis-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``clawk tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``clawk tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def register_plugin_override_policy(
        self, module_namespace: str, allowed: bool
    ) -> None:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("clawk_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``clawk_plugins.{name}``), not the exact module string.
                # A handler defined in ``clawk_plugins.pkg.handlers`` is
                # still owned by the ``clawk_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("clawk_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``clawk tools enable foo``)
