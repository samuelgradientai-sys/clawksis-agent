...
async def _handle_scrapegraph(args, **kw):
    ...
    try:
        ...
        return tool_result(
            ...
            urls=urls,
            error=(
                f"ScrapeGraphAI extraction failed: {exc}. The page may need "
                "JavaScript (set render_js=true), be behind anti-bot (try the "
                "`scrape` tool), or the prompt may need to be more specific."
            ),
        )
