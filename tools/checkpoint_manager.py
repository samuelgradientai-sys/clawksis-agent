...
-----------------------------------------------------------------------------
    ...
    ~/.clawksis/checkpoints/
        ...
        store/                          — single bare-ish git repo
            ...
            refs/clawksis/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_clawk_home() / "checkpoints"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Clawksis convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("CLAWK_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal Clawksis infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``clawk checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `clawk checkpoints clear-legacy` when safe.",
            legacy_root,
        )
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "clawk@local"], store, cfg_wd)

    _run_git(["config", "user.name", "Clawksis Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``CLAWK_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at CLAWK_WORKDIR

    # (write in addition to the JSON metadata).

    try:
        (shadow_repo / "CLAWK_WORKDIR").write_text(
