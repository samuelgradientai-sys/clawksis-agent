...
提供以下工具函数，供 clawksis-agent 的 "clawk-yuanbao" toolset 使用：
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（CLAWK_SESSION_CHAT_ID）。
    ...
    target = (chat_id or "").strip() or get_session_env("CLAWK_SESSION_CHAT_ID", "")
...
def _check_yuanbao():
    ...
    try:
        ...
        if get_session_env("CLAWK_SESSION_PLATFORM", "") == "yuanbao":
            return True
