...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Clawksis was already launched from (for example ``clawksis-agent``
    while the process cwd is ``~/.clawksis/clawksis-agent``), passing it through
    unchanged makes the wrapper run ``cd clawksis-agent`` *inside* the project
    ...
    # Common recovery for config values like ``clawksis-agent`` when Clawksis was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./clawksis-agent``; use the current directory instead.
...
# Clawksis-internal env vars that should NOT leak into terminal subprocesses.
_CLAWK_PROVIDER_ENV_FORCE_PREFIX = "_CLAWK_FORCE_"

# Clawksis-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Clawksis inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Clawksis-managed inference credential — Claude subscription auth is not a
    # working Clawksis provider path.  Stripping it broke agent-spawned
...
_CLAWK_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Clawksis venv path instead of the project's own
# ``.venv`` — silently clobbering the Clawksis environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Clawksis venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_clawk_internal_secret(key: str) -> bool:
    """Return True for Clawksis-internal secrets injected under *dynamic* names.

    ``_CLAWK_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Clawksis-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`clawk_subprocess_env` helper all call it, so the
...
def _inject_context_clawk_home(env: dict) -> None:
    """Bridge the context-local Clawksis home override into subprocess env."""
    try:
        from clawk_constants import get_clawk_home_override

        value = get_clawk_home_override()
        if value:
            env["CLAWK_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (CLAWK_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_CLAWK_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_clawk_internal_secret(key):
            continue
        if key not in _CLAWK_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
    ...
    for key, value in (extra_env or {}).items():
        ...
        elif _is_clawk_internal_secret(key):
            continue
        elif key not in _CLAWK_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
# legitimate child Clawksis spawns needs them, and they are the highest-value
...
# narrow subset of _CLAWK_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in clawk_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_clawk_internal_secret, but _ID has no secret suffix, so it must be
...
def clawk_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_CLAWK_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Clawksis spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_CLAWK_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Clawksis-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_clawk_internal_secret`.
    for key in list(env):
        if key.startswith(_CLAWK_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_clawk_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _CLAWK_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)
    ...
    # copies os.environ, whose CLAWK_SESSION_* mirror is a last-writer-wins
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("CLAWK_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale CLAWK_GIT_BASH_PATH pointing at one) must not
    ...
    # CLAWK_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\clawk\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "CLAWK_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Clawksis requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set CLAWK_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Clawksis. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``clawk`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_CLAWK_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_clawk_bin_dir() -> str | None:
    """Return the directory holding the ``clawk`` console-script, or None.
    ...
    clawk install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``clawk``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``clawk`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``clawk`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("clawk")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``clawk`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _CLAWK_BIN_DIR
    if _CLAWK_BIN_DIR is not _SENTINEL:
        return _CLAWK_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("clawk")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "clawk" or base.startswith("clawk."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "clawk.exe" if _IS_WINDOWS else "clawk"
    ...
    _CLAWK_BIN_DIR = candidate
    return candidate


def _prepend_clawk_bin_dir(existing_path: str) -> str:
    """Prepend the clawk install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_clawk_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Clawksis
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_CLAWK_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if _is_clawk_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_clawk_internal_secret(k):
            continue
        elif k not in _CLAWK_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    if path_key is not None:
        ...
        # Ensure the clawk install dir is reachable so plugins can shell out
        # to bare ``clawk`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_clawk_bin_dir(new_path)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Clawksis trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``CLAWK_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under CLAWK_HOME.  Using
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._clawk_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_clawk_pgid", None)
                    if pgid is None:
                        raise
