...
def _load_clawk_env_vars() -> dict[str, str]:
    """Load ~/.clawksis/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Clawksis profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale clawk-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=clawksis-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Clawksis process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=clawk-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a clawk process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Clawksis
    process leaves its container behind permanently because there's no
    subsequent Clawksis process scheduled to reuse that exact (task, profile)
    pair.
    """
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``CLAWK_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("CLAWK_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using CLAWK_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'clawk'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``clawksis-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.clawksis/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. clawksis-agent:latest) already use /init as PID 1
        ...
        container_name = f"clawk-{uuid.uuid4().hex[:8]}"
        # Labels make clawk-created containers identifiable to:
        #   * the orphan reaper (`clawksis-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`clawk-task-id`, `clawk-profile`)
        #   * operators running `docker ps --filter label=clawksis-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Clawksis profile is captured at
        ...
        self._labels = {
            "clawksis-agent": "1",
            "clawk-task-id": task_label,
            "clawk-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Clawksis process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Clawksis secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Clawksis-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_clawk_internal_secret.
        _implicit_forward = {
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("clawk-task-id", "")
        profile_label = self._labels.get("clawk-profile", "")
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``clawk-*`` but were
        ...
        # container) but can happen if a previous Clawksis process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Clawksis exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Clawksis process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Clawksis startup. That covers the
        ...
        # outlives Clawksis processes, processes inside it stay alive, and
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``clawk /quit``
