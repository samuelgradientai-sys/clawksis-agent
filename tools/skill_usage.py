...
Tracks per-skill usage metadata in a sidecar JSON file (~/.clawksis/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.clawksis/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.clawksis/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.clawksis/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``clawk update`` would
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.clawksis/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``clawk curator
    list-archived`` to help users pass a name to ``clawk curator restore``.
    """
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.clawksis/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.clawksis/skills/<skill>/SKILL.md) and category-nested
    (~/.clawksis/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
