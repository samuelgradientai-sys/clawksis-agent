...
<url>``) is spawned as a direct child of the Clawksis process. Clawksis's own
...
Clawksis process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Clawksis startup's opt-in
