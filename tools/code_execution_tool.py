...
Lets the LLM write a Python script that calls Clawksis tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `clawk_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `clawk_tools.py` with file-based RPC stubs
...
# available on every platform Clawksis itself runs on.
...
# match a safe prefix, the operational CLAWK_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "CLAWK_" prefix was deliberately removed (#27303) — it leaked
# CLAWK_*-named config that lacks a secret substring (e.g. CLAWK_BASE_URL,
...
_CLAWK_CHILD_ALLOWED = frozenset({
    "CLAWK_HOME",
    "CLAWK_PROFILE",
    "CLAWK_CONFIG",
    "CLAWK_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational CLAWK_* vars (_CLAWK_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret CLAWK_* vars dropped by the tightened allowlist (#27303). The
    # broad "CLAWK_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like CLAWK_KANBAN_DB / CLAWK_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _CLAWK_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("CLAWK_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped CLAWK_* var.
            _dropped_clawk.append(k)
    if _dropped_clawk:
        logger.debug(
            "execute_code: dropped %d non-allowlisted CLAWK_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Clawksis-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Clawksis-profile soft guard."""',
...
def _connect():
    ...
    CLAWK_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["CLAWK_RPC_SOCKET"]
...
_RPC_DIR = os.environ.get("CLAWK_RPC_DIR") or os.path.join(tempfile.gettempdir(), "clawk_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated clawk_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/clawk_exec_{sandbox_id}"
...
def execute_code(
    ...
    # --- Set up temp directory with clawk_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="clawk_sandbox_")
    ...
    # selector from CLAWK_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"clawk_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated clawk_tools module.
        ...
        tools_src = generate_clawk_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "clawk_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  CLAWK_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # Ensure the clawksis-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from clawk_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _clawk_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _clawk_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # CLAWK_TIMEZONE is an internal Clawksis setting and must not leak
        # into child processes.
        _tz_name = os.getenv("CLAWK_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("CLAWK_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {CLAWK_HOME}/home/ when that directory exists.
...
def _load_config() -> dict:
    ...
    ``clawk --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (clawksis-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    # temp-dir staging and clawksis-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.clawksis/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Clawksis tools programmatically. "
        ...
        f"Available via `from clawk_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into clawk_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from clawk_tools import {import_str}` "
