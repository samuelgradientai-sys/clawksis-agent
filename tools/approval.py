...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CLAWK_YOLO_MODE", ""))
...
# os.environ["CLAWK_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export CLAWK_INTERACTIVE.
_clawk_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "clawk_interactive",
    default=None,
)


def set_clawk_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["CLAWK_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``CLAWK_INTERACTIVE`` env var for legacy callers.
    """
    return _clawk_interactive_ctx.set("1" if interactive else "")


def reset_clawk_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_clawk_interactive_context`."""
    _clawk_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``CLAWK_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _clawk_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("CLAWK_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CLAWK_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("CLAWK_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("CLAWK_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CLAWK_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CLAWK_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CLAWK_SESSION_PLATFORM via
    ...
    if env_var_enabled("CLAWK_CRON_SESSION"):
        return False
    if env_var_enabled("CLAWK_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $CLAWK_HOME, or by the resolved absolute
# active profile home path such as /home/clawk/.clawksis/config.yaml. The
# resolved-absolute form is folded into the ~/.clawksis/ patterns at detection
...
# go stale when CLAWK_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CLAWK_ENV_PATH = (
    r'(?:~\/\.clawksis/|'
    r'(?:\$home|\$\{home\})/\.clawksis/|'
    r'(?:\$clawk_home|\$\{clawk_home\})/)'
    r'\.env\b'
)
# ~/.clawksis/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _CLAWK_ENV_PATH; matches the CLAWK_HOME override form as
# well as ~/.clawksis/.
_CLAWK_CONFIG_PATH = (
    r'(?:~\/\.clawksis/|'
    r'(?:\$home|\$\{home\})/\.clawksis/|'
...
DANGEROUS_PATTERNS = [
    ...
    # `clawk` and `gateway` (e.g. `clawk -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bclawk\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart clawk gateway (kills running agents)"),
    (r'\bclawk\s+update\b', "clawk update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `clawk gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart clawk-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart clawk-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(clawk|gateway|cli\.py)\b', "kill clawk/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill clawk` but not
    # `kill -9 $(pgrep -f clawk)` because the substitution is opaque
    ...
    # the `clawk gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.clawk.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(clawk|ai\.clawk)\b', "stop/restart clawk launchd service (kills running agents)"),
    ...
    # ~/.clawksis/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Clawksis-managed security file (~/.clawksis/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_CLAWK_CONFIG_PATH}|{_CLAWK_ENV_PATH})', "in-place edit of Clawksis config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_CLAWK_CONFIG_PATH}|{_CLAWK_ENV_PATH})', "in-place edit of Clawksis config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_CLAWK_CONFIG_PATH}|{_CLAWK_ENV_PATH})', "in-place edit of Clawksis config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.clawksis/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / CLAWK_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Clawksis home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\clawk), so folding the user home
    # first would eat the prefix the Clawksis-home fold needs.
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / CLAWK_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.clawksis``); the matched
...
def _rewrite_resolved_clawk_home(command: str) -> str:
    ...
    /home/clawk/.clawksis/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$CLAWK_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.clawksis")
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Clawksis ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"clawk-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["CLAWK_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "CLAWK_SPINNER_PAUSE" in os.environ:
            del os.environ["CLAWK_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Clawksis approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``CLAWK_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            CLAWK_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("CLAWK_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set CLAWK_INTERACTIVE or "
                "CLAWK_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set CLAWK_INTERACTIVE or "
            "CLAWK_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("CLAWK_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``CLAWK_INTERACTIVE``) fails CLOSED — a plugin-
        ...
        if env_var_enabled("CLAWK_CRON_SESSION"):
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from clawk_cli.config import load_config as _load_cfg
