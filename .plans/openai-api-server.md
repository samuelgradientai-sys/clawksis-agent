# OpenAI-Compatible API Server for Clawksis
...
SSE streaming. By exposing this endpoint, clawksis-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  clawksis-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.clawksis/.env`
2. Run `clawk gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with clawksis-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns clawksis-agent as a model) |
...
Authorization: Bearer clawk-api-key-here
...
But clawksis-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "clawksis-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured clawksis-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of clawksis-agent's core prompt
...
Once implemented, clawksis-agent works as a drop-in backend for:
