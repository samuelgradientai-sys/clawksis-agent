...
from clawk_constants import get_clawk_dir, get_clawk_home
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from clawk_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{CLAWK_HOME}/pairing`` while newer code/docs may
    write ``{CLAWK_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<CLAWK_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<CLAWK_HOME>/pairing/``
    directory (backward-compat for the ``clawk pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp CLAWK_HOME
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `clawk` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> clawk pairing approve ...` as root, "
