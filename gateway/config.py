...
# dashboard's pre-write mutation validation (clawk_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Clawksis is thinking…"). Platforms with textless indicators (Discord,
...
DEFAULT_STREAMING_FRESH_FINAL_AFTER_SECONDS: float = 0.0
...
class GatewayConfig:
    ...
    # gateway behaves exactly as before — single CLAWK_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``clawk config set gateway.multiplex_profiles true``.
            multiplex_profiles = nested_gateway.get("multiplex_profiles")
        # Operator override: GATEWAY_MULTIPLEX_PROFILES wins over config.yaml when
        # set to a recognized value. Hosted deployments (Fly, etc.) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.clawksis/config.yaml (primary user-facing config)
    3. ~/.clawksis/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_clawk_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through clawk_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `clawk config set gateway.<key> ...`, which naturally
