...
The PID file lives at ``{CLAWK_HOME}/gateway.pid``.  CLAWK_HOME defaults to
``~/.clawksis`` but can be overridden via the environment variable.  This means
separate CLAWK_HOME directories naturally get separate PID files — a property
...
from clawk_constants import get_clawk_home, _get_platform_default_clawk_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_clawk_home() / "gateway-starts.log"
...
def _get_process_clawk_home() -> Path:
    """Return the process-level CLAWK_HOME, skipping context-local overrides.
    ...
    ``get_clawk_home()`` honors ``_CLAWK_HOME_OVERRIDE`` contextvar used for
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting CLAWK_HOME."""
    home = _get_process_clawk_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("CLAWK_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "clawk" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Clawksis gateway lifecycle subcommand from a command line.
    ...
    test also matched ``clawk_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\clawk-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Clawksis's
    ...
    for token in tokens:
        ...
        if basename in ("clawk-gateway", "clawk-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "clawk_cli.main" in joined
        or "clawk_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("clawk", "clawk.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `clawk gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Clawksis-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Clawksis gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a CLAWK_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.clawksis`` or ``$CLAWK_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``clawk_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``CLAWK_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"clawk_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit CLAWK_HOME= on the argv. CLAWK_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "clawk_home=" in command_lc and f"clawk_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `clawk status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``CLAWK_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    CLAWK_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(
    ...
    at once (e.g. the same Telegram bot token across different CLAWK_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``clawk.service`` + ``clawk-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different CLAWK_HOME. When two profile gateway
    # services share the same default ~/.clawksis (CLAWK_HOME not set
    ...
    # field is absent in markers written by older Clawksis versions; treat
    ...
    replacer_home = record.get("replacer_clawk_home")
    if replacer_home is not None and replacer_home != str(_get_process_clawk_home()):
        return False
    ...
    # ``clawk gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_clawk_home": str(_get_process_clawk_home()),
