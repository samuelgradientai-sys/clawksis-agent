...
# Audio file extensions Clawksis recognizes for native audio delivery.
...
def _thread_metadata_for_source(
    ...
    value. Telegram private-chat topics created through Clawksis' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Clawksis-created Telegram private-chat
...
from clawk_constants import get_default_clawk_root, get_clawk_dir, get_clawk_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.clawksis/.env manually."
)
...
IMAGE_CACHE_DIR = get_clawk_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_clawk_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_clawk_dir(new_subpath, old_name)
...
AUDIO_CACHE_DIR = get_clawk_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_clawk_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_clawk_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_clawk_dir("cache/screenshots", "browser_screenshots")
...
_CLAWK_ROOT = get_default_clawk_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "CLAWK_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "CLAWK_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "CLAWK_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "CLAWK_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _CLAWK_HOME / "image_cache",
    _CLAWK_HOME / "audio_cache",
    _CLAWK_HOME / "video_cache",
    _CLAWK_HOME / "document_cache",
    _CLAWK_HOME / "browser_screenshots",
    ...
    _CLAWK_HOME / "cache" / "images",
    _CLAWK_HOME / "cache" / "audio",
    _CLAWK_HOME / "cache" / "videos",
    _CLAWK_HOME / "cache" / "documents",
    _CLAWK_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Clawksis root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* CLAWK_HOME's cache, so a gateway running at
    the root (e.g. ``CLAWK_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when CLAWK_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _CLAWK_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("CLAWK_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _CLAWK_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Clawksis profile and shared Clawksis root both contain control
    ...
    # CLAWK_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.clawksis
    ...
    for clawk_root in (_CLAWK_HOME, _CLAWK_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(clawk_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Clawksis
    secrets (``~/.clawksis/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``CLAWK_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Clawksis-managed cache, under an operator-allowlisted root
    (``CLAWK_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Clawksis root (~/.clawksis/.env,
    ...
    # ``MEDIA:~/.clawksis/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(
        r"^(?:please\s+)?restart\s+(?:the\s+)?clawk\s+gateway[.!?\s]*$", re.IGNORECASE
    ),
    re.compile(r"^(?:please\s+)?restart\s+clawk[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``CLAWK_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Clawksis commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("CLAWK_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "CLAWK_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "CLAWK_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Clawksis sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Clawksis home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Clawksis home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.clawksis/media/generated/stale.png"}
    ...
    async def interrupt_session_activity(
        self, session_key: str, chat_id: str, metadata=None
    ) -> None:
    ...
    def _final_delivery_adapter(
        ...
        try:
            ...
            logger.debug(
                "[%s] Failed to resolve live adapter for final delivery", self.name
            )
            return self
        if (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          CLAWK_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          CLAWK_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          CLAWK_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("CLAWK_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("CLAWK_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("CLAWK_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(
        ...
        try:
            ...
            # not at the top of this task.  _clawk_run_generation is set on
    ...
    def build_source(
        ...
        per-profile CLAWK_HOME isolation.
        """
