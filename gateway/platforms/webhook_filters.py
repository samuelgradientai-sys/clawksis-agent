...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.clawksis to the active profile home."""
    ...
    if raw == "~/.clawksis":
        return clawk_home
    if raw.startswith("~/.clawksis/"):
        return clawk_home / raw.removeprefix("~/.clawksis/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return clawk_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under CLAWK_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from clawk_constants import get_clawk_home

    scripts_root = (get_clawk_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.clawksis" or raw_text.startswith("~/.clawksis/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(
        ...
        if (
            ...
            or transformed.get("__clawk_ignore__") is True
        ):
            return False, None
