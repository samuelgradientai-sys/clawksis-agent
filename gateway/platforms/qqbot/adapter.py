...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.clawksis/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Clawksis upgrades overwrite this
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    ...
                    "$browser": "clawksis-agent",

                    "$device": "clawksis-agent",
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          ...
          writes the answer to ``~/.clawksis/.update_response`` for the

          detached ``clawk update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``clawk update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `clawk setup gateway` / `clawk gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``clawk update --gateway`` watcher. Button
        ...
        ``~/.clawksis/.update_response`` so the detached update process
