...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    When the install tree is read-only (e.g., Docker /opt/clawksis), this function
    mirrors the bridge source to a writable CLAWK_HOME location and returns that
    ...
    clawk_home_bridge = clawk_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to CLAWK_HOME if needed
    if clawk_home_bridge.exists():
        return clawk_home_bridge

    # Mirror the bridge source to CLAWK_HOME
    try:
        clawk_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            clawk_home_bridge,
            dirs_exist_ok=False,
