...
There are two ways to add a platform to the Clawksis gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.clawksis/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Clawksis code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `clawk gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `clawk_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Clawksis core.
...
And add it to the `clawk-gateway` composite:

```python
"clawk-gateway": {
    "includes": [..., "clawk-your-platform"]
}
```
...
## 12. Status Display (`clawk_cli/status.py`)
...
## 13. Gateway Setup Wizard (`clawk_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ clawk_cli/ toolsets.py \
  --include="*.py" -l | sort -u
