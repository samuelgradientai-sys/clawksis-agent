...
Connects Clawksis to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_dir(clawk_home: str) -> Path:
    path = Path(clawk_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(clawk_home: str, account_id: str) -> Path:
    return _account_dir(clawk_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(clawk_home, account_id)
...
def load_weixin_account(clawk_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, clawk_home: str):
        self._root = _account_dir(clawk_home)
...
def _sync_buf_path(clawk_home: str, account_id: str) -> Path:
    return _account_dir(clawk_home) / f"{account_id}.sync.json"


def _load_sync_buf(clawk_home: str, account_id: str) -> str:
    path = _sync_buf_path(clawk_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(clawk_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(clawk_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Clawksis adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._clawk_home = clawk_home
        self._token_store = ContextTokenStore(clawk_home)
        ...
        self._account_id = str(
            extra.get("account_id") or get_secret("WEIXIN_ACCOUNT_ID", "")
        ).strip()
        self._token = str(
            config.token or extra.get("token") or get_secret("WEIXIN_TOKEN", "")
        ).strip()
        self._base_url = (
            str(extra.get("base_url") or get_secret("WEIXIN_BASE_URL", ILINK_BASE_URL))
            .strip()
            .rstrip("/")
        )
        self._cdn_base_url = (
            str(
                ...
                or get_secret("WEIXIN_CDN_BASE_URL", WEIXIN_CDN_BASE_URL)
            )
            .strip()
            .rstrip("/")
        )
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(clawk_home, self._account_id)
            if persisted:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Clawksis regardless of this "
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._clawk_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._clawk_home, self._account_id, sync_buf)
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"clawk-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"clawk-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    resolved_token = str(
        token or extra.get("token") or get_secret("WEIXIN_TOKEN", "")
    ).strip()
    ...
    token_store = ContextTokenStore(str(get_clawk_home()))
