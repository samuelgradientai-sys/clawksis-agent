...
_APP_VERSION = _CLAWK_VERSION
_BOT_VERSION = _CLAWK_VERSION
_YUANBAO_INSTANCE_ID = str(
    CLAWK_INSTANCE_ID
)  # single source: yuanbao_proto.CLAWK_INSTANCE_ID
...
class MediaResolveMiddleware(InboundMiddleware):
    ...
    async def _resolve_media_urls(
        ...
        for ref in media_refs:
            ...
            if rid and cls._append_cached_resource(
...
class MessageSender:
    ...
    def strip_cron_wrapper(content: str) -> str:
        """Strip the scheduler's natural cron header (``⏰ <name>\\n\\n``) so
        Yuanbao renders just the body."""
...
class YuanbaoAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs: Any) -> None:
        ...
        try:
            _raw_concurrency = int(
                _extra.get("media_resolve_concurrency", _DEFAULT_RESOLVE_CONCURRENCY)
            )
        ...
        # ------------------------------------------------------------------
        dm_policy: str = (
            (_extra.get("dm_policy") or os.getenv("YUANBAO_DM_POLICY", "pairing"))
            .strip()
            .lower()
        )
        ...
        group_policy: str = (
            (_extra.get("group_policy") or os.getenv("YUANBAO_GROUP_POLICY", "pairing"))
            .strip()
            .lower()
        )
