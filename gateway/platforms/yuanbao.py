...
class QuoteContextMiddleware(InboundMiddleware):
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        ctx.reply_to_message_id, ctx.reply_to_text, ctx.quote_media_refs = (
            self._extract_quote_context(ctx.cloud_custom_data)
        )
        ...
        async def _dispatch_inbound_event() -> None:
            ...
            if ctx.reply_to_message_id is not None:
                ...
                for rid, kind, filename in ctx.quote_media_refs:
                    ...
                    try:
                        fresh_url = (
                            await MediaResolveMiddleware._resolve_by_resource_id(
                                adapter, rid
                            )
                        )
