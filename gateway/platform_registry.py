...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``clawk setup`` display).
    ...
    # built-ins).  Used by ``clawk gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `clawk`
        # invocation -- including plain `clawk chat`, which never touches any
        ...
        # `clawk setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``clawk setup``/``gateway status``, channel
