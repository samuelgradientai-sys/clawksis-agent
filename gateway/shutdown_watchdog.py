...
2. An event-loop heartbeat file at ``<CLAWK_HOME>/state/gateway.heartbeat`` so
...
def _process_clawk_home() -> Path:
    """CLAWK_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<CLAWK_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_clawk_home()
