...
window before ``clawk update``'s graceful restart fires), a first-time lazy
import on a new code path can resolve a freshly-pulled consumer module against a
stale cached dependency -> ImportError.
...
This matters for Clawksis specifically: production deploys are ``git pull`` +
``systemctl restart clawk-gateway`` (see the VPS runbook), so a pull landing
before the restart is a realistic window.
"""
...
def _fingerprint() -> str | None:
    ...
    ``clawk_cli.main`` is always already imported in a gateway process (it's
    the entry point), so this import is free and avoids duplicating the
    worktree-aware ref resolution.
    """
    try:
        from clawk_cli.main import _read_git_revision_fingerprint
