...
~/.clawksis/channel_directory.json.  The send_message tool reads this file for
...
DIRECTORY_PATH = get_clawk_home() / "channel_directory.json"

# User-maintained friendly-name overlay. The directory is fully regenerated from
# live adapters + session data on a timer, so hand-edits to
# channel_directory.json don't survive. Aliases declared here are re-applied on
# every build AND every load, giving durable human-friendly names (and letting
# you pre-name a chat — e.g. "Ventas", "Soporte" — before it has produced any
# traffic). Format: {"<platform>": {"<chat_id>": "<friendly name>", ...}, ...}
CHANNEL_ALIASES_PATH = get_clawk_home() / "channel_aliases.json"
...
async def build_channel_directory(adapters: Dict[Any, Any]) -> Dict[str, Any]:
    ...
    # Overlay user-maintained friendly names (durable across rebuilds).
...
def _build_from_sessions(platform_name: str) -> List[Dict[str, str]]:
    ...
    sessions_path = get_clawk_home() / "sessions" / "sessions.json"
...
def load_directory() -> Dict[str, Any]:
    ...
    if not DIRECTORY_PATH.exists():
        empty: Dict[str, Any] = {"updated_at": None, "platforms": {}}
        _apply_channel_aliases(empty["platforms"])
        return empty
    ...
    try:
        ...
        if isinstance(directory, dict):
            _apply_channel_aliases(directory.setdefault("platforms", {}))
        return directory
