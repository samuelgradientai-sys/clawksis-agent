...
Defenses 1 and 2 (the ``_CLAWK_GATEWAY`` guard on ``clawk gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.clawk.gateway")``,
...
State lives in ``<CLAWK_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_clawk_home() / "gateway" / "restart_loop.json"
