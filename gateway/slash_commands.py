...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _clawk_home, _platform_config_key
        import yaml
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + f"\n… (truncated — full diff: `/skills diff {pending_id}` "
                     f"on the CLI, or ~/.clawksis/pending/skills/{pending_id}.json)")
        return out

    async def _handle_fast_command(self, event: MessageEvent) -> str:
        """Handle /fast — mirror the CLI Priority Processing toggle in gateway chats."""
        from gateway.run import _clawk_home, _load_gateway_config, _resolve_gateway_model
        import yaml
