...
Module-level run.py helpers a handler needs (``_clawk_home``,
...
from clawk_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: clawk gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Clawksis commands.
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _clawk_home
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _clawk_home, _load_gateway_config
        import yaml
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _clawk_home) / "config.yaml"
        try:
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from clawk_cli.goals import draft_contract
    ...
    def _reasoning_picker_choices(self, current_effort: str) -> list:
        ...
        from clawk_constants import VALID_REASONING_EFFORTS
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _clawk_home
        from clawk_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.clawksis/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``clawk skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _clawk_home
        from clawk_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `clawk skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.clawksis/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _clawk_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _clawk_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from clawk_cli.session_listing import (
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Compat alias for the BYOK fork: /topup maps to the /credits view.

        run.py still dispatches ``canonical == "topup"`` here (upstream naming),
        so the mixin must expose this method. The fork keeps its own /credits
        handler and deliberately does NOT ship upstream's Nous portal-billing
        handoff wording, so this delegates to _handle_credits_command.
        """
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _clawk_home, _resolve_clawk_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
