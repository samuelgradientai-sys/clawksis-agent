"""
Session-scoped context variables for the Clawksis gateway.

Replaces the previous ``os.environ``-based session state
(``CLAWK_SESSION_PLATFORM``, ``CLAWK_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["CLAWK_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("CLAWK_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("CLAWK_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("CLAWK_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("CLAWK_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("CLAWK_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("CLAWK_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("CLAWK_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("CLAWK_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("CLAWK_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("CLAWK_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("CLAWK_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from CLAWK_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("CLAWK_UI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("CLAWK_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("CLAWK_SESSION_PROFILE", default=_UNSET)
...
_VAR_MAP = {
    ...
    "CLAWK_SESSION_SOURCE": _SESSION_SOURCE,
    "CLAWK_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "CLAWK_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "CLAWK_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "CLAWK_SESSION_USER_ID": _SESSION_USER_ID,
    "CLAWK_SESSION_USER_NAME": _SESSION_USER_NAME,
    "CLAWK_SESSION_KEY": _SESSION_KEY,
    "CLAWK_SESSION_ID": _SESSION_ID,
    "CLAWK_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "CLAWK_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "CLAWK_SESSION_PROFILE": _SESSION_PROFILE,
    "CLAWK_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "CLAWK_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "CLAWK_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``CLAWK_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("CLAWK_SESSION_ID")`` with an ``os.environ`` fallback,
...
def reset_session_vars() -> None:
    ...
    *A's* ``CLAWK_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``CLAWK_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``CLAWK_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("CLAWK_SESSION_*", default)``.
...
def declare_stateless_channel() -> None:
    ...
    See samuelgradientai-sys/clawksis-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``clawk -z``, cron — see :func:`declare_stateless_channel`). The real
