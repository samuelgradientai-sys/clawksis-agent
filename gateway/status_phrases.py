...
Clawksis' long-running gateway status surface into short status lines suitable
for chat surfaces.

Built-in defaults live in ``gateway/assets/status_phrases.yaml``. Users can add
portable, profile-relative phrase catalogs under ``CLAWK_HOME`` either by using
conventional paths::

    ~/.clawksis/status_phrases.yaml
    ~/.clawksis/status_phrases/*.yaml
...
or by pointing config at a relative file/directory::
    ...
    display:
      status_phrases:
        path: status_phrases/whatsapp.yaml  # relative to CLAWK_HOME
...
# These are Clawksis UI surfaces, not app/vendor/domain buckets.  Keep this
...
def classify_status_context(
    ...
    *,
    ...
    """Classify an internal gateway event into a Clawksis UI-surface bucket."""
