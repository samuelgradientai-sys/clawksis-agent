...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_clawk_home() under the active profile.
...
#   - cli.py, clawk_cli/*      -> inline on startup (no event loop)
...
# bound the cache grows unboundedly. (#19251)
#
# Clawksis raises the cap to 64 and makes the cache a true LRU (OrderedDict +
# popitem(last=False)): a multi-tenant gateway juggles far more distinct
# fingerprints than upstream's single-agent assumption, and a plain dict with
# an 8-entry FIFO thrashed on every platform/toolset rotation.
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("CLAWK_KANBAN_TASK")),
    ...
    if quiet_mode:
        ...
        if (
            ...
            _tool_defs_cache.popitem(last=False)  # evict least-recently-used
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("CLAWK_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by CLAWK_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like clawk-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("clawk-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (clawk-*) include _CLAWK_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("clawk-")
    ...
    # window). Core Clawksis tools (toolsets._CLAWK_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from clawk_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to clawksis-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from clawk_cli.plugins import has_hook, invoke_hook
