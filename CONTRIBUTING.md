...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Clawksis to work everywhere.
...
Bundled skills (in `skills/`) ship with every Clawksis install. They should be **broadly useful to most users**:
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `clawk skills browse` (labeled "official") and install it with `clawk skills install` (no third-party warning, built-in trust).



If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `clawk skills install`.
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.clawksis/plugins/` (or via a pip entry point).
...
- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(clawk_home, config)` for setup-wizard integration
...
- Integrate with `clawk memory setup` via `post_setup()` — no need to touch core code
...
mkdir -p ~/.clawksis/{cron,sessions,logs,memories,skills}

cp cli-config.yaml.example ~/.clawksis/config.yaml
...
echo "OPENROUTER_API_KEY=***" >> ~/.clawksis/.env
...
ln -sf "$(pwd)/venv/bin/clawk" ~/.local/bin/clawk
...
├── cli.py                    # ClawksisCLI class — interactive TUI, prompt_toolkit integration

├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)

├── toolsets.py               # Tool groupings and presets (clawk-cli, clawk-telegram, etc.)

├── clawk_state.py           # SQLite session database with FTS5 full-text search, session titles
...
├── clawk_cli/               # CLI command implementations
...
├── skills/                   # Bundled skills (copied to ~/.clawksis/skills/ on install)
...
├── cli-config.yaml.example   # Example configuration (copied to ~/.clawksis/config.yaml)
...
### User configuration (stored in `~/.clawksis/`)
...
| `~/.clawksis/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |

| `~/.clawksis/.env` | API keys and secrets |

| `~/.clawksis/auth.json` | OAuth credentials (Nous Portal) |

| `~/.clawksis/skills/` | All active skills (bundled + hub-installed + agent-created) |

| `~/.clawksis/memories/` | Persistent memory (MEMORY.md, USER.md) |

| `~/.clawksis/state.db` | SQLite session database |

| `~/.clawksis/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |

| `~/.clawksis/cron/` | Scheduled job data |

| `~/.clawksis/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`clawk_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.clawksis/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_CLAWK_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.clawk`:
...
The user may skip setup and keep loading the skill. Clawksis only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `clawk setup` or update `~/.clawksis/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Clawksis tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Clawksis" is the secondary collaborator. If the contributor's commit shows "Clawksis" as author because they used Clawksis to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Clawksis tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `clawk --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Clawksis uses a data-driven skin system — no code changes needed to add a new skin.
...
Create `~/.clawksis/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `clawk_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.
...
See `clawk_cli/skin_engine.py` for the full schema and existing skins as examples.
...
Clawksis runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `clawk_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `clawk_cli/gateway_windows.py::_spawn_detached` for the reference
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `clawk_cli/gateway_windows.py::
...
Clawksis has terminal access. Security matters.
...
2. **Test manually**: Run `clawk` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/samuelgradientai-sys/clawksis-agent/issues)

- Include: OS, Python version, Clawksis version (`clawk version`), full error traceback
