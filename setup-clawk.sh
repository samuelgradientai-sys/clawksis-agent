...
# 5. Symlinks the 'clawk' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/clawk-uv-install.$$.log")"
...
# PATH setup — symlink clawk into a user-facing bin dir

# ============================================================================



echo -e "${CYAN}→${NC} Setting up clawk command..."



CLAWK_BIN="$SCRIPT_DIR/venv/bin/clawk"
...
ln -sf "$CLAWK_BIN" "$COMMAND_LINK_DIR/clawk"

echo -e "${GREEN}✓${NC} Symlinked clawk → $COMMAND_LINK_DISPLAY_DIR/clawk"
...
# Seed bundled skills into ~/.clawksis/skills/

# ============================================================================



CLAWK_SKILLS_DIR="${CLAWK_HOME:-$HOME/.clawksis}/skills"

mkdir -p "$CLAWK_SKILLS_DIR"



echo ""

echo "Syncing bundled skills to ~/.clawksis/skills/ ..."
...
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    ...
    if [ -d "$SCRIPT_DIR/skills" ]; then
        ...
        cp -rn "$SCRIPT_DIR/skills/"* "$CLAWK_SKILLS_DIR/" 2>/dev/null || true
...
echo "  clawk status        # Check configuration"

if is_termux; then

    echo "  clawk gateway       # Run gateway in foreground"

else

    echo "  clawk gateway install # Install gateway service (messaging + cron)"

fi

echo "  clawk cron list     # View scheduled jobs"

echo "  clawk doctor        # Diagnose issues"
