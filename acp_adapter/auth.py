"""ACP auth helpers — detect and advertise Clawksis authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "clawk-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Clawksis runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Clawksis can resolve any runtime provider credentials."""

    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Clawksis.
    ...
    not have Clawksis provider credentials configured yet, so Clawksis always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Clawksis using the currently configured "
