"""CLI entry point for the clawksis-agent ACP adapter.

Loads environment variables from ``~/.clawksis/.env``, configures logging
...
try:
    ...
    # shadowing Clawksis's own modules — ``clawk acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    clawk_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from CLAWK_HOME (default ``~/.clawksis``)."""
    from clawk_cli.env_loader import load_clawk_dotenv

    clawk_home = get_clawk_home()
    loaded = load_clawk_dotenv(clawk_home=clawk_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", clawk_home / ".env"
        )
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ClawksisACPAgent  # noqa: F401

    print("Clawksis ACP check OK")


def _run_setup() -> None:
    from clawk_cli.main import main as clawk_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "clawk", "model"]
        clawk_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``clawk postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting clawksis-agent ACP adapter")
    ...
    import acp
    from .server import ClawksisACPAgent
