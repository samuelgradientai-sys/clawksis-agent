"""ACP agent server — exposes Clawksis via the Agent Client Protocol."""
...
try:
    from clawk_cli import __version__ as CLAWK_VERSION
except Exception:
    CLAWK_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Clawksis running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Clawksis/OpenAI-compatible user content payload."""
...
class ClawksisACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Clawksis AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Clawksis version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Clawksis maps edit approval
    ...
    def _resolve_model_selection(
        ...
        try:
            from clawk_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Clawksis estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_clawk_session_id: str,
        previous_clawk_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.clawk.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_clawk_session_id,
                previous_clawk_session_id=previous_clawk_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_clawk_session_id: Optional[str] = None,
        previous_clawk_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Clawksis changes it.

        When the internal Clawksis head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_clawk_session_id`` so the
        attached ``_meta.clawk.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # clawk_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_clawk_session_id or session_id,
            previous_clawk_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["clawk-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="clawksis-agent", version=CLAWK_VERSION),
    ...
    async def authenticate(
        ...
        # Clawksis' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Clawksis setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Clawksis
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Clawksis on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Clawksis' local kawaii waiting/status
        ...
        def _run_agent() -> dict:
            ...
            previous_session_id = os.environ.get("CLAWK_SESSION_ID")
            os.environ["CLAWK_SESSION_ID"] = session_id
            try:
                ...
                if previous_session_id is None:
                    ...
                    os.environ["CLAWK_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            pre_turn_clawk_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (CLAWK_SESSION_KEY in
        ...
        # _meta.clawk.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_clawk_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_clawk_id
            and pre_turn_clawk_id
            and post_turn_clawk_id != pre_turn_clawk_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_clawk_session_id=post_turn_clawk_id,
                    previous_clawk_session_id=pre_turn_clawk_id,
                )
            except Exception:
                logger.debug(
        ...
        # Clawksis' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_reset(self, args: str, state: SessionState) -> str:
        ...
        try:
            ...
            reset_failed = True
            logger.warning(
                "ACP session state reset failed for %s", state.session_id, exc_info=True
            )
        finally:
            self.session_manager.save_session(state.session_id)
        if reset_failed:
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Clawksis v{CLAWK_VERSION}"
