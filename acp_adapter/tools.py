"""ACP tool-call helpers for mapping clawk tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map clawk tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
_POLISHED_TOOLS = {
    ...
    "yb_send_dm",
    "yb_send_sticker",
    # Fork-only: Clawksis still ships tools/mixture_of_agents_tool.py.
    "mixture_of_agents",
}


def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a clawk tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some Clawksis tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Clawksis tool result clearly failed.
    ...
    # Clawksis core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(
    ...
    # Clawksis read_file output is line-numbered with `|`. If we send it as raw
...
def _format_session_search_result(result: Optional[str]) -> Optional[str]:
    ...
    lines = [
        "Recent sessions"
        if mode == "recent"
        else "Session search results" + (f" for `{query}`" if query else "")
    ]
    if not results:
...
def build_tool_start(
    ...
    *,
    ...
    """Create a ToolCallStart event for the given clawk tool invocation.
